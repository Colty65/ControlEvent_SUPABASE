...
(function(){
  ...
  function metricStyle(label, value, cls){
    ...
    const soft = (color, bg) => `--ce-v19-metric-color:${color};background:${bg}!important;border-color:rgba(15,23,42,.08)!important`;
    if(key === 'INGRESOS') return soft('#1d4ed8','rgba(37,99,235,.11)');
    if(key === 'COMPRAS') return soft('#b91c1c','rgba(220,38,38,.10)');
    if(key === 'DONACIONES') return soft('#b45309','rgba(245,158,11,.14)');
    if(key === 'PRODUCTO DISPONIBLE') return soft('#0f766e','rgba(15,118,110,.11)');
    if(key === 'SALDO ACTUAL') return bad ? soft('#b91c1c','rgba(220,38,38,.10)') : soft('#15803d','rgba(22,163,74,.11)');
    if(key === 'SALDO OPERATIVO') return bad ? soft('#b91c1c','rgba(220,38,38,.10)') : soft('#0e7490','rgba(8,145,178,.11)');
    return soft('#475569','rgba(100,116,139,.10)');
  }
  ...
  function injectStyle(){
    ...
    style.textContent = `
      ...
      .ce-v19-metrics{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:10px;margin-bottom:12px;}.ce-v19-metric{border-radius:18px;border:1px solid rgba(15,23,42,.08);padding:13px;box-shadow:0 6px 18px rgba(15,23,42,.055);position:relative;overflow:hidden;backdrop-filter:blur(4px);}.ce-v19-metric::after{content:"";position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.55),rgba(255,255,255,.10));pointer-events:none}.ce-v19-metric span{display:block;font-size:11px;font-weight:1000;color:#475569!important;letter-spacing:.05em;text-shadow:none!important;position:relative;z-index:1;}.ce-v19-metric strong{display:block;margin-top:5px;font-size:24px;font-weight:1000;color:var(--ce-v19-metric-color,#0f172a)!important;text-shadow:none!important;position:relative;z-index:1;}
