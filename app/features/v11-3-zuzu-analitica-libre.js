/* ControlEvent v28.5_prod - Zuzu / Analítica libre de explotación del evento.
...
(function(){
  ...
  function responsePdfTitle(data, prompt){
    ...
    return 'ControlEvent_v28.5_prod-responde_Zuzu_a_'+subject+'-'+stamp+'.pdf';
  }
  ...
  function modalHtml(){
    return '<div class="ce-ai-overlay" id="ceGeminiLibreOverlay" role="dialog" aria-modal="true">'+
      '<div class="ce-ai-modal">'+
        '<div class="ce-ai-head"><h2>✨ Soy Zuzu, pregúntame lo que quieras...</h2><span class="ce-ai-version-badge">v28.5_prod</span><div id="ceAiEventTitle">'+eventTitleHtml()+'</div><div class="spacer"></div><button type="button" class="ce-ai-close" id="ceAiClose">Cerrar</button></div>'+
  ...
  function zuzuStoragePrefix(){ var v=String(window.__ceVersionLabel||'v28.5_prod').trim(); return 'ControlEvent_'+v+'_zuzu_'; }
  ...
  async function runAi(){
    ...
    // v28.5_prod: Gemini interpreta el ámbito; el evento de pantalla es solo contexto ambiental.
    // No se bloquean preguntas globales o personales por no haber evento activo.
    saveZuzuInteractionId(''); // v28.5_prod: no conservar IDs acumulativos de Interactions entre turnos.
    setStatus('Zuzu está pensando...', 'ok');
    var resEl=$('ceAiResult');
    startZuzuThinking(prompt);
    try{
      var history=loadZuzuConversation().slice(-6);
      var previousInteractionId=''; // v28.5_prod: memoria local compacta; no se encadena Interactions.
  ...
  function traceHtml(data){
    // v28.5_prod: la traza está SIEMPRE disponible en pantalla, plegada por defecto.
