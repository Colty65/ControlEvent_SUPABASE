/* ControlEvent v27_prod_1.0 - asistencia canónica compartida en navegador.
...
(function(root){
  ...
  function census(state,eventId){
    // El snapshot FIX9.3 solo existe para personas vinculadas al evento. Por tanto,
    // no puede sustituir al censo completo: se parte de PERSONAS y el snapshot
    // histórico del evento solo sobrescribe nombre/rango de quien sí tiene snapshot.
    ...
    arr(state?.eventPersonSnapshots).filter(row=>eventIdOf(row)===trim(eventId)).forEach(row=>{
      const id=personIdOf(row); const current=byId.get(id)||{};
  ...
  root.ControlEventCanonicalAttendance={version:'v27_prod_1.0',calculate};
})(window);
