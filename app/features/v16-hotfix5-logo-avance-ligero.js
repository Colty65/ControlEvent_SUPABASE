/* ControlEvent v27_prod_1.0 - HOTFIX5: avance ColtyLAB ligero y sin bloqueo.
...
(function(){
  ...
  function socioBasePermitido(p){
    ...
    if(/^Personas/i.test(n)) return false;
    return !!n;
  }
  ...
  function noSociosAsistentes(col){
    ...
    col.filter(c=>rowRango(c)!=='SOCIO').forEach(c=>{
      const name=rowName(c); const n=norm(name);
      if(!name || /^GRUPO\b/.test(n) || /^PEÑA\b/.test(n) || /^PERSONAS\b/.test(n) || /^Z\s*_?\s*DEV/.test(n) || /^Z\s*_?\s*DE/.test(n)) return;
      const item={name,size:Math.max(1,rowNumero(c))};
      const old=byName.get(n);
      if(!old || item.size>old.size) byName.set(n,item);
    });
