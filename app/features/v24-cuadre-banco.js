/* ControlEvent v26_prod_1.2 FIX9.3.12 · Cuadre Banco: visor contable junto a cada justificante. */
(function(root){
  'use strict';
  if(root.__ceV24BankReconciliation) return;
  root.__ceV24BankReconciliation = true;

  const VERSION = 'v26_prod_1.2';
  ...
  async function load({force=false,preserveNotice=false,preserveMovementId='',preserveScroll=false}={}){
    ...
    try{
      ...
      if(error.code==='BANK_SCHEMA_MISSING') notice('Ejecuta en Supabase el fichero ControlEvent_SQL_V26_PROD_1_2_CUADRE_BANCO.sql actualizado.','warning',true);
    }finally{
