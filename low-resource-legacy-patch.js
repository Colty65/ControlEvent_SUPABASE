...
async function exportSeedWorkbook(){
  ...
  a.download = 'ControlEvent_v24_prod-02_descarga_datos.xlsx';
  a.click();
...
(function(){
  ...
  function excelFileName(ev){
    ...
    return `ControlEvent_v24_prod-02-${cleanFilePart(ev?.titulo || 'evento')}_${dd}${mm}${yyyy}.xlsx`;
  }
  function graphData(){
    const b = budgetSummary();
...
(function(){
  ...
  window.excelFileName = function(ev){
    ...
    const mm = String(d.getMonth()+1).padStart(2,'0');
    const yyyy = String(d.getFullYear());
    return `ControlEvent_v24_prod-02_INFOEVENTO-${title}_${yyyy}${mm}${dd}.xlsx`;
  };
...
(function(){
  ...
  function emittedByTextV171(date=new Date()){
    ...
    return `Emitido por “©oltyLAB ’26_ControlEvent_v24_prod-02_${dd}${mm}${yyyy}_${hh}:${mi}:${ss}”`;
  }
...
(function(){
  ...
  function normalizeDownloadName(name){
    let n = String(name || '');
    n = n.replace(/^ControlEvent_v\d+_\d+(?:_\d+)?/i, VERSION_FILE);
    n = n.replace(/ControlEvent_v24_prod-02_\d(?:_\d+)?/ig, VERSION_FILE);
    return n;
  }
...
(function(){
  'use strict';
  const VERSION='ControlEvent v24_prod-02';
  const SESSION_KEY='ControlEvent_v24_prod-02_session';
  const $=id=>document.getElementById(id);
