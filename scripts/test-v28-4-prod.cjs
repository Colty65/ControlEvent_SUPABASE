...
test('identidad central v28.4_prod',()=>{
  const v=read('public/app/version.js');
  assert(v.includes("VERSION = 'v28.4_prod'"));
  assert(v.includes("VERSION_TEXT = 'ControlEvent v28.4_prod'"));
  assert(v.includes("VERSION_FILE = 'ControlEvent_v28.4_prod'"));
  assert(v.includes("ZIP_NAME = 'ControlEvent_v28.4_prod.zip'"));
  const pkg=JSON.parse(read('package.json'));
  assert.equal(pkg.name,'controlevent-v28-4-prod');
  assert.equal(pkg.version,'28.4.0');
  assert(pkg.scripts['test:v28.4'].includes('test-v28-4-prod.cjs'));
});

test('migración conserva v28.3 como origen histórico',()=>{
  const v=read('public/app/version.js');
  assert(v.includes("'ControlEvent_v28.3_prod'"));
  assert(v.includes("oldKey.replace(prefix, 'ControlEvent_v28.4_prod')"));
});

test('INFOEVENTO y BACKUP externo/interno usan v28.4',()=>{
  const legacy=read('public/app/legacy/legacy-bundle-before-modules-v30.7.js');
  assert(legacy.includes('ControlEvent_v28.4_prod_INFOEVENTO-'));
  for(const rel of ['public/modules/excel/backup.js','routes/export.routes.js']){
    const s=read(rel);
    assert(s.includes("BACKUP_VERSION = 'ControlEvent v28.4_prod'"),rel);
    assert(s.includes("BACKUP_VERSION_FILE = 'ControlEvent_v28.4_prod'"),rel);
  }
});

test('hardlock final no destruye cabecera',()=>{
  const html=read('public/index.html'),lock=read('public/app/features/v28-4-prod-version-hardlock.js');
  assert(html.lastIndexOf('v28-4-prod-version-hardlock.js')>html.lastIndexOf('v28-4-prod-detail-globes.js'));
  ...
  assert(lock.includes('__ceV284VersionHardlock'));
...
test('Por destino conserva la lógica estable anterior',()=>{
  const s=read('public/app/features/v28-4-prod-detail-globes.js');
...
test('informe preconsultado usa una llamada de razonamiento sin tools ni previous_interaction_id',()=>{
  ...
  assert(chunk.includes("stage:'v28.4 · Gemini razonamiento único'"));
...
test('memoria conversacional usa cápsula compacta y no encadena Interactions',()=>{
  ...
  assert(front.includes("var previousInteractionId=''; // v28.4_prod"));
});
...
test('informe de movimientos conciliados es ruta directa sin Gemini',()=>{
  const s=read('services/event-ai.service.js');
  const a=s.indexOf('if(v283BankReportRequest(userPrompt,conversationHistory))'),b=s.indexOf('// v28.4_prod: un «sí / hazlo»',a),chunk=s.slice(a,b);
  assert(a>0&&b>a);
...
test('limpiar Zuzu reinicia también el total acumulado',()=>{
  ...
  assert(chunk.includes('window.__ceZuzuUsageTotalV284=emptyZuzuUsageTotal()'));
...
test('no se hardcodean entidades de la batería en la lógica nueva v28.4',()=>{
  const s=read('services/event-ai.service.js');
  const a=s.indexOf('// v28.4_prod · RUTAS DETERMINISTAS DE BAJO COSTE'),b=s.indexOf('/* Código histórico conservado',a),chunk=s.slice(a,b);
  assert(a>0&&b>a);
  for(const token of ['SySA 2026','FUNCION 2025','Pocholo','Carmelo','TK05','CUBATAS','ALMACEN']) assert(!chunk.includes(token),token);
});

console.log(`OK ${n} pruebas v28.4_prod`);
