...
test('identidad central v28.2_prod',()=>{
  const v=read('public/app/version.js');assert(v.includes("VERSION = 'v28.2_prod'"));assert(v.includes("VERSION_TEXT = 'ControlEvent v28.2_prod'"));assert(v.includes("VERSION_FILE = 'ControlEvent_v28.2_prod'"));assert(v.includes("ZIP_NAME = 'ControlEvent_v28.2_prod.zip'"));
  const pkg=JSON.parse(read('package.json'));assert.equal(pkg.name,'controlevent-v28-2-prod');assert.equal(pkg.version,'28.2.0');
});

test('migración conserva v28.1 como origen, no como versión activa',()=>{
  const v=read('public/app/version.js');assert(v.includes("legacyPrefixes = ['ControlEvent_v28.1_prod'"));assert(v.includes("oldKey.replace(prefix, 'ControlEvent_v28.2_prod')"));
});

test('INFOEVENTO y BACKUP externo/interno son v28.2',()=>{
  const legacy=read('public/app/legacy/legacy-bundle-before-modules-v30.7.js');assert(legacy.includes('ControlEvent_v28.2_prod_INFOEVENTO-'));
  for(const rel of ['public/modules/excel/backup.js','routes/export.routes.js']){const s=read(rel);assert(s.includes("BACKUP_VERSION = 'ControlEvent v28.2_prod'"),rel);assert(s.includes("BACKUP_VERSION_FILE = 'ControlEvent_v28.2_prod'"),rel);}
});

test('hardlock v28.2 queda al final sin destruir cabecera',()=>{
  const html=read('public/index.html'),lock=read('public/app/features/v28-2-prod-version-hardlock.js');
  assert(html.lastIndexOf('v28-2-prod-version-hardlock.js')>html.lastIndexOf('v28-2-prod-detail-globes.js'));
  ['controlevent-welcome-v44.png','headerDateTime','btnLogout'].forEach(x=>assert(html.includes(x),x));
  assert(!lock.includes("querySelectorAll('.appname,.appname span,.appname-stack"));assert(lock.includes("source:'v28-2-prod-version-hardlock.js'"));
});

test('globo Por destino conserva exactamente la lógica estable',()=>{
  const s=read('public/app/features/v28-2-prod-detail-globes.js');assert(s.includes('ControlEventV280RestoredGraphDetails'));assert(s.includes("const subset=all.filter(r=>norm(r.destino)===norm(destination)&&r.kind===k)"));assert(!s.includes('expandCompletePurchasedTickets'));assert(!s.includes('TOTAL TICKETS COMPLETOS MOSTRADOS'));
});
...
console.log(`OK ${n} pruebas v28.2_prod`);
