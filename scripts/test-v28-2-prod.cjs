...
test('identidad central v28.5.3_prod',()=>{
  const v=read('public/app/version.js');assert(v.includes("VERSION = 'v28.5.3_prod'"));assert(v.includes("VERSION_TEXT = 'ControlEvent v28.5.3_prod'"));assert(v.includes("VERSION_FILE = 'ControlEvent_v28.5.3_prod'"));assert(v.includes("ZIP_NAME = 'ControlEvent_v28.5.3_prod.zip'"));
...
test('migración conserva v28.1 como origen, no como versión activa',()=>{
  const v=read('public/app/version.js');assert(v.includes("legacyPrefixes = ['ControlEvent_v28.1_prod'"));assert(v.includes("oldKey.replace(prefix, 'ControlEvent_v28.5.3_prod')"));
});

test('INFOEVENTO y BACKUP externo/interno son v28.2',()=>{
  const legacy=read('public/app/legacy/legacy-bundle-before-modules-v30.7.js');assert(legacy.includes('ControlEvent_v28.5.3_prod_INFOEVENTO-'));
  for(const rel of ['public/modules/excel/backup.js','routes/export.routes.js']){const s=read(rel);assert(s.includes("BACKUP_VERSION = 'ControlEvent v28.5.3_prod'"),rel);assert(s.includes("BACKUP_VERSION_FILE = 'ControlEvent_v28.5.3_prod'"),rel);}
});
