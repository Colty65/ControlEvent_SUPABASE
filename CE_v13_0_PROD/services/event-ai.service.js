...
function systemPrompt(userPrompt, context) {
  ...
  const ctx = rawCtx;
  return `Eres Zuzu, la AnalÃ­tica libre integrada en ControlEvent, una aplicaciÃ³n de gestiÃ³n de eventos solidarios.

Arquitectura obligatoria ya ejecutada por ControlEvent:
1) Gemini/planificador ha deducido los mÃ³dulos necesarios.
2) ControlEvent ha extraÃ­do esos mÃ³dulos desde la app, en registros legibles y sin cÃ³digos internos.
3) Ahora recibes TODOS los registros entregados por esos mÃ³dulos y el prompt original del usuario. Tu trabajo es cocinar/formatear la respuesta final exactamente segÃºn lo pedido.

Reglas obligatorias:
- Usa exclusivamente modulosExtraidos y metricasCanonicas. No inventes datos ni completes huecos por intuiciÃ³n.
- La respuesta principal debe ser legible para usuario final: mÃ¡ximo 10-12 lÃ­neas de explicaciÃ³n; no pegues JSON, arrays ni listados brutos dentro de answer.
- Si hay muchos registros, resume y crea tablas de resumen. El detalle completo debe ir en tables o files, no en answer.
- Devuelve SIEMPRE JSON vÃ¡lido. No uses markdown fuera de los campos de texto. No cortes strings. Si no puedes generar todo, prioriza resumen + tablas cortas + aviso.
...
- Si el usuario menciona varios mÃ³dulos o conceptos, responde a todos: por ejemplo DONACIONES, COMPRAS, COLABORADORES/INGRESOS, TICKETS y DOCUMENTOS deben aparecer todos si los pidiÃ³.
- Si el usuario pide comparativa, crea una tabla comparativa por evento y por mÃ³dulo solicitado. No te quedes solo con el primer mÃ³dulo.
- Si pide agrupar, totalizar, calcular, ordenar, resumir o graficar, hazlo sobre TODOS los registros entregados del mÃ³dulo correspondiente, no sobre una muestra.
- Si el usuario pide una grÃ¡fica, devuelve al menos un objeto en charts. No digas que has pintado una grÃ¡fica si charts estÃ¡ vacÃ­o.
- Tipos de grÃ¡fica disponibles: bar, horizontalBar, pie, donut, line y stackedBar. Para comparativas entre eventos usa bar/stackedBar. Para repartos por tipo usa pie/donut. Para rankings largos usa horizontalBar.
...
- Para â€œproducto/artÃ­culo mÃ¡s utilizado comprado/donadoâ€, mide por Unidades, separando Comprado y Donado si el usuario lo pide.
...
- Si detectas que el contexto no contiene un mÃ³dulo necesario para responder, dilo claramente en warnings.
- Responde siempre en espaÃ±ol.

Campos oficiales por mÃ³dulo:
- INGRESOS: Evento; Nombre; Numero; Importe obligatorio; Importe voluntario; Ingreso; Rango; Just.ing.
- DONACIONES: Evento; Producto; Unidades; Precio; Valor; Tipo de donaciÃ³n; Donante; Responsable.
...
- TICKETS: Evento; TKxx; Tienda; Responsable; Total ticket; NÂº lÃ­neas; Ticket SI/NO; LÃ­neas contables.
...
Formato de salida: SOLO JSON vÃ¡lido con el esquema indicado. Evita respuestas excesivamente largas: usa tablas y ficheros para detalle cuando sea necesario.
LÃ­mites de presentaciÃ³n: answer <= 2500 caracteres; mÃ¡ximo 8 tablas; mÃ¡ximo 80 filas por tabla; mÃ¡ximo 8 grÃ¡ficas. Si necesitas devolver mÃ¡s detalle, usa files en CSV.
...
function isTransformAnalysisPrompt(prompt) {
  const p = norm(prompt);
  // Cuando el usuario pide operar sobre los datos, ControlEvent debe extraer los mÃ³dulos
  ...
  // cÃ¡lculos, comparativas, grÃ¡ficos o formatos pedidos por el usuario.
  return /\b(agrupa|agrupar|agrupado|agrupados|agrupacion|agrupaciÃ³n|totaliza|totalizar|totalizado|subtotal|subtotales|suma|sumar|sumatorio|calcula|calcular|calculo|cÃ¡lculo|media|promedio|porcentaje|porcentajes|ratio|ranking|ordena|ordenar|filtra|filtrar|resume|resumen|resumir|analiza|analisis|anÃ¡lisis|compara|comparar|comparativa|evolucion|evoluciÃ³n|tendencia|grafica|grÃ¡fica|grafico|grÃ¡fico|diagrama|tabla dinamica|tabla dinÃ¡mica|desglose|desglosa|desglosar)\b/.test(p);
}
function isListExtractionPrompt(prompt) {
  const p = norm(prompt);
  if (isTransformAnalysisPrompt(prompt)) return false;
  return /\b(lista|listado|relacion|relaciÃ³n|detalle|detallame|detalla|dame|muestra|muÃ©strame|ensena|enseÃ±a|ver|cuales|cuÃ¡les|todos|todas)\b/.test(p);
}
function directModuleResultIfApplicable(prompt, context) {
  ...
  if (/\bingreso|ingresos|recaudacion|recaudaciÃ³n|asistente|asistentes|entrada|entradas|colaborador|colaboradores|socio|socios\b/.test(p) && Array.isArray(mods.INGRESOS)) priority.push('INGRESOS');
  ...
  if (/\bevento|eventos|fecha|estado|situacion|situaciÃ³n\b/.test(p) && Array.isArray(mods.EVENTOS)) priority.push('EVENTOS');
  if (/\bproducto|productos|catalogo|catÃ¡logo\b/.test(p) && Array.isArray(mods.PRODUCTOS)) priority.push('PRODUCTOS');
  ...
  return {
    ...
    warnings: arr(context.advertencias).concat(rows.length ? [] : [`El mÃ³dulo ${moduleName} no tiene registros con los filtros solicitados.`]),
    charts: [],
...
function detectGroupField(moduleName, prompt) {
  ...
  if (moduleName === 'DONACIONES') {
    if (/\btipo(?:\s+de)?\s+donaci|donado\s+socio|donado\s+no\s+socio|donado\s+otros|donado\s+tienda\b/.test(p)) return 'Tipo de donaciÃ³n';
...
function detectAggregateModule(prompt, mods) {
  ...
  if (/\bingreso|ingresos|recaudacion|recaudaciÃ³n|asistente|asistentes|entrada|entradas|colaborador|colaboradores|socio|socios\b/.test(p) && Array.isArray(mods.INGRESOS)) return 'INGRESOS';
  return '';
}
...
function auditRowsForAggregate(moduleName, rows, ag, audit, context) {
  ...
  return [
    ['MÃ³dulo usado', moduleName],
    ['Evento(s) detectado(s)', eventos || 'No indicado'],
    ['Registros extraÃ­dos del mÃ³dulo', String(rows.length)],
    ...
    ['Motor de cÃ¡lculo', 'ControlEvent local, sin Gemini para sumas/agrupaciones']
  ];
}
...
function directAggregateResultIfApplicable(prompt, context) {
  if (!context || context.needsClarification) return null;
  const p = norm(prompt);
  // DiagnÃ³stico fiable: detecciÃ³n amplia, sin depender de que Gemini interprete el prompt.
  // Captura formas con y sin acento: agrÃºpalas/agrupar/agrupado, totalÃ­zalas/totalizar,
  // suma, subtotales, desglose, etc.
  if (!/(agrup|totaliz|subtotal|subtot|sumator|sumar|suma|desglos|calcula|calculo|cÃ¡lculo|conteo|contar|recuento)/.test(p)) return null;
  const mods = context.modulosExtraidos || {};
  ...
  if (!rows.length) {
    return {
      ...
      answer: `ControlEvent no puede agrupar porque el mÃ³dulo ${moduleName} ha entregado 0 registros${eventos ? ` para ${eventos}` : ''}.`,
      warnings: [audit ? `AuditorÃ­a ${moduleName}: fuente sin filtros ${audit.registrosFuenteSinFiltros}, entregados ${audit.registrosEntregados}.` : `El mÃ³dulo ${moduleName} no tiene registros.`],
  ...
  groupedColumns.push(`Total ${ag.valueColumn} (â‚¬)`);
  ...
  const groupedCsvRows = ag.groups.map(g => {
    const row = { [ag.groupField]: g.key, Registros: g.registros, [`Total ${ag.valueColumn} (â‚¬)`]: round(g.total, 2), '% sobre total': ag.totalGeneral ? `${round((num(g.total) * 100) / ag.totalGeneral, 2)} %` : '0 %' };
    if (groupedColumns.includes('Unidades')) row.Unidades = round(g.unidades, 3);
    return row;
  });
  const groupedCsvColumns = groupedColumns;
  const warningSynonyms = /donado\s+no\s+socio/.test(p) && moduleName === 'DONACIONES' ? ['En los datos reales el tipo equivalente a â€œDONADO NO SOCIOâ€ suele venir como â€œDONADO OTROSâ€; se agrupa por el valor real del campo Tipo de donaciÃ³n.'] : [];
  ...
  return {
    ...
    answer: `AgrupaciÃ³n por ${ag.groupField}. Total general: ${ag.totalGeneral} â‚¬.` ,
    warnings: arr(context.advertencias).concat(warningSynonyms),
    charts: [{ title: `${moduleName} por ${ag.groupField} (cÃ¡lculo local ControlEvent)`, type: /\btarta|pie\b/.test(p) ? 'pie' : 'bar', labels: ag.groups.map(g => g.key).slice(0, 30), values: ag.groups.map(g => round(g.total, 2)).slice(0, 30), unit: 'â‚¬' }],
    tables: [
...
function directComparativeAllDataResultIfApplicable(prompt, context) {
  ...
  const rows = events.map(eventName => {
    ...
    return {
      ...
      'Ingresos total (â‚¬)': importeIngresos,
      'Compras lÃ­neas': can['Compras registros'] ?? com.length,
      'Compras realizadas (â‚¬)': importeCompras,
      'Compras pendientes (â‚¬)': round(can['Compras pendientes'] ?? 0, 2),
      'Donaciones lÃ­neas': can['Donaciones registros'] ?? don.length,
      'Donaciones valor (â‚¬)': valorDonaciones,
      TKxx: can['Tickets numero'] ?? tk.length,
      'TKxx total (â‚¬)': totalTk,
      Documentos: can['Documentos numero'] ?? doc.length,
      'Saldo actual ingresos - compras (â‚¬)': round(can['Saldo actual'] ?? (importeIngresos - importeCompras), 2),
      'ValoraciÃ³n compras + donaciones (â‚¬)': round(can['Valoracion con donaciones'] ?? (importeCompras + valorDonaciones), 2)
    };
  });

  const columns = ['Evento','Colaboradores','Asistentes / Numero','Just.ing SI','Ingresos total (â‚¬)','Compras lÃ­neas','Compras realizadas (â‚¬)','Compras pendientes (â‚¬)','Donaciones lÃ­neas','Donaciones valor (â‚¬)','TKxx','TKxx total (â‚¬)','Documentos','Saldo actual ingresos - compras (â‚¬)','ValoraciÃ³n compras + donaciones (â‚¬)'];
  ...
  const auditRows = [
    ...
    ['MÃ³dulos usados', ['INGRESOS','COMPRAS','DONACIONES','TICKETS','DOCUMENTOS'].filter(m => Array.isArray(mods[m])).join(', ')],
    ['Motor de cÃ¡lculo', 'ControlEvent local, sin Gemini para selecciÃ³n de eventos ni sumas']
  ];
  const charts = [
    { title: 'Ingresos total por evento', type: 'bar', labels: events, values: rows.map(r => round(r['Ingresos total (â‚¬)'], 2)), unit: 'â‚¬' },
    { title: 'Compras total por evento', type: 'bar', labels: events, values: rows.map(r => round(r['Compras realizadas (â‚¬)'], 2)), unit: 'â‚¬' },
    { title: 'Donaciones valor por evento', type: 'bar', labels: events, values: rows.map(r => round(r['Donaciones valor (â‚¬)'], 2)), unit: 'â‚¬' },
...
function quotedNames(prompt) {
  const out = [];
  const re = /["â€œâ€']([^"â€œâ€']{2,120})["â€œâ€']/g; let m;
...
function directEventPriceExtremesIfApplicable(prompt, context) {
  ...
  const tableRows = [
    ['MÃ¡s barato', barato?.['Titulo del evento'] || '', String(round(barato?.Precio,2)), text(barato?.['fecha ini']), text(barato?.['fecha fin']), text(barato?.Estado)],
    ['MÃ¡s costoso', caro?.['Titulo del evento'] || '', String(round(caro?.Precio,2)), text(caro?.['fecha ini']), text(caro?.['fecha fin']), text(caro?.Estado)]
  ];
  const warnings = positive.length && positive.length < rows.length ? ['Se han ignorado eventos con precio 0 para no confundir â€œsin precio definidoâ€ con el evento mÃ¡s barato.'] : [];
  return { ok:true, rejected:false, title:'Precio de eventos', answer:`ControlEvent ha revisado ${rows.length} evento(s) y ha calculado localmente el mÃ¡s barato y el mÃ¡s costoso.`, warnings, charts:[{title:'Precio de eventos extremos', type:'bar', labels:['MÃ¡s barato','MÃ¡s costoso'], values:[round(barato?.Precio,2), round(caro?.Precio,2)], unit:'â‚¬'}], tables:[{title:'Evento mÃ¡s barato y mÃ¡s costoso', columns, rows: tableRows}], files:[{filename:fileSafe('EVENTOS_precios_extremos_v13.0_prod.csv'), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, tableRows.map(r=>Object.fromEntries(columns.map((c,i)=>[c,r[i]]))))}], provider:'control-event-local-eventos-precios', model:'sin-gemini-para-calculos' };
}
function directPersonAppearanceIfApplicable(prompt, context) {
  const p = norm(prompt);
  if (!/\b(busca|buscar|aparece|aparecen|cuantos|cuÃ¡ntos|revisa)\b/.test(p) || !/\b(persona|colaborador|colaboradores|responsable|responsables|donante|donantes)\b/.test(p)) return null;
  ...
  return { ok:true, rejected:false, title:`Apariciones de ${needle}`, answer:`ControlEvent ha buscado a â€œ${needle}â€ en todos los mÃ³dulos disponibles: colaboradores/ingresos, responsables de compras y responsables de donaciones${/\bdonante|donantes\b/.test(p)?', y donantes de donaciones':''}. Aparece en ${rows.length} evento(s).`, warnings:[], charts:[{title:`Eventos donde aparece ${needle}`, type:'bar', labels:rows.map(r=>r.Evento), values:rows.map(r=>r.Colaborador+r['Responsable compras']+r['Responsable donaciones']+r['Donante donaciones']), unit:'apariciones'}], tables:[{title:`Apariciones de ${needle} por evento`, columns, rows: rows.map(r=>columns.map(c=>text(r[c])))}], files:[{filename:fileSafe(`Apariciones_${needle}_v13.0_prod.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, rows)}], provider:'control-event-local-busqueda-persona', model:'sin-gemini-para-busquedas' };
}
function directBoughtDonatedUsageIfApplicable(prompt, context) {
  const p = norm(prompt);
  if (!/\b(comprado\s*\/\s*donado|comprado\s+y\s+donado|compras?\s+y\s+donaciones?|donaciones?\s+y\s+compras?|separa\s+comprado|mas\s+utilizado|mÃ¡s\s+utilizado)\b/.test(p)) return null;
  ...
  const map = new Map();
  function rec(prod){ const k=trim(prod)||'Sin producto'; if(!map.has(k)) map.set(k,{Producto:k,'Unidades compradas':0,'Unidades donadas':0,'Total unidades':0,'Importe comprado (â‚¬)':0,'Valor donado (â‚¬)':0}); return map.get(k); }
  compras.forEach(r=>{ const o=rec(r.Producto); o['Unidades compradas']+=num(r.Unidades); o['Total unidades']+=num(r.Unidades); o['Importe comprado (â‚¬)']+=num(r.Importe); });
  donaciones.forEach(r=>{ const o=rec(r.Producto); o['Unidades donadas']+=num(r.Unidades); o['Total unidades']+=num(r.Unidades); o['Valor donado (â‚¬)']+=num(r.Valor); });
  const limit = firstIntInPrompt(prompt, 25);
  const rows = [...map.values()].map(r=>({ ...r, 'Unidades compradas':round(r['Unidades compradas'],3), 'Unidades donadas':round(r['Unidades donadas'],3), 'Total unidades':round(r['Total unidades'],3), 'Importe comprado (â‚¬)':round(r['Importe comprado (â‚¬)'],2), 'Valor donado (â‚¬)':round(r['Valor donado (â‚¬)'],2)})).sort((a,b)=>num(b['Total unidades'])-num(a['Total unidades']) || String(a.Producto).localeCompare(String(b.Producto),'es'));
  const eventos = eventNamesFromContext(context).join(', ');
  const columns = ['Producto','Unidades compradas','Unidades donadas','Total unidades','Importe comprado (â‚¬)','Valor donado (â‚¬)'];
  const shown = rows.slice(0, limit);
  return { ok:true, rejected:false, title:`Productos mÃ¡s utilizados${eventos?` - ${eventos}`:''}`, answer:`ControlEvent ha unido COMPRAS y DONACIONES y ha calculado localmente el producto mÃ¡s utilizado por unidades, separando Comprado y Donado.`, warnings:[], charts:[{title:'Top productos por unidades compradas + donadas', type:'bar', labels:shown.slice(0,30).map(r=>r.Producto), values:shown.slice(0,30).map(r=>r['Total unidades']), unit:'uds'}], tables:[{title:`Top ${shown.length} productos por unidades`, columns, rows:shown.map(r=>columns.map(c=>text(r[c])))}], files:[{filename:fileSafe(`Productos_comprado_donado_${eventos||'ControlEvent'}_v13.0_prod.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, rows)}], provider:'control-event-local-comprado-donado', model:'sin-gemini-para-calculos' };
}
function directProductCatalogIfApplicable(prompt, context) {
  const p = norm(prompt);
  const rows0 = arr(context?.modulosExtraidos?.PRODUCTOS);
  if (!rows0.length || !/\b(producto|productos|ce_productos|catalogo|catÃ¡logo|segmento|destino|precio\s+rfa|precio\s+referencia)\b/.test(p)) return null;
  let rows = rows0.slice();
  const top = /\b(mas\s+caros|mÃ¡s\s+caros|mayor\s+precio|top)\b/.test(p);
  ...
  if (/\b(agrupa|agrupad|agrupados|agrupadas|por\s+segmento|por\s+destino)\b/.test(p)) {
    const groups = new Map();
    rows.forEach(r=>{ const key=`${trim(r.Segmento)||'Sin segmento'} / ${trim(r.Destino)||'Sin destino'}`; const g=groups.get(key)||{Grupo:key, Productos:0,'Precio mÃ­nimo':Infinity,'Precio mÃ¡ximo':0,'Precio medio':0, _sum:0}; g.Productos++; const price=num(r['Precio rfa.']); g['Precio mÃ­nimo']=Math.min(g['Precio mÃ­nimo'],price); g['Precio mÃ¡ximo']=Math.max(g['Precio mÃ¡ximo'],price); g._sum+=price; groups.set(key,g); });
    const groupRows=[...groups.values()].map(g=>({Grupo:g.Grupo, Productos:g.Productos, 'Precio mÃ­nimo':g['Precio mÃ­nimo']===Infinity?0:round(g['Precio mÃ­nimo'],2), 'Precio mÃ¡ximo':round(g['Precio mÃ¡ximo'],2), 'Precio medio':round(g._sum/g.Productos,2)})).sort((a,b)=>String(a.Grupo).localeCompare(String(b.Grupo),'es'));
    const gcols=['Grupo','Productos','Precio mÃ­nimo','Precio mÃ¡ximo','Precio medio'];
  ...
  return { ok:true, rejected:false, title:'PRODUCTOS extraÃ­do por ControlEvent', answer:`ControlEvent ha consultado el catÃ¡logo de productos con filtros exactos y cÃ¡lculo local. Registros entregados: ${rows.length}.`, warnings:[], charts: top ? [{title:`Top ${shown.length} productos por precio rfa.`, type:'bar', labels:shown.slice(0,30).map(r=>r['Nombre producto']), values:shown.slice(0,30).map(r=>round(r['Precio rfa.'],2)), unit:'â‚¬'}] : [], tables, files:[{filename:fileSafe('PRODUCTOS_catalogo_v13.0_prod.csv'), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, rows)}], provider:'control-event-local-productos', model:'sin-gemini-para-catalogos' };
}
...
function directComparativeModuleTotalsIfApplicable(prompt, context) {
  ...
  return {ok:true,rejected:false,title,answer:`ControlEvent ha comparado estrictamente ${moduleName} entre los eventos citados. No se han mezclado otros eventos.`,warnings:[],charts:[{title,type:'bar',labels:out.map(r=>r.Evento),values:out.map(r=>r.Total),unit:'â‚¬'}],tables:[{title,columns:cols,rows:out.map(r=>cols.map(c=>text(r[c])))}],files:[{filename:fileSafe(`${moduleName}_comparativa_eventos_v13.0_prod.csv`),mime:'text/csv;charset=utf-8',content:csvFromRows(cols,out)}],provider:'control-event-local-comparativa-modulo',model:'sin-gemini-para-calculos'};
}
function directDeterministicResultIfApplicable(prompt, context) {
  if (!context || context.needsClarification) return null;
  // Fase de diagnÃ³stico: todo lo que sea peticiÃ³n de datos de mÃ³dulos se resuelve primero y, salvo anÃ¡lisis libre puro,
  // se devuelve desde ControlEvent para poder auditar si los mÃ³dulos sirven todos los registros.
  ...
  const auditRows = [
    ...
    ['Motor', 'ControlEvent local, diagnÃ³stico de mÃ³dulos']
  ];
  return {
    ok: true,
    rejected: false,
    title: `${first} extraÃ­do por ControlEvent`,
    answer: `ControlEvent ha extraÃ­do ${rows.length} registro(s) del mÃ³dulo ${first}${eventos ? ` para ${eventos}` : ''}. En esta fase de diagnÃ³stico no se usa Gemini para transformar los datos; se prioriza comprobar que el mÃ³dulo devuelve los registros correctos y legibles.`,
    warnings: arr(context.advertencias),
    charts: [],
    tables: [
      { title: 'AuditorÃ­a de extracciÃ³n', columns: ['Dato','Valor'], rows: auditRows },
...
function directGraphResultIfApplicable(prompt, context) {
  if (!context || context.needsClarification) return null;
  const p = norm(prompt);
  if (!/\b(grafica|grÃ¡fica|grafico|grÃ¡fico|diagrama|barras|tarta)\b/.test(p)) return null;
  const mods = context.modulosExtraidos || {};
  let moduleName = '';
  ...
  else if (/\bingreso|ingresos|recaudacion|recaudaciÃ³n|asistente|asistentes|entrada|entradas|colaborador|colaboradores|socio|socios\b/.test(p) && Array.isArray(mods.INGRESOS)) moduleName = 'INGRESOS';
  if (!moduleName) return null;
  const rows = arr(mods[moduleName]);
  const eventos = arr(context.eventosObjetivo).map(e => trim(e?.['Titulo del evento'] || e?.Titulo || e?.EVENTO || e?.Evento)).filter(Boolean).join(', ');
  const audit = arr(context.auditoriaModulos).find(a => a.modulo === moduleName);
  if (!rows.length) {
    return {
      ok: true,
      rejected: false,
      title: `GrÃ¡fica de ${moduleName}`,
      answer: `ControlEvent no ha podido generar la grÃ¡fica porque el mÃ³dulo ${moduleName} ha entregado 0 registros${eventos ? ` para ${eventos}` : ''}.`,
      warnings: [audit ? `AuditorÃ­a ${moduleName}: fuente sin filtros ${audit.registrosFuenteSinFiltros}, entregados ${audit.registrosEntregados}, filtros ${audit.filtrosAplicados ? JSON.stringify(audit.filtros) : 'NO'}.` : `El mÃ³dulo ${moduleName} no tiene registros.`],
  ...
  return {
    ok: true,
    rejected: false,
    title: `GrÃ¡fica de ${moduleName}${eventos ? ` - ${eventos}` : ''}`,
    answer: `GrÃ¡fica por ${g.groupField} con ${rows.length} registro(s).`,
    warnings: arr(context.advertencias),
    charts: [{ title: `${moduleName} por ${g.groupField}`, type: /\btarta|pie\b/.test(p) ? 'pie' : 'bar', labels: g.labels, values: g.values, unit: 'â‚¬' }],
...
function normalizeResult(raw, model) {
  ...
  const charts = arr(out.charts).map(ch => {
    ...
    return { title: trim(ch.title || 'GrÃ¡fica'), type, labels, values, series, unit: trim(ch.unit || '') };
...
async function callGeminiEvent(prompt, context) {
  ...
  for (const model of configuredGeminiModels()) {
    ...
    try {
      ...
      if (!outText) throw new Error('Gemini no devolviÃ³ texto analizable.');
      let parsed;
      try { parsed = JSON.parse(stripJsonText(outText)); }
        ...
        // con los datos canÃ³nicos y una advertencia.
        ...
        if (fallback) {
          fallback.warnings = arr(fallback.warnings).concat('Gemini no devolviÃ³ JSON estructurado vÃ¡lido; se ha usado una salida estructurada de ControlEvent para no mostrar datos crudos ni desformateados.');
        ...
        return {
          ...
          answer: 'Gemini no devolviÃ³ un JSON vÃ¡lido. ControlEvent ha evitado mostrar la respuesta cruda para no entregar una pantalla ilegible. Repite la consulta de forma algo mÃ¡s concreta o revisa la cuota/modelo de Gemini.',
          warnings: ['Gemini no devolviÃ³ JSON estructurado vÃ¡lido y no hubo una salida local aplicable.'],
          charts: [],
          tables: [],
...
function plannerPrompt(userPrompt, catalog) {
  ...
  return `Eres el planificador seguro de Zuzu para ControlEvent. Tu Ãºnica tarea es decidir quÃ© mÃ³dulos de extracciÃ³n debe usar ControlEvent para responder bien al usuario. NO respondas la pregunta final.

MÃ³dulos disponibles:
- INGRESOS: colaboradores, ingresos, recaudaciÃ³n, asistentes, socios/no socios, justificantes de ingreso.
...
- PRODUCTOS: catÃ¡logo de productos, segmento, destino, precio de referencia.
- TIENDAS: catÃ¡logo de tiendas.
- PERSONAS: catÃ¡logo de personas, rango.

Reglas de planificaciÃ³n:
- Si el prompt menciona explÃ­citamente un mÃ³dulo o uno de sus sinÃ³nimos, inclÃºyelo.
- Si el prompt pide una comparativa con varios conceptos, incluye TODOS esos mÃ³dulos, no solo el primero.
- Si pide "colaboradores", "asistentes", "justificantes de ingreso" o "recaudaciÃ³n", incluye INGRESOS.
- Si pide "comprado/donado" o "separa comprado y donado", incluye COMPRAS y DONACIONES.
- Si pide "tickets", incluye TICKETS y, si pide importes o lÃ­neas, tambiÃ©n COMPRAS.
- Si pide "documentos adjuntos", incluye DOCUMENTOS y EVENTOS.
- Si cita eventos entre comillas, pon sus tÃ­tulos en eventos exactamente como aparecen en el prompt o como mejor coincidan con el catÃ¡logo.
- Si dice "todos los eventos", "eventos registrados" o "entre todos los eventos", todosLosEventos=true.
- De momento NO propongas filtros de persona, producto, tienda, responsable, donante, segmento ni destino. ControlEvent traerÃ¡ todos los registros de cada mÃ³dulo y la respuesta final se filtrarÃ¡ en la segunda llamada a Gemini usando el prompt original.
- needsClarification=true solo si no puedes identificar ningÃºn mÃ³dulo Ãºtil. No pidas concreciÃ³n por volumen.
...
async function callGeminiPlanner(userPrompt, catalog) {
  ...
  for (const model of configuredGeminiModels()) {
    ...
    try {
      ...
      const outText = trim(geminiOutText(payload));
      if (!outText) throw new Error('Planificador no devolviÃ³ texto.');
...
async function buildZuzuPlan(userPrompt, state, selectedEventId) {
  ...
  try {
    ...
    return {
      ...
      filters: {},
      reasoning: trim(ai?.reasoning || '') || 'Planificador Gemini: mÃ³dulos deducidos desde el prompt. ControlEvent extrae todos los registros de esos mÃ³dulos sin filtros de reducciÃ³n.',
    ...
    return {
      ...local,
      filters: {},
      reasoning: `${local.reasoning || 'Plan local de respaldo.'} Aviso: el planificador Gemini no respondiÃ³ (${trim(error?.message || error)}).`,
...
export async function analyzeEventPrompt({ prompt, selectedEventId, stateOverride } = {}) {
  ...
  if (!userPrompt) {
    const err = new Error('Escribe una pregunta o peticiÃ³n para Gemini.');
    err.status = 400;
    throw err;
  }
  if (userPrompt.length > 3000) {
    const err = new Error('El prompt es demasiado largo. Resume la peticiÃ³n.');
    err.status = 413;
    throw err;
  }
  const forbidden = /(contraseÃ±a|password|clave api|api key|token|sql|drop table|delete from|insert into|hack|exfiltra|sistema operativo|receta|chiste|horÃ³scopo|fÃºtbol|politic[ao]s?)/i;
  const eventish = /(evento|eventos|compra|compras|donaci[oÃ³]n|donaciones|ingreso|ingresos|producto|productos|ticket|tk\d+|tienda|responsable|socio|donante|colaborador|gr[aÃ¡]fica|estad[iÃ­]stica|presupuesto|segmento|destino|coste|cantidad|valoraci[oÃ³]n|recurso|mapa|resumen|compar)/i;
  if (forbidden.test(userPrompt) && !eventish.test(userPrompt)) {
    return { ok: true, rejected: true, title: 'PeticiÃ³n rechazada', answer: 'La peticiÃ³n no parece relacionada con la gestiÃ³n de eventos de ControlEvent.', warnings: [], charts: [], tables: [], files: [], provider: 'local-guard', model: '' };
  }
  ...
  // 1) planificaciÃ³n de mÃ³dulos, 2) extracciÃ³n oficial por ControlEvent, 3) respuesta final con Gemini.
  // Las respuestas locales quedan solo como respaldo si Gemini falla.
  if (context?.needsClarification) {
    return {
      ok: true,
      rejected: true,
      title: 'Zuzu necesita una peticiÃ³n mÃ¡s concreta',
      answer: context.clarification || 'Debes ser mÃ¡s concreto en tu peticiÃ³n. Piensa un poco mÃ¡s lo que quieres.',
