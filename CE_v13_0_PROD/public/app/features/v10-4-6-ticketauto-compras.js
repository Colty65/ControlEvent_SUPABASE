...
(function(){
  ...
  function openPanel(){
    ...
    // v10.4.6: primera apertura siempre limpia y reconstruida. Evita panel congelado en iPad/iPhone/PC
  ...
  function aiPendingSimilar(pname, a){
    ...
    if(!pTok.length || !aTok.length) return false;
    var common=commonTokens(pTok,aTok);
    var commonDistinct=common.filter(function(t){ return !isGenericProductToken(t); });
    var priceOk=(closePrice(a.pendingUnit||0,a.price) || closePrice(a.pendingTotal||0,a.total) || closePrice(a.pendingTotal||0,a.price));
    // Palabras cortas tipo SAL solo coinciden si son palabra exacta en ambos lados. SAL no coincide con ROSAL.
    if(common.indexOf('SAL')>=0) return hasToken('SAL',pTok) && hasToken('SAL',aTok);
    // TÃ© al limÃ³n: exige TE/TEA y otro rasgo comÃºn; asÃ­ FANTA LIMON no se empareja con TE AL LIMON.
    if((hasToken('TEA',pTok)||hasToken('TEA',aTok)) && hasToken('LIMON',common)) return true;
    // Una marca/producto distintivo exacto ya vale: ACUARIUS, SPRITE, PIMENTON, HUEVOS, BUTANO, etc.
    if(hasStrongSingleMatch(commonDistinct)) return true;
    if(commonDistinct.length>=2) return true;
    if(commonDistinct.length>=1 && priceOk) return true;
    // La similitud difusa solo se permite si ademÃ¡s hay una palabra real comÃºn, no solo precio o letras internas.
    var sc=productSimilarityScore(pname, a.name);
    return sc>=88 && commonDistinct.length>=1;
  }
  ...
  console.info('[CE v10.4.6 Alta IA] producto similar, candidatos Pte.Compra, segmento/destino editables y actualizaciÃ³n PRODUCTOS instalada. Prueba: window.__ceOpenTicketAutoV104()');
})();
