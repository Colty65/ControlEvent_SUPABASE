...
(function(){
  ...
  function upsertProductByName(row, tiendaId, warnings){
    ...
    if(existing){
      // v9.5.2: ruta dedicada. Solo refresca Ãºltimo precio conocido y tienda habitual en PRODUCTOS.
      return apiJson('/api/crud/productos/'+encodeURIComponent(existing.id)+'/precio',{method:'PUT',headers:crudHeaders(),body:JSON.stringify({__crudRowOnly:true,defaultPrecio:price,precio:price,defaultTiendaId:tiendaId||existing.defaultTiendaId||'',tiendaId:tiendaId||existing.tiendaId||''})})
        ...
        .catch(function(err){
          warnings.push('No se actualizÃ³ precio de PRODUCTOS para "'+name+'": '+(err.message||err)+'. La compra se grabarÃ¡ con el precio del ticket.');
