...
(function(){
  ...
  function openPanel(){
    ...
    // Reentrada limpia: si quedÃ³ un panel anterior a medias por una recarga dura, lo retiramos y lo reconstruimos.
    var oldPanel=$('ceAiTicketPanel');
    if(oldPanel && (!oldPanel.querySelector('#ceAiFile') || !oldPanel.querySelector('#ceAiRows') || !oldPanel.querySelector('#ceAiClose'))){ try{ oldPanel.remove(); }catch(_){} }
  ...
  function productSimilarityScore(input, productName){
    ...
    if(ca===cb) return 98;
    if(ca.length>=4 && cb.indexOf(ca)>=0) return 92;
    if(cb.length>=4 && ca.indexOf(cb)>=0) return 90;
    var d=lev(a,b), max=Math.max(ca.length,cb.length)||1;
    var score=Math.round((1-(d/max))*100);
    var toks=a.split(/[^A-Z0-9]+/).filter(function(x){return x.length>2;});
    var hits=0; toks.forEach(function(t){ if(b.indexOf(t)>=0 || cb.indexOf(t)>=0) hits++; });
    if(toks.length) score=Math.max(score, Math.round((hits/toks.length)*88));
    return score;
  }
  ...
  console.info('[CE v10.4.2 Alta IA] producto similar, candidatos Pte.Compra, segmento/destino editables y actualizaciÃ³n PRODUCTOS instalada. Prueba: window.__ceOpenTicketAutoV104()');
})();
