...
(function(){
  'use strict';
  var TAG='__ceV1042TicketAutoComprasSmartProducts';
  ...
  function userRole(){ var u=authObj(); return trim(u && u.nivel).toUpperCase(); }
  function isGD(){ return userRole()==='GD'; }
  function isRW(){ return userRole()==='RW'; }
  function canWriteCompras(){ var r=userRole(); return r==='GD' || r==='RW'; }
  ...
  function ensureUi(){
    ...
    if(!$('ceAiTicketPanel')){
      ...
      div.innerHTML='<div class="ce-ai-modal">'+
        '<div class="ce-ai-head"><div class="ce-ai-title">ðŸ§¾âœ¨ Alta asistida de COMPRAS</div><div id="ceAiStatus" class="ce-ai-status info"></div><button type="button" id="ceAiClose" class="ce-ai-secondary">Cerrar</button></div>'+ 
        '<div class="ce-ai-work">'+
          ...
          '<section class="ce-ai-right">'+
            ...
            '</div>'+ 
            '<div class="ce-ai-hintbox"><textarea id="ceAiGeminiHint" placeholder="Ej.: responsable Juan PÃ©rez; aÃ±ade PROPINAS 1 ud 73,05 â‚¬; aplica IVA general a las lÃ­neas si procede."></textarea></div>'+
            '<div class="ce-ai-actions ce-ai-actions-top"><button type="button" id="ceAiAnalyze" class="ce-ai-primary ce-ai-icon-btn" aria-label="Analizar foto con IA"><span aria-hidden="true">ðŸ¤–</span></button><button type="button" id="ceAiAddRow" class="ce-ai-secondary ce-ai-icon-btn" aria-label="AÃ±adir fila manual"><span aria-hidden="true">ðŸ¤šï¼‹</span></button><button type="button" id="ceAiClear" class="ce-ai-danger ce-ai-icon-btn" aria-label="Limpiar"><span aria-hidden="true">ðŸ§¹</span></button></div>'+ 
  ...
  function openPanel(){
    if(!canWriteCompras()){ alert('Esta funciÃ³n estÃ¡ disponible para usuarios GD o RW.'); return; }
    ...
    // v10.4.5: primera apertura siempre limpia y reconstruida. Evita panel congelado en iPad/iPhone/PC
    ...
    setTimeout(function(){ try{ var f=$('ceAiFile'); if(f){ f.disabled=false; f.removeAttribute('disabled'); f.style.pointerEvents='auto'; f.style.visibility='visible'; f.style.opacity='1'; } var c=$('ceAiClose'); if(c){ c.disabled=false; c.removeAttribute('disabled'); } }catch(_){} }, 40);
  }
  ...
  function renderRows(){
    ...
    body.innerHTML=rows.map(function(r,i){
      ...
      var attrTitle=existing ? 'Producto existente: puedes corregir SEGMENTO/DESTINO; al procesar se actualizarÃ¡n PRODUCTOS junto con el precio.' : 'Producto nuevo: informa segmento y destino antes de confirmar.';
      return '<tr class="'+cls+'" data-ce-ai-row="'+i+'" data-ce-ai-conf="'+htmlEscape(r.confianza||0)+'" data-ce-ai-existing="'+(existing?'1':'0')+'">'+
        ...
        '<td><input list="ceAiProducts" data-ce-ai-field="descripcion" value="'+htmlEscape((existing && existing.nombre) || r.descripcion || '')+'" placeholder="Nombre producto"></td>'+ 
        ...
        '<td><button type="button" class="ce-ai-danger ce-ai-trash-btn" data-ce-ai-del="'+i+'" aria-label="Eliminar lÃ­nea" title="Eliminar lÃ­nea">ðŸ—‘ï¸</button></td>'+ 
      '</tr>';
    }).join('');
  ...
  function enrichRowDefaults(row){
    ...
    if(p){
      r.descripcion=p.nombre||r.descripcion;
      if(!trim(r.segmento)) r.segmento=p.segmento||'';
      if(!trim(r.destino)) r.destino=p.destino||'';
      r.productoIdDetectado=p.id||'';
      r.nombreDetectadoOriginal=row && row.descripcion && normalizeName(row.descripcion)!==normalizeName(p.nombre) ? row.descripcion : '';
    } else {
  ...
  var CE_PENDING_STOP={DE:1,DEL:1,LA:1,EL:1,LOS:1,LAS:1,UN:1,UNA:1,UD:1,UDS:1,KG:1,KGS:1,GR:1,G:1,ML:1,L:1,LITRO:1,LITROS:1,BOLSA:1,BOTE:1,PACK:1,PAQ:1,PK:1,CAJA:1,DOCE:1,DOCENA:1,MEDIA:1,MEDIO:1,PARA:1,CON:1,SIN:1};
  function pendingTokens(v){ return normalizePlain(v).split(/\s+/).filter(function(t){ return t.length>=4 && !CE_PENDING_STOP[t] && !/^\d+$/.test(t); }); }
  ...
  function currentAiProductRows(){
    ...
    (window.__ceAiTicketLines||[]).forEach(function(r){
      ...
      if(d) rows.push({name:d, units:u, price:p, total:im});
      if(od && normalizeName(od)!==normalizeName(d)) rows.push({name:od, units:u, price:p, total:im});
    });
    return rows;
  }
  ...
  function sameTicketStore(compra){
    ...
    if(compraStore && compraStore===selectedStore) return true;
  ...
  function tokenStemMatch(x,y){
    ...
    if(!x || !y) return false;
    if(x===y) return true;
    // Evita falsos positivos tipo SAL dentro de ROSAL. Solo permite prefijos si ambos tokens son suficientemente largos.
    if(x.length>=5 && y.length>=5 && (x.indexOf(y)===0 || y.indexOf(x)===0)) return true;
    return false;
  }
  function tokenOverlapCount(a,b){
    var c=0;
    a.forEach(function(x){ if(b.some(function(y){ return tokenStemMatch(x,y); })) c++; });
    return c;
  }
  function aiPendingSimilar(pname, a){
    pname=trim(pname); if(!pname || !a || !trim(a.name)) return false;
    if(normalizeName(pname)===normalizeName(a.name)) return true;
    var pTok=pendingTokens(pname), aTok=pendingTokens(a.name);
    var common=tokenOverlapCount(pTok,aTok);
    var priceOk=(closePrice(a.pendingUnit||0,a.price) || closePrice(a.pendingTotal||0,a.total) || closePrice(a.pendingTotal||0,a.price));
    var shortSide=Math.min(pTok.length,aTok.length);
    // Productos de una sola palabra corta: exigir palabra completa y precio compatible.
    if(shortSide<=1){ return common>=1 && priceOk; }
    // Productos con varias palabras: dos coincidencias o una coincidencia fuerte + precio.
    if(common>=2) return true;
    if(common>=1 && priceOk) return true;
    var sc=productSimilarityScore(pname, a.name);
    return sc>=82 && (common>=1 || priceOk);
  }
  function pendingCandidateKind(compra){
    var pname=productNameById(compra && compra.productoId);
    if(!trim(pname)) return '';
    var sameStore=sameTicketStore(compra);
  ...
  function aiLineCandidateKind(line){
    ...
    for(var r=0;r<rows.length;r++){
      ...
      for(var i=0;i<ai.length;i++){
        var a=Object.assign({},ai[i],{pendingUnit:pUnit,pendingTotal:pTotal});
        if(aiPendingSimilar(pname,a)){
          if(sameTicketStore(c)) return 'same';
          foundOther=true;
        }
      }
    }
  ...
  function pendingLooksCandidate(compra){ return !!pendingCandidateKind(compra); }
    ...
    box.innerHTML=rows.map(function(c){
      ...
      var title=kind==='same' ? 'Posible compra ya realizada en este ticket y en la misma tienda. MÃ¡rcala solo si quieres eliminarla.' : (kind==='other' ? 'Producto parecido al ticket, pero de otra tienda. Revisa antes de marcar.' : '');
  ...
  function productSimilarityScore(input, productName){
    ...
    // Solo considerar contenciÃ³n compacta cuando el texto corto no es una palabra pequeÃ±a aislada.
  ...
  function applyBestProductFromName(input){
    var p=findBestProductByName(input && input.value); if(!p) return;
  ...
  function productKey(v){ return normalizePlain(v).replace(/[^A-Z0-9]+/g,' ').replace(/\s+/g,' ').trim(); }
  function findProductByName(name){
    var n=productKey(name); if(!n) return null;
    var ps=arr('productos');
    for(var i=0;i<ps.length;i++){ if(productKey(ps[i].nombre)===n) return ps[i]; }
    var compact=n.replace(/\s+/g,'');
    for(var j=0;j<ps.length;j++){ if(productKey(ps[j].nombre).replace(/\s+/g,'')===compact) return ps[j]; }
    return null;
  }
  ...
  function upsertProductByName(row, tiendaId, warnings){
    ...
    price=round2(price);
    var existing=findBestProductByName(name) || findProductByName(name);
    ...
    if(existing){
      // v10.3: doble vÃ­a segura para refrescar PRODUCTOS cuando ya existe.
      // Primero usa la ruta especÃ­fica y despuÃ©s fuerza el mismo cambio por el CRUD normal
      ...
      return apiJson('/api/crud/productos/'+encodeURIComponent(existing.id)+'/precio',{method:'PUT',headers:crudHeaders(),body:JSON.stringify(body)})
        ...
        .catch(function(err1){
          return genericUpdate().catch(function(err2){
            warnings.push('No se actualizÃ³ precio de PRODUCTOS para "'+name+'": '+((err2&&err2.message)||(err1&&err1.message)||err2||err1)+'. La compra se grabarÃ¡ con el precio del ticket.');
  ...
  console.info('[CE v10.4.5 Alta IA] producto similar, candidatos Pte.Compra, segmento/destino editables y actualizaciÃ³n PRODUCTOS instalada. Prueba: window.__ceOpenTicketAutoV104()');
})();
