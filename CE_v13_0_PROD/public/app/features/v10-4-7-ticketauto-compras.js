...
(function(){
  'use strict';
  var TAG='__ceV1047TicketAutoComprasSmartProducts';
  ...
  function openPanel(){
    ...
    // v10.4.7: primera apertura siempre limpia y reconstruida. Evita panel congelado en iPad/iPhone/PC
  ...
  function renderRows(){
    ...
    body.innerHTML=rows.map(function(r,i){
      ...
      return '<tr class="'+cls+'" data-ce-ai-row="'+i+'" data-ce-ai-conf="'+htmlEscape(r.confianza||0)+'" data-ce-ai-existing="'+(existing?'1':'0')+'">'+
        ...
        '<td><input list="ceAiProducts" data-ce-ai-field="descripcion" value="'+htmlEscape(r.descripcion || '')+'" placeholder="Nombre producto" title="Conserva el literal leÃ­do del ticket. Puedes elegir un producto existente del desplegable si corresponde."></td>'+ 
  ...
  function enrichRowDefaults(row){
    ...
    if(p){
      // v10.4.7: no sustituye el literal leÃ­do por la IA. Mantiene el texto del ticket
  ...
  function aiPendingScore(pname, a){
    ...
    if(normalizeName(pname)===normalizeName(a.name)) return 120;
    ...
    // Palabras cortas tipo SAL solo coinciden como palabra real completa en ambos lados. SAL no coincide con ROSAL.
    if(common.indexOf('SAL')>=0 && !(hasToken('SAL',pTok) && hasToken('SAL',aTok))) return 0;
    // Si solo coincide un genÃ©rico de sabor/formato (LIMON, LATA, BOTELLA...), no es candidato.
    if(!commonDistinct.length) return 0;
    // TE al limÃ³n <-> FUZE Tea limÃ³n: TE/TEA puede ser distintivo si coincide precio o hay LIMON comÃºn.
    if(common.some(function(t){return normalizeToken(t)==='TEA';}) && (priceOk || common.some(function(t){return normalizeToken(t)==='LIMON';}))) return 88;
    // Una marca/producto distintivo largo vale: ACUARIUS/ACUARIOS, SPRITE, HUEVOS, PIMENTON...
    if(hasStrongSingleMatch(commonDistinct)) return 92;
    if(commonDistinct.length>=2) return 90;
    if(commonDistinct.length>=1 && priceOk) return 84;
    var sc=productSimilarityScore(pname, a.name);
    return (sc>=92 && commonDistinct.length>=1) ? 80 : 0;
  }
  ...
  console.info('[CE v10.4.7 Alta IA] producto similar, candidatos Pte.Compra, segmento/destino editables y actualizaciÃ³n PRODUCTOS instalada. Prueba: window.__ceOpenTicketAutoV104()');
})();
