...
(function(){
  ...
  function ensureUi(){
    ...
    btn.setAttribute('onclick','window.__ceOpenTicketAutoV103&&window.__ceOpenTicketAutoV103();return false;');
  ...
  function pendingLooksCandidate(compra){
    ...
    for(var i=0;i<names.length;i++){
      var sc=productScore(pname, names[i]);
  ...
  function renderPendingPurchases(){
    ...
    box.innerHTML=rows.map(function(c){
      ...
      return '<label class="ce-ai-pending-row '+(cand?'ce-ai-pending-candidate':'')+'" title="'+(cand?'Posible compra ya realizada en este ticket. MÃ¡rcala solo si quieres eliminarla.':'')+'"><input type="checkbox" data-ce-ai-pending-delete="'+htmlEscape(c.id)+'"><strong>'+htmlEscape(productNameById(c.productoId))+'</strong><span>'+htmlEscape(tiendaNameById(c.tiendaId)||'Sin tienda')+'</span><span>'+htmlEscape((money(c.unidades)||0)+' ud x '+dec(c.precio))+'</span><span>'+htmlEscape(euro(compraValue(c)))+'</span></label>';
    }).join('');
  }
  ...
  console.info('[CE v10.3 Alta IA] producto similar, segmento/destino editables y actualizaciÃ³n PRODUCTOS instalada. Prueba: window.__ceOpenTicketAutoV103()');
})();
