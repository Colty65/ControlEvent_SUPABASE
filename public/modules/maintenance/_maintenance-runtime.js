...
export function callLegacy(name, ...args){
  ...
  try{
    ...
    console.warn(`[maintenance/v30.3] Error en ${name}`, error);
    return undefined;
  }
}
...
export function safeStep(label, fn){
  try{
    ...
    console.warn(`[maintenance/v30.3] ${label}`, error);
    return undefined;
  }
}
...
export function setSectionRoot(root, name){
  ...
  root.dataset.ceMaintenanceVersion = 'v30.3';
}
...
export function createMaintenanceSection({name, render = [], afterActivate, beforeActivate} = {}){
  return {
    meta: {name, version:'v30.3', mode:'maintenance-legacy-controller'},
    mount(context = {}){
...
export function scheduleCurrentMaintenance(options = {}){
  ...
  scheduleIdle(() => {
    activateMaintenanceSection(name, {reason:'maintenance-lazy-current', ...options, scheduledAt: startedAt})
      .catch(error => console.warn('[maintenance/v30.3] No se pudo activar mantenimiento diferido', name, error));
  }, delay);
...
function scheduleActivation(section, options = {}){
  ...
  window.setTimeout(() => {
    activateMaintenanceSection(section.name, options).catch(error => {
      console.error('[maintenance/v30.3] No se pudo activar mantenimiento', section.name, error);
    });
  }, 0);
}
...
export function installMaintenanceModules(){
  ...
  window.ControlEventMaintenance = {
    ...
    print(){ const report = maintenanceInfo(); console.group('[ControlEventMaintenance/v30.3] Mantenimiento diferido'); console.info(report); console.groupEnd(); return report; },
    actions: {
