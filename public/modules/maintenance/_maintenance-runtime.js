...
export function callLegacy(name, ...args){
  ...
  try{
    ...
    console.warn(`[maintenance/v28.7.1] Error en ${name}`, error);
    return undefined;
  }
}
...
export function safeStep(label, fn){
  try{
    ...
    console.warn(`[maintenance/v28.7.1] ${label}`, error);
    return undefined;
  }
}
...
export function setSectionRoot(root, name){
  ...
  root.dataset.ceMaintenanceVersion = 'v28.7.1';
}
...
export function createMaintenanceSection({name, render = [], afterActivate, beforeActivate} = {}){
  return {
    meta: {name, version:'v28.7.1', mode:'maintenance-legacy-controller'},
    mount(context = {}){
...
export function scheduleCurrentMaintenance(options = {}){
  ...
  scheduleIdle(() => {
    activateMaintenanceSection(name, {reason:'maintenance-lazy-current', ...options, scheduledAt: startedAt})
      .catch(error => console.warn('[maintenance/v28.7.1] No se pudo activar mantenimiento diferido', name, error));
  }, delay);
...
function scheduleActivation(section, options = {}){
  ...
  window.setTimeout(() => {
    activateMaintenanceSection(section.name, options).catch(error => {
      console.error('[maintenance/v28.7.1] No se pudo activar mantenimiento', section.name, error);
    });
  }, 0);
}
...
export function installMaintenanceModules(){
  ...
  window.ControlEventMaintenance = {
    ...
    print(){ const report = maintenanceInfo(); console.group('[ControlEventMaintenance/v28.7.1] Mantenimiento diferido'); console.info(report); console.groupEnd(); return report; },
    actions: {
