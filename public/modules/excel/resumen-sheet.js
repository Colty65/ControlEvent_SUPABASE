...
const RESUMEN_SHEET_VERSION = 'v27.7.1';
let lastSnapshot = null;
let lastWorksheetBuild = null;
let installed = false;
let lastInfoEventoAttach = null;
const AUDIT_STORAGE_KEY = 'controlevent:v27.7:resumenModularAudit';

export const meta = {
  name: 'resumen-sheet',
  version: RESUMEN_SHEET_VERSION,
  mode: 'modular-infoevento-audit-writer',
  description: 'Módulo real para preparar, validar y escribir una hoja RESUMEN modular. En v27.7 mantiene el modelo modular para auditoría interna; la descarga standalone queda desactivada porque INFOEVENTO es la fuente fiable.'
};
...
function sanitizeStandaloneWorkbook(workbook, worksheet){
  try{
    ...
    // v27.7: no se vacían drawings/media porque los gráficos standalone son imágenes PNG protegidas.
...
export async function downloadStandaloneResumen(options = {}){
  const message = 'RESUMEN standalone desactivado en v27.7: no se genera un Excel independiente porque la fuente fiable es INFOEVENTO. Usa el botón normal de INFOEVENTO para obtener RESUMEN y GRAFICAS correctos.';
