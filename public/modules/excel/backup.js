...
const BACKUP_VERSION = 'ControlEvent v28.7.3';
...
async function getBestState(){
  ...
  try{ state = await fetchServerState(); }
    ...
    console.warn('[ControlEventExcel/v28.7.3] No se pudo leer /api/state; se usa estado de la app.', error);
    source = 'app-fallback';
    state = fallbackState();
  }
...
export async function run(options = {}){
  ...
  console.info('[ControlEventExcel/v28.7.3] Descarga de datos solicitada', {source, counts, scope, scopedCounts});
  try{
    const serverResult = await downloadServerBackup(scope);
    console.info('[ControlEventExcel/v28.7.3] Backup generado por servidor', serverResult);
    return {...serverResult, counts, scopedCounts};
  }catch(serverError){
    console.warn('[ControlEventExcel/v28.7.3] Fallback a backup cliente', serverError);
  }
  if(dataCount === 0){
