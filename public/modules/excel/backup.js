...
export const meta = {
  name: 'backup',
  version: 'v27.0.1',
  mode: 'fresh-state-modular-backup',
  description: 'Descarga de datos/backup construida desde /api/state para evitar hojas solo con cabeceras por estado legacy vacío o desfasado.'
};

const BACKUP_VERSION = 'ControlEvent v27.0.1';
const BACKUP_VERSION_FILE = 'ControlEvent_v27_0_1';
...
async function getBestState(){
  ...
  try{ state = await fetchServerState(); }
    ...
    console.warn('[ControlEventExcel/v27.0.1] No se pudo leer /api/state; se usa estado de la app.', error);
    source = 'app-fallback';
    state = fallbackState();
  }
...
function chooseBackupScope(state){
  return new Promise(resolve => {
    ...
    overlay.innerHTML = `<div class="ce-backup-modal-v181"><h3>Descarga de datos</h3><p>Elige si quieres descargar todos los datos o solo los vinculados a un evento concreto.</p><div class="field"><label>Evento a descargar</label><select id="ceBackupScopeV2701"><option value="TODOS">TODOS los eventos</option>${events.map(e => `<option value="${esc(e.id)}" ${String(e.id)===String(selectedId)?'selected':''}>${esc(e.titulo || e.id)}</option>`).join('')}</select></div><div class="ce-backup-actions-v181"><button type="button" class="outline" id="ceBackupCancelV2701">Cancelar</button><button type="button" id="ceBackupOkV2701">Descargar</button></div></div>`;
    document.body.appendChild(overlay);
    const done = value => { overlay.remove(); resolve(value); };
    overlay.querySelector('#ceBackupCancelV2701')?.addEventListener('click', () => done(null));
    overlay.querySelector('#ceBackupOkV2701')?.addEventListener('click', () => done(overlay.querySelector('#ceBackupScopeV2701')?.value || 'TODOS'));
...
export async function run(options = {}){
  ...
  console.info('[ControlEventExcel/v27.0.1] Descarga de datos', {source, counts, scope, scopedCounts});
