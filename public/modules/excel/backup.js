...
const BACKUP_VERSION = 'ControlEvent v27.4.8';
const BACKUP_VERSION_FILE = 'ControlEvent_v27_4_8';
...
async function getBestState(){
  ...
  try{ state = await fetchServerState(); }
    ...
    console.warn('[ControlEventExcel/v27.4.8] No se pudo leer /api/state; se usa estado de la app.', error);
    source = 'app-fallback';
    state = fallbackState();
  }
...
export async function run(options = {}){
  ...
  console.info('[ControlEventExcel/v27.4.8] Descarga de datos solicitada', {source, counts, scope, scopedCounts});
  try{
    const serverResult = await downloadServerBackup(scope);
    console.info('[ControlEventExcel/v27.4.8] Backup generado por servidor', serverResult);
    return {...serverResult, counts, scopedCounts};
  }catch(serverError){
    console.warn('[ControlEventExcel/v27.4.8] Fallback a backup cliente', serverError);
  }
  if(dataCount === 0){
