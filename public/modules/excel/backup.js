...
export async function run(options = {}){
  ...
  addRows('CE_COMPRAS_BBDD', ['COMPRA_ID','EVENT_ID','PRODUCTO_ID','UNIDADES','PRECIO','TICKET_DONACION','DONACION_SITUACION','TIENDA_ID','RESPONSABLE_ID','DONOR_REF','CREATED_AT','UPDATED_AT'], scoped.compras.map(c=>[c.id||'',c.eventId||'',c.productoId||'',num(c.unidades),num(c.precio),ticket(c),c.donacionSituacion||c.donacion_situacion||'',c.tiendaId||'',c.responsableId||'',c.donorRef||'',c.createdAt||'',c.updatedAt||'']));
  ...
  addRows('DONACIONES', ['EVENTO_CODIGO','DONACION_ID','PRODUCTO_CODIGO','UNIDADES','PRECIO','TIPO_DONACION','SITUACION_ENTREGA','DONANTE_TIPO','DONANTE_CODIGO','RESPONSABLE_PERSONA_CODIGO'], scoped.compras.filter(c => isDonation(ticket(c))).map(c => { const parts = String(c.donorRef || '').split(':'); const kind = parts[0], id = parts[1]; return [eventCode[c.eventId] || '', c.id || '', productCode[c.productoId] || '', num(c.unidades), price(c, productMap), ticket(c), c.donacionSituacion||c.donacion_situacion||'Comprometida', kind === 'P' ? 'PERSONA' : (kind === 'T' ? 'TIENDA' : ''), kind === 'P' ? (personCode[id] || '') : (kind === 'T' ? (storeCode[id] || '') : ''), personCode[c.responsableId] || '']; }));
  ...
  try{
    ...
    addRows('BANCO_INGRESOS_LINKS',['ID','MOVEMENT_ID','EVENT_ID','INCOME_ID','INCOME_AMOUNT_SNAPSHOT','CREATED_BY','CREATED_AT'],(bank.incomeLinks||[]).map(r=>[r.id||'',r.movementId||'',r.eventId||'',r.incomeId||'',num(r.incomeAmountSnapshot),r.createdBy||'',r.createdAt||'']));
    addRows('BANCO_CIERRE_MVTO',['MOVEMENT_ID','ACCEPTED_DIFFERENCE','NOTE','ACCEPTED_BY','ACCEPTED_AT','UPDATED_AT'],(bank.movementSettlements||[]).map(r=>[r.movementId||'',num(r.acceptedDifference),r.note||'',r.acceptedBy||'',r.acceptedAt||'',r.updatedAt||'']));
  ...
  }catch(bankError){['BANCO_IMPORTACIONES','BANCO_MVTOS','BANCO_TK_LINKS','BANCO_INGRESOS_LINKS','BANCO_CIERRE_MVTO','BANCO_PERIODOS','BANCO_ESTADO_MVTO'].forEach(name=>addRows(name,['AVISO'],[[bankError?.message||String(bankError)]]));}
