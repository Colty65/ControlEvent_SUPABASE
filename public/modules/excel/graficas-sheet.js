...
const GRAFICAS_SHEET_VERSION = 'v27.4';
const AUDIT_STORAGE_KEY = 'controlevent:v27.4:graficasModularAudit';
let installed = false;
...
export const meta = {
  ...
  description: 'Módulo real para preparar y escribir una hoja GRAFICAS modular. En v27.4 se integra en INFOEVENTO como hoja GRAFICAS_MODULAR para comparar sin sustituir todavía la hoja GRAFICAS legacy.'
};
...
export async function downloadStandaloneGraficas(options = {}){
  ...
  if(!ExcelJS?.Workbook) throw new Error('ExcelJS no está disponible para generar GRAFICAS modular.');
  ...
  a.href = url;
  a.download = `ControlEvent_v27_4_GRAFICAS_MODULAR-${safeName(result.model.event.titulo)}_${stamp}.xlsx`;
