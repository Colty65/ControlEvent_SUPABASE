/* ControlEvent v28.5.3_prod - Bundle legacy generado desde scripts legacy-inline extraídos. */
/* Mantiene el orden original de ejecución para compatibilidad. */

;/* ===== BEGIN legacy-inline-01.js ===== */

/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #1. */
...
function mergeLoadedState(parsed, defaults){
  ...
  return {
    ...
    eventDocuments: (parsed.eventDocuments || defaults.eventDocuments || []).map(d => ({
      ...d,
      id: d.id || ((d.eventId || '') + '|' + (d.codigo || d.imageKey || 'DOC')),
      eventId: d.eventId || '',
      codigo: d.codigo || d.imageKey || '',
      imageKey: d.imageKey || d.codigo || '',
      imageUrl: d.imageUrl || '',
      fecha: d.fecha || '',
...
function ceParseEventDateKeyV310(value){
  ...
  if(m){
    ...
    return Number(String(y).padStart(4,'0') + mo + d) || 99999999;
  }
  m = raw.match(/^(\d{4})[\/\-.](\d{1,2})[\/\-.](\d{1,2})$/);
  if(m){ return Number(m[1] + String(Number(m[2])).padStart(2,'0') + String(Number(m[3])).padStart(2,'0')) || 99999999; }
  const dt = new Date(raw);
  if(!Number.isNaN(dt.getTime())) return Number(String(dt.getFullYear()) + String(dt.getMonth()+1).padStart(2,'0') + String(dt.getDate()).padStart(2,'0'));
  return 99999999;
}
function ceSortEventosByFechaInicioV310(a,b){
  const da = ceParseEventDateKeyV310(a?.fechaIni), db = ceParseEventDateKeyV310(b?.fechaIni);
...
function renderHeader(){
  ...
  document.getElementById('selectedEvent').innerHTML = state.eventos
    .slice()
    .sort(ceSortEventosByFechaInicioV310)
...
function renderTabVisibility(){
  ...
  const mapaPanelV303 = document.getElementById('tabMapaProductos');
  if(mapaPanelV303) mapaPanelV303.classList.toggle('hidden', currentMainTab !== 'mapa' || !hasEvent);
  ...
  const mapaBtnV303 = document.getElementById('tabMapaBtn');
  if(mapaBtnV303) mapaBtnV303.classList.toggle('active', currentMainTab === 'mapa');
...
function renderEventos(){
  ...
  const list = state.eventos.slice().sort(ceSortEventosByFechaInicioV310);
  if(!list.length){
...
function handleClick(e){
  ...
  if(isLocked() && !['btnToggleMaintenance','mtEventosBtn','btnLogout','toggleEventDesc','save-evento','btnExportExcel','tabIngresosBtn','tabComprasBtn','tabDonacionesBtn','tabMapaBtn','tabResumenBtn','tabGraficasBtn'].includes(action)) return;
  ...
  switch(action){
    ...
    case 'tabMapaBtn': currentMainTab = 'mapa'; render(); if(window.renderMapaProductos) window.renderMapaProductos(); break;
    ...
    case 'delete-evento':
      ...
      if(Array.isArray(state.eventDocuments)) state.eventDocuments = state.eventDocuments.filter(d => String(d.eventId || '') !== String(btn.dataset.id));
...
async function exportSeedWorkbook(){
  ...
  wb.creator = 'ControlEvent v28.5.3_prod - ©oltyLAB ’26';
  wb.created = new Date();
  ...
  a.download = 'ControlEvent_v28.5.3_prod_descarga_datos.xlsx';
  a.click();
...
async function exportExcel(){
  ...
  putText(wsRes,r,1,'Generado por'); putText(wsRes,r++,2,'ControlEvent v28.5.3_prod - ©oltyLAB ’26');
  ...
  link.download = `${(ev.titulo || 'evento').replace(/\s+/g,'_')}_ControlEvent_v28.5.3_prod.xlsx`;
  link.click();
...
function donorOptions(){
  ...
  renderTabVisibility = function(){
    ...
    const mapaPanelV303 = opt('tabMapaProductos'); if(mapaPanelV303) mapaPanelV303.classList.toggle('hidden', currentMainTab !== 'mapa' || !hasEvent);
    ...
    const mapaBtnV303 = opt('tabMapaBtn'); if(mapaBtnV303) mapaBtnV303.classList.toggle('active', currentMainTab === 'mapa');
  ...
  async function importV92Workbook(){
    ...
    try{
      ...
      const baseState = mode === 'RESUME' ? {
        ...
        eventCodeMap:{...(state.eventCodeMap||{})},
        entityCodeMaps: JSON.parse(JSON.stringify(state.entityCodeMaps||{personas:{},tiendas:{},productos:{}})),
        selectedEventId: state.selectedEventId || ''
      } : {
        personas:[], eventos:[], tiendas:[], productos:[], colaboradores:[], compras:[], ticketImages:{}, eventCodeMap:{}, entityCodeMaps:{personas:{},tiendas:{},productos:{}}, selectedEventId:''
      };
      ...
      const storeMap = {};
      const productMap = {};
      const usedImportIdsV831 = new Set();
      const stableImportIdV831 = (value, seed) => {
      ...
      eventosRows.forEach(row => {
        ...
        const titulo = String(pick(row, ['EVENTO_TITULO','TITULO'])).trim();
        if(!code || !titulo) return;
        const id = stableImportIdV831(pick(row, ['EVENTO_ID','ID_EVENTO','EVENT_ID'], ''), code);
        eventMap[code] = id;
        baseState.eventCodeMap[id] = code;
      ...
      personasRows.forEach(row => {
        ...
        const id = stableImportIdV831(pick(row, ['PERSONA_ID','ID_PERSONA','PERSON_ID'], ''), code);
        personMap[code] = id;
        if(!baseState.entityCodeMaps.personas) baseState.entityCodeMaps.personas = {};
        baseState.entityCodeMaps.personas[id] = code;
        const rangoRaw = String(pick(row, ['PERSONA_RANGO','RANGO'], 'SOCIO')).trim().toUpperCase();
        baseState.personas.push({id, personaCodigo: code, nombre, rango: PERSONA_RANGOS.includes(rangoRaw) ? rangoRaw : 'SOCIO'});
      });
      ...
      tiendasRows.forEach(row => {
        ...
        const nombre = String(pick(row, ['TIENDA_NOMBRE','NOMBRE'])).trim();
        if(!code || !nombre) return;
        const id = stableImportIdV831(pick(row, ['TIENDA_ID','ID_TIENDA','STORE_ID'], ''), code);
        storeMap[code] = id;
        if(!baseState.entityCodeMaps.tiendas) baseState.entityCodeMaps.tiendas = {};
        baseState.entityCodeMaps.tiendas[id] = code;
        baseState.tiendas.push({id, tiendaCodigo: code, nombre});
      });
      ...
      productosRows.forEach(row => {
        ...
        const nombre = String(pick(row, ['PRODUCTO_NOMBRE','NOMBRE'])).trim();
        if(!code || !nombre) return;
        const id = stableImportIdV831(pick(row, ['PRODUCTO_ID','ID_PRODUCTO','PRODUCT_ID'], ''), code);
        productMap[code] = id;
        if(!baseState.entityCodeMaps.productos) baseState.entityCodeMaps.productos = {};
        baseState.entityCodeMaps.productos[id] = code;
        const precioReferencia = parseEuroInput(pick(row, [
          'PRODUCTO_PRECIO_REFERENCIA',
          'PRODUCTO_PRECIO',
          'PRECIO_REFERENCIA',
      ...
      ingresosRows.forEach(row => {
        ...
        baseState.colaboradores.push({
          id: stableImportIdV831(pick(row, ['INGRESO_ID','COLABORADOR_ID','ID_INGRESO','ID'], ''), `${eventCode}|${personCode}|INGRESO`),
          eventId: eventMap[eventCode],
      ...
      comprasRows.forEach(row => {
        ...
        baseState.compras.push({
          id: stableImportIdV831(pick(row, ['COMPRA_ID','ID_COMPRA','ID'], ''), `${eventCode}|${productCode}|COMPRA|${pick(row, ['TICKET_U_OTROS_GASTOS','TICKET_DONACION','TICKET'], '')}`),
          eventId: eventMap[eventCode],
      ...
      donacionesRows.forEach(row => {
        ...
        baseState.compras.push({
          id: stableImportIdV831(pick(row, ['DONACION_ID','COMPRA_ID','ID_DONACION','ID_COMPRA','ID'], ''), `${eventCode}|${productCode}|DONACION|${pick(row, ['TIPO_DONACION','TICKET_DONACION'], '')}`),
          eventId: eventMap[eventCode],
          productoId: productMap[productCode],
          unidades: Number(pick(row, ['UNIDADES','UD'], 0)) || 0,
          precio: parseEuroInput(pick(row, ['PRECIO'], 0)),
      ...
      state.eventCodeMap = baseState.eventCodeMap || {};
      state.entityCodeMaps = baseState.entityCodeMaps || {personas:{},tiendas:{},productos:{}};
  ...
  handleClick = function(e){
    ...
    if(!['btnTogglePower','btnExportExcel','btnOpenImport','btnExportSeed','btnToggleMaintenance','mtEventosBtn','btnAddEvento','mtAccesoBtn','tabIngresosBtn','tabComprasBtn','tabDonacionesBtn','tabMapaBtn','tabResumenBtn','tabGraficasBtn'].includes(action) && isLocked()) return;
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #2. */
(function(){
  ...
  function excelFileName(ev){
    ...
    return `ControlEvent_v28.5.3_prod-${cleanFilePart(ev?.titulo || 'evento')}_${dd}${mm}${yyyy}.xlsx`;
  }
  function graphData(){
    const b = budgetSummary();
...
(function(){
  ...
  window.excelFileName = function(ev){
    ...
    const mm = String(d.getMonth()+1).padStart(2,'0');
    const yyyy = String(d.getFullYear());
    return `ControlEvent_v28.5.3_prod_INFOEVENTO-${title}_${yyyy}${mm}${dd}.xlsx`;
  };
...
(function(){
  ...
  renderGraphOnly = function(){
    ...
    wrap.innerHTML = `
      <div class="chart-shell">
        <div class="chart-bars">
          <div class="chart-row">
            <div class="chart-label">INGRESOS: ${esc(moneyFmt(g.incomes.total))}</div>
            <div>
              <div class="chart-track">
  ...
  makeChartImageDataUrl = async function(){
    ...
    const rows = [
      {
        label:`INGRESOS: ${fmt(g.incomes.total)}`,
        items:[
          {label:'Socios Banco', value:g.incomes.socioBanco, color:'#2563eb'},
          {label:'Socios Bizum', value:g.incomes.socioBizum, color:'#16a34a'},
          {label:'Socios Efectivo', value:g.incomes.socioEfectivo, color:'#84cc16'},
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #3. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #4. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #5. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #6. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #7. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #8. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #9. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #10. */
/* ==== V15.1 AJUSTES ==== */
(function(){
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #11. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #12. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #13. */
/* ==== V16.0 PATCH FINAL ==== */
(function(){
  const $ = id => document.getElementById(id);
  const esc = v => (typeof escapeHtml === 'function' ? escapeHtml(v) : String(v ?? ''));
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #14. */
/* ==== V16.2 REWORK ==== */
(function(){
  const $ = id => document.getElementById(id);
  const esc = v => typeof escapeHtml === 'function' ? escapeHtml(v) : String(v ?? '');
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #15. */
/* ==== V16.2 FIXES ==== */
(function(){
  const $ = id => document.getElementById(id);
  const esc = v => typeof escapeHtml === 'function' ? escapeHtml(v) : String(v ?? '');
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #16. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #17. */
...
(function(){
  ...
  function emittedByTextV171(date=new Date()){
    ...
    return `Emitido por “©oltyLAB ’26_ControlEvent_v28.5.3_prod_${dd}${mm}${yyyy}_${hh}:${mi}:${ss}”`;
  }
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #18. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #19. */
/* ==== V18.1: tooltips redondeados, backup por evento y Excel INGRESOS ==== */
(function(){
  const VERSION = 'ControlEvent v28.5.3_prod';
  const VERSION_FILE = 'ControlEvent_v28.5.3_prod';
  const norm = v => String(v ?? '').trim();
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #20. */
...
(function(){
  ...
  function hasDependency(action, id){
    ...
    if(action === 'delete-evento') return false;
    return false;
  }
  document.addEventListener('click', function(e){
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #21. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #22. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #23. */
...
(function(){
  ...
  window.addEventListener('scroll', ev => { /* v46.8: mantener globo al usar ruleta/ascensor */ }, true);
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #24. */
(function(){
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #25. */
...
(function(){
  ...
  function normalizeDownloadName(name){
    let n = String(name || '');
    n = n.replace(/^ControlEvent_v\d+_\d+(?:_\d+)?/i, VERSION_FILE);
    n = n.replace(/ControlEvent_v28.5.3_prod/ig, VERSION_FILE);
    return n;
  }
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #26. */
...
(function(){
  ...
  function normalizeDownloadName(name){
    let n = String(name || '');
    n = n.replace(/^ControlEvent_v\d+_\d+(?:_\d+)?/i, VERSION_FILE);
    n = n.replace(/ControlEvent_v24_prod_\d(?:_\d+)?/ig, VERSION_FILE);
    return n;
  }
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #27. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #28. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #29. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #30. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #31. */
/* ==== V21.0: producto duplicado permitido si cambia la tienda + globos estables ==== */
(function(){
  const VERSION='ControlEvent v28.5.3_prod'; const VERSION_FILE='ControlEvent_v28.5.3_prod';
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #32. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #33. */
...
(function(){
  ...
  function donationRowsGrouped(code){
    ...
    rows.forEach((c,idx)=>{const d=donor(c); if(cur!==null&&d!==cur){out.push(`Total ${cur} | | | ${money(total)}`); out.push(''); total=0;} cur=d; const u=Number(c.unidades||0), pr=price(c), v=val(c); total+=v; out.push(`${d} | ${pName(c)} | ${num(u)} uds x ${money(pr)} | ${money(v)}`); if(idx===rows.length-1){out.push(`Total ${d} | | | ${money(total)}`);}});
    return out;
  }
  function expenseRowsGrouped(kind){
    const rows=compras().filter(c=>{const tk=ticket(c); if(kind==='ticket')return !isDon(tk)&&!isCurrent(tk)&&tk!==''; if(kind==='current')return isCurrent(tk); return !isDon(tk)&&tk==='';}).sort((a,b)=>cmp(tName(a),tName(b))||cmp(ticket(a)||'PTE.COMPRA',ticket(b)||'PTE.COMPRA')||cmp(pName(a),pName(b)));
    const out=[]; let store=null,ticketCur=null,ticketTotal=0,storeTotal=0; function closeTicket(){if(ticketCur!==null){out.push(`Total ${store}, ${ticketCur} | | | | ${money(ticketTotal)}`); out.push('');}} function closeStore(){if(store!==null){out.push(`Total ${store} | | | | ${money(storeTotal)}`); out.push('');}}
  ...
  function applyGraphTips(){
    ...
    expSegs.forEach((seg,i)=>{const [title,kind,item]=expSpecs[i]||[]; if(!item)return; const lines=expenseRowsGrouped(kind); setTip(seg,tipTitle(title,item.value||0,lines,'Tienda | Ticket | Producto | Uds x precio | Total'),item.color||getComputedStyle(seg).backgroundColor||'#fff','graphexpensev212');});
  }
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #34. */
...
(function(){
  ...
  function donationRowsGrouped(code){const rows=compras().filter(c=>!code||ticket(c)===code).sort((a,b)=>cmp(donorName(a),donorName(b))||cmp(productName(a),productName(b))); const out=[]; let cur=null,total=0; rows.forEach((c,i)=>{const d=donorName(c); if(cur!==null&&d!==cur){out.push(`Total ${cur} | | | | ${money(total)}`); out.push(''); total=0;} cur=d; total+=value(c); out.push(`${d} | ${productName(c)} | ${num(Number(c.unidades||0))} | ${money(price(c))} | ${money(value(c))}`); if(i===rows.length-1)out.push(`Total ${d} | | | | ${money(total)}`);}); return out;}
  ...
  function applyGraphTips(){
    ...
    expSegs.forEach((seg,i)=>{const [title,kind]=specs[i]||[]; if(!kind)return; const lines=expenseRowsGrouped(kind); const total=lines.total; const text=titleWithLines(title,total,'Tienda | Ticket | Producto | Uds x precio | Total',lines.rows); setTip(seg,text,expItems[i]?.color||getComputedStyle(seg).backgroundColor||'#fff','graphexpensev213');});
  }
  function expenseRowsGrouped(kind){
    ...
    const rows=compras().filter(filt).sort((a,b)=>cmp(storeName(a),storeName(b))||cmp(ticket(a)||'PTE.COMPRA',ticket(b)||'PTE.COMPRA')||cmp(productName(a),productName(b)));
    const out=[]; let store=null,ticketCur=null,ticketTotal=0,storeTotal=0,grand=0; function closeTicket(){if(ticketCur!==null){out.push(`Total ${store}, ${ticketCur} | | | | ${money(ticketTotal)}`); out.push('');}} function closeStore(){if(store!==null){out.push(`Total ${store} | | | | ${money(storeTotal)}`); out.push('');}}
    rows.forEach((c,i)=>{const st=storeName(c), tk=kind==='current'?'GASTOS CORRIENTES':(ticket(c)||'PTE.COMPRA'); if(store!==null&&(st!==store||tk!==ticketCur)){closeTicket(); ticketTotal=0;} if(store!==null&&st!==store){closeStore(); storeTotal=0;} store=st; ticketCur=tk; const v=value(c); ticketTotal+=v; storeTotal+=v; grand+=v; out.push(`${st} | ${tk} | ${productName(c)} | ${num(Number(c.unidades||0))} uds x ${money(price(c))} | ${money(v)}`); if(i===rows.length-1){closeTicket(); closeStore();}}); while(out.length&&out[out.length-1]==='')out.pop(); return {rows:out,total:grand};
  }
  ...
  window.exportExcel=exportExcel=async function(){
    try{
      ...
      console.group('ERROR INFOEVENTO - ControlEvent v28.5.3_prod');
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #35. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #36. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #37. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #38. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #39. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #40. */
...
(function(){
  ...
  setInterval(()=>{ try{ patchExcelJSAddImage(); patchProtectionAndDownloads(); }catch(_){ } }, window.ControlEventLowResource?.interval?.(1200) || 1200);
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #41. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #42. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #43. */
(function(){
  'use strict';
  const VERSION='v22.8';
  const $=id=>document.getElementById(id);
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #44. */
...
(function(){
  ...
  setInterval(applyAll, window.ControlEventLowResource?.interval?.(1800) || 1800);
  applyAll();
})();
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #45. */
...
(function(){
  ...
  function isAllowedInFinalized(el){
    ...
    if(['selectedEvent','btnExportExcel','btnLogout','toggleEventDesc','toggleIngresosSummary','toggleComprasSummary','toggleComprasEvent','ceMobileMenuBtn','ceMobileDrawerClose','btnToggleMaintenance','mtEventosBtn','tabIngresosBtn','tabComprasBtn','tabDonacionesBtn','tabMapaBtn','tabResumenBtn','tabGraficasBtn'].includes(id)) return true;
  ...
  function applyFinalizedConsulta(){
    ...
    document.querySelectorAll('#mainTabs,#tabIngresos,#tabCompras,#tabDonaciones,#tabMapaProductos,#tabResumen,#tabGraficas,#maintenanceWrapper').forEach(el=>{if(el){el.style.pointerEvents='auto';el.style.opacity='1';}});
  ...
  setInterval(applyAll, window.ControlEventLowResource?.interval?.(900) || 900);
  applyAll();
})();
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #46. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #47. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #48. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #49. */
/* ==== v23.6.4 local: no precargar state pesado, no localStorage pesado, fotos como archivos ==== */
(function(){
  'use strict';
  const VERSION='ControlEvent v28.5.3_prod';
  const SESSION_KEY='ControlEvent_v24_prod_session';
  const $=id=>document.getElementById(id);
  ...
  const imageWriteHeaders=()=>({'Content-Type':'application/json','X-ControlEvent-Write-Scope':'ticket-image-v8-5-fix26'});
  const photoVariants=(label)=>{
    const ev=eventId();
    const raw=String(label||'').trim();
    const clean=raw.startsWith(`${ev}|`) ? raw.slice(ev.length+1).trim() : raw;
    const out=[];
    const add=v=>{ v=String(v||'').trim(); if(v&&!out.includes(v)) out.push(v); };
    [raw, clean, imgKey(raw), imgKey(clean), ev?`${ev}|${raw}`:'', ev?`${ev}|${clean}`:''].forEach(add);
    const parts=clean.split('|').map(x=>String(x||'').trim()).filter(Boolean);
    if(parts.length>=2 && ev){
      add(`${ev}|${parts[0]} | ${parts[1]}`);
      add(`${ev}|${parts[0]}|${parts[1]}`);
      add(`${ev}|${parts[1]}`);
    }
    return out;
  };
  function clearPhotoLocal(label){
    const s=stateRef();
    photoVariants(label).forEach(k=>{ if(s.ticketImages) delete s.ticketImages[k]; if(s.ticketImageRefs) delete s.ticketImageRefs[k]; });
  }
  function setVersion(){
    try{ document.title=VERSION; }catch(_){ }
    document.querySelectorAll('.appname-stack span').forEach(el=>{ if(/ControlEvent/i.test(el.textContent||'')) el.textContent=VERSION; });
  }
  function sessionSave(user){ /* v50.27: no guardar sesion ligera; la app siempre pide login limpio. */ try{ localStorage.removeItem(SESSION_KEY); }catch(_){ } }
  ...
  async function uploadPhoto(label,file){
    ...
    clearPhotoLocal(label);
    const dataUrl=await fileToCompressedDataUrl(file);
    const res=await fetch('/api/ticket-images',{method:'POST',headers:imageWriteHeaders(),body:JSON.stringify({eventId:eventId(),key,dataUrl})});
    const data=await res.json().catch(()=>({}));
    ...
    const savedKey=data.image.key||key;
    const savedUrl=data.image.pathname||data.image.url;
    if(savedUrl){
      s.ticketImages[key]=savedUrl;
      s.ticketImages[savedKey]=savedUrl;
    }
    if(!s.ticketImageRefs) s.ticketImageRefs={};
    if(savedUrl) s.ticketImageRefs[savedKey]={...(data.image||{}),key:savedKey,url:data.image.url||savedUrl,pathname:data.image.pathname||savedUrl};
    render();
  }
  function pickPhotoFor(label){
    const input=document.createElement('input'); input.type='file'; input.accept='image/*';
    input.value='';
    input.onchange=()=>uploadPhoto(label,input.files&&input.files[0]).catch(e=>{alert(e.message||String(e)); console.error(e);}).finally(()=>{ try{ input.value=''; input.remove(); }catch(_){ } });
    input.click();
  }
  async function removePhoto(label){
    const ev=eventId();
    const variants=photoVariants(label);
    for(const k of variants){
      const res=await fetch(`/api/ticket-images?eventId=${encodeURIComponent(ev)}&key=${encodeURIComponent(k)}`,{method:'DELETE',headers:imageWriteHeaders()});
      if(!res.ok){
        const data=await res.json().catch(()=>({}));
        throw new Error(data.error||data.message||'No se pudo eliminar la foto en servidor');
      }
    }
    clearPhotoLocal(label);
    render();
  ...
  try{
    ...
    removeTicketImage=function(encoded){ removePhoto(decodeURIComponent(String(encoded||''))).catch(e=>{alert(e.message||String(e)); console.error(e);}); return false; };
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #50. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #51. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #52. */
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #53. */
...
(function(){
  ...
  function imageCandidates(label){
    ...
    const scoped = v => { v = norm(v); if(!v || !id) return; add(v.startsWith(id + '|') ? v : `${id}|${v}`); };
    [base, clean, ticketImageKey(base,id), ticketImageKey(clean,id)].forEach(scoped);
  ...
  function imageRefFor(label){
    ...
    for(const bag of stores){
      for(const [k,v] of Object.entries(bag)){
        const ks = String(k);
        if(id && !ks.startsWith(id + '|')) continue;
...
/* ControlEvent v28.5.3_prod - JS legacy extraido de public/index.html. Bloque inline #54. */
(function(){
  const VERSION = 'ControlEvent v28.5.3_prod';
  const VERSION_FILE = 'ControlEvent_v28.5.3_prod';
  const $ = id => document.getElementById(id);
  ...
  setInterval(() => { applyVersionV241(); fixAccessVisibilityV241(); }, window.ControlEventLowResource?.interval?.(1500) || 1500);
