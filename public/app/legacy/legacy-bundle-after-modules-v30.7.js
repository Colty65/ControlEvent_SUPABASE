...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #55. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #56. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #57. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #58. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #59. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #60. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #61. */
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #62. */
...
(function(){
  ...
  function grouping(kind){
    // INFOEVENTO necesita una fila moderna por agrupación. Algunas correcciones antiguas
    // devolvían pares {k,v}; al leerlos como {label,comprado,...} aparecían Undefined,
    // hojas vacías y gráficas sin cifras. Solo aceptamos el formato moderno y, si no,
    // recalculamos directamente desde COMPRAS + catálogo PRODUCTOS.
    try{
      const external = kind === 'segmento'
        ? (typeof summaryBySegmento === 'function' ? summaryBySegmento() : [])
        : (typeof summaryByDestino === 'function' ? summaryByDestino() : []);
      if(Array.isArray(external) && external.length && external.every(item => item && norm(item.label) && ['comprado','donado','pendiente','total'].every(key => Object.prototype.hasOwnProperty.call(item,key)))){
        return external.map(item => ({
          ...item,
          label:norm(item.label),
          comprado:num(item.comprado),
          donado:num(item.donado),
          pendiente:num(item.pendiente),
          total:num(item.total)
        }));
      }
    }catch(_){ }
    const field = kind === 'segmento' ? 'segmento' : 'destino';
    const defaults = kind === 'segmento'
      ? ['COMIDA','BEBIDA','INFRAESTRUCTURA']
      : ['APERITIVO','COMIDA','CENA','CUBATAS','INFRAESTRUCTURA'];
    const map = new Map(defaults.map(label => [label, {label, comprado:0, donado:0, pendiente:0, total:0}]));
    compras().forEach(c => {
      const p = c.producto || producto(c.productoId);
      const label = norm(c?.[field] || c.producto?.[field] || p[field] || `Sin ${field}`) || `Sin ${field}`;
      ...
      else if(!tk || /PTE\.?\s*COMPRA|PENDIENTE/i.test(tk)) row.pendiente += v;
      else row.comprado += v;
      row.total += v;
    });
  ...
  function imageForRow(row){
    ...
    for(const c of candidates){
      for(const k of [imageKey(c), `${ev}|${c}`]){
  ...
  function backupFileName(scope = 'TODOS', eventTitle = 'TODOS'){
    ...
    return `${VERSION_FILE}_BACKUP_${name}_${t.yyyy}${t.mm}${t.dd}_${t.hh}${t.mi}${t.ss}.xlsx`;
  }
  ...
  async function addBankInfoEventoV24(wb,x,ev){
    ...
    let r=5;
    const movements=(Array.isArray(data?.movements)?data.movements:[]).filter(movement=>movement?.included===true);
  ...
  function ticketImageSourceV80(value){
    if(!value) return '';
  ...
  function decodeBase64UrlTextV80(value){
  ...
  function ticketEventIdFromImageValueV80(value){ const m=ticketImageSourceV80(value).match(/\/ticket-images\/([^\/?#]+)\//i); return m ? decodeURIComponent(m[1]) : ''; }
  function ticketDecodedKeyFromImageValueV80(value){ const m=ticketImageSourceV80(value).match(/\/ticket-images\/[^\/?#]+\/([^\/?#]+?)(?:\.[a-z0-9]+)?(?:[?#].*)?$/i); return m ? decodeBase64UrlTextV80(m[1]) : ''; }
  function ticketBackupPartsV80(fullKey,image,eventCode){
    const rawKey=norm(fullKey); const keyEvent=ticketEventIdFromKey(rawKey); const keyInner=ticketInnerKeyFromKey(rawKey) || rawKey;
    const decoded=ticketDecodedKeyFromImageValueV80(image); const de=decoded ? ticketEventIdFromKey(decoded) : ''; const di=decoded ? (ticketInnerKeyFromKey(decoded)||decoded) : '';
    const srcEvent=ticketEventIdFromImageValueV80(image); let eventToken='';
    if(eventCode[de]) eventToken=de; else if(eventCode[srcEvent]) eventToken=srcEvent; else if(eventCode[keyEvent]) eventToken=keyEvent;
    let innerKey=''; if(eventToken && de===eventToken && di) innerKey=di; else if(eventToken && keyEvent===eventToken && keyInner) innerKey=keyInner; else innerKey=di || keyInner || rawKey;
  ...
  function ticketTokenV81(value){ const m=String(value||'').match(/\bTK\s*\d+[A-Z0-9_-]*\b/i); return m ? m[0].replace(/\s+/g,'').toUpperCase() : ''; }
  function isIngresoTicketKeyV81(value){ return /^INGRESO[:|]/i.test(String(value||'').trim()); }
  ...
  function scopedBackupState(scope){
    ...
    Object.entries(base.ticketImages || {}).forEach(([key, value]) => {
      const srcEvent = ticketEventIdFromImageValueV80(value);
      const decoded = ticketDecodedKeyFromImageValueV80(value);
      if(all || eventIds.has(String(ticketEventIdFromKey(key))) || eventIds.has(String(srcEvent)) || (decoded && eventIds.has(String(ticketEventIdFromKey(decoded))))) ticketImages[key] = value;
    });
  ...
  async function exportSeedWorkbookV257(){
    ...
    const totalCounts = {
      eventos: rows('eventos').length,
      personas: rows('personas').length,
      productos: rows('productos').length
    };
    make('METADATOS', ['CAMPO','VALOR'], [
      ...
      ['REGISTROS_EVENTOS', scoped.eventos.length],
      ['REGISTROS_PERSONAS', scoped.personas.length],
      ['REGISTROS_TIENDAS', scoped.tiendas.length],
      ['REGISTROS_PRODUCTOS', scoped.productos.length],
      ['REGISTROS_INGRESOS', scoped.colaboradores.length],
      ['REGISTROS_COMPRAS', scoped.compras.length],
      ['REGISTROS_TICKETS', Object.keys(scoped.ticketImages || {}).length],
    ...
    make('EVENTOS', ['EVENTO_CODIGO','EVENTO_ID','EVENTO_TITULO','EVENTO_NOMBRE_HABLADO','EVENTO_PRECIO','EVENTO_FECHAINI','EVENTO_FECHAFIN','EVENTO_SITUACION','EVENTO_DESCRIPCION'], scoped.eventos.map(e => [eventCode[e.id], e.id, e.titulo || '', e.nombreHablado || e.nombre_hablado || '', num(e.precio), e.fechaIni || '', e.fechaFin || '', e.situacion || 'En curso', e.descripcion || '']));
    ...
    const seenTicketImageRowsV81 = new Set();
    Object.entries(scoped.ticketImages || {}).forEach(([fullKey, image]) => {
      const info = ticketBackupPartsV80(fullKey, image, eventCode);
      const evCode = info.eventCode || '';
      const key = info.innerKey || ticketInnerKeyFromKey(fullKey);
      const hasTk = !!(ticketTokenV81(fullKey) || ticketTokenV81(key));
      if(hasTk && (!info.eventToken || !key)) return;
      const rowId = (info.eventToken || '') + '|' + key;
      if(seenTicketImageRowsV81.has(rowId)) return;
      seenTicketImageRowsV81.add(rowId);
    ...
    try{
      ...
      console.warn('[ControlEvent v4_0_exp] No se pudo añadir Cuadre Banco al BACKUP.',bankBackupError);
  ...
  async function addDocumentosInfoEventoV85(wb, x){
    ...
    try{
      ...
      console.warn('[ControlEvent v4_0_exp] No se pudo añadir DOCUMENTOS DEL EVENTO al INFOEVENTO.', error);
    }
  }
      ...
      if(window.ControlEventExcel && typeof window.ControlEventExcel.protectWorkbook === 'function'){
        ...
        for(const ws of wb.worksheets){
          try{
            ...
            console.warn('[ControlEvent v4_0_exp] No se pudo proteger hoja INFOEVENTO', ws?.name, protectError);
          }
        }
      }
    }catch(error){
      console.warn('[ControlEvent v4_0_exp] No se pudo aplicar protección final al INFOEVENTO.', error);
    }
  ...
  document.addEventListener('click', ev => {
    ...
    if(excel){ ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation(); const direct = window.ControlEventV460?.directInfoEvento; if(typeof direct === 'function') direct(); else exportExcelSafe(); return false; }
    const seed = ev.target?.closest?.('#btnExportSeed,.mobile-menu-action[data-target="btnExportSeed"]');
    if(seed && isGD()){ ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation(); const direct = window.ControlEventV460?.directBackup; if(typeof direct === 'function') direct(); else exportSeedSafe(); return false; }
  }, true);
...
/* ControlEvent v4_0_exp - JS legacy extraido de public/index.html. Bloque inline #63. */
