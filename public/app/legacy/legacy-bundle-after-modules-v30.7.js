...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #55. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #56. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #57. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #58. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #59. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #60. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #61. */
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #62. */
...
(function(){
  ...
  function isPenaDonaFundacionIncomeV457(c){
    const name = up(c?.persona?.nombre || personName(c?.personaId) || c?.nombre || '');
    return name === 'PENA_DONA_A_FUNDACION' || name.includes('PENA_DONA_A_FUNDACION') || name.includes('PENA DONA A FUNDACION');
  }
  function isPendingIncomeV457(c){
    const s = up(c?.situacion || c?.ingreso || c?.tipoIngreso || '');
    return s === 'PENDIENTE' || s.includes('PENDIENTE') || s.includes('PTE');
  }
  function penaDonaFundacionInfoV457(){
    const rowsPena = collabs().filter(c => isPenaDonaFundacionIncomeV457(c));
    const paidRowsPena = rowsPena.filter(c => !isPendingIncomeV457(c));
    return {exists: rowsPena.length > 0, paid: paidRowsPena.length > 0, amount: sum(paidRowsPena.map(c => c.total))};
  }
  function opValues(){
    const b = budget();
    const op = b.operativa || {};
    ...
    const saldoActual = Number.isFinite(Number(op.saldoActual)) ? num(op.saldoActual) : ingresoDinero - gastosRealizados;
    const saldoOperativo = Number.isFinite(Number(op.saldoOperativo)) ? num(op.saldoOperativo) : presupuesto - gastosPrevistos;
    const penaInfo = penaDonaFundacionInfoV457();
    const penaDonaFundacion = penaInfo.paid ? num(op.penaDonaFundacion ?? penaInfo.amount) : 0;
    const donacionFinal = penaInfo.paid ? penaDonaFundacion : 0;
    const hasDonacionFinal = !!penaInfo.paid;
    return {donation,presupuesto,gastosRealizados,pendiente,gastosPrevistos,ingresoDinero,saldoActual,saldoOperativo,penaDonaFundacion,donacionFinal,donacion:donacionFinal,hasPenaDonaFundacion:!!penaInfo.exists,penaDonaFundacionPaid:!!penaInfo.paid,hasDonacionFinal,valoracion:gastosPrevistos + donation};
  }
  ...
  function makeEventChart(){
    const canvas = document.createElement('canvas');
    canvas.width = 1800; canvas.height = 1080;
    const ctx = canvas.getContext('2d');
    const op = opValues();
    const cs = collabs();
    const buys = compras();
    ...
    if(op.hasDonacionFinal){
      rows.splice(4, 0, ['DONACION', op.donacionFinal, [{label:'Donacion', value:Math.abs(op.donacionFinal), displayValue:op.donacionFinal, color:'#0b1f3a'}]]);
    }
  ...
  function setupWorkbook(ExcelJS){
    ...
    wb.created = new Date();
    const border = {top:{style:'thin', color:{argb:'FFDDE2EA'}},left:{style:'thin', color:{argb:'FFDDE2EA'}},bottom:{style:'thin', color:{argb:'FFDDE2EA'}},right:{style:'thin', color:{argb:'FFDDE2EA'}}};
    const fills = {title:'FF111827', navy:'FF0B1F3A', ok:'FFECFDF5', bad:'FFFEF2F2', warn:'FFFFE4EC', soft:'FFF8FAFC', white:'FFFFFFFF'};
    ...
    function euro(ws,r,c,v,fill='white',bold=false,color='FF111827'){ const cell = ws.getCell(r,c); cell.value = num(v); cell.numFmt = moneyFmt; paint(cell,fill,bold,color); cell.alignment = {vertical:'middle', horizontal:'right', wrapText:false}; return cell; }
  ...
  async function exportExcelV257(){
    ...
    wsRes.getRow(r++).height = 8;
    const opRowsV455 = [['INGRESO TOTAL',op.presupuesto,'white'],['GASTOS PREVISTOS',op.gastosPrevistos,'white'],['GASTOS REALIZADOS',op.gastosRealizados,'white'],['PTE. COMPRA U OTROS GASTOS',op.pendiente,'warn'],['SALDO ACTUAL',op.saldoActual,op.saldoActual >= 0 ? 'ok' : 'bad']];
    if(op.hasDonacionFinal) opRowsV455.push(['DONACION',op.donacionFinal,'navy']);
    opRowsV455.push(['SALDO OPERATIVO',op.saldoOperativo,op.saldoOperativo >= 0 ? 'ok' : 'bad'],['VALORACION DEL EVENTO',op.valoracion,'white']);
    opRowsV455.forEach(([label,val,fill]) => { const color = fill==='warn'?'FFBE123C':(fill==='title'||fill==='navy'?'FFFFFFFF':'FF111827'); x.text(wsRes,r,1,label,fill,true,color); x.euro(wsRes,r++,2,val,fill,true,color); });
    ...
    addImage(wb, wsGraf, makeEventChart(), 3, 1, 1500, 880);
    for(let rr = 3; rr <= 47; rr++) wsGraf.getRow(rr).height = 20;
      ...
      if(window.ControlEventExcel && typeof window.ControlEventExcel.protectWorkbook === 'function'){
        ...
        for(const ws of wb.worksheets){
          try{
            ...
            console.warn('[ControlEvent v45.7] No se pudo proteger hoja INFOEVENTO', ws?.name, protectError);
          }
        }
      }
    }catch(error){
      console.warn('[ControlEvent v45.7] No se pudo aplicar protección final al INFOEVENTO.', error);
    }
...
/* ControlEvent v45.7 - JS legacy extraido de public/index.html. Bloque inline #63. */
