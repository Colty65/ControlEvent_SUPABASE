/* ControlEvent v8.2.1_prod - Fotos TKxx/INGRESOS con claves vivas y exportación limpia.
  ...
  function normalizeStore(){
    ...
    ['ticketImages','ticketImageRefs'].forEach(name => {
      ...
      Object.entries({...store}).forEach(([key,value]) => {
        ...
        if(!lk){ delete store[key]; changed = true; return; }
        if(lk !== key){
          if(!store[lk]) store[lk] = value;
          delete store[key];
          changed = true;
        }
      });
