/* ControlEvent v44.6 - Optimización controlada de render y gráficas.
...
(function(){
  ...
  function call(name, args){
    ...
    try{ return fn.apply(window, args || []); }catch(error){ console.warn('[ControlEvent v44.6] Error en ' + name, error); return undefined; }
  }
  function currentTab(){
  ...
  function pruneInactive(active, reason){
    // v44.6: limpieza selectiva. No se barre todo en cada render.
  ...
  function renderStableGraficas(){
    ...
    if(typeof stable === 'function') return stable({force:true, reason:'v44.6'});
  ...
  function patchEventChange(){
    ...
    const wrapped = async function(){
      ...
      setTimeout(() => {
        try{ pruneInactive(currentTab(), 'event-change'); renderV443(); }catch(error){ console.warn('[ControlEvent v44.6] render tras cambio de evento', error); }
      }, 0);
      return result;
    };
  ...
  function applyVersion(){
    ...
    // v44.6: actualización ligera de versión. Evita recorrer todo el DOM en cada instalación.
  ...
  function install(){
    window.__ceV443Stats = window.__ceV443Stats || {prunes:0, clearedNodes:0, installedAt:new Date().toISOString(), mode:'v44.6-selective'};
    try{ window.__ceDisableLegacyBarGraficas = true; window.__ceStableGraficasV435 = true; }catch(_){ }
    applyVersion();
