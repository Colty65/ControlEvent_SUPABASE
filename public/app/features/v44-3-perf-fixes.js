/* ControlEvent v3.2_prod - Optimización controlada de render y gráficas.
...
(function(){
  ...
  function call(name, args){
    ...
    try{ return fn.apply(window, args || []); }catch(error){ console.warn('[ControlEvent v3.2_prod] Error en ' + name, error); return undefined; }
  }
  function currentTab(){
  ...
  function patchEventChange(){
    ...
    const wrapped = async function(){
      ...
      setTimeout(() => {
        try{ pruneInactive(currentTab(), 'event-change'); renderV443(); }catch(error){ console.warn('[ControlEvent v3.2_prod] render tras cambio de evento', error); }
      }, 0);
      return result;
    };
