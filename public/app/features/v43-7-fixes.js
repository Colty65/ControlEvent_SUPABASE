/* ControlEvent v44.0 - exportaciones seguras, edición sin falso duplicado, mapa de recursos y globos de borrado. */
(function(){
  'use strict';
  ...
  async function tryServerBackup(scope){
    const res = await fetch(`/api/export/backup?scope=${encodeURIComponent(scope)}&eventId=${encodeURIComponent(selectedEventId())}&v=44.0`, {cache:'no-store'});
