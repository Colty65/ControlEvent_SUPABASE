...
(function(){
  ...
  const ITV_LANGUAGE_BUILD='20260902-P123-DIALOGUE-STATE-AUTHORITY-ADAPTIVE-ITV-PDF-GUARD-NHC';
  ...
  function renderFinishState(s,updatePhase=true){
    ...
    if(isLanguage&&!s.aborted){cert.textContent=isDialogue?`🗣️ DIÁLOGO · ${fmtN(s.done)} turnos · HILO ${dm.continuityPct}% · ${fmtN(errors)} KO · foco ${dm.focusBreaks} · promesas ${dm.emptyPromises} · PERF ${fmtN(pv.KO)} KO/${fmtN(pv.WARN)} avisos`:`🧭 ${manifest?.languageProfile?.label||'LENGUAJE'} · FUNCIONAL ${coverage}% · ${fmtN(s.ok)}/${fmtN(s.done)} · ${fmtN(errors)} KO · ${fmtN(warns)} avisos · PERF ${fmtN(pv.KO)} KO/${fmtN(pv.WARN)} avisos`;const certScore=isDialogue?dm.continuityPct:coverage;cert.className='zt-cert '+(certScore>=90?'good':certScore>=50?'warn':'bad');}
...
Fallback: ${esc(r.usage.fallbackReason)}</span>`:''}${perfBadge}</div><div class="zt-ms">${fmtN(r.durationMs)} ms${r.usage?`
${fmtE(r.usage.costEur)} · ${fmtN(r.usage.calls)} IA · ${fmtN(r.usage.tokens)} tok`:''}${phase}</div></div>`;}
  ...
  function lightCapabilityCall(x={}){return{tool:trim(x.tool),effectiveOperation:trim(x.effectiveOperation),effectiveSubject:x.effectiveSubject||{},rawArgs:x.rawArgs||{},normalizedArgs:x.normalizedArgs||{},envelope:x.envelope||x?.audit?.envelope||null,audit:x.audit?{ok:x.audit.ok!==false,classification:trim(x.audit.classification),issues:arr(x.audit.issues),repairs:arr(x.audit.repairs),operation:trim(x.audit.operation),effectiveOperation:trim(x.audit.effectiveOperation),signature:trim(x.audit.signature),registryVersion:trim(x.audit.registryVersion)}:null,error:trim(x.error)}}
