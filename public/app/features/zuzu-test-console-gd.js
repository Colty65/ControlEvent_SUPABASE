...
(function(){
  ...
  function modal(){return `<div id="ceZuzuTestOverlay" role="dialog" aria-modal="true" aria-label="ITV de Zuzu"><div class="zt-modal">
    <div class="zt-head"><h2>🧪 ITV de Zuzu</h2><span class="zt-sub">Baterías autogeneradas o Excel · ORÁCULO ACTIVO · misma tubería que Zuzu · build ${ITV_BUILD} · SOLO LECTURA · solo GD</span><div class="zt-spacer"></div><div class="zt-head-actions"><button type="button" class="zt-action" id="ztAntonioLab" title="Abrir ZUZU LAB V3.12.2A · base estable + fecha/personaje/ghost guard">🎙️ ZUZU</button><button class="zt-action" id="ztInterpreter" title="Prueba aislada: Gemini interpreta 30 escenarios × 3, sin ejecutar CE">🧠 INTÉRPRETE</button><button class="zt-action" id="ztExecution" title="Gemini V2.3 interpreta y ControlEvent ejecuta realmente en solo lectura">⚙️ EJECUCIÓN CE</button><button class="zt-action report" id="ztDownload">⬇ JSON LIGHT</button><button class="zt-action report" id="ztDownloadFull" title="Informe forense completo, más pesado">⬇ FULL</button><button class="zt-action print" id="ztPrint">🖨 PDF</button><button class="zt-action close" id="ztClose">✕ CERRAR</button></div></div>
    <div class="zt-top">
  ...
  function openAntonioLab(){
    const root=$('ceZuzuTestOverlay');if(!root)return;
    closeAntonioLab();
    const ov=document.createElement('div');ov.id='ceAntonioLabOverlay';ov.innerHTML=`<div class="cal-shell"><div class="cal-head"><b>🎙️ ZUZU LAB V3.12.2A</b><span>Base estable · VNext/CE · Algenib · fecha/personaje</span><span class="cal-spacer"></span><button type="button" class="cal-close" id="calClose">✕ VOLVER A ITV</button></div><iframe id="calFrame" title="ZUZU LAB V3.12.2A" allow="microphone; autoplay" src="/antonio-lab.html?embedded=itv&v=20260905-V3122A"></iframe></div>`;
