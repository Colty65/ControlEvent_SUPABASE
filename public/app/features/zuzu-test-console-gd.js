...
(function(){
  ...
  const ITV_LANGUAGE_BUILD='20260901-P1221-ENTITY-QUERY-CURRENT-VIEW-SUMMARY-FALLBACK-GOLDEN110-NHC';
  ...
  function modal(){return `<div id="ceZuzuTestOverlay" role="dialog" aria-modal="true" aria-label="ITV de Zuzu"><div class="zt-modal">
    ...
    <div class="zt-top">
      <div class="zt-panel"><div class="zt-panel-head"><h3>Datos reales · batería</h3><button id="ztImportExcel" class="zt-mini-primary">📥 CARGAR EXCEL</button><button id="ztGenerate" class="zt-mini-primary">↻ NUEVA BATERÍA</button><input id="ztExcelFile" type="file" accept=".xlsx,.xlsm" style="display:none!important"></div><div id="ztData" class="zt-data"><span class="zt-pill">Cargando datos…</span></div><div id="ztSeedInfo" class="zt-live zt-seed-strip"></div>
        <div class="zt-language"><div class="zt-language-title"><span>🧭 ITV ZUZU · GOLDEN 110 fija + 260 preguntas de exploración</span></div><div class="zt-language-grid">
  ...
  async function loadLanguageBattery(level='BASIC'){
    ...
    try{
      ...
      setPhase(`Batería ${data?.languageProfile?.label||lvl} preparada · ${count} preguntas · MOTOR: VNEXT · expectativa inicial ${data?.languageProfile?.expectedBand||'—'}. Pulsa INICIAR y la ITV recorrerá toda la batería automáticamente.`);
  ...
  function renderFinishState(s,updatePhase=true){
    ...
    if(isLanguage&&!s.aborted){cert.textContent=`🧭 ${manifest?.languageProfile?.label||'LENGUAJE'} · FUNCIONAL ${coverage}% · ${fmtN(s.ok)}/${fmtN(s.done)} · ${fmtN(errors)} KO · ${fmtN(warns)} avisos · PERF ${fmtN(pv.KO)} KO/${fmtN(pv.WARN)} avisos`;cert.className='zt-cert '+(coverage>=90?'good':coverage>=50?'warn':'bad');}
