...
(function(){
  ...
  function modal(){return `<div id="ceZuzuTestOverlay" role="dialog" aria-modal="true" aria-label="ITV de Zuzu"><div class="zt-modal">
    ...
    <div class="zt-top">
      <div class="zt-panel"><div class="zt-panel-head"><h3>Datos reales · batería</h3><button id="ztImportExcel" class="zt-mini-primary">📥 CARGAR EXCEL</button><button id="ztGenerate" class="zt-mini-primary">↻ NUEVA BATERÍA</button><input id="ztExcelFile" type="file" accept=".xlsx,.xlsm" style="display:none!important"></div><div id="ztData" class="zt-data"><span class="zt-pill">Cargando datos…</span></div><div id="ztSeedInfo" class="zt-live zt-seed-strip"></div>
        <div class="zt-language"><div class="zt-language-title"><span>🧭 ALCANCE DEL LENGUAJE ZUZU · 260 preguntas en 4 baterías</span></div><div class="zt-language-grid">
  ...
  async function loadLanguageBattery(level='BASIC'){
    ...
    try{
      ...
      setPhase(`Batería ${data?.languageProfile?.label||lvl} preparada · ${count} preguntas · expectativa inicial ${data?.languageProfile?.expectedBand||'—'}. Pulsa INICIAR y la ITV recorrerá toda la batería automáticamente.`);
  ...
  function renderPreview(){const c=preview?.dataCounts||{},t=preview?.tests||{},isExcel=(preview?.source==='excel'||batterySource==='excel'),isLanguage=(preview?.source==='language'||batterySource==='language'),lp=preview?.languageProfile||{};$('ztData').innerHTML=(isLanguage?[["Origen","Alcance lenguaje"],["Nivel",lp.label||batteryCode],["Preguntas",t['FULL-CERT']||lp.count],["Expectativa",lp.expectedBand||'—'],["NHC","solo ITV"]]:isExcel?[['Origen','Excel'],['Código',preview?.batteryCode||batteryCode],['Preguntas',t['FULL-CERT']||t['AI-SMOKE']],['AI-SMOKE',t['AI-SMOKE']],['FULL-CERT',t['FULL-CERT']]]:[['Eventos',c.events],['Personas',c.people],['Productos',c.products],['Tiendas',c.stores],['Compras',c.purchases],['Ingresos',c.incomes],['DOC',c.documents],['Fototickets',c.ticketImages],['Donaciones',c.donationLines],['Hitos',c.hitos],['LG',c.lgs],['FAST',t.FAST],['AI-SMOKE',t['AI-SMOKE']],['FULL-CERT',t['FULL-CERT']]]).map(x=>`<span class="zt-pill">${esc(x[0])}: <strong>${typeof x[1]==='number'?fmtN(x[1]):esc(x[1]||'—')}</strong></span>`).join('');if($('ztSeedInfo'))$('ztSeedInfo').textContent=isLanguage?`Batería ${lp.label||batteryCode} · ${lp.count||t['FULL-CERT']||0} preguntas · expectativa inicial ${lp.expectedBand||'—'} · NHC: estas frases son solo pruebas, nunca reglas del runtime.`:isExcel?`Batería Excel ${preview?.batteryCode||batteryCode} · semilla estable ${batterySeed} · ${t['FULL-CERT']||0} preguntas · FULL-CERT conserva toda la conversación en el mismo ledger.`:`Batería ${batteryClock||'reloj local'} · semilla ${batterySeed} · FAST recorre todos los registros estructurales y FULL-CERT incluye continuidad Z1 sobre todos los eventos reales.`;}
