...
(function(){
  ...
  let streamWatchdog=null,lastStreamAt=0,currentCase=null;
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='ceZuzuTestConsoleStyle';s.textContent=`
      ...
      #ceZuzuTestOverlay button{font-family:inherit;color:#0f172a!important;-webkit-text-fill-color:currentColor!important;opacity:1}
      ...
      .zt-action.report{background:#0f766e!important;border-color:#0f766e!important;color:#fff!important}.zt-action.print{background:#475569!important;border-color:#475569!important;color:#fff!important}.zt-action.close{min-width:92px!important;color:#991b1b!important;border-color:#fecaca!important}
      ...
      .zt-controls{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:7px}.zt-controls button{border:1px solid #94a3b8!important;background:#fff!important;border-radius:9px!important;padding:7px 10px!important;font-weight:950!important;cursor:pointer!important;white-space:nowrap!important}.zt-controls .primary{background:#0284c7!important;color:#fff!important;border-color:#0284c7!important}.zt-controls .danger{background:#fff7f7!important;color:#b91c1c!important;border-color:#fca5a5!important}.zt-controls .next{background:#eef2ff!important;color:#3730a3!important;border-color:#a5b4fc!important}.zt-controls label{font-size:11px;font-weight:850;color:#475569}.zt-controls input,.zt-controls select{border:1px solid #cbd5e1;border-radius:8px;padding:6px;font-weight:850;background:#fff;color:#0f172a}.zt-controls input{width:78px}.zt-controls .grow{margin-left:auto}
  ...
  function close(){stopStreamWatchdog();if(currentAbort)currentAbort.abort();currentAbort=null;$('ceZuzuTestOverlay')?.remove();}
  ...
  function setRunning(on){$('ztStart').disabled=on;$('ztGenerate').disabled=on;$('ztStop').disabled=!on;$('ztNext').disabled=on;document.querySelectorAll('.zt-mode').forEach(b=>b.disabled=on);}
  ...
  function stop(){currentAbort?.abort();setPhase('Detención solicitada. Se cancela el caso actual y no se lanzarán más pruebas.');setLive('Cancelando…');}
  ...
  async function run(onlyIssues){
    ...
    lastMode=document.querySelector('.zt-mode.active')?.dataset.mode||lastMode;resetRun();currentAbort=new AbortController();setRunning(true);setPhase(`${lastMode}: preparando ejecución…`);startStreamWatchdog();
    try{
    ...
    }catch(e){if(e.name==='AbortError'){if(!/dejó de enviar señal/.test($('ztPhase')?.textContent||''))setPhase('Prueba detenida. Puedes reanudar o cambiar de chequeo sin cerrar la ventana.');}else setPhase('Error de ejecución: '+(e.message||e),true);}
    finally{stopStreamWatchdog();currentAbort=null;setRunning(false);cacheCurrent();}
  }
  ...
  function renderFinishState(s,updatePhase=true){const good=s.ko===0&&!s.aborted&&s.done>0,warnings=num(s.warn);if(updatePhase)setPhase(s.aborted?'Ejecución detenida.':s.ko?'Ejecución terminada con KO.':warnings?'Ejecución terminada sin KO, pero con avisos de tiempo/servicio.':'Ejecución terminada sin incidencias.');const cert=$('ztCert');if(!cert)return;if(s.aborted){cert.textContent='⏹ PRUEBA DETENIDA';cert.className='zt-cert bad';}else if(s.ko){cert.textContent=`🔴 ${fmtN(s.ko)} KO · REVISAR`;cert.className='zt-cert bad';}else if(warnings){cert.textContent=`🟠 SUPERADA CON ${fmtN(warnings)} AVISO${warnings===1?'':'S'}`;cert.className='zt-cert warn';}else if(good){cert.textContent='🟢 CERTIFICACIÓN DEL MODO SUPERADA';cert.className='zt-cert good';}}
  ...
  function renderModeStatuses(){for(const mode of MODES){const el=$('ztModeStatus'+mode),s=modeCache[mode]?.summary;if(!el)continue;el.className='zt-mode-status';if(!s){el.textContent='Pendiente';continue;}if(s.ko){el.textContent=`${s.ko} KO`;el.classList.add('bad');}else if(s.warn){el.textContent=`${s.warn} aviso${s.warn===1?'':'s'}`;el.classList.add('warn');}else{el.textContent='✓ OK';el.classList.add('good');}}}
