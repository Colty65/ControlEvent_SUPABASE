...
(function(){
  ...
  const ITV_BUILD='20260823-PRESENTACION-RAW4';
  const ITV_OBSERVATION_MODE=true;
  ...
  async function runPaidCases(onlyIssues){
    ...
    try{
      ...
      lastSummary={type:'summary',mode:lastMode,done,total,ok,warn,ko,costEur,calls,tokens,aborted,incomplete,budgetStopped,contextWarmups,contextWarmupFailures,finishedAt:new Date().toISOString(),certified:false,observationMode:'ESCAPE_LIBRE',oracleEnabled:false};
  ...
  function renderFinishState(s,updatePhase=true){const incomplete=num(s.done)<num(s.total),errors=num(s.ko);if(updatePhase)setPhase(s.aborted?'Ejecución detenida.':errors?`Observación terminada con ${fmtN(errors)} error${errors===1?'':'es'} técnico${errors===1?'':'s'}.`:incomplete?'Observación incompleta.':'Observación completa. No se ha evaluado si las respuestas son buenas o malas.');const cert=$('ztCert');if(!cert)return;if(s.aborted){cert.textContent='⏹ OBSERVACIÓN DETENIDA';cert.className='zt-cert bad';}else if(errors){cert.textContent=`🔴 ${fmtN(errors)} ERROR${errors===1?'':'ES'} TÉCNICO${errors===1?'':'S'} · SIN JUICIO SEMÁNTICO`;cert.className='zt-cert bad';}else if(incomplete){cert.textContent=`🟠 OBSERVACIÓN INCOMPLETA · ${fmtN(s.done)}/${fmtN(s.total)}`;cert.className='zt-cert warn';}else{cert.textContent=`👁 OBSERVACIÓN COMPLETA · ${fmtN(s.done)} RESPUESTAS · ORÁCULO DESCONECTADO`;cert.className='zt-cert good';}}
  ...
  function rowHtml(r){const mb=r.usage?modelBadge(r.usage):'';return `<div class="zt-row ${esc(r.status)}" data-status="${esc(r.status)}" data-group="${esc(r.group)}"><div class="zt-status">${esc(r.status)}</div><div class="zt-cell"><b>${esc(r.group)}</b><span>${esc(r.id)}</span></div><div class="zt-cell"><b>${esc(r.label)}</b><span>${esc(r.prompt||'')}</span></div><div class="zt-cell zt-expected"><b>Referencia de batería · NO EVALÚA</b><span>${esc(r.expected||'')}</span></div><div class="zt-cell zt-actual"><b>Obtenido</b><span>${esc(r.actual||'')}</span>${r.tools?.length?`<span>\nHerramientas: ${esc(r.tools.join(', '))}</span>`:''}${mb?`<span>\n${esc(mb)}</span>`:''}${r.usage?.fallbackReason?`<span>\nFallback: ${esc(r.usage.fallbackReason)}</span>`:''}</div><div class="zt-ms">${fmtN(r.durationMs)} ms${r.usage?`\n${fmtE(r.usage.costEur)}`:''}${mb?`\n${esc(mb.replace('Modelo IA: ',''))}`:''}</div></div>`;}
  ...
  function groups(){return ['TODOS','OBSERVED','ERROR',...Array.from(new Set(rows.map(r=>r.group).filter(Boolean)))];}
  function renderFilters(){if(!$('ztFilters'))return;$('ztFilters').innerHTML=groups().map(g=>`<button class="zt-filter ${activeFilter===g?'active':''}" data-f="${esc(g)}">${esc(g)}${g==='OBSERVED'?` (${rows.filter(r=>r.status==='OBSERVED').length})`:g==='ERROR'?` (${rows.filter(r=>r.status==='ERROR').length})`:''}</button>`).join('');$('ztFilters').querySelectorAll('button').forEach(b=>b.onclick=()=>{activeFilter=b.dataset.f;renderFilters();applyFilter();});}
  function applyFilter(){document.querySelectorAll('#ztResults .zt-row').forEach(el=>{const show=activeFilter==='TODOS'||(activeFilter==='OBSERVED'&&el.dataset.status==='OBSERVED')||(activeFilter==='ERROR'&&el.dataset.status==='ERROR')||el.dataset.group===activeFilter;el.style.display=show?'grid':'none';});}
  ...
  function saveHistory(s){const h=history();h.unshift({at:new Date().toISOString(),mode:lastMode,done:s.done,total:s.total,observed:s.ok,errors:s.ko,costEur:s.costEur,calls:s.calls,tokens:s.tokens,observationMode:lastMode==='FAST'?false:'ESCAPE_LIBRE'});localStorage.setItem(HISTORY_KEY,JSON.stringify(h.slice(0,30)));}
