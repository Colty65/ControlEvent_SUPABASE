...
(function(){
  ...
  const ITV_LANGUAGE_BUILD='20260902-P127-NATURAL-VISIBLE-CONTENT-DIALOGUE-NHC';
  ...
  function modal(){return `<div id="ceZuzuTestOverlay" role="dialog" aria-modal="true" aria-label="ITV de Zuzu"><div class="zt-modal">
    ...
    <div class="zt-top">
      <div class="zt-panel"><div class="zt-panel-head"><h3>Datos reales · batería</h3><button id="ztImportExcel" class="zt-mini-primary">📥 CARGAR EXCEL</button><button id="ztGenerate" class="zt-mini-primary">↻ NUEVA BATERÍA</button><input id="ztExcelFile" type="file" accept=".xlsx,.xlsm" style="display:none!important"></div><div id="ztData" class="zt-data"><span class="zt-pill">Cargando datos…</span></div><div id="ztSeedInfo" class="zt-live zt-seed-strip"></div>
        <div class="zt-language"><div class="zt-language-title"><span>🧭 ITV ZUZU · GOLDEN 110 + DIÁLOGO adaptativo + 260 preguntas de exploración</span></div><div class="zt-language-grid">
          <button class="zt-language-btn" data-level="GOLDEN"><b>GOLDEN · 110</b><small>Regresión fija P1.17 · mismas preguntas cada versión.</small></button>
  ...
  async function loadLanguageBattery(level='BASIC'){
    ...
    try{
      ...
      setPhase(`Batería ${data?.languageProfile?.label||lvl} preparada · ${count} ${lvl==='DIALOGUE'?'turnos adaptativos':'preguntas'} · MOTOR: VNEXT · expectativa ${data?.languageProfile?.expectedBand||'—'}. ${lvl==='DIALOGUE'?'Solo el primer mensaje está fijado; los siguientes los genera el usuario sintético a partir de la respuesta real de Zuzu. ':''}Pulsa INICIAR.`);
  ...
  function renderPreview(){const c=preview?.dataCounts||{},t=preview?.tests||{},isExcel=(preview?.source==='excel'||batterySource==='excel'),isLanguage=(preview?.source==='language'||batterySource==='language'),lp=preview?.languageProfile||{},isDialogue=trim(lp.id).toUpperCase()==='DIALOGUE';$('ztData').innerHTML=(isLanguage?[["Origen",isDialogue?"Conversación adaptativa":"Alcance lenguaje"],["Motor","VNEXT"],["Nivel",lp.label||batteryCode],[isDialogue?"Turnos":"Preguntas",t['FULL-CERT']||lp.count],["Expectativa",lp.expectedBand||'—'],["NHC","solo ITV"]]:isExcel?[['Origen','Excel'],['Código',preview?.batteryCode||batteryCode],['Preguntas',t['FULL-CERT']||t['AI-SMOKE']],['AI-SMOKE',t['AI-SMOKE']],['FULL-CERT',t['FULL-CERT']]]:[['Eventos',c.events],['Personas',c.people],['Productos',c.products],['Tiendas',c.stores],['Compras',c.purchases],['Ingresos',c.incomes],['DOC',c.documents],['Fototickets',c.ticketImages],['Donaciones',c.donationLines],['Hitos',c.hitos],['LG',c.lgs],['FAST',t.FAST],['AI-SMOKE',t['AI-SMOKE']],['FULL-CERT',t['FULL-CERT']]]).map(x=>`<span class="zt-pill">${esc(x[0])}: <strong>${typeof x[1]==='number'?fmtN(x[1]):esc(x[1]||'—')}</strong></span>`).join('');if($('ztSeedInfo'))$('ztSeedInfo').textContent=isLanguage?`Batería ${lp.label||batteryCode} · MOTOR VNEXT · ${lp.count||t['FULL-CERT']||0} ${isDialogue?'turnos adaptativos':'preguntas'} · expectativa ${lp.expectedBand||'—'} · NHC: las pruebas no son reglas del runtime.`:isExcel?`Batería Excel ${preview?.batteryCode||batteryCode} · semilla estable ${batterySeed} · ${t['FULL-CERT']||0} preguntas · FULL-CERT conserva toda la conversación en el mismo ledger.`:`Batería ${batteryClock||'reloj local'} · semilla ${batterySeed} · FAST recorre todos los registros estructurales y FULL-CERT incluye continuidad Z1 sobre todos los eventos reales.`;}
  ...
  function renderFinishState(s,updatePhase=true){
    ...
    if(isLanguage&&!s.aborted){cert.textContent=isDialogue?`🗣️ DIÁLOGO · ${fmtN(s.done)} turnos · HILO ${dm.continuityPct}% · ${fmtN(errors)} KO · ZUZU ${(num(pf.medianMs)/1000).toFixed(1)}s med · SIM ${(num(pf.simulatorMedianMs)/1000).toFixed(1)}s · foco ${dm.focusBreaks} · promesas ${dm.emptyPromises}`:`🧭 ${manifest?.languageProfile?.label||'LENGUAJE'} · FUNCIONAL ${coverage}% · ${fmtN(s.ok)}/${fmtN(s.done)} · ${fmtN(errors)} KO · ${fmtN(warns)} avisos · PERF ${fmtN(pv.KO)} KO/${fmtN(pv.WARN)} avisos`;const certScore=isDialogue?dm.continuityPct:coverage;cert.className='zt-cert '+(certScore>=90?'good':certScore>=50?'warn':'bad');}
...
${fmtE(r.usage.costEur)} · ${fmtN(r.usage.calls)} IA lab · ${fmtN(r.usage.tokens)} tok`:''}${phase}</div></div>`;}
