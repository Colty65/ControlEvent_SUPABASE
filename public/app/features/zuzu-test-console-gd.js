/* ControlEvent v4_1_exp · PRUEBAS ZUZU · Consola GD.
...
(function(){
  ...
  const ITV_LANGUAGE_BUILD='20260902-P2R-MINIMAL-PLANNER-GOLDEN-NHC';
  ...
  function performanceSummary(list=[]){const a=list.filter(r=>num(r?.durationMs)>0),dialogue=a.some(r=>num(r?.dialogueRuntime?.zuzuMs)>0),zuzu=a.map(r=>dialogue?(num(r?.dialogueRuntime?.zuzuMs)||num(r?.performance?.totalMs)||num(r.durationMs)):num(r.durationMs)),sim=a.map(r=>num(r?.dialogueRuntime?.simulatorMs)).filter(v=>v>0),wall=a.map(r=>num(r?.dialogueRuntime?.wallMs)||num(r.durationMs)),calls=a.map(r=>num(r?.usage?.calls)),tokens=a.map(r=>num(r?.usage?.tokens)),cost=a.map(r=>num(r?.usage?.costEur));const avg=x=>x.length?x.reduce((m,v)=>m+v,0)/x.length:0;return{cases:a.length,dialogue,medianMs:percentile(zuzu,.5),p90Ms:percentile(zuzu,.9),p95Ms:percentile(zuzu,.95),maxMs:zuzu.length?Math.max(...zuzu):0,simulatorMedianMs:percentile(sim,.5),simulatorP90Ms:percentile(sim,.9),wallMedianMs:percentile(wall,.5),wallP90Ms:percentile(wall,.9),avgCalls:Number(avg(calls).toFixed(2)),avgTokens:Math.round(avg(tokens)),avgCostEur:Number(avg(cost).toFixed(6)),...usageSplit(a),over12s:zuzu.filter(x=>x>12000).length,over18s:zuzu.filter(x=>x>18000).length};}
  function usageSplit(list=[]){let zuzuCalls=0,simulatorCalls=0,labCalls=0,zuzuTokens=0,simulatorTokens=0,labTokens=0,zuzuCostEur=0,simulatorCostEur=0,labCostEur=0;for(const r of list){const zu=r?.zuzuUsage||r?.usage||{},si=r?.simulatorUsage||{},la=r?.labUsage||{};const zc=num(zu.calls),sc=num(si.calls),zt=num(zu.tokens),st=num(si.totalTokens||si.tokens),ze=num(zu.costEur),se=num(si.costEurApprox||si.costEur);zuzuCalls+=zc;simulatorCalls+=sc;zuzuTokens+=zt;simulatorTokens+=st;zuzuCostEur+=ze;simulatorCostEur+=se;labCalls+=num(la.calls)||(zc+sc);labTokens+=num(la.tokens)||(zt+st);labCostEur+=num(la.costEur)||(ze+se);}const turns=Math.max(1,list.length);return{zuzuCalls,simulatorCalls,labCalls,zuzuTokens,simulatorTokens,labTokens,zuzuCostEur:Number(zuzuCostEur.toFixed(6)),simulatorCostEur:Number(simulatorCostEur.toFixed(6)),labCostEur:Number(labCostEur.toFixed(6)),avgZuzuCalls:Number((zuzuCalls/turns).toFixed(2)),avgSimulatorCalls:Number((simulatorCalls/turns).toFixed(2)),avgLabCalls:Number((labCalls/turns).toFixed(2))};}
  function performanceVerdicts(list=[]){const out={OK:0,WARN:0,KO:0};for(const r of list){const st=trim(r?.performanceStatus||'OK').toUpperCase();if(st==='KO')out.KO++;else if(st==='WARN')out.WARN++;else out.OK++;}return out;}
  function dialogueMetrics(list=[]){const a=arr(list).filter(r=>r?.dialogue);let coherent=0,focusBreaks=0,emptyPromises=0,missingTools=0,focusChanges=0;for(const r of a){const d=r.dialogue||{},as=d.assessment||{},m=d.userMove||{};if(as.previous_coherent!==false)coherent++;if(as.focus_preserved===false&&m.changeFocus!==true)focusBreaks++;if(as.empty_promise===true)emptyPromises++;if(m.requiresTool===true&&!arr(r.tools).length)missingTools++;if(m.changeFocus===true)focusChanges++;}return{turns:a.length,coherent,focusBreaks,emptyPromises,missingTools,focusChanges,continuityPct:a.length?Math.round(coherent*100/a.length):0};}
  ...
  function storedAuth(){
    ...
    for(const key of ['ControlEvent_v4_1_exp_login_user','ControlEvent_ce_acceso_usuario','ControlEvent_auth_user_v509']){
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='ceZuzuTestConsoleStyle';s.textContent=`
      ...
      #ceAntonioLabOverlay{position:absolute!important;inset:0!important;z-index:50!important;background:rgba(15,23,42,.5)!important;padding:10px!important;display:flex!important;align-items:center!important;justify-content:center!important}
      #ceAntonioLabOverlay .cal-shell{width:100%!important;height:100%!important;background:#fff!important;border-radius:16px!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;box-shadow:0 20px 70px rgba(15,23,42,.35)!important}
      #ceAntonioLabOverlay .cal-head{height:46px!important;min-height:46px!important;display:flex!important;align-items:center!important;gap:8px!important;padding:6px 10px!important;background:#0e273b!important;color:#fff!important}
      #ceAntonioLabOverlay .cal-head b{font-size:14px!important}.cal-spacer{flex:1!important}.cal-close{border:1px solid #cbd5e1!important;background:#fff!important;color:#991b1b!important;border-radius:9px!important;padding:7px 11px!important;font-weight:950!important}
      #ceAntonioLabOverlay iframe{width:100%!important;height:100%!important;border:0!important;background:#eef4f8!important;flex:1 1 auto!important;min-height:0!important}
      ...
      .zt-language{margin-top:7px;padding-top:7px;border-top:1px dashed #bae6fd}.zt-language-title{display:flex;align-items:center;gap:7px;margin-bottom:5px;font-size:10px;font-weight:950;color:#075985}.zt-language-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:5px}.zt-language-btn{border:1px solid #cbd5e1!important;background:#fff!important;color:#0f172a!important;border-radius:9px!important;padding:6px 7px!important;text-align:left!important;min-height:45px!important}.zt-language-btn b{display:block;font-size:10px}.zt-language-btn small{display:block;font-size:8px;color:#64748b!important;margin-top:2px;line-height:1.15}.zt-language-btn[data-level="GOLDEN"]{border-top:3px solid #7c3aed!important}.zt-language-btn[data-level="GOLDEN_DIALOGUE"]{border-top:3px solid #4f46e5!important}.zt-language-btn[data-level="BASIC"]{border-top:3px solid #16a34a!important}.zt-language-btn[data-level="MEDIUM"]{border-top:3px solid #ca8a04!important}.zt-language-btn[data-level="HARD"]{border-top:3px solid #ea580c!important}.zt-language-btn[data-level="EXTREME"]{border-top:3px solid #b91c1c!important}.zt-language-btn[data-level="DIALOGUE"]{border-top:3px solid #0e7490!important}.zt-language-btn.active{background:#eff6ff!important;border-color:#0284c7!important}.zt-modes{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.zt-mode{position:relative!important;border:1px solid #cbd5e1!important;border-top:3px solid #cbd5e1!important;background:#fff!important;color:#0f172a!important;-webkit-text-fill-color:#0f172a!important;border-radius:10px!important;padding:7px 8px!important;cursor:pointer!important;text-align:left!important;min-height:56px!important;height:auto!important;overflow:hidden!important}.zt-mode.active{border-color:#0ea5e9!important;border-top-color:#0284c7!important;background:#eff6ff!important;box-shadow:0 0 0 1px rgba(14,165,233,.10)}.zt-mode b{display:block;font-size:11px}.zt-mode small{display:block;color:#64748b;-webkit-text-fill-color:#64748b!important;margin-top:2px;line-height:1.15;font-size:9px}.zt-mode .free{color:#15803d!important;-webkit-text-fill-color:#15803d!important}.zt-mode .paid{color:#b45309!important;-webkit-text-fill-color:#b45309!important}.zt-mode-status{position:absolute;right:7px;top:6px;font-style:normal;font-size:9px;font-weight:950;padding:2px 5px;border-radius:999px;background:#e2e8f0;color:#475569}.zt-mode-status.good{background:#dcfce7;color:#166534}.zt-mode-status.warn{background:#fef3c7;color:#92400e}.zt-mode-status.bad{background:#fee2e2;color:#991b1b}
  ...
  function modal(){return `<div id="ceZuzuTestOverlay" role="dialog" aria-modal="true" aria-label="ITV de Zuzu"><div class="zt-modal">
    <div class="zt-head"><h2>🧪 ITV de Zuzu</h2><span class="zt-sub">Baterías autogeneradas o Excel · ORÁCULO ACTIVO · misma tubería que Zuzu · build ${ITV_BUILD} · SOLO LECTURA · solo GD</span><div class="zt-spacer"></div><div class="zt-head-actions"><button type="button" class="zt-action" id="ztAntonioLab" title="Abrir ZUZU LAB V3.16.1 · conversación continua + Gemini Live 3.1/2.5 + fecha autoritativa">🎙️ ZUZU</button><button class="zt-action" id="ztInterpreter" title="Prueba aislada: Gemini interpreta 30 escenarios × 3, sin ejecutar CE">🧠 INTÉRPRETE</button><button class="zt-action" id="ztExecution" title="Gemini V2.3 interpreta y ControlEvent ejecuta realmente en solo lectura">⚙️ EJECUCIÓN CE</button><button class="zt-action report" id="ztDownload">⬇ JSON LIGHT</button><button class="zt-action report" id="ztDownloadFull" title="Informe forense completo, más pesado">⬇ FULL</button><button class="zt-action print" id="ztPrint">🖨 PDF</button><button class="zt-action close" id="ztClose">✕ CERRAR</button></div></div>
    <div class="zt-top">
      <div class="zt-panel"><div class="zt-panel-head"><h3>Datos reales · batería</h3><button id="ztImportExcel" class="zt-mini-primary">📥 CARGAR EXCEL</button><button id="ztGenerate" class="zt-mini-primary">↻ NUEVA BATERÍA</button><input id="ztExcelFile" type="file" accept=".xlsx,.xlsm" style="display:none!important"></div><div id="ztData" class="zt-data"><span class="zt-pill">Cargando datos…</span></div><div id="ztSeedInfo" class="zt-live zt-seed-strip"></div>
        <div class="zt-language"><div class="zt-language-title"><span>🧭 ITV ZUZU · GOLDEN 110 + GOLDEN DIÁLOGO 14 + DIÁLOGO adaptativo + exploración</span></div><div class="zt-language-grid">
          <button class="zt-language-btn" data-level="GOLDEN"><b>GOLDEN · 110</b><small>Regresión factual fija · mismas preguntas cada versión.</small></button>
          <button class="zt-language-btn" data-level="GOLDEN_DIALOGUE"><b>GOLDEN DIÁLOGO · 14</b><small>Conversación fija P2 · misma carretera cada versión.</small></button>
          <button class="zt-language-btn" data-level="BASIC"><b>BÁSICA · 50</b><small>Simple y explícita · objetivo casi pleno.</small></button>
          <button class="zt-language-btn" data-level="MEDIUM"><b>MEDIA · 60</b><small>Continuidad y composición · ≈90% esperado.</small></button>
          <button class="zt-language-btn" data-level="HARD"><b>DIFÍCIL · 70</b><small>Tablas, contexto, gráficos · &lt;50% de partida.</small></button>
          <button class="zt-language-btn" data-level="EXTREME"><b>EXTREMA · 80</b><small>Cruces y peticiones abiertas · &lt;25% de partida.</small></button>
          <button class="zt-language-btn" data-level="DIALOGUE"><b>DIÁLOGO · 24</b><small>Usuario sintético adaptativo · cada turno sigue la respuesta real.</small></button>
        </div></div>
  ...
  function closeAntonioLab(){const x=$('ceAntonioLabOverlay');if(x)x.remove();}
  window.ceCloseAntonioLab=closeAntonioLab;
  function openAntonioLab(){
    const root=$('ceZuzuTestOverlay');if(!root)return;
    closeAntonioLab();
    const ov=document.createElement('div');ov.id='ceAntonioLabOverlay';ov.innerHTML=`<div class="cal-shell"><div class="cal-head"><b>🎙️ ZUZU LAB V3.16.1</b><span>Oído continuo · VNext/CE · Gemini Live 3.1/2.5 · Iapetus</span><span class="cal-spacer"></span><button type="button" class="cal-close" id="calClose">✕ VOLVER A ITV</button></div><iframe id="calFrame" title="ZUZU LAB V3.16.1" allow="microphone; autoplay" src="/antonio-lab.html?embedded=itv&v=20260905-V3161-LIVE-PROTOCOL"></iframe></div>`;
  ...
  function close(){closeAntonioLab();stopStreamWatchdog();stopOverlayGuard();stopRequested=true;try{currentCaseCancel?.();}catch(_){}try{currentFetchAbort?.abort();}catch(_){}try{currentReader?.cancel();}catch(_){}try{currentAbort?.abort();}catch(_){}currentAbort=null;currentFetchAbort=null;currentCaseCancel=null;currentReader=null;$('ceZuzuTestOverlay')?.remove();document.body.classList.remove('ce-zuzu-itv-open');}

  function bind(){
    $('ztClose').onclick=close;$('ztAntonioLab').disabled=false;$('ztAntonioLab').removeAttribute('disabled');$('ztAntonioLab').onclick=openAntonioLab;$('ztInterpreter').onclick=()=>window.ceOpenZuzuInterpreterLab?.();$('ztExecution').onclick=()=>window.ceOpenZuzuExecutionLab?.();$('ceZuzuTestOverlay').addEventListener('click',e=>{if(e.target.id==='ceZuzuTestOverlay')close();});
    document.querySelectorAll('.zt-mode').forEach(b=>b.onclick=()=>selectMode(b.dataset.mode));
    document.querySelectorAll('.zt-language-btn').forEach(b=>b.onclick=()=>loadLanguageBattery(b.dataset.level));
    $('ztGenerate').onclick=()=>loadPreview(true);$('ztImportExcel').onclick=()=>{if(uiRunning){setPhase('Detén la ejecución antes de cargar otro Excel.');return;}$('ztExcelFile').value='';$('ztExcelFile').click();};$('ztExcelFile').onchange=()=>{const f=$('ztExcelFile').files&&$('ztExcelFile').files[0];if(f)importBatteryExcel(f);};$('ztStart').onclick=()=>run(false);$('ztRetry').onclick=()=>run(true);$('ztNext').onclick=nextMode;$('ztDownload').onclick=()=>downloadReport(false);$('ztDownloadFull').onclick=()=>downloadReport(true);$('ztPrint').onclick=printReport;$('ztHistoryView').onclick=loadHistoricalRun;$('ztHistoryReplay').onclick=replayHistoricalRun;$('ztHistoryDelete').onclick=deleteHistoricalRun;$('ztReplaySeed').onclick=replayManualSeed;
  ...
  async function loadLanguageBattery(level='BASIC'){
    ...
    try{
      ...
      setPhase(`Batería ${data?.languageProfile?.label||lvl} preparada · ${count} ${lvl==='DIALOGUE'?'turnos adaptativos':lvl==='GOLDEN_DIALOGUE'?'turnos fijos':'preguntas'} · MOTOR: VNEXT · expectativa ${data?.languageProfile?.expectedBand||'—'}. ${lvl==='DIALOGUE'?'Solo el primer mensaje está fijado; los siguientes los genera el usuario sintético a partir de la respuesta real de Zuzu. ':lvl==='GOLDEN_DIALOGUE'?'Los 14 turnos están congelados para comparar versiones sobre la misma carretera. ':''}Pulsa INICIAR.`);
    }catch(e){setPhase('No se pudo preparar la batería de alcance: '+(e.message||e),true);}
  }
  ...
  function renderPreview(){const c=preview?.dataCounts||{},t=preview?.tests||{},isExcel=(preview?.source==='excel'||batterySource==='excel'),isLanguage=(preview?.source==='language'||batterySource==='language'),lp=preview?.languageProfile||{},pid=trim(lp.id).toUpperCase(),isDialogue=['DIALOGUE','GOLDEN_DIALOGUE'].includes(pid),adaptiveDialogue=pid==='DIALOGUE',fixedDialogue=pid==='GOLDEN_DIALOGUE',origin=adaptiveDialogue?'Conversación adaptativa':fixedDialogue?'Conversación fija comparable':'Alcance lenguaje';$('ztData').innerHTML=(isLanguage?[["Origen",origin],["Motor","VNEXT"],["Nivel",lp.label||batteryCode],[isDialogue?"Turnos":"Preguntas",t['FULL-CERT']||lp.count],["Expectativa",lp.expectedBand||'—'],["NHC","solo ITV"]]:isExcel?[["Origen","Excel"],["Código",preview?.batteryCode||batteryCode],["Preguntas",t['FULL-CERT']||t['AI-SMOKE']],["AI-SMOKE",t['AI-SMOKE']],["FULL-CERT",t['FULL-CERT']]]:[["Eventos",c.events],["Personas",c.people],["Productos",c.products],["Tiendas",c.stores],["Compras",c.purchases],["Ingresos",c.incomes],["DOC",c.documents],["Fototickets",c.ticketImages],["Donaciones",c.donationLines],["Hitos",c.hitos],["LG",c.lgs],["FAST",t.FAST],["AI-SMOKE",t['AI-SMOKE']],["FULL-CERT",t['FULL-CERT']]]).map(x=>`<span class="zt-pill">${esc(x[0])}: <strong>${typeof x[1]==='number'?fmtN(x[1]):esc(x[1]||'—')}</strong></span>`).join('');if($('ztSeedInfo'))$('ztSeedInfo').textContent=isLanguage?`Batería ${lp.label||batteryCode} · MOTOR VNEXT · ${lp.count||t['FULL-CERT']||0} ${adaptiveDialogue?'turnos adaptativos':fixedDialogue?'turnos fijos':'preguntas'} · expectativa ${lp.expectedBand||'—'} · NHC: las pruebas no son reglas del runtime.`:isExcel?`Batería Excel ${preview?.batteryCode||batteryCode} · semilla estable ${batterySeed} · ${t['FULL-CERT']||0} preguntas · FULL-CERT conserva toda la conversación en el mismo ledger.`:`Batería ${batteryClock||'reloj local'} · semilla ${batterySeed} · FAST recorre todos los registros estructurales y FULL-CERT incluye continuidad Z1 sobre todos los eventos reales.`;}
  ...
  function auditRunManifest(expectedCases=[],actualRows=[],done=0){
    ...
    for(const c of expected){const r=byId.get(String(c?.id||''));if(!r){reasons.push(`falta resultado ${c?.id}`);continue;}if(c?.dialogue?.adaptive!==true&&trim(r?.prompt)!==trim(c?.prompt))reasons.push(`prompt distinto en ${c?.id}`);}
  ...
  function localCaseOutcome(c,status,actual,usage={}){return{id:c.id,group:c.group||'IA',label:c.label||c.prompt,prompt:c.prompt||'',expected:c.expected||'Regla/invariante satisfecha',actual,status,functionalStatus:status,functionalReasons:[],performanceStatus:'OK',performanceReasons:[],usage,durationMs:0};}
  ...
  async function runPaidCases(onlyIssues){
    ...
    try{
      for(let i=0;i<cases.length;i++){
        ...
        if(lastMode==='FULL-CERT'&&onlyIssues&&fullCases.length){
          ...
          if(targetIdx>=0){
            ...
            for(let j=Math.max(blockStart,fullThrough+1);j<targetIdx;j++){
              ...
              const prepResult=prepGot.data?.case||{};const pu=prepResult.labUsage||prepResult.usage||{};
        ...
        r.durationMs=num(r.durationMs)||Date.now()-started;const u=r.labUsage||r.usage||{};costEur=Number((costEur+num(u.costEur)).toFixed(6));calls+=num(u.calls);tokens+=num(u.tokens);done++;
        {const fst=trim(r.functionalStatus||r.status).toUpperCase();r.status=fst||r.status;if(fst==='OBSERVED'||fst==='OK')ok++;else if(fst==='WARN')warn++;else ko++;}
      ...
      {const us=usageSplit(rows);lastSummary={type:'summary',mode:lastMode,done,total,ok,warn,ko,costEur,calls,tokens,...us,aborted,incomplete,budgetStopped,contextWarmups,contextWarmupFailures,finishedAt:new Date().toISOString(),certified:!aborted&&!incomplete&&ko===0&&auditValidity.valid,observationMode:'ORACLE_ACTIVE',oracleEnabled:true,performance:performanceSummary(rows),performanceVerdicts:performanceVerdicts(rows),diagnosis:diagnosisSummary(rows),auditValidity};}
  ...
  function renderFinishState(s,updatePhase=true){
    const incomplete=num(s.done)<num(s.total),errors=num(s.ko),warns=num(s.warn),pf=s.performance||performanceSummary(rows),pv=s.performanceVerdicts||performanceVerdicts(rows),perfText=lastMode==='FAST'?'':(pf.dialogue?` · LATENCIA REAL ZUZU mediana ${(num(pf.medianMs)/1000).toFixed(1)} s · P90 ${(num(pf.p90Ms)/1000).toFixed(1)} s · SIM ${(num(pf.simulatorMedianMs)/1000).toFixed(1)} s · LAB ${(num(pf.wallMedianMs)/1000).toFixed(1)} s · PERF ${fmtN(pv.KO)} KO/${fmtN(pv.WARN)} avisos`:` · RENDIMIENTO ${fmtN(pv.KO)} KO / ${fmtN(pv.WARN)} avisos · mediana ${(num(pf.medianMs)/1000).toFixed(1)} s · P90 ${(num(pf.p90Ms)/1000).toFixed(1)} s`),manifest=modeManifests[lastMode]||preview,isLanguage=(manifest?.source==='language'||batterySource==='language'),auditValid=s?.auditValidity?.valid!==false,coverage=num(s.done)?Math.round(num(s.ok)*100/num(s.done)):0,coverageText=isLanguage&&auditValid?` · COBERTURA FUNCIONAL ${coverage}% (referencia ${manifest?.languageProfile?.expectedBand||'—'})`:'',profileId=trim(manifest?.languageProfile?.id).toUpperCase(),isDialogue=['DIALOGUE','GOLDEN_DIALOGUE'].includes(profileId),dm=dialogueMetrics(rows),us=s?.zuzuCalls!=null?s:usageSplit(rows),usageText=isDialogue?` · IA ZUZU ${num(us.avgZuzuCalls).toFixed(2)}/turno · SIM ${num(us.avgSimulatorCalls).toFixed(2)} · LAB ${num(us.avgLabCalls).toFixed(2)}`:'',dialogueText=isDialogue?` · HILO ${dm.continuityPct}% · foco ${dm.focusBreaks} roturas · promesas vacías ${dm.emptyPromises} · acciones sin tool ${dm.missingTools}${usageText}`:'';
    if(updatePhase){if(!auditValid)setPhase(`INFORME NO VÁLIDO: ${arr(s?.auditValidity?.reasons).join(' · ')||'manifiesto/resultados no coinciden'}. No se calcula cobertura.`,true);else setPhase(s.aborted?'Ejecución detenida.':errors?`ITV funcional terminada con ${fmtN(errors)} KO y ${fmtN(warns)} avisos.${coverageText}${dialogueText}${perfText}`:incomplete?`ITV incompleta.${coverageText}${dialogueText}${perfText}`:`ITV funcional completa: ${fmtN(s.ok)} OK, ${fmtN(warns)} avisos, 0 KO.${coverageText}${dialogueText}${perfText}`);}
    ...
    if(isLanguage&&!s.aborted){cert.textContent=isDialogue?`🗣️ ${profileId==='GOLDEN_DIALOGUE'?'GOLDEN DIÁLOGO':'DIÁLOGO'} · ${fmtN(s.done)} turnos · HILO ${dm.continuityPct}% · ${fmtN(errors)} KO · IA ZUZU ${num(us.avgZuzuCalls).toFixed(2)}/t · SIM ${num(us.avgSimulatorCalls).toFixed(2)}/t · LAB ${num(us.avgLabCalls).toFixed(2)}/t · ZUZU ${(num(pf.medianMs)/1000).toFixed(1)}s med`:`🧭 ${manifest?.languageProfile?.label||'LENGUAJE'} · FUNCIONAL ${coverage}% · ${fmtN(s.ok)}/${fmtN(s.done)} · ${fmtN(errors)} KO · ${fmtN(warns)} avisos · PERF ${fmtN(pv.KO)} KO/${fmtN(pv.WARN)} avisos`;const certScore=isDialogue?dm.continuityPct:coverage;cert.className='zt-cert '+(certScore>=90?'good':certScore>=50?'warn':'bad');}
  ...
  function setRetryState(){const b=$('ztRetry');if(!b)return;const n=rows.filter(r=>['KO','WARN'].includes(trim(r?.status).toUpperCase())).length;b.style.display=n?'':'none';b.disabled=!n;b.title=n?`Repetir solo ${n} incidencia(s); FULL-CERT reconstruirá el contexto del escenario cuando sea necesario.`:'No hay incidencias que repetir.';}
  ...
  function rowHtml(r){const mb=r.usage?modelBadge(r.usage):'',perfSt=trim(r?.performanceStatus||'OK').toUpperCase(),perfReasons=arr(r?.performanceReasons),perfBadge=perfSt==='OK'?'':`<span>Rendimiento: ${esc(perfSt)}${perfReasons.length?' · '+esc(perfReasons.join(' | ')):''}</span>`,reasons=Array.isArray(r.validationReasons)&&r.validationReasons.length?`<span>
...
Fallback: ${esc(r.usage.fallbackReason)}</span>`:''}${perfBadge}</div><div class="zt-ms">${r?.dialogueRuntime?`Zuzu ${fmtN(r.dialogueRuntime.zuzuMs)} ms<br>Sim ${fmtN(r.dialogueRuntime.simulatorMs)} ms<br>Lab ${fmtN(r.dialogueRuntime.wallMs||r.durationMs)} ms`:`${fmtN(r.durationMs)} ms`}${r.usage?`
${fmtE(r.usage.costEur)} · ${fmtN(r.usage.calls)} IA Zuzu · ${fmtN(r.usage.tokens)} tok`:''}${phase}</div></div>`;}
  ...
  function groups(){return ['TODOS','OK','WARN','KO','PERF_WARN','PERF_KO',...Array.from(new Set(rows.map(r=>r?.decisionDiagnosis?.category).filter(Boolean))),...Array.from(new Set(rows.map(r=>r.group).filter(Boolean)))];}
  function renderFilters(){if(!$('ztFilters'))return;$('ztFilters').innerHTML=groups().map(g=>`<button class="zt-filter ${activeFilter===g?'active':''}" data-f="${esc(g)}">${esc(g)}${['OK','WARN','KO'].includes(g)?` (${rows.filter(r=>r.status===g).length})`:g==='PERF_WARN'?` (${rows.filter(r=>trim(r.performanceStatus).toUpperCase()==='WARN').length})`:g==='PERF_KO'?` (${rows.filter(r=>trim(r.performanceStatus).toUpperCase()==='KO').length})`:''}</button>`).join('');$('ztFilters').querySelectorAll('button').forEach(b=>b.onclick=()=>{activeFilter=b.dataset.f;renderFilters();applyFilter();});}
  function applyFilter(){document.querySelectorAll('#ztResults .zt-row').forEach(el=>{const row=rows.find(r=>String(r.id)===el.querySelector('.zt-cell span')?.textContent),perf=trim(row?.performanceStatus).toUpperCase();const show=activeFilter==='TODOS'||(['OK','WARN','KO'].includes(activeFilter)&&el.dataset.status===activeFilter)||(activeFilter==='PERF_WARN'&&perf==='WARN')||(activeFilter==='PERF_KO'&&perf==='KO')||el.dataset.diagnosis===activeFilter||el.dataset.group===activeFilter;el.style.display=show?'grid':'none';});}
  ...
  async function saveServerRun(){
    ...
    try{
      ...
      const report=reportPayload(),payload={runKey:currentRunKey,seed:batterySeed,batteryClock,appVersion:'v4_1_exp',generatedAt:executed?.generatedAt,dataCounts:executed?.dataCounts||{},generatedBattery:report.generatedBattery||executed,report,summary};
  ...
  function itvVersionToken(){
    const candidates=[window.ControlEventVersion?.version,window.ControlEventVersion?.versionFile,window.__ceVersion,document.querySelector?.('[data-ce-version-label]')?.textContent,'v4_1_exp'];
  ...
  function lightCapabilityCall(x={}){return{tool:trim(x.tool),effectiveOperation:trim(x.effectiveOperation),effectiveSubject:x.effectiveSubject||{},rawArgs:x.rawArgs||{},normalizedArgs:x.normalizedArgs||{},envelope:x.envelope||x?.audit?.envelope||null,durationMs:num(x.durationMs),audit:x.audit?{ok:x.audit.ok!==false,classification:trim(x.audit.classification),issues:arr(x.audit.issues),repairs:arr(x.audit.repairs),operation:trim(x.audit.operation),effectiveOperation:trim(x.audit.effectiveOperation),signature:trim(x.audit.signature),registryVersion:trim(x.audit.registryVersion)}:null,error:trim(x.error)}}
  function lightRow(r={}){const rc=r?.resultContext||r?.meta?.resultContext||{},perf=r?.performance||r?.meta?.performance||{};return{id:trim(r.id),group:trim(r.group),label:trim(r.label),prompt:trim(r.prompt),expected:trim(r.expected),actual:trim(r.actual),status:trim(r.status),functionalStatus:trim(r.functionalStatus),functionalReasons:arr(r.functionalReasons),performanceStatus:trim(r.performanceStatus),performanceReasons:arr(r.performanceReasons),validationReasons:arr(r.validationReasons),scenario:trim(r.scenario),engine:trim(r.engine),provider:trim(r.provider),architecture:trim(r.architecture),usage:r.usage||{},performance:perf,tools:arr(r.tools),resultContext:{kind:trim(rc.kind),operation:trim(rc.operation),event:trim(rc.event),person:trim(rc.person),focus_mode:trim(rc.focus_mode),focus_type:trim(rc.focus_type),focus_entities:arr(rc.focus_entities),requested_fields:arr(rc.requested_fields),derive_operation:trim(rc.derive_operation),derive_field:trim(rc.derive_field),source_operation:trim(rc.source_operation),source_event:trim(rc.source_event),source_events:arr(rc.source_events),source_person:trim(rc.source_person),table_key:trim(rc.table_key),order_by:trim(rc.order_by)},capabilityCalls:arr(r.capabilityCalls||r?.meta?.capabilityCalls).map(lightCapabilityCall),decisionDiagnosis:r.decisionDiagnosis||null,dialogue:r.dialogue||null,dialogueRuntime:r.dialogueRuntime||null,presentationEvidence:r.presentationEvidence||null,zuzuUsage:r.zuzuUsage||null,simulatorUsage:r.simulatorUsage||null,durationMs:num(r.durationMs)}}
  function lightOracle(o={}){const scalar={};for(const [k,v] of Object.entries(o||{})){if(v==null||['string','number','boolean'].includes(typeof v))scalar[k]=v;}if(Array.isArray(o?.requiredMetrics))scalar.requiredMetrics=o.requiredMetrics.slice(0,20);if(Array.isArray(o?.events))scalar.events=o.events.slice(0,20);if(o?.row&&typeof o.row==='object')scalar.row=Object.fromEntries(Object.entries(o.row).filter(([,v])=>v==null||['string','number','boolean'].includes(typeof v)));if(o?.data&&typeof o.data==='object')scalar.data=Object.fromEntries(Object.entries(o.data).filter(([k,v])=>!/(rows?|tables?|lines?|details?|documents?|tickets?|summaryRows|incomeRows|productRows|donorRows)/i.test(k)&&(v==null||['string','number','boolean'].includes(typeof v))));if(o?.compare&&typeof o.compare==='object')scalar.compare={events:arr(o.compare.events).slice(0,20),rows:arr(o.compare.rows).slice(0,20).map(r=>Object.fromEntries(Object.entries(r||{}).filter(([,v])=>v==null||['string','number','boolean'].includes(typeof v))))};return scalar;}
  function lightGeneratedBattery(executed={}){return{ok:executed?.ok!==false,replayContractVersion:num(executed?.replayContractVersion),source:trim(executed?.source),batteryCode:trim(executed?.batteryCode),languageProfile:executed?.languageProfile||null,generatedAt:trim(executed?.generatedAt),seed:num(executed?.seed),dataCounts:executed?.dataCounts||{},tests:executed?.tests||{},cases:Object.fromEntries(MODES.map(m=>[m,arr(executed?.cases?.[m]).map(c=>({id:trim(c.id),group:trim(c.group),label:trim(c.label),prompt:trim(c.prompt),expected:trim(c.expected),scenario:trim(c.scenario),mode:trim(c.mode),event:trim(c.event),events:arr(c.events),person:trim(c.person),engine:trim(c.engine),oracle:lightOracle(c.oracle||{}),dialogue:c.dialogue||null}))]))}}
  function reportPayloadLight(){const full=reportPayload(),modes={};for(const m of MODES){const c=modeCache[m],mm=modeManifests[m];modes[m]={summary:c.summary||null,dialogueMetrics:dialogueMetrics(c.rows),results:arr(c.rows).map(lightRow),manifestHash:mm?manifestHash(mm):''};}const executed=modeManifests[lastMode]||preview||{};return{type:'ControlEvent Zuzu ITV LIGHT',reportFormat:'LIGHT-P2R',version:full.version,itvContractVersion:full.itvContractVersion,itvBuild:full.itvBuild,itvLanguageBuild:full.itvLanguageBuild,languageProfile:full.languageProfile,itvObservationMode:full.itvObservationMode,oracleEnabled:full.oracleEnabled,batteryReplayContractVersion:full.batteryReplayContractVersion,exportedAt:full.exportedAt,batterySeed:full.batterySeed,batteryClock:full.batteryClock,batterySource:full.batterySource,batteryCode:full.batteryCode,historicReplayKey:full.historicReplayKey,generatedBattery:lightGeneratedBattery(executed),executedManifestHash:full.executedManifestHash,manifestValidation:full.manifestValidation,dataCounts:full.dataCounts,modes,history:full.history};}
  function reportPayload(){
    const modes={};for(const mode of MODES){modes[mode]={summary:modeCache[mode].summary||null,dialogueMetrics:dialogueMetrics(modeCache[mode].rows||[]),results:modeCache[mode].rows||[],manifestHash:modeManifests[mode]?manifestHash(modeManifests[mode]):''};}
  ...
  function modeFileSuffix(mode=lastMode){return mode==='FAST'?'fast-ce':mode==='AI-SMOKE'?'ai-smoke':'full-cert';}
  function batteryFileSuffix(mode=lastMode){const executed=modeManifests[mode]||preview||{},profile=trim(executed?.languageProfile?.id).toUpperCase(),code=trim(executed?.batteryCode||batteryCode).toUpperCase();if(profile==='GOLDEN_DIALOGUE'||code.startsWith('GOLDEN-DIALOG-')){const count=num(executed?.languageProfile?.count)||num(executed?.tests?.['FULL-CERT'])||14;return`golden-dialog-${count}`;}if(profile==='DIALOGUE'||code.startsWith('DIALOGUE-')){const count=num(executed?.languageProfile?.count)||num(executed?.tests?.['FULL-CERT'])||24;return`dialog-${count}`;}if(profile&&!['DIALOGUE','GOLDEN_DIALOGUE'].includes(profile)){const label=trim(executed?.languageProfile?.label||profile).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');if(label)return label;}if(trim(executed?.source)==='excel'||batterySource==='excel')return`excel-${trim(executed?.batteryCode||batteryCode||'battery').toLowerCase().replace(/[^a-z0-9]+/g,'-')}`;return modeFileSuffix(mode);}
  function downloadReport(full=false){const payload=full?reportPayload():reportPayloadLight(),has=MODES.some(m=>modeCache[m].rows.length||modeCache[m].summary);if(!has){alert('Todavía no hay resultados que exportar.');return;}const suffix=batteryFileSuffix(lastMode),tag=full?'FULL':'LIGHT',downloadName=`${itvFilePrefix()}_ITV_Zuzu_${new Date().toISOString().replace(/[:.]/g,'-')}-${suffix}-${tag}.json`;payload.intendedDownloadName=downloadName;const blob=new Blob([JSON.stringify(payload,null,full?2:0)],{type:'application/json;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=downloadName;document.body.appendChild(a);const nativeClick=window.__CE_NATIVE_ANCHOR_CLICK__;if(typeof nativeClick==='function')nativeClick.call(a);else a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),2000);setPhase(`Informe ${tag} ${suffix} descargado como ${downloadName}. ${full?'Úsalo solo para forense detallado.':'Este es el informe recomendado para análisis normal.'}`);}
  function printReport(){const mode=lastMode,c=modeCache[mode],s=c.summary||{},date=new Date().toLocaleString('es-ES'),suffix=batteryFileSuffix(mode),body=c.rows.map(r=>`<tr><td class="${esc(r.status)}">${esc(r.status)}${trim(r.performanceStatus)&&trim(r.performanceStatus)!=='OK'?` / PERF ${esc(r.performanceStatus)}`:''}</td><td>${esc(r.group)}</td><td>${esc(r.label)}</td><td>${esc(r.prompt||'')}</td><td>${esc(r.expected||'')}</td><td>${esc(r.actual||'')}</td></tr>`).join('');if(!c.rows.length){alert('Este modo todavía no tiene resultados.');return;}const w=window.open('','_blank');if(!w){setPhase('El navegador ha bloqueado la ventana de impresión. Usa ⬇ INFORME para descargar el JSON.',true);return;}w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${itvFilePrefix()}_ITV_Zuzu_${suffix}</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:Arial,sans-serif;margin:16px;color:#0f172a}h1{color:#075985}table{width:100%;border-collapse:collapse;font-size:8.5px;table-layout:fixed}th,td{border:1px solid #cbd5e1;padding:4px;vertical-align:top;overflow-wrap:anywhere}th:nth-child(1){width:5%}th:nth-child(2){width:8%}th:nth-child(3){width:13%}th:nth-child(4){width:22%}th:nth-child(5){width:20%}th:nth-child(6){width:32%}.OK{color:#15803d;font-weight:bold}.KO{color:#b91c1c;font-weight:bold}.WARN{color:#b45309;font-weight:bold}.summary{display:flex;gap:18px;flex-wrap:wrap;margin:8px 0 12px}.summary b{font-size:16px}</style></head><body><h1>🧪 ITV de Zuzu · ${esc(mode)}</h1><p>${esc(date)} · semilla <b>${esc(batterySeed)}</b> · tablas reales · solo lectura</p><div class="summary"><span>OK <b>${fmtN(s.ok)}</b></span><span>AVISOS <b>${fmtN(s.warn)}</b></span><span>KO <b>${fmtN(s.ko)}</b></span><span>IA Zuzu <b>${fmtN(s.zuzuCalls??s.calls)} (${num(s.avgZuzuCalls||0).toFixed(2)}/turno)</b></span><span>Simulador ITV <b>${fmtN(s.simulatorCalls||0)}</b></span><span>Total laboratorio <b>${fmtN(s.labCalls??s.calls)}</b></span><span>Tokens Zuzu <b>${fmtN(s.zuzuTokens??s.tokens)}</b></span><span>Coste Zuzu <b>${fmtE(s.zuzuCostEur??s.costEur)}</b></span><span>Coste lab <b>${fmtE(s.labCostEur??s.costEur)}</b></span><span>Rendimiento <b>${fmtN((s.performanceVerdicts||performanceVerdicts(c.rows)).KO)} KO / ${fmtN((s.performanceVerdicts||performanceVerdicts(c.rows)).WARN)} avisos</b></span></div><table><thead><tr><th>Estado</th><th>Grupo</th><th>Prueba</th><th>Pregunta realizada</th><th>Oráculo / referencia</th><th>Respuesta literal de Zuzu</th></tr></thead><tbody>${body}</tbody></table><script>window.onload=()=>setTimeout(()=>window.print(),250)<\/script></body></html>`);w.document.close();}
