/* ControlEvent v2.0_exp · PRUEBAS ZUZU · Consola GD.
...
(function(){
  ...
  function storedAuth(){
    ...
    for(const key of ['ControlEvent_v2.0_exp_login_user','ControlEvent_ce_acceso_usuario','ControlEvent_auth_user_v509']){
  ...
  function modal(){return `<div id="ceZuzuTestOverlay" role="dialog" aria-modal="true" aria-label="ITV de Zuzu"><div class="zt-modal">
    ...
    <div class="zt-top">
      ...
      </div><div class="zt-controls zt-run-controls"><label>Máx. coste<br><input id="ztMaxCost" type="number" min="0.02" max="5" step="0.05" value="0.25"> €</label><label title="Máximo de preguntas de IA que se ejecutarán en AI-SMOKE o FULL-CERT. No crea casos nuevos; si pones más que los disponibles, se ejecutan todos.">Máx. casos IA<br><input id="ztMaxCases" type="number" min="1" max="100" step="1" value="48"></label><button class="primary" id="ztStart">▶ INICIAR</button><button class="danger" id="ztStop">■ DETENER</button><button id="ztRetry">↻ REPETIR KO/AVISOS</button><button class="next" id="ztNext">SIGUIENTE →</button></div></div>
    </div>
  ...
  async function saveServerRun(){
    ...
    try{
      ...
      const payload={runKey:currentRunKey,seed:batterySeed,batteryClock,appVersion:'v2.0_exp',generatedAt:preview?.generatedAt,dataCounts:preview?.dataCounts||{},generatedBattery:preview,report:reportPayload(),summary};
