/* ControlEvent v26_prod_1.2 FIX36 - COMPRAS RPC desde HEAD
...
(function(){
  'use strict';
  const TAG='__ceV85ComprasRpcHeadFix36';
  if(window[TAG]) return; window[TAG]=true;
  const LOG='[CE FIX36 COMPRAS RPC HEAD]';
  ...
  function renderCompraViewsFull(opts){
    opts = opts || {};
    // Recalcula la pantalla con el estado local ya filtrado. No llamamos a render()
    // porque render() ejecuta saveState legacy; aquí solo queremos repintar vistas.
    try{
      const s = stateObj();
      deletedIds.forEach(id=>{
  ...
  async function syncStateNoRender(){
    try{
      const fresh=await apiJson('/api/state?fix36=1&_='+Date.now(), {method:'GET', headers:{'Cache-Control':'no-cache','Pragma':'no-cache'}});
  ...
  style.textContent=`
    ...
    .ce-crud-flash-add{ animation:ceCrudPulseFix36 1.1s ease-in-out 0s 7 alternate; outline:4px solid #22c55e !important; box-shadow:0 0 0 6px rgba(34,197,94,.22) !important; font-weight:900!important; transition:outline .25s ease, box-shadow .25s ease; }
    .ce-crud-flash-update{ animation:ceCrudPulseFix36 1.1s ease-in-out 0s 7 alternate; outline:4px solid #0ea5e9 !important; box-shadow:0 0 0 6px rgba(14,165,233,.22) !important; font-weight:900!important; transition:outline .25s ease, box-shadow .25s ease; }
    ...
    @keyframes ceCrudPulseFix36{from{filter:saturate(1.0) brightness(1)}to{filter:saturate(1.18) brightness(1.06)}}
  `;
  ...
  console.info(LOG,'activo en HEAD: COMPRAS RPC FIX36 con baja no bloqueante');
})();
