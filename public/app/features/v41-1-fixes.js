/* ControlEvent v41.1 - correcciones de navegación, casitas, menú, backup y Mapa de recursos. */
(function(){
  'use strict';
  ...
  function patchRenderForLoginAndSync(){
    ...
    const wrapped = function(){
      ...
      if(hasAuthNow && !hadAuth){
        // v41.1: al entrar desde LOGIN, la primera ventana de trabajo será GRAFICAS.
  ...
  function patchChangeSelectedEventPreserveTab(){
    ...
    const wrapped = async function(value){
      ...
      // v41.1: al cambiar de evento desde dentro, mantener la misma ventana activa.
      if(before) setCurrentTab(before);
      try{ if(typeof render === 'function') render(); else window.render?.(); }catch(_){ }
      setTimeout(() => {
        try{ window.ControlEventModules?.activate?.(before, {reason:'v41.1-event-change-preserve-tab'}); }catch(_){ }
