/* ControlEvent v2.0_exp - HOTFIX47: avance en logo, carga scoped estable, documentos/mantenimiento. */
(function(){
  'use strict';
  ...
  async function loadScopedEventForSwitch(eventId, token){
    ...
    for(let attempt = 1; attempt <= 3; attempt++){
      ...
      try{
        ...
        if(attempt >= 3) console.warn('[ControlEvent v2.0_exp HF47] carga scoped de evento fallida:', error?.message || error);
