/* ControlEvent v27_prod_1.1 - ajustes quirurgicos: version, INFOEVENTO/BACKUP, fotos ingresos/docs, resumen sin descarga. */
(function(){
  'use strict';
  ...
  function normalizeDownloadName(name){
    var n=text(name);
    n=n.replace(/ControlEvent_v27_prod_1.1(?:_r1)?/ig, VERSION_FILE)
       ...
       .replace(/ControlEvent\s+v27_prod_1.1(?:_r1)?/ig, VERSION_TEXT)
  ...
  function applyVersion(){
    ...
    document.querySelectorAll('.ce-v104-brand-mini,.ce-v1045-brand-mini,.ce-v1047-brand-mini,[data-ce-version-label]').forEach(function(el){
      ...
      else if(/v\d/i.test(el.textContent||'')) el.textContent=(el.textContent||'').replace(/v27_prod_1.1(?:_r1)?|v\d+(?:[._]\d+){1,4}_prod/ig, VERSION);
    });
    document.querySelectorAll('.appname-stack span,.appname span').forEach(function(el){ if(/v\d/i.test(el.textContent||'')) el.textContent=(el.textContent||'').replace(/v27_prod_1.1(?:_r1)?|v\d+(?:[._]\d+){1,4}_prod/ig, VERSION); });
  }
