(function(){
  'use strict';
  if(window.__ceV15Hotfix16AvanceDonacionesPrompt) return;
  window.__ceV15Hotfix16AvanceDonacionesPrompt = true;
  ...
  function avRow(n,label,pct,color,detail,warn){
    ...
    return `<div class="ce-hf16-av-row"><div class="ce-hf16-av-head"><span><b>${n}</b>${esc(label)}</span><strong>${esc(num(p))}%</strong></div><div class="ce-hf16-av-track"><i style="width:${p}%;background:${esc(color)}"></i></div><small>${esc(detail||'')}</small>${warn?`<em>${esc(warn)}</em>`:''}</div>`;
  }
  function avanceHtml(){
    const p=computeAvance();
    return `<div class="ce-hf16-av-box">
      <div class="ce-hf16-av-title"><span>AVANCE</span><strong>del evento</strong></div>
  ...
  window.ceHf16ToggleAvance = function(ev){
    try{ ev?.preventDefault?.(); ev?.stopPropagation?.(); ev?.stopImmediatePropagation?.(); }catch(_){}
    const existing=$('ceHf16MapaAvancePanel');
    if(existing){ existing.remove(); return false; }
    const {card,title}=panelContainer();
    if(!card || !title) return false;
    const panel=document.createElement('div');
    panel.id='ceHf16MapaAvancePanel';
  ...
  function injectStyle(){
    ...
    const css=`
      ...
      .ce-hf16-av-box{padding:9px!important;border:3px solid #0f172a!important;border-radius:18px!important;background:linear-gradient(180deg,#fff,#f8fafc)!important;box-shadow:0 8px 22px rgba(15,23,42,.12)!important;font-size:11px!important;line-height:1.12!important;max-width:650px!important;margin-left:auto!important}
      .ce-hf16-av-title{display:flex!important;justify-content:space-between!important;align-items:center!important;margin-bottom:5px!important;padding-bottom:5px!important;border-bottom:2px solid #e2e8f0!important}.ce-hf16-av-title span{font-size:12px!important;font-weight:950!important;letter-spacing:.08em!important}.ce-hf16-av-title strong{font-size:10px!important;text-transform:uppercase!important;color:#475569!important}
      .ce-hf16-av-row{display:grid!important;grid-template-columns:minmax(150px,1.25fr) 52px minmax(120px,1fr)!important;gap:5px!important;align-items:center!important;margin:4px 0!important;padding:5px 6px!important;border:1px solid #dbe4ee!important;border-radius:12px!important;background:#fff!important}.ce-hf16-av-head{display:contents!important}.ce-hf16-av-head span{font-size:9.5px!important;font-weight:900!important;text-transform:uppercase!important}.ce-hf16-av-head b{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:16px!important;height:16px!important;margin-right:4px!important;border-radius:999px!important;background:#0f172a!important;color:#fff!important;font-size:9px!important}.ce-hf16-av-head strong{font-size:12px!important;text-align:right!important}.ce-hf16-av-track{height:8px!important;background:#e5e7eb!important;border-radius:999px!important;overflow:hidden!important}.ce-hf16-av-track i{display:block!important;height:100%!important;border-radius:999px!important}.ce-hf16-av-row small{grid-column:1/4!important;font-size:9.5px!important;font-weight:750!important;color:#334155!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.ce-hf16-av-row em{grid-column:1/4!important;font-size:9.5px!important;padding:3px 5px!important;border:1px solid #fdba74!important;border-radius:9px!important;background:#fff7ed!important;color:#9a3412!important;font-style:normal!important;font-weight:850!important}
      ...
      @media(max-width:900px){.ce-hf16-av-box{max-width:none!important;margin-left:0!important}.ce-hf16-av-row{grid-template-columns:1fr 48px!important}.ce-hf16-av-track{grid-column:1/3!important}.ce-hf16-av-row small,.ce-hf16-av-row em{grid-column:1/3!important;white-space:normal!important}}
    `;
