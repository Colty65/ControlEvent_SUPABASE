/* ControlEvent v27_prod_1.0 - HOTFIX mínimo: documentos con título evento, descarga ticket en resumen y botones sin heredar rojo. */
(function(){
  'use strict';
  ...
  function safeFile(base){ return trim(base || 'foto_ticket').replace(/[\\/:*?"<>|]+/g,' ').replace(/\s+/g,' ').trim().slice(0,170) || 'foto_ticket'; }
  ...
  function eventTitleForTicket(){
    var ev = selectedEvent() || {};
    return trim(ev.titulo || ev.nombre || (($('selectedEvent') && $('selectedEvent').selectedIndex >= 0 && $('selectedEvent').options[$('selectedEvent').selectedIndex]) ? $('selectedEvent').options[$('selectedEvent').selectedIndex].textContent : '') || 'Evento');
  }
  function guessTicketName(img, idx){
    ...
    var raw = trim((box && (box.innerText || box.textContent)) || '');
    var txt = fold(raw);
    var m = txt.match(/\bTK\s*0*([0-9]{1,3})\b/i);
    var tk = m ? ('TK'+String(m[1]).padStart(2,'0')) : ('TK'+String(idx+1).padStart(2,'0'));
    var tienda = 'Tienda';
    var lines = raw.split(/\n+/).map(function(x){return trim(x);}).filter(Boolean);
    for(var i=0;i<lines.length;i++){
      var line = lines[i];
      var mm = line.match(/^(.+?)\s*\|\s*TK\s*0*[0-9]{1,3}\b/i);
      if(mm){ tienda = trim(mm[1]); break; }
    }
    if(tienda === 'Tienda'){
      var mm2 = raw.match(/([^\n|]{2,80})\s*\|\s*TK\s*0*[0-9]{1,3}\b/i);
      if(mm2) tienda = trim(mm2[1]);
    }
    return tk + '-' + eventTitleForTicket() + '-' + tienda;
  }
