/* ControlEvent v45.3 - selector de evento unificado y render activo único.
...
(function(){
  ...
  function call(name, args){ const fn = window[name] || safe(() => eval(name), null); if(typeof fn !== 'function') return undefined; try{ return fn.apply(window, args || []); }catch(error){ console.warn('[v45.3] Error en ' + name, error); return undefined; } }
  ...
  function persistLocal(){
    // v45.3: el cambio de evento NO debe serializar todo el estado.
    // En móviles, JSON.stringify(state) puede incluir muchos registros o imágenes de tickets y bloquear la UI.
    return undefined;
  }
  function scheduleRemoteSave(){
    // v45.3: seleccionar evento es una preferencia local, no un cambio de datos del evento.
  ...
  function armWatchdog(token, ms){
    ...
    transition.watchdog = setTimeout(() => {
      ...
      console.warn('[v45.3] Watchdog libera transición bloqueada', {eventId:transition.eventId, targetTab:transition.targetTab});
  ...
  function unlockMenuShell(){
    if(!auth()) return;
    // v45.3: si venimos de un usuario RO, quitar restos de ocultación antes de reconstruir menú/paneles.
    if(!isRO()){
  ...
  async function doLoginFast(){
    ...
    try{
      ...
      console.error('[v45.3] login rápido', err);
  ...
  function renderStableGraficas(){
    ...
    if(typeof stable === 'function') return stable({force:true, reason:'v45.3-active-only'});
  ...
  function queueActive(tab, token, options = {}){
    ...
    transition.timer = setTimeout(() => {
      ...
      const run = () => {
        ...
        catch(error){ console.warn('[v45.3] render activo', error); finishTransition(token); }
      };
