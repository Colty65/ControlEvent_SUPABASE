/* ControlEvent v28.5.1_prod - selector de evento unificado y render activo único.
...
(function(){
  ...
  const OLD_CHOSEN_KEY = 'ControlEvent_v28.5.1_prod_event_chosen';
  const LEGACY_CHOSEN_KEYS = ['ce_v250_event_chosen','ControlEvent_v28.5.1_prod_event_chosen','ControlEvent_v28.5.1_prod_event_chosen','controlevent_v5022_user_picked_event'];
  ...
  function notice(text){
    // v28.5.1_prod_opt_2D: cambio de evento silencioso. No mostrar pastillas negras.
  ...
  function clearDynamic(activeTab, options = {}){
    ...
    Object.entries(DYNAMIC_IDS).forEach(([tab, ids]) => {
      ...
      ids.forEach(id => {
        // v28.5.1_prod_opt_2D: en GRAFICAS no vaciar eventChartWrap durante el cambio; mantener el último gráfico válido.
  ...
  function showLoading(tab, label){
    // v28.5.1_prod_opt_2D: no pintar pantallas intermedias de carga; dejan la UI fea y provocan reflujo.
    return;
  }
  ...
  async function doLoginFast(){
    ...
    try{
      ...
      try{ localStorage.removeItem('ControlEvent_v28.5.1_prod_session'); }catch(_){ } // v50.27: no persistir sesion ligera
