/* ControlEvent v4_1_exp OPT2E - GRAFICAS: un solo pintado final sin retemblores.
...
(function(){
  ...
  function finalPaint(reason){
    ...
    try{
      fn.call(window.ControlEventV434 || window.ControlEventV462 || window, {force:true, reason:'v4_1_exp_opt_2e_final'});
      const html = rawGet(w) || '';
      if(looksChart(html) && !looksBlank(html)){
        w.classList.remove('ce-opt2e-frozen');
        w.classList.add('ce-opt2e-final');
    ...
    }catch(err){ console.warn('[v4_1_exp_opt_2e] final render', err); }
    finally{ allowDirectCommit = false; queuedHtml = ''; }
  }
