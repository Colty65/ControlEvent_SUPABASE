/* ControlEvent v26_prod_1.0 OPT2E - GRAFICAS: un solo pintado final sin retemblores.
...
(function(){
  ...
  const VERSION = 'v26_prod_1.0_opt_2e';
  ...
  function finalPaint(reason){
    ...
    try{
      fn.call(window.ControlEventV434 || window.ControlEventV462 || window, {force:true, reason:'v26_prod_1.0_opt_2e_final'});
      const html = rawGet(w) || '';
      if(looksChart(html) && !looksBlank(html)){
        w.classList.remove('ce-opt2e-frozen');
        w.classList.add('ce-opt2e-final');
    ...
    }catch(err){ console.warn('[v26_prod_1.0_opt_2e] final render', err); }
    finally{ allowDirectCommit = false; queuedHtml = ''; }
  }
