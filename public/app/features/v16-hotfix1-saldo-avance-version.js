/* ControlEvent v3_0_exp - HOTFIX1: cierre/colores avance + versión v16 visible. */
(function(){
  'use strict';
  ...
  function applyVersion(){
    ...
    document.querySelectorAll('[data-ce-version-label],.ce-v104-brand-mini span,.appname span,.appname-stack span').forEach(el=>{
      try{
        ...
        if(/v\d+_prod/i.test(txt) || /ControlEvent\s+v\d+_prod/i.test(txt)) el.textContent=txt.replace(/ControlEvent\s+v\d+_prod/ig,VERSION).replace(/v\d+_prod/ig,'v3_0_exp');
      }catch(_){ }
    });
  }
