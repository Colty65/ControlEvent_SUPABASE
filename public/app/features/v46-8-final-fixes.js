/* ControlEvent v46.8 - persistencia real de justificantes de INGRESOS, retorno al globo y negrita PRODUCTOS.
...
(function(){
  ...
  function enrichGraphTooltip(){
    ...
    tip.querySelectorAll('table.ce-v21-table').forEach(table => {
      if(table.dataset.ceV468Receipts === '1') return;
  ...
  function productCard(id){
    ...
    const sels = [`button[data-action="save-producto"][data-id="${safe}"]`, `input[data-action="edit-producto-nombre"][data-id="${safe}"]`, `select[data-action="edit-producto-segmento"][data-id="${safe}"]`, `[data-id="${safe}"]`];
    for(const sel of sels){ try{ const el = wrap.querySelector(sel) || document.querySelector(sel); const card = el?.closest?.('.itemcard,.rowline,.card,tr,li,[data-record-id],.maintenance-card') || el; if(card) return card; }catch(_){ } }
    return null;
  }
  function applyProductBold(){
    modifiedProducts.forEach(id => { const card = productCard(id); if(!card) return; card.classList.add('ce-v468-modified-product','ce-v464-modified','ce-v46-modified'); card.querySelectorAll('input,select,textarea,button,label,span,strong,td,th,div').forEach(el => { try{ el.style.fontWeight='900'; }catch(_){ } }); });
  }
  function rememberProductModified(btn){ const id=btn?.dataset?.id || ''; if(!id) return; modifiedProducts.add(String(id)); [40,120,260,520,1000,1800,3200].forEach(ms => setTimeout(applyProductBold, ms)); }
