/* ControlEvent v28.5.2_prod FIX37 - COMPRAS RPC + SITUACION EVENTO desde HEAD
...
(function(){
  'use strict';
  const TAG='__ceV85ComprasRpcHeadFix37';
  if(window[TAG]) return; window[TAG]=true;
  const LOG='[CE FIX37 COMPRAS RPC HEAD]';
  ...
  function withNoStateWrites(fn){
    ...
    const localOk = function(){ try{ if(typeof persistStateLocal==='function') persistStateLocal(); }catch(_){} return Promise.resolve({ok:true, localOnly:true, fix:'FIX37'}); };
    try{
      ...
      if(typeof oldFetch === 'function'){
        window.fetch = function(input, init){
          ...
          if(method === 'PUT' && /\/api\/state(?:$|\?)/i.test(url)){
            return Promise.resolve(new Response(JSON.stringify({ok:true, localOnly:true, blockedBy:'FIX37_RENDER'}), {status:200, headers:{'Content-Type':'application/json'}}));
          }
  ...
  async function syncStateNoRender(){
    try{
      const fresh=await apiJson('/api/state?fix37=1&_='+Date.now(), {method:'GET', headers:{'Cache-Control':'no-cache','Pragma':'no-cache'}});
  ...
  async function deleteCompra(id, btn){
    ...
    try{
      ...
      // Confirmado por BBDD: quitamos de memoria, sincronizamos y repintamos
      // COMPRAS + RESUMEN con la realidad filtrada. Así no queda saldo viejo ni
      // botones Eliminar fantasma sobre filas ya borradas.
      ...
      renderCompraViewsFull();
      scrubDeletedFromDom();
      // Segundo pase: si algún render legacy vuelve a pintar saldo o ficha vieja,
      // volvemos a leer BBDD y renderizamos protegido otra vez.
      setTimeout(()=>{ syncStateNoRender().then(()=>{ removeCompraLocal(id); renderCompraViewsFull(); }).catch(()=>renderCompraViewsFull()); }, 350);
      setTimeout(()=>{ syncStateNoRender().then(()=>{ removeCompraLocal(id); renderCompraViewsFull(); }).catch(()=>scrubDeletedFromDom()); }, 1200);
      unlockUiRepeated();
    }finally{
  ...
  style.textContent=`
    ...
    .ce-crud-flash-add{ animation:ceCrudPulseFix37 1.1s ease-in-out 0s 7 alternate; outline:4px solid #22c55e !important; box-shadow:0 0 0 6px rgba(34,197,94,.22) !important; font-weight:900!important; transition:outline .25s ease, box-shadow .25s ease; }
    .ce-crud-flash-update{ animation:ceCrudPulseFix37 1.1s ease-in-out 0s 7 alternate; outline:4px solid #0ea5e9 !important; box-shadow:0 0 0 6px rgba(14,165,233,.22) !important; font-weight:900!important; transition:outline .25s ease, box-shadow .25s ease; }
    ...
    @keyframes ceCrudPulseFix37{from{filter:saturate(1.0) brightness(1)}to{filter:saturate(1.18) brightness(1.06)}}
  `;
  ...
  console.info(LOG,'activo en HEAD: COMPRAS RPC FIX37 con baja recalculada y situación por RPC');
})();
