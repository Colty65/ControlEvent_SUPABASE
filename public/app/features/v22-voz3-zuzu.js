/* ControlEvent v2.0_exp · Zuzu Voice · FIX27 motor estable recuperado de FIX14
...
(function(){
  ...
  var BUILD='v2.0_exp-FIX27-VOICE-FIX14-CORE';
  ...
  function voiceAddressName(formal){var u=currentVoiceUser(),informal=clean(u.identificacion||u.Identificacion||u.usuario||u.user||u.nombre||u.Nombre||''),formalName=clean(u.nombre||u.Nombre||u.name||informal);return formal?(formalName||'usuario'):(informal||formalName||'amigo');}
  function isFormalVoiceTurn(prompt,title){var t=norm(clean(prompt)+' '+clean(title));return /\b(informe formal|informe ejecutivo|informe oficial|informe tecnico|informe tecnica|auditoria|memoria formal|acta|documento formal|comunicado oficial|para la directiva|para los socios)\b/.test(t);}
  function stripVoiceAnswerLead(v){return clean(v).replace(/^(?:respuesta|contestacion|contestación|informe)\s+(?:de\s+)?zuzu\s*[:.\-–—]*\s*/i,'').replace(/^zuzu\s+(?:responde|contesta)\s*[:.\-–—]*\s*/i,'').trim();}
  ...
  function newRecognition(kind){
    ...
    r.onstart=function(){if(gen!==state.recognitionGeneration)return;state.recognitionStarting=false;state.recognitionLive=true;state.needsGesture=false;setMic(kind==='user');if(kind==='ambient')setStatus(state.wakeCapture?'Sigue hablando…':'Escucha activa. Di «Hola Zuzu».','ok');if(kind==='user')setStatus('Te escucho…','ok');updateBadge();};
    r.onerror=function(ev){if(gen!==state.recognitionGeneration)return;state.recognitionStarting=false;state.recognitionLive=false;var code=clean(ev&&ev.error);if(code==='not-allowed'||code==='service-not-allowed'){state.needsGesture=true;setStatus('La escucha se rearmará con tu siguiente pulsación normal.','err');}else if(code&&code!=='no-speech'&&code!=='aborted')setStatus('Micrófono: '+code,'err');updateBadge();};
    r.onend=function(){if(gen!==state.recognitionGeneration)return;state.recognitionStarting=false;state.recognitionLive=false;state.recognition=null;setMic(false);if(state.needsGesture){updateBadge();return;}if(kind==='ambient'&&state.ambientEnabled&&!state.conversationMode)setTimeout(function(){startAmbient(false);},90);else if(kind==='user'&&state.conversationMode&&!state.speaking&&!state.requestInFlight&&!state.awaitingResponse)setTimeout(startUser,90);};
  ...
  function startAmbient(fromGesture){if(!state.ambientEnabled||state.conversationMode||state.speaking||state.requestInFlight)return;state.mode='ambient';startRecognition('ambient',!!fromGesture);updateBadge();}
  function primeAmbientFromGesture(ev){
    if(!state.ambientEnabled||state.conversationMode||!supportsRecognition())return;
    if(ev&&ev.isTrusted===false)return;
    var t=ev&&ev.target;if(t&&t.closest&&t.closest('#ceVoz3Mic,#ceZuzuWakeBadge'))return;
    // Si el intento automático quedó bloqueado esperando permiso/activación, lo cortamos y
    // lo arrancamos AHORA, dentro del mismo gesto real. Esto evita perder la user activation.
  ...
  function startUser(seed){if(!state.conversationMode||state.speaking||state.requestInFlight||state.awaitingResponse)return;stopBarge();state.turnPrefix=clean(seed||state.turnPrefix);state.turnSession='';if(state.turnPrefix){setPrompt(state.turnPrefix);scheduleTurnCommit();}state.mode='user';startRecognition('user',false);updateBadge();}
  ...
  function scheduleEntertainment(delay){clearTimeout(state.entertainmentTimer);state.entertainmentTimer=setTimeout(function(){if(!state.conversationMode||!state.requestInFlight)return;speakEntertainmentPhrase();},Number(delay)||4000);}
  ...
  function startEntertainment(){stopEntertainment(true);state.entertainmentCount=0;state.lastEntertainmentIndex=-1;if(state.conversationMode&&state.requestInFlight)scheduleEntertainment(4000);}
  ...
  function bindPanel(){var b=$('ceVoz3Mic');if(b)b.onclick=function(){if(state.conversationMode){if(state.recognition)stopRecognition();else startUser();}else{state.needsGesture=false;state.ambientEnabled=true;safeSet(STORAGE.ambient,'1');startAmbient(true);}};b=$('ceVoz3AutoRead');if(b)b.onchange=function(){safeSet(STORAGE.auto,b.checked?'1':'0');};b=$('ceVoz3Read');if(b)b.onclick=speakResponse;b=$('ceVoz3Stop');if(b)b.onclick=function(){stopSpeaking(true);startUser();};b=$('ceVoz3RecordDownload');if(b)b.onclick=toggleRecording;b=$('ceVoz3Preview');if(b)b.onclick=previewVoice;b=$('ceVoz3VoiceMode');if(b)b.onchange=function(){safeSet(STORAGE.mode,b.value);populateVoices();};b=$('ceVoz3VoiceChoice');if(b)b.onchange=function(){safeSet(voiceKey(),b.value);};b=$('ceVoz3Rate');if(b)b.onchange=function(){safeSet(STORAGE.rate,b.value);};loadVoices();updateRecordButton();}
  ...
  document.addEventListener('ce:zuzu-request-started',function(ev){if(!state.conversationMode)return;state.requestInFlight=true;state.awaitingResponse=true;state.requestPrompt=clean(ev&&ev.detail&&ev.detail.prompt||state.requestPrompt);stopRecognition();clearTurnTimer();setStatus('Consultando ControlEvent…','ok');startEntertainment();});
  ...
  function install(){
    ...
    if(state.ambientEnabled)setTimeout(function(){startAmbient(false);},350);
    ...
    window.addEventListener('beforeunload',function(){stopRecognition();stopBarge();stopEntertainment(true);try{state.recorder&&state.recordingActive&&state.recorder.stop();}catch(_){}try{state.recorderStream&&state.recorderStream.getTracks().forEach(function(t){t.stop();});}catch(_){} });
  }
  ...
  window.ControlEventVoiceTurns={
    ...
    debugState:function(){return{build:BUILD,ambientEnabled:state.ambientEnabled,conversationMode:state.conversationMode,recognitionStarting:state.recognitionStarting,recognitionLive:state.recognitionLive,needsGesture:state.needsGesture,mode:state.mode};}
  };
