/* ControlEvent v2.0_exp · Zuzu Voice · FIX33 conversación robusta + repertorio persistente
...
(function(){
  ...
  var BUILD='v2.0_exp-FIX33-VOICE-RESUME-PERSISTENT-20-PHRASES';
  ...
  function loadEntertainmentState(){
    ...
    try{var raw=JSON.parse(safeGet(STORAGE.entertainmentUsed,'[]'));state.entertainmentUsed=Array.isArray(raw)?raw.map(Number).filter(function(x){return Number.isInteger(x)&&x>=0&&x<ENTERTAINMENT_PHRASES.length;}):[];}catch(_){state.entertainmentUsed=[];}
  ...
  function resumeVoiceAfterDownload(){
    if(!state.conversationMode||state.speaking||state.requestInFlight||state.awaitingResponse)return;
    setStatus('Te escucho…','ok');
    setTimeout(function(){if(state.conversationMode&&!state.speaking&&!state.requestInFlight&&!state.awaitingResponse)startUser();},180);
  }
  ...
  document.addEventListener('click',function(ev){var t=ev.target;if(t&&t.closest&&t.closest('#ceAiDownloadResult')&&state.conversationMode){stopEntertainment(true);resumeVoiceAfterDownload();}},true);
