...
(function(){
  ...
  var BUILD='v2.0_exp-FIX29-VOICE-CE-STT';
  ...
  var state={
    ...
    cloudFallback:false, cloudStream:null, cloudAudioContext:null, cloudSource:null, cloudAnalyser:null, cloudMonitor:null, cloudWanted:false, cloudKind:'ambient', cloudRecorder:null, cloudChunks:[], cloudRecording:false, cloudBusy:false, cloudLastVoiceAt:0, cloudRecordStartedAt:0, cloudThreshold:0.018, cloudGeneration:0, cloudLastTranscript:'',
  ...
  function cloudStatus(kind){setStatus(kind==='user'?'Te escucho con Voz CE…':'Voz CE activa. Di «Hola Zuzu».','ok');updateBadge();}
  ...
  function startCloudUtterance(kind){if(state.cloudRecording||state.cloudBusy||!state.cloudStream)return;var mime=cloudMime(),mr;try{mr=mime?new MediaRecorder(state.cloudStream,{mimeType:mime}):new MediaRecorder(state.cloudStream);}catch(err){setStatus('Voz CE no puede grabar este micrófono.','err');return;}var gen=state.cloudGeneration;state.cloudRecorder=mr;state.cloudChunks=[];state.cloudRecording=true;state.cloudRecordStartedAt=Date.now();state.cloudLastVoiceAt=Date.now();mr.ondataavailable=function(e){if(e.data&&e.data.size)state.cloudChunks.push(e.data);};mr.onstop=function(){var discard=!!mr.__discard,chunks=state.cloudChunks.slice(),type=mr.mimeType||mime||'audio/webm';if(state.cloudRecorder===mr)state.cloudRecorder=null;state.cloudChunks=[];if(discard||gen!==state.cloudGeneration)return;var blob=new Blob(chunks,{type:type});sendCloudAudio(blob,kind,gen);};try{mr.start(180);}catch(err){state.cloudRecording=false;state.cloudRecorder=null;setStatus('Voz CE no pudo iniciar la captura.','err');}}
  function cloudMonitorTick(){if(!state.cloudWanted||state.speaking||state.requestInFlight||state.awaitingResponse||state.cloudBusy||!state.cloudAnalyser)return;var arr=new Float32Array(state.cloudAnalyser.fftSize);try{state.cloudAnalyser.getFloatTimeDomainData(arr);}catch(_){return;}var sum=0;for(var i=0;i<arr.length;i++)sum+=arr[i]*arr[i];var rms=Math.sqrt(sum/Math.max(1,arr.length)),now=Date.now();if(rms>=state.cloudThreshold){if(!state.cloudRecording)startCloudUtterance(state.cloudKind);state.cloudLastVoiceAt=now;}if(state.cloudRecording){var elapsed=now-state.cloudRecordStartedAt,silent=now-state.cloudLastVoiceAt;if((elapsed>520&&silent>780)||elapsed>9000)finishCloudUtterance(state.cloudKind);}}
  function setupCloudStream(stream,kind){state.cloudStream=stream;state.cloudKind=kind;var AC=window.AudioContext||window.webkitAudioContext;if(!AC)throw new Error('AudioContext no disponible');var ac=new AC();state.cloudAudioContext=ac;state.cloudSource=ac.createMediaStreamSource(stream);var an=ac.createAnalyser();an.fftSize=1024;an.smoothingTimeConstant=0.25;state.cloudAnalyser=an;state.cloudSource.connect(an);state.cloudWanted=true;clearInterval(state.cloudMonitor);state.cloudMonitor=setInterval(cloudMonitorTick,45);try{ac.resume();}catch(_){}cloudStatus(kind);return true;}
  function startCloudRecognition(kind,fromGesture){kind=kind||'ambient';state.cloudFallback=true;state.cloudKind=kind;state.cloudWanted=true;state.needsGesture=false;if(!supportsCeVoice()){setStatus('Este navegador no permite la Voz CE por micrófono.','err');return false;}if(state.cloudStream){cloudStatus(kind);setMic(kind==='user');return true;}var gen=++state.cloudGeneration;setStatus('Activando Voz CE…','ok');navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false}).then(function(stream){if(gen!==state.cloudGeneration){try{stream.getTracks().forEach(function(t){t.stop();});}catch(_){}return;}setupCloudStream(stream,kind);setMic(kind==='user');}).catch(function(err){state.needsGesture=true;setStatus('Micrófono bloqueado: '+clean(err&&err.name||err&&err.message||'permiso')+'. Pulsa una vez en ControlEvent.','err');updateBadge();});return true;}
