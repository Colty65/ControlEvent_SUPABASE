...
(function(){
  ...
  var BUILD='v4_1_exp-FIX12-NEURAL-TTS-BARGEIN-V59';
  ...
  function neuralVoiceName(){var e=$('ceVoz3NeuralVoice'),v=clean(e?e.value:safeGet(STORAGE.neuralVoice,'Algenib'));return v||'Algenib';}
  function neuralPlaybackRate(){var e=$('ceVoz3NeuralRate'),n=Number(e?e.value:safeGet(STORAGE.neuralRate,'1.08'));return Number.isFinite(n)?Math.max(0.92,Math.min(1.18,n)):1.08;}
  ...
  function speakChunksBrowser(answer){if(!supportsSpeech()||!state.conversationMode){state.speaking=false;startUser();return;}pauseCloudListening();stopRecognition();stopBarge();cleanupNeuralAudio();try{window.speechSynthesis.cancel();}catch(_){}state.speechGeneration++;var gen=state.speechGeneration;state.speechChunks=chunkSpeech(answer);state.speechIndex=0;state.speaking=true;state.mode='speaking';setVoicePhase('SPEAKING','fallback navegador');updateBadge();setStatus('Zuzu está hablando. Di «Perdona Zuzu», «Para Zuzu» o «Calla Zuzu» para cortar.','ok');function next(){if(gen!==state.speechGeneration||!state.speaking)return;if(state.speechIndex>=state.speechChunks.length){finishSpokenAnswer(gen);return;}var seg=state.speechChunks[state.speechIndex++]||{},phrase=typeof seg==='string'?seg:seg.text,pause=Number(seg&&seg.pauseAfter)||0;if(!phrase){setTimeout(next,pause);return;}var u=new SpeechSynthesisUtterance(phrase);u.lang='es-ES';u.rate=Number(seg&&seg.rate)||speechRate();u.pitch=Number(seg&&seg.pitch)||0.78;u.volume=Number(seg&&seg.volume)||1;var voice=chooseVoice();if(voice)u.voice=voice;state.currentUtterance=u;u.onstart=function(){if(gen===state.speechGeneration)startBarge();};u.onend=function(){if(gen===state.speechGeneration)setTimeout(next,pause);};u.onerror=function(){if(gen===state.speechGeneration)setTimeout(next,Math.min(120,pause));};try{window.speechSynthesis.speak(u);}catch(_){setTimeout(next,Math.min(120,pause));}}next();}
  function speakChunks(answer){answer=clean(answer);if(!answer){startUser();return;}if(!state.conversationMode){state.conversationMode=true;}if(!supportsNeuralTts()){speakChunksBrowser(answer);return;}pauseCloudListening();stopRecognition();stopBarge();cleanupNeuralAudio();try{window.speechSynthesis.cancel();}catch(_){}state.speechGeneration++;var gen=state.speechGeneration;state.speaking=true;state.mode='speaking';setVoicePhase('SPEAKING_PREP','voz neural Zuzu');updateBadge();setStatus('Zuzu responde… puedes cortarla en cuanto empiece a hablar.','ok');var ctrl=new AbortController();state.neuralAbort=ctrl;fetch('/api/zuzu-voice/synthesize',{method:'POST',headers:{'Content-Type':'application/json'},signal:ctrl.signal,body:JSON.stringify({text:prepareSpeechText(answer),voice:neuralVoiceName()})}).then(function(res){return res.json().then(function(j){if(!res.ok||j.ok===false)throw new Error(j.error||('HTTP '+res.status));return j;});}).then(function(j){if(gen!==state.speechGeneration||!state.speaking)return;if(!j.audioBase64)throw new Error('Voz neural sin audio');state.neuralAbort=null;var blob=base64AudioBlob(j.audioBase64,j.mimeType||'audio/wav'),url=URL.createObjectURL(blob),a=new Audio(url);state.neuralObjectUrl=url;state.currentAudio=a;a.preload='auto';a.playbackRate=neuralPlaybackRate();try{a.preservesPitch=true;a.mozPreservesPitch=true;a.webkitPreservesPitch=true;}catch(_){}a.onplay=function(){if(gen!==state.speechGeneration)return;setVoicePhase('SPEAKING','Gemini TTS · '+clean(j.voice||neuralVoiceName()));setStatus('Zuzu está hablando. «Perdona Zuzu», «Para Zuzu», «Calla Zuzu» o «Espera Zuzu» la cortan.','ok');startBarge();};a.onended=function(){finishSpokenAnswer(gen);};a.onerror=function(){if(gen!==state.speechGeneration)return;cleanupNeuralAudio();state.speaking=false;state.neuralFailureUntil=Date.now()+60000;speakChunksBrowser(answer);};return a.play();}).catch(function(err){if(gen!==state.speechGeneration||!state.speaking||clean(err&&err.name)==='AbortError')return;cleanupNeuralAudio();state.speaking=false;state.neuralFailureUntil=Date.now()+60000;setVoicePhase('SPEAKING','TTS neural no disponible; fallback navegador');speakChunksBrowser(answer);});}
  ...
  function panelHtml(){var auto=safeGet(STORAGE.auto,'1')!=='0',mode='male',rate=safeGet(STORAGE.rate,'0.90');return '<div id="'+PANEL_ID+'">'+
    ...
    '<select id="ceVoz3NeuralVoice" title="Voz neural de Zuzu"><option value="Algenib">Algenib · áspera</option><option value="Orus">Orus · firme</option><option value="Gacrux">Gacrux · madura</option><option value="Charon">Charon · sobria</option></select><select id="ceVoz3NeuralRate" title="Ritmo de la voz neural"><option value="1.00">Natural</option><option value="1.08" selected>Ágil</option><option value="1.14">Muy ágil</option></select>'+
    '<select id="ceVoz3VoiceChoice" title="Fallback de voz del navegador"><option value="auto">Fallback navegador</option></select><select id="ceVoz3Rate" title="Ritmo del fallback"><option value="0.82"'+(rate==='0.82'?' selected':'')+'>Lento fallback</option><option value="0.90"'+(rate==='0.90'?' selected':'')+'>Normal fallback</option><option value="1.06"'+(rate==='1.06'?' selected':'')+'>Rápido fallback</option></select>'+
