...
(function(){
  ...
  var BUILD='v4_1_exp-FIX13-UNIFIED-VOICE-BUS-V60';
  ...
  var state={
    ...
    speaking:false, speechGeneration:0, speechChunks:[], speechIndex:0, currentUtterance:null, currentAudio:null, neuralAbort:null, neuralObjectUrl:'', neuralFailureUntil:0,
  ...
  function isBargeWord(w){w=norm(w).replace(/[^a-záéíóúüñ]/g,'');if(!w)return false;var roots=['perdona','perdon','espera','esperate','calla','callate','corta','detente','alto'];return roots.some(function(r){return w===r||(w.length>=5&&Math.abs(w.length-r.length)<=1&&controlEditDistance(w,r)<=1);});}
  function bargeControlInfo(v){var raw=clean(v),words=norm(raw).split(/\s+/).filter(Boolean),hasZ=words.some(function(w){return isZuzuWord(w);}),strong=words.some(isBargeWord),stop=words.some(function(w){w=w.replace(/[^a-záéíóúüñ]/g,'');return w==='para'||w==='parate'||w==='párate';});return{ok:!!(strong||(hasZ&&stop)),hasZuzu:hasZ,strong:strong,stop:stop,text:raw};}
  function hasBarge(v){return bargeControlInfo(v).ok;}
  function bargeTail(v){var raw=clean(v),patterns=[/\b(?:perdona|perd[oó]n|espera|esp[eé]rate|calla|c[aá]llate|corta|detente|alto)\s+(?:zuzu|susu|suzu|zusu|zulu|yuyu)\b/ig,/\b(?:zuzu|susu|suzu|zusu|zulu|yuyu)\s*[,;:]?\s*(?:para|p[aá]rate|perdona|perd[oó]n|espera|esp[eé]rate|calla|c[aá]llate|corta|detente|alto)\b/ig,/\b(?:para|p[aá]rate)\s+(?:zuzu|susu|suzu|zusu|zulu|yuyu)\b/ig,/\b(?:perdona|perd[oó]n|espera|esp[eé]rate|calla|c[aá]llate|corta|detente|alto)\b/ig],last=null;patterns.forEach(function(re){var m;while((m=re.exec(raw))){var end=re.lastIndex;if(!last||end>last.end)last={end:end};}});return last==null?'':stripLeadingVoiceControl(clean(raw.slice(last.end).replace(/^[\s,;:.!?-]+/,'')));}
  function applyBargeInterruption(text,source){if(!hasBarge(text)||!state.speaking)return false;var seed=bargeTail(text);if(voiceControlResidueInfo(seed).only)seed='';stopSpeaking(true);state.voiceResidueUntil=Date.now()+Number(state.voiceResidueQuarantineMs||4200);state.turnPrefix=seed;state.turnSession='';setPrompt(seed);setStatus('Te escucho…','ok');setVoicePhase('LISTENING',(source||'interrupción')+' detectada; Zuzu callada al instante');setTimeout(function(){startUser(seed);},55);return true;}
  ...
  function stripReservedFromSpeech(v){return clean(v).replace(/\bperdona\b/ig,'disculpa').replace(/\bperd[oó]n\b/ig,'disculpa').replace(/\bespera\b/ig,'aguarda').replace(/\besp[eé]rate\b/ig,'aguarda').replace(/\bcalla\b/ig,'mira').replace(/\bc[aá]llate\b/ig,'mira').replace(/\bdetente\b/ig,'un momento').replace(/\bcorta\b/ig,'cambia');}
  ...
  function speakLocalControl(text,after,phase){state.localControlGeneration++;state.localControlSpeaking=false;state.lastLocalControlEndedAt=Date.now();setVoicePhase(phase||'LOCAL_CONTROL',text);setStatus(text,'ok');if(typeof after==='function')setTimeout(after,80);}
  ...
  function speakLocalGreeting(){var name=voiceGreetingName(),text='Hola, '+name+'. ¿Tienes ganas de que hablemos? Pregúntame algo.';setStatus('Zuzu está lista para escucharte.','ok');speakChunks(text);}
  ...
  function cloudAudioConstraints(){var id=clean(safeGet(STORAGE.mic,'')),audio={echoCancellation:true,noiseSuppression:true,autoGainControl:true};if(id)audio.deviceId={exact:id};return{audio:audio,video:false};}
  ...
  function stopCloudBarge(discard){var mr=state.cloudBargeRecorder;state.cloudBargeRecorder=null;state.cloudBargeRecording=false;state.cloudBargeVoiceFrames=0;if(mr){try{mr.__discard=discard!==false;mr.stop();}catch(_){}}}
  function startCloudBargeUtterance(){if(state.cloudBargeRecording||state.cloudBargeBusy||!state.cloudStream||!state.speaking)return;var mime=cloudMime(),mr;try{mr=mime?new MediaRecorder(state.cloudStream,{mimeType:mime}):new MediaRecorder(state.cloudStream);}catch(_){return;}var gen=state.cloudGeneration;state.cloudBargeRecorder=mr;state.cloudBargeChunks=[];state.cloudBargeRecording=true;state.cloudBargeStartedAt=Date.now();state.cloudBargeLastVoiceAt=Date.now();mr.ondataavailable=function(e){if(e.data&&e.data.size)state.cloudBargeChunks.push(e.data);};mr.onstop=function(){var discard=!!mr.__discard,chunks=state.cloudBargeChunks.slice(),type=mr.mimeType||mime||'audio/webm';if(state.cloudBargeRecorder===mr)state.cloudBargeRecorder=null;state.cloudBargeChunks=[];state.cloudBargeRecording=false;if(discard||gen!==state.cloudGeneration||!state.speaking)return;var blob=new Blob(chunks,{type:type});state.cloudBargeBusy=true;sendCloudAudio(blob,'barge',gen).finally(function(){state.cloudBargeBusy=false;});};try{mr.start(120);}catch(_){state.cloudBargeRecording=false;state.cloudBargeRecorder=null;}}
  function cloudBargeMonitorTick(){if(!state.speaking||!state.cloudFallback||!state.cloudStream||state.cloudBargeBusy)return;var ans=(state.cloudAnalysers&&state.cloudAnalysers.length)?state.cloudAnalysers:(state.cloudAnalyser?[state.cloudAnalyser]:[]);if(!ans.length)return;var levels=ans.map(analyserRms),rms=0;for(var j=0;j<levels.length;j++)if(levels[j]>rms)rms=levels[j];state.cloudChannelRms=levels;state.cloudRms=rms;var now=Date.now(),floor=Math.max(0.0002,Number(state.cloudNoiseFloor)||0.001),thr=Math.max(Number(state.cloudThreshold)||0.006,floor*2.8+0.0012),voice=rms>=thr;state.cloudBargeVoiceFrames=voice?state.cloudBargeVoiceFrames+1:0;if(voice){state.cloudBargeLastVoiceAt=now;if(state.cloudBargeVoiceFrames>=3&&!state.cloudBargeRecording)startCloudBargeUtterance();}if(state.cloudBargeRecording){var elapsed=now-state.cloudBargeStartedAt,silent=now-state.cloudBargeLastVoiceAt;if((elapsed>650&&silent>520)||elapsed>2800){var mr=state.cloudBargeRecorder;try{mr&&mr.stop();}catch(_){state.cloudBargeRecording=false;}}}updateMicMeter();}
  function cloudMonitorTick(){if(state.speaking&&state.cloudFallback&&state.cloudStream){cloudBargeMonitorTick();return;}if(!state.cloudWanted||state.requestInFlight||state.awaitingResponse||state.cloudBusy)return;var ans=(state.cloudAnalysers&&state.cloudAnalysers.length)?state.cloudAnalysers:(state.cloudAnalyser?[state.cloudAnalyser]:[]);if(!ans.length)return;var levels=ans.map(analyserRms),rms=0;for(var j=0;j<levels.length;j++)if(levels[j]>rms)rms=levels[j];state.cloudChannelRms=levels;var now=Date.now();state.cloudRms=rms;state.cloudPeak=Math.max(rms,state.cloudPeak*0.985);if(!state.cloudCalibrationDone){state.cloudNoiseSamples.push(rms);if(state.cloudNoiseSamples.length>90)state.cloudNoiseSamples.shift();updateMicMeter();if(now>=state.cloudCalibratingUntil)finishCloudCalibration();return;}if(!state.cloudRecording&&rms<state.cloudThreshold){state.cloudNoiseFloor=clamp(state.cloudNoiseFloor*0.985+rms*0.015,0.00015,0.02);var target=clamp(state.cloudNoiseFloor*2.35+0.0010,0.0022,0.022);state.cloudThreshold=state.cloudThreshold*0.985+target*0.015;}var ratio=rms/Math.max(0.0002,state.cloudNoiseFloor),voice=(rms>=state.cloudThreshold)||(rms>=0.0014&&ratio>=1.65);state.cloudVoiceFrames=voice?state.cloudVoiceFrames+1:0;if(voice&&state.cloudVoiceFrames>=2){if(state.cloudKind==='user')touchReplyWindow();if(!state.cloudRecording)startCloudUtterance(state.cloudKind);state.cloudLastVoiceAt=now;}if(state.cloudRecording){var elapsed=now-state.cloudRecordStartedAt,silent=now-state.cloudLastVoiceAt;if((elapsed>700&&silent>3000)||elapsed>45000)finishCloudUtterance(state.cloudKind);}updateMicMeter();}
  ...
  function startAmbient(fromGesture){if(!state.ambientEnabled||state.conversationMode||state.speaking||state.requestInFlight)return;state.mode='ambient';state.cloudFallback=true;setVoicePhase('AMBIENT_STARTING',fromGesture?'gesto':'automático');startCloudRecognition('ambient',!!fromGesture);updateBadge();}
  ...
  function startUser(seed){if(!state.conversationMode||state.speaking||state.requestInFlight||state.awaitingResponse)return;if(state.manualDraftOwned){setVoicePhase('MANUAL_DRAFT','no rearmar micro sobre texto escrito');setStatus('Borrador escrito protegido. Envíalo cuando quieras.','ok');setMic(false);return;}stopBarge();armReplyWindow(state.replyWindowMs);state.turnPrefix=clean(seed||state.turnPrefix);state.turnSession='';if(state.turnPrefix){setPrompt(state.turnPrefix);scheduleTurnIfReady(state.turnLastAt||Date.now());}state.mode='user';setVoicePhase('USER_STARTING',state.turnPrefix?'reanuda con buffer':'nueva escucha');state.cloudFallback=true;startCloudRecognition('user',false);updateBadge();}

  function stopBarge(){state.bargeGeneration++;var r=state.bargeRecognition;state.bargeRecognition=null;try{r&&r.abort();}catch(_){try{r&&r.stop();}catch(__){}}stopCloudBarge(true);}
  function startBarge(){stopRecognition();stopBarge();if(!state.conversationMode||!state.speaking)return;if(state.cloudFallback){state.cloudBargeVoiceFrames=0;return;}if(!supportsRecognition())return;var C=recognitionCtor(),r=new C(),gen=++state.bargeGeneration;r.lang='es-ES';r.continuous=true;r.interimResults=true;r.maxAlternatives=5;state.bargeRecognition=r;r.onresult=function(ev){if(gen!==state.bargeGeneration||!state.speaking)return;var alternatives=eventAlternativeTexts(ev),t=alternatives.find(hasBarge)||sessionText(ev);applyBargeInterruption(t,'interrupción Web Speech');};r.onend=function(){if(gen!==state.bargeGeneration)return;state.bargeRecognition=null;if(state.speaking&&state.conversationMode&&!state.cloudFallback)setTimeout(startBarge,80);};r.onerror=function(){};try{r.start();}catch(_){setTimeout(function(){if(state.speaking&&!state.cloudFallback)startBarge();},120);}}
  ...
  function supportsNeuralTts(){return typeof fetch==='function'&&typeof Audio!=='undefined'&&safeGet(STORAGE.neural,'1')!=='0'&&Date.now()>Number(state.neuralFailureUntil||0);}
  function neuralVoiceName(){return 'Algenib';}
  function neuralPlaybackRate(){return 1.10;}
  function cleanupNeuralAudio(){if(state.neuralAbort){try{state.neuralAbort.abort();}catch(_){}}state.neuralAbort=null;var a=state.currentAudio;state.currentAudio=null;if(a){try{a.pause();a.src='';}catch(_){}}if(state.neuralObjectUrl){try{URL.revokeObjectURL(state.neuralObjectUrl);}catch(_){}state.neuralObjectUrl='';}}
  function base64AudioBlob(b64,mime){var bin=atob(String(b64||'')),len=bin.length,arr=new Uint8Array(len);for(var i=0;i<len;i++)arr[i]=bin.charCodeAt(i);return new Blob([arr],{type:mime||'audio/wav'});}
  ...
  function safeVoiceAgainstScreen(screen,spoken){var w=String(screen==null?'':screen),s=String(spoken==null?'':spoken);if(!s)return w;if(!w)return s;var allowed=voiceExplicitMoneyValues(w),claims=voiceExplicitMoneyValues(s),bad=claims.some(function(n){return !allowed.some(function(a){return a===n||Math.round(a)===Math.round(n);});});if(bad){try{console.warn('[CE VOZ FIX13] spoken_answer contiene un importe no acreditado; se conserva la respuesta oral solo si coincide exacta o redondeada con CE.');}catch(_){}return w;}try{if(s!==w)console.info('[CE VOZ FIX13] TTS usa spoken_answer certificado; se permiten importes CE redondeados al euro.');}catch(_){}return s;}
  function stripDecorativeEmoji(v){var out=String(v==null?'':v);try{out=out.replace(/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}]/gu,' ');}catch(_){out=out.replace(/[\u2600-\u27BF]/g,' ');}return out.replace(/[\uFE0E\uFE0F\u200D]/g,' ');}
  function prepareSpeechText(v){var text=stripDecorativeEmoji(String(v==null?'':v)).replace(/[*_`#>|]/g,' ');text=humanizeSpokenMoney(text);text=humanizeSpokenListRhythm(text);text=humanizeSpokenLabels(text);return stripReservedFromSpeech(clean(text));}
  function speechProsodySegments(v){
    ...
    push(0);return voicePunchTailSegments(out);
  }
  function voicePunchTailSegments(segments){var out=[],rx=/^(.*?)(?:,\s*)\b(crack|maquina|máquina|figura|fenomeno|fenómeno|campeon|campeón)\s*([!?]*)$/i;segments.forEach(function(seg){var t=clean(seg&&seg.text),m=t.match(rx);if(!m||clean(m[1]).length<5){out.push(seg);return;}var lead=clean(m[1]).replace(/[,;:\s]+$/,'');if(t.slice(-1)==='?'&&!/[?!.]$/.test(lead))lead+='?';else if(t.slice(-1)==='!'&&!/[?!.]$/.test(lead))lead+='!';out.push({text:lead,pauseAfter:18});out.push({text:'¡'+m[2]+'!',pauseAfter:Number(seg&&seg.pauseAfter)||0,rate:Math.min(1.10,speechRate()+0.11),pitch:1.03,volume:1});});return out;}
  function chunkSpeech(v){return speechProsodySegments(v);}
  function stopSpeaking(interrupted){state.speechGeneration++;state.localControlGeneration++;state.localControlSpeaking=false;state.speaking=false;state.currentUtterance=null;state.speechChunks=[];state.speechIndex=0;stopBarge();cleanupNeuralAudio();try{window.speechSynthesis.pause();window.speechSynthesis.cancel();}catch(_){}updateBadge();if(!interrupted&&state.conversationMode&&!state.requestInFlight&&!state.awaitingResponse)setTimeout(startUser,180);}
  function finishSpokenAnswer(gen){if(gen!==state.speechGeneration)return;state.speaking=false;stopBarge();cleanupNeuralAudio();updateBadge();setStatus('Te escucho…','ok');setVoicePhase('REPLY_WINDOW','respuesta terminada');setTimeout(startUser,150);}

  function neuralSpeechFailure(answer,gen,err){if(gen!==state.speechGeneration)return;cleanupNeuralAudio();state.speaking=false;state.neuralFailureUntil=Date.now()+4000;setVoicePhase('VOICE_RECOVERY','TTS neural estándar no disponible');setStatus('La voz estándar no ha podido sonar en este turno. Sigo escuchándote; no usaré otra voz.','err');updateBadge();setTimeout(function(){if(state.conversationMode&&!state.requestInFlight&&!state.awaitingResponse)startUser();},180);}
  function speakChunks(answer){answer=clean(answer);if(!answer){startUser();return;}if(!state.conversationMode){state.conversationMode=true;}pauseCloudListening();stopRecognition();stopBarge();cleanupNeuralAudio();try{window.speechSynthesis&&window.speechSynthesis.cancel();}catch(_){}state.speechGeneration++;var gen=state.speechGeneration;state.speaking=true;state.mode='speaking';setVoicePhase('SPEAKING_PREP','voz neural estándar única');updateBadge();if(!supportsNeuralTts()){neuralSpeechFailure(answer,gen,new Error('TTS neural temporalmente no disponible'));return;}setStatus('Zuzu responde… una sola voz. Puedes cortarla con «Perdona Zuzu» o «Para Zuzu».','ok');var ctrl=new AbortController();state.neuralAbort=ctrl;fetch('/api/zuzu-voice/synthesize',{method:'POST',headers:{'Content-Type':'application/json'},signal:ctrl.signal,body:JSON.stringify({text:prepareSpeechText(answer),voice:neuralVoiceName(),style:'Voz masculina adulta, grave, ligeramente áspera y cercana. Español de España. Habla ágil y enlazado, como un amigo que conoce bien el tema. Evita dicción de locutor, pausas artificiales y tono de GPS. No sobreactúes.'})}).then(function(res){return res.json().then(function(j){if(!res.ok||j.ok===false)throw new Error(j.error||('HTTP '+res.status));return j;});}).then(function(j){if(gen!==state.speechGeneration||!state.speaking)return;if(!j.audioBase64)throw new Error('Voz neural sin audio');state.neuralAbort=null;var blob=base64AudioBlob(j.audioBase64,j.mimeType||'audio/wav'),url=URL.createObjectURL(blob),a=new Audio(url);state.neuralObjectUrl=url;state.currentAudio=a;a.preload='auto';a.playbackRate=neuralPlaybackRate();try{a.preservesPitch=true;a.mozPreservesPitch=true;a.webkitPreservesPitch=true;}catch(_){}a.onplay=function(){if(gen!==state.speechGeneration)return;setVoicePhase('SPEAKING','voz estándar '+clean(j.voice||neuralVoiceName()));setStatus('Zuzu está hablando. «Perdona Zuzu», «Para Zuzu», «Calla Zuzu» o «Espera Zuzu» la cortan.','ok');startBarge();};a.onended=function(){finishSpokenAnswer(gen);};a.onerror=function(){neuralSpeechFailure(answer,gen,new Error('Error reproduciendo audio neural'));};return a.play();}).catch(function(err){if(gen!==state.speechGeneration||clean(err&&err.name)==='AbortError')return;neuralSpeechFailure(answer,gen,err);});}
  ...
  function previewVoice(){var was=state.conversationMode;state.conversationMode=true;speakChunks('Esta es la voz de Zuzu. Vamos al lío.');if(!was)setTimeout(function(){if(!state.speaking)state.conversationMode=false;},2500);}
  ...
  function speakEntertainmentPhrase(){
    if(!state.conversationMode||!state.requestInFlight||state.entertainmentCount>=state.entertainmentMaxPerRequest)return;
    var item=renderEntertainmentPhrase(contextualEntertainmentPhrase(state.requestPrompt,state.entertainmentCount)),phrase=item.display;state.entertainmentCount++;
    // FIX13: las microseñales de espera quedan SOLO en pantalla. Nunca activan speechSynthesis ni una segunda voz.
    setStatus(phrase||'Pensando…','ok');entertainmentEnded();
  }
  ...
  function activateDirectConversation(){
    ...
    else setStatus('Este navegador no ofrece getUserMedia + MediaRecorder, que es la entrada estándar de Zuzu.','err');
  ...
  function panelHtml(){var auto=safeGet(STORAGE.auto,'1')!=='0',mode='male',rate=safeGet(STORAGE.rate,'0.90');return '<div id="'+PANEL_ID+'">'+
    ...
    '<select id="ceVoz3NeuralVoice" disabled title="Voz estándar única de Zuzu"><option value="Algenib" selected>Zuzu · Neural estándar</option></select><select id="ceVoz3NeuralRate" disabled title="Ritmo estándar"><option value="1.10" selected>Ágil natural</option></select>'+
  ...
  function bindPanel(){var b=$('ceVoz3Mic');if(b)b.onclick=toggleDirectConversationMic;b=$('ceVoz3AutoRead');if(b)b.onchange=function(){safeSet(STORAGE.auto,b.checked?'1':'0');};b=$('ceVoz3Read');if(b)b.onclick=speakResponse;b=$('ceVoz3Stop');if(b)b.onclick=function(){stopSpeaking(true);activateDirectConversation();};b=$('ceVoz3RecordDownload');if(b)b.onclick=toggleRecording;b=$('ceVoz3Preview');if(b)b.onclick=previewVoice;b=$('ceVoz3VoiceMode');if(b)b.onchange=function(){safeSet(STORAGE.mode,b.value);populateVoices();};b=$('ceVoz3NeuralVoice');if(b){b.value=safeGet(STORAGE.neuralVoice,'Algenib');b.onchange=function(){safeSet(STORAGE.neuralVoice,b.value||'Algenib');};}b=$('ceVoz3NeuralRate');if(b){b.value=safeGet(STORAGE.neuralRate,'1.08');b.onchange=function(){safeSet(STORAGE.neuralRate,b.value||'1.08');};}b=$('ceVoz3VoiceChoice');if(b)b.onchange=function(){safeSet(voiceKey(),b.value);};b=$('ceVoz3Rate');if(b)b.onchange=function(){safeSet(STORAGE.rate,b.value);};b=$('ceVoz3MicChoice');if(b)b.onchange=function(){safeSet(STORAGE.mic,b.value||'');var kind=state.conversationMode?'user':'ambient';closeCloudVoice();state.cloudFallback=true;setTimeout(function(){if(kind==='user')activateDirectConversation();else startCloudRecognition('ambient',true);},80);};loadVoices();refreshMicDevices();updateMicMeter(true);updateRecordButton();}
  ...
  function install(){
    ...
    safeSet(STORAGE.ambient,'1');safeSet(STORAGE.mode,'male');state.ambientEnabled=true;state.cloudFallback=true;
    try{window.speechSynthesis&&window.speechSynthesis.cancel();}catch(_){}
    // FIX13: transporte de voz único. Todos los dispositivos usan Voz CE (getUserMedia + MediaRecorder + transcripción servidor).
    // Web Speech queda fuera del camino normal para evitar diferencias PC/iOS/Android.
    state.localSpeechReady=false;state.localSpeechUnavailable=true;
    if(state.ambientEnabled)setTimeout(function(){state.cloudFallback=true;startCloudRecognition('ambient',false);},260);
