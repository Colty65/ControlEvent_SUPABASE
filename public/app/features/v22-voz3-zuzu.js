/* ControlEvent v3_0_exp · Zuzu Voice · RAW14L turn-taking + ventana de réplica + espera de entretenimiento completa
...
(function(){
  ...
  var BUILD='v3_0_exp-RAW14L-TURN-TAKING-MIC-FIX42';
  ...
  var state={
    ...
    turnPrefix:'', turnSession:'', turnTimer:null, turnLastAt:0, replyWindowTimer:null, replyWindowUntil:0, replyWindowMs:12000,
  ...
  function armReplyWindow(delay){
    ...
    var ms=Math.max(4000,Number(delay)||state.replyWindowMs);state.replyWindowUntil=Date.now()+ms;
  ...
  function handleUserText(text){if(!text)return;touchReplyWindow();state.turnSession=text;setPrompt(currentTurn());scheduleTurnCommit();}
  function commitUserTurn(){clearReplyWindow();var elapsed=Date.now()-state.turnLastAt;if(elapsed<1650){clearTurnTimer();state.turnTimer=setTimeout(commitUserTurn,1700-elapsed);return;}var text=clean(currentTurn());if(!text)return;clearTurnTimer();stopRecognition();state.turnPrefix='';state.turnSession='';state.requestPrompt=text;state.requestInFlight=true;state.awaitingResponse=true;state.mode='request';setPrompt(text);setStatus('Procesando…','ok');updateBadge();var b=$('ceAiRun');if(b)b.click();}
  function startUser(seed){if(!state.conversationMode||state.speaking||state.requestInFlight||state.awaitingResponse)return;stopBarge();armReplyWindow(state.replyWindowMs);state.turnPrefix=clean(seed||state.turnPrefix);state.turnSession='';if(state.turnPrefix&&!state.cloudFallback){setPrompt(state.turnPrefix);scheduleTurnCommit();}state.mode='user';if(state.cloudFallback||!supportsRecognition())startCloudRecognition('user',false);else startRecognition('user',false);updateBadge();}
  ...
  document.addEventListener('ce:zuzu-request-started',function(ev){if(!state.conversationMode)return;clearReplyWindow();pauseCloudListening();state.requestInFlight=true;state.awaitingResponse=true;state.requestPrompt=clean(ev&&ev.detail&&ev.detail.prompt||state.requestPrompt);stopRecognition();clearTurnTimer();setStatus('Consultando ControlEvent…','ok');startEntertainment();});
