/* ControlEvent v28.2_prod OPT2F - GRAFICAS: renderer único v46 / 3 quesos de ancho.
...
(function(){
  ...
  const VERSION = 'v28.2_prod_opt_2f';
  ...
  function finalPaint(reason){
    ...
    allowCommit = true;
    try{
      fn.call(window.ControlEventV462 || window.ControlEventV461 || window.ControlEventV460 || window, {force:true, reason:'v28.2_prod_opt_2f_' + (reason || 'final')});
    }catch(err){
      console.warn('[v28.2_prod_opt_2f] render final gráficas', err);
    }finally{
      allowCommit = false;
    }
    const html = rawGet(w) || '';
