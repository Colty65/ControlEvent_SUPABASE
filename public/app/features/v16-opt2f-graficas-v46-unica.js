/* ControlEvent v24_prod-02 OPT2F - GRAFICAS: renderer único v46 / 3 quesos de ancho.
...
(function(){
  ...
  function finalPaint(reason){
    ...
    allowCommit = true;
    try{
      fn.call(window.ControlEventV462 || window.ControlEventV461 || window.ControlEventV460 || window, {force:true, reason:'v24_prod_opt_2f_' + (reason || 'final')});
    }catch(err){
      console.warn('[v24_prod_opt_2f] render final gráficas', err);
    }finally{
      allowCommit = false;
    }
    const html = rawGet(w) || '';
