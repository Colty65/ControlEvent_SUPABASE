/* ControlEvent v27_prod_1.1 OPT2F - GRAFICAS: renderer único v46 / 3 quesos de ancho.
...
(function(){
  ...
  const VERSION = 'v27_prod_1.1_opt_2f';
  ...
  function finalPaint(reason){
    ...
    allowCommit = true;
    try{
      fn.call(window.ControlEventV462 || window.ControlEventV461 || window.ControlEventV460 || window, {force:true, reason:'v27_prod_1.1_opt_2f_' + (reason || 'final')});
    }catch(err){
      console.warn('[v27_prod_1.1_opt_2f] render final gráficas', err);
    }finally{
      allowCommit = false;
    }
    const html = rawGet(w) || '';
