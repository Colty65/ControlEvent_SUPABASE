/* ControlEvent v45.7 - DONACION por colaborador PEÑA_DONA_A_FUNDACION, quesos y barra INFOEVENTO.
...
(function(){
  ...
  function isPenaDonaFundacion(row){
    const n = fold(personName(row));
    return n === 'PENA_DONA_A_FUNDACION' || n.includes('PENA_DONA_A_FUNDACION') || n.includes('PENA DONA A FUNDACION');
  }
  function isPendingIncome(row){
    const s = fold(row?.situacion || row?.ingreso || row?.tipoIngreso || '');
    return s === 'PENDIENTE' || s.includes('PENDIENTE') || s.includes('PTE');
  }
  function penaDonaFundacionInfo(){
    const rows = ingresos().filter(r => isPenaDonaFundacion(r));
    const paid = rows.filter(r => !isPendingIncome(r));
    return {exists: rows.length > 0, paid: paid.length > 0, amount: paid.reduce((a,r)=>a+incomeTotal(r),0), rows, paidRows: paid};
  }
  ...
  function computed(){
    ...
    const penaInfo = penaDonaFundacionInfo();
    const donacion = penaInfo.paid ? penaInfo.amount : 0;
    const showDonacion = !!penaInfo.paid;
    return {ingresosRealizados,gastosRealizados,saldoActual,ingresoTotal,gastosPrevistos,saldoOperativo,penaDonaFundacion:donacion,donacion,hasPenaDonaFundacion:!!penaInfo.exists,penaDonaFundacionPaid:!!penaInfo.paid,showDonacion};
  }
  function formulaTip(kind, c){
    ...
    return `Concepto | Condición | Importe\nDONACION | PEÑA_DONA_A_FUNDACION pagado | ${money(c.donacion)}\nColaborador | No pendiente de ingreso | ${money(c.penaDonaFundacion ?? c.donacion)}`;
  }
  ...
  function enhanceBudget(){
    ...
    const c = computed();
    const sig = [eventId(), c.saldoActual, c.penaDonaFundacionPaid, c.donacion].join('|');
  ...
  function enhanceGraficas(){
    ...
    const c = computed();
    const sig = [eventId(), c.saldoActual, c.saldoOperativo, c.penaDonaFundacionPaid, c.donacion].join('|');
  ...
  function donationFromBudget(b){
    ...
    const penaInfo = penaDonaFundacionInfo();
    const donacion = penaInfo.paid ? penaInfo.amount : 0;
    return {penaDonaFundacion:donacion, donacion, hasPenaDonaFundacion: !!penaInfo.exists, penaDonaFundacionPaid: !!penaInfo.paid, showDonacion: !!penaInfo.paid};
  }
  function patchBudgetSummary(){
    const old = (typeof budgetSummary === 'function') ? budgetSummary : window.budgetSummary;
    if(typeof old !== 'function' || old.__ceV457Donacion) return;
    const wrapped = function(){
      const b = old.apply(this, arguments) || {};
      try{
        const op = b.operativa || {};
        const d = donationFromBudget(b);
        const nextOp = Object.assign({}, op, {penaDonaFundacion:d.penaDonaFundacion, penaDonaFundacionExists:d.hasPenaDonaFundacion, penaDonaFundacionPaid:d.penaDonaFundacionPaid});
        if(d.showDonacion){
    ...
    wrapped.__ceV457Donacion = true; wrapped.__ceV455Donacion = true;
  ...
  function schedule(){
    ...
    setTimeout(()=>{
      scheduled = false;
      try{ patchBudgetSummary(); enhanceBudget(); enhanceGraficas(); }catch(e){ console.warn('[v45.7] donacion operativa', e); }
    }, 60);
  }
  ...
  window.ControlEventV457Donacion = {version:VERSION, install, computed, enhanceBudget, enhanceGraficas};
