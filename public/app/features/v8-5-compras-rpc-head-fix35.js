/* ControlEvent v26_prod_1.0 FIX35 - COMPRAS RPC desde HEAD
...
(function(){
  'use strict';
  const TAG='__ceV85ComprasRpcHeadFix35';
  if(window[TAG]) return; window[TAG]=true;
  const LOG='[CE FIX35 COMPRAS RPC HEAD]';
  ...
  function flashRow(id, cls){
    setTimeout(()=>{
      ...
      setTimeout(()=>card.classList.remove(cls||'ce-crud-flash'), 9000);
    }, 50);
  }
  function unlockUi(){
  ...
  async function syncStateNoRender(){
    try{
      const fresh=await apiJson('/api/state?fix35=1&_='+Date.now(), {method:'GET', headers:{'Cache-Control':'no-cache','Pragma':'no-cache'}});
  ...
  style.textContent=`
    ...
    .ce-crud-flash-add{ animation:ceCrudPulseFix35 1.1s ease-in-out 0s 7 alternate; outline:4px solid #22c55e !important; box-shadow:0 0 0 6px rgba(34,197,94,.22) !important; font-weight:900!important; transition:outline .25s ease, box-shadow .25s ease; }
    .ce-crud-flash-update{ animation:ceCrudPulseFix35 1.1s ease-in-out 0s 7 alternate; outline:4px solid #0ea5e9 !important; box-shadow:0 0 0 6px rgba(14,165,233,.22) !important; font-weight:900!important; transition:outline .25s ease, box-shadow .25s ease; }
