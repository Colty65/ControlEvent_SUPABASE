/* ControlEvent v9.6_prod - Baja segura de evento con eliminación en cascada controlada.
  ...
  function scheduleTicketImageCleanup(eventId){
    ...
    window.setTimeout(() => {
      deleteTicketImagesServer(eventId).catch(error => {
        console.warn('[ControlEvent v9.6_prod] Limpieza diferida de CE_TICKET_IMAGES no bloqueante:', error?.message || error);
      });
    }, 1400);
  }
