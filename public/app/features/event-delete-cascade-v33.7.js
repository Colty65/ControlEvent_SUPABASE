/* ControlEvent v1.0_exp - Baja segura de evento con eliminación en cascada controlada.
  ...
  function scheduleTicketImageCleanup(eventId){
    ...
    window.setTimeout(() => {
      deleteTicketImagesServer(eventId).catch(error => {
        console.warn('[ControlEvent v1.0_exp] Limpieza diferida de CE_TICKET_IMAGES no bloqueante:', error?.message || error);
      });
    }, 1400);
  }
