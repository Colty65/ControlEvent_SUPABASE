/* ControlEvent v26_prod_1.2 FIX38 - COMPRAS RPC + RECARGA DETERMINISTA tras baja
...
(function(){
  'use strict';
  const TAG='__ceV85ComprasRpcHeadFix38';
  if(window[TAG]) return; window[TAG]=true;
  const LOG='[CE FIX38 COMPRAS RPC HEAD]';
  ...
  function withNoStateWrites(fn){
    ...
    const localOk = function(){ try{ if(typeof persistStateLocal==='function') persistStateLocal(); }catch(_){} return Promise.resolve({ok:true, localOnly:true, fix:'FIX38'}); };
    try{
      ...
      if(typeof oldFetch === 'function'){
        window.fetch = function(input, init){
          ...
          if(method === 'PUT' && /\/api\/state(?:$|\?)/i.test(url)){
            return Promise.resolve(new Response(JSON.stringify({ok:true, localOnly:true, blockedBy:'FIX38_RENDER'}), {status:200, headers:{'Content-Type':'application/json'}}));
          }
  ...
  async function syncStateNoRender(){
    try{
      const fresh=await apiJson('/api/state?fix38=1&_='+Date.now(), {method:'GET', headers:{'Cache-Control':'no-cache','Pragma':'no-cache'}});
  ...
  function toast(msg){
    ...
    try{ console.info(LOG || '[ControlEvent v26_prod_1.2]', msg); }catch(_){}
  }
  function reloadAfterDelete(id){
    ...
    try{ sessionStorage.setItem('ce_fix38_restore_event', selectedEventId()); }catch(_){ }
    try{ sessionStorage.setItem('ce_fix38_restore_tab', 'compras'); }catch(_){ }
    try{ sessionStorage.setItem('ce_fix38_deleted_compra', String(id||'')); }catch(_){ }
    persistLocalNow();
  ...
  function restoreAfterReload(){
    const evId = (()=>{ try{return sessionStorage.getItem('ce_fix38_restore_event')||'';}catch(_){return '';} })();
    const tab = (()=>{ try{return sessionStorage.getItem('ce_fix38_restore_tab')||'';}catch(_){return '';} })();
    if(!evId && !tab) return;
    try{ sessionStorage.removeItem('ce_fix38_restore_event'); sessionStorage.removeItem('ce_fix38_restore_tab'); }catch(_){ }
    const apply=()=>{
      try{
        const s=stateObj();
  ...
  console.info(LOG,'activo en HEAD: COMPRAS RPC FIX38 con recarga determinista tras baja y situación por RPC');
})();
