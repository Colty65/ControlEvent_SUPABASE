/* ControlEvent v4_0_exp BANK4 · informes por responsable: Compras, Donaciones y combinado Mapa/Vista aérea. */
(function(root){
  'use strict';
  ...
  function enrich(row,kind){
    ...
    const units=num(row.unidades),price=num(row.precio??row.precioCalc??p.precio),amount=num(row.valor??row.importe??units*price);
  ...
  function sortLines(list,kind){
    return list.sort((a,b)=>kind==='donacion'
      ? a.producto.localeCompare(b.producto,'es',{sensitivity:'base'})||a.tienda.localeCompare(b.tienda,'es',{sensitivity:'base'})
      : a.tienda.localeCompare(b.tienda,'es',{sensitivity:'base'})||a.producto.localeCompare(b.producto,'es',{sensitivity:'base'}));
  }
  function grouped(kind){
    ...
    return [...map.entries()].map(([name,list])=>({name,rows:sortLines(list,kind),total:list.reduce((s,r)=>s+r.importe,0)})).sort((a,b)=>a.name.localeCompare(b.name,'es'));
  }
  ...
  function combinedHtml(group){
    return `<div class="ce-resp-subhead"><span>Compras · Tienda + Producto</span><b>${esc(money(group.purchaseTotal))}</b></div>${tableHtmlRows(group.purchases,'compra')}<div class="ce-resp-combined-gap"></div><div class="ce-resp-subhead donation"><span>Donaciones · Producto</span><b>${esc(money(group.donationTotal))}</b></div>${tableHtmlRows(group.donations,'donacion')}`;
  }
  function openReport(kind){
    ...
    modal.innerHTML=`<section class="ce-resp-report-card" role="dialog" aria-modal="true"><header><div class="ico">${combined?'🧭':kind==='donacion'?'🎁':'🛒'}</div><div class="copy"><span>CONTROL EVENT · ${combined?'RECURSOS POR RESPONSABLE':kind==='donacion'?'VALORACIÓN ESTIMADA':'DETALLE DE COMPRAS'}</span><h3>${esc(title)} · ${esc(eventTitle())}</h3><p>${groups.length} responsable${groups.length===1?'':'s'} · ${count} línea${count===1?'':'s'}${combined?' · donaciones con valor estimado':''}</p></div><button type="button" class="pdf-all" data-ce-resp-pdf-all>🖨 PDF todos</button><button type="button" class="close" data-ce-resp-close>×</button></header><div class="ce-resp-report-body">${body}</div></section>`;
  ...
  function printableRows(list,kind){
    ...
    return `<table><thead><tr><th>${kind==='donacion'?'Origen / tienda':'Tienda'}</th><th>Producto</th><th>Uds.</th><th>Precio</th><th>${kind==='donacion'?'Valor estimado':'Importe'}</th></tr></thead><tbody>${list.map(r=>`<tr><td>${esc(r.tienda)}</td><td>${esc(r.producto)}</td><td class="num">${esc(r.unidades.toLocaleString('es-ES'))}</td><td class="num">${esc(money(r.precio))}</td><td class="num">${esc(money(r.importe))}</td></tr>`).join('')}</tbody></table>`;
  }
  function printable(groups,kind){
    if(kind==='combinado')return groups.map(g=>`<section class="group"><h2>${esc(g.name)} <span>Compras ${esc(money(g.purchaseTotal))} · Donaciones ${esc(money(g.donationTotal))}</span></h2><h3>Compras · Tienda + Producto</h3>${printableRows(g.purchases,'compra')}<div class="gap"></div><h3 class="don">Donaciones · Producto · valor estimado</h3>${printableRows(g.donations,'donacion')}</section>`).join('');
  ...
  function printGroups(groups,kind){
    ...
    const note=kind==='donacion'||kind==='combinado'?'<p class="note">Los importes de Donaciones corresponden a valoración estimada.</p>':'';
    ...
    win.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${esc(title)}</title><style>@page{size:A4 landscape;margin:10mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#243847;margin:0}header{border-bottom:3px solid #0f766e;padding:0 0 10px;margin-bottom:12px}h1{font-size:20px;margin:0;color:#173a63}header p,.note{font-size:10px;color:#607789}.group{break-inside:avoid;margin:0 0 16px}.group h2{display:flex;justify-content:space-between;margin:0;padding:7px 9px;background:#edf6fa;color:#173a63;font-size:13px}.group h2 span{color:#0f766e}.group h3{margin:8px 0 4px;font-size:10px;color:#173a63;text-transform:uppercase}.group h3.don{color:#9a5d0c}.gap{height:12px}table{width:100%;border-collapse:collapse;font-size:9px}th,td{padding:5px 6px;border-bottom:1px solid #dfe8ee;text-align:left}th{background:#f8fafc;text-transform:uppercase;font-size:8px;color:#64748b}.num{text-align:right;white-space:nowrap}.empty{font-size:9px;color:#7a8b97}</style></head><body><header><h1>${esc(h1)} · ${esc(eventTitle())}</h1><p>ControlEvent v4_0_exp · ${new Date().toLocaleString('es-ES')}</p>${note}</header>${printable(groups,kind)}<script>window.onload=function(){setTimeout(function(){window.focus();window.print()},220)}<\/script></body></html>`);win.document.close();
  }
  ...
  function bind(){
    document.addEventListener('click',ev=>{
      ...
      if(ev.target?.closest?.('#btnMapaResponsables,#btnVistaAereaResponsables')){ev.preventDefault();openReport('combinado');return;}
    ...
    const obs=new MutationObserver(()=>ensureEntryButtons());obs.observe(document.documentElement,{childList:true,subtree:true});
    ...
    [0,120,500,1200].forEach(ms=>setTimeout(ensureEntryButtons,ms));
  }
