/* ControlEvent v8.5_prod FIX24 - CRUD EXPLICITO Y EVENTO FINALIZADO INMUTABLE
   Regla:
...
(function(){
  'use strict';
  const TAG='__ceV85ExplicitCrudFix24';
  if(window[TAG]) return; window[TAG]=true;
  const WRITE_SCOPE='row-crud-v8-5-fix23';

  function warn(msg, extra){try{console.warn('[FIX24 CRUD]', msg, extra||'');}catch(_){}}
  ...
  async function handleAction(action,id){
    ...
    if(action==='save-evento'){ const target=eventById(id); if(blockFinalized(target,'modificar el evento'))return; const row=rowEventoFromForm(id); await upsert('eventos',row); replaceLocal('eventos',row); rerender(); return; }
  ...
  document.addEventListener('click', function(ev){
    ...
    (async()=>{
      ...
      }catch(e){console.error('[FIX24 CRUD]',e); alert(e.message||String(e));}
    })();
    return false;
  }, true);
  ...
  window.ControlEventExplicitCrudFix24={active:true, version:'v8.5_prod_fix24', rule:'Finalizado inmutable; CRUD inmediato fila-a-fila'};
  warn('Activo: mantenimiento por CRUD explícito y Finalizado inmutable.');
})();
