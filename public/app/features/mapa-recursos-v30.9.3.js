/* ControlEvent v28.1_prod - Mapa de recursos
...
(function(){
  ...
  const FILTER_ID = 'mapaResponsablesFilterV309';
  ...
  function isDonation(value){
    ...
    return DONATION_TYPES.includes(up(value));
  }
  ...
  function donorName(row){
    ...
    return row?.donante || row?.donor || row?.donorLabel || 'Sin donante';
  }
  function responsibleId(row){ return String(row?.responsableId || row?.responsablePersonaId || row?.socioResponsableId || row?.responsable || ''); }
  ...
  function unitPrice(row){
    const p = productOf(row) || {};
    const candidates = [row?.precio, row?.precioCalc, p.precio, p.defaultPrecio];
  ...
  function productName(row){ return productOf(row)?.nombre || row?.productoNombre || row?.producto || 'Producto sin nombre'; }
  ...
  function ticketLabel(row){
    ...
    if(!raw) return 'Pte. comprar / otros gastos';
    if(isDonation(raw)) return raw;
    if(isTk(raw)) return raw.replace(/^TK\s*/i, 'TK');
    const t = up(raw);
    if(t.includes('PTE') || t.includes('PENDIENTE') || t.includes('COMPRA') || t.includes('GASTO')) return 'Pte. comprar / otros gastos';
    return raw;
  }
  ...
  function buildMapaProductos(){
    ...
    const onlyDonations = Array.from(donationByProduct.entries())
      ...
      .map(([productId, items]) => {
        ...
        return {
          productId,
          tienda: 'Solo donación',
          ticket: 'Donación sin compra planificada',
    ...
    return {groups: result, onlyDonations, buys, donations, responsableOptions, totalRows: rowsAll.length};
  }
  ...
  function bindResponsableFilter(){
    const filter = $(FILTER_ID);
    if(!filter || filter.__ceBoundV309) return;
    filter.__ceBoundV309 = true;
  ...
  function sectionKey(group){ return `${group.tienda}||${group.ticket}`; }
  function renderCardsWithHeaders(groups){
    let lastKey = '';
    return groups.map(group => {
      const key = sectionKey(group);
      const header = key !== lastKey ? `<div class="mapa-group-header ${group.isResolvedTk ? 'resolved' : 'pending'}"><strong>${esc(group.tienda)}</strong><span>${esc(group.ticket)}</span></div>` : '';
      lastKey = key;
      return header + `
      <article class="mapa-product-card ${group.isResolvedTk ? 'resolved-tk' : 'pending-buy'}">
        ...
        <div class="mapa-product-meta">
          <div><b>Ticket / estado:</b> ${esc(group.ticket)}</div>
          <div><b>Responsable:</b> ${esc(Array.from(group.responsables.values()).join(', ') || '—')}</div>
        </div>
  ...
  function normalizeMapaLabels(){
    ...
    const fixes = [[/PENDIENTE\s+COMPRAR/gi, 'COMPRAS PRODUCTO'],[/Pendiente\s+comprar/g, 'COMPRAS PRODUCTO'],[/Pendiente\s+de\s+comprar/g, 'COMPRAS PRODUCTO'],[/Pdte\.Compra/g, 'COMPRAS PRODUCTO'],[/Mapa de productos/g, 'Mapa de recursos']];
    const nodes = [];
  ...
  function renderMapaProductos(){
    ...
    if(lastRenderedEventId !== currentEventId){ selectedResponsables = null; lastRenderedEventId = currentEventId; }
    ...
    summary.innerHTML = `
      ...
      <div class="mapa-summary-metrics">
        ...
        ${renderMetric('Compras producto', moneyFmt(totalCompra), `${data.groups.length} tienda-ticket-producto`, 'warn')}
    ...
    const cards = renderCardsWithHeaders(data.groups);
  ...
  function applyMapVisibility(){
    if(mapPinned && currentTab() !== TAB_NAME) setCurrentTab(TAB_NAME);
    ...
    if(btn){ btn.classList.toggle('active', active); btn.classList.remove('hidden-by-role-v228'); btn.style.removeProperty('display'); btn.removeAttribute('aria-hidden'); btn.disabled = false; btn.removeAttribute('aria-disabled'); btn.setAttribute('aria-label', 'Mapa de recursos'); btn.removeAttribute('title'); }
    document.querySelectorAll('.mobile-menu-action[data-target="tabMapaBtn"]').forEach(el => { el.classList.remove('hidden-by-role-v228'); el.style.removeProperty('display'); el.removeAttribute('aria-hidden'); el.disabled = false; el.removeAttribute('aria-disabled'); el.classList.toggle('primary', active); el.innerHTML = '<span class="mi">🧭</span>Mapa de recursos'; });
  ...
  function forceShowMapa(options = {}){
    ...
    applyMapVisibility(); renderMapaProductos(); normalizeMapaLabels(); closeMobileDrawer();
    try{ window.ControlEventModules?.activate?.(TAB_NAME, {reason: options.reason || 'mapa-recursos-show'}); }catch(_){ }
  }
  function ensureMobileMenuAction(){
    Array.from(document.querySelectorAll('.mobile-menu-grid')).forEach(grid => {
      ...
      if(existing){ existing.innerHTML = '<span class="mi">🧭</span>Mapa de recursos'; return; }
  ...
  function renderTabVisibilityV309(){ const result = oldRenderTabVisibility ? oldRenderTabVisibility.apply(this, arguments) : undefined; applyMapVisibility(); return result; }
  ...
  function renderV309(){ const result = oldRender ? oldRender.apply(this, arguments) : undefined; applyMapVisibility(); ensureMobileMenuAction(); if(currentTab() === TAB_NAME) renderMapaProductos(); return result; }
  ...
  function bindDirectMapaButton(){
    document.querySelectorAll('#tabMapaBtn,.mobile-menu-action[data-target="tabMapaBtn"]').forEach(el => {
      if(el.__ceMapaV309Bound) return; el.__ceMapaV309Bound = true;
      el.addEventListener('click', ev => openMapaFromEvent(ev, 'direct-click'));
      el.addEventListener('keydown', ev => { if(ev.key === 'Enter' || ev.key === ' '){ openMapaFromEvent(ev, 'direct-keyboard'); } });
      el.onclick = function(ev){ openMapaFromEvent(ev || window.event || {target:el}, 'direct-onclick'); return false; };
      el.removeAttribute('title');
      el.setAttribute('aria-label', 'Mapa de recursos');
    });
  }
  function bindOtherTabs(){
    document.querySelectorAll('#tabIngresosBtn,#tabDonacionesBtn,#tabComprasBtn,#tabResumenBtn,#tabGraficasBtn,.mobile-menu-action[data-target="tabIngresosBtn"],.mobile-menu-action[data-target="tabDonacionesBtn"],.mobile-menu-action[data-target="tabComprasBtn"],.mobile-menu-action[data-target="tabResumenBtn"],.mobile-menu-action[data-target="tabGraficasBtn"]').forEach(el => {
      if(el.__ceMapaClearV309) return; el.__ceMapaClearV309 = true;
      el.addEventListener('click', () => { mapPinned = false; try{ window.__ceMapaProductosPinned = false; }catch(_){ } });
    });
  }

  document.addEventListener('change', event => { if(event.target && event.target.id === 'selectedEvent' && currentTab() === TAB_NAME){ setTimeout(() => forceShowMapa({reason:'event-change'}), 60); } });
  try{ renderTabVisibility = renderTabVisibilityV309; }catch(_){ }
  try{ window.renderTabVisibility = renderTabVisibilityV309; }catch(_){ }
  try{ render = renderV309; }catch(_){ }
  try{ window.render = renderV309; }catch(_){ }
  ...
  function refreshHooks(){ applyMapVisibility(); ensureMobileMenuAction(); bindDirectMapaButton(); bindOtherTabs(); patchAppActions(); if(currentTab() === TAB_NAME) renderMapaProductos(); }
  patchAppActions(); bindDirectMapaButton(); bindOtherTabs();
  window.addEventListener('controlevent:app-ready', refreshHooks);
  window.addEventListener('controlevent:runtime-ready', refreshHooks);
  const observer = new MutationObserver(() => refreshHooks());
  try{ observer.observe(document.body || document.documentElement, {childList:true, subtree:true}); }catch(_){ }
  [0, 120, 400, 900, 1800].forEach(ms => setTimeout(refreshHooks, ms));
  window.ControlEventMapaProductos = {version: VERSION, mode: 'mapa-recursos-v309', render: renderMapaProductos, build: buildMapaProductos, show: forceShowMapa, sync: refreshHooks};
})();
