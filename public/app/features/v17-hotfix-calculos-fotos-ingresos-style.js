/* ControlEvent v23_prod - Cálculos por tienda/ticket: fotos con método tipo Documentos.
...
(function(){
  ...
  async function loadServerImages(force=false){
    ...
    fetchPromise=fetch('/api/ticket-images?eventId='+encodeURIComponent(ev),{cache:'no-store'})
      ...
      .catch(err=>{console.warn('[ControlEvent v23_prod] No se pudieron cargar fotos de tickets:',err?.message||err); return serverImages;})
  ...
  async function attachPhoto(label,ev){
    ...
    input.addEventListener('change',async()=>{
      ...
      try{
        ...
        await deleteServer(label).catch(err=>console.warn('[ControlEvent v23_prod] Limpieza previa TK:',err?.message||err));
  ...
  window.ControlEventV17CalculosFotos={install,redraw,attachPhoto,removePhoto,loadServerImages,serverImages,version:'v23_prod_doc_method_fix12_visor_detalle_miniaturas'};
})();
