/* ControlEvent v28.5.2_prod - Cálculos por tienda/ticket: fotos con método tipo Documentos.
...
(function(){
  ...
  async function loadServerImages(force=false){
    ...
    fetchPromise=fetch('/api/ticket-images?eventId='+encodeURIComponent(ev)+'&_='+Date.now(),{cache:'no-store'})
      ...
      .catch(err=>{loadedEvent=ev;mergeKnownImages({});console.warn('[ControlEvent v28.5.2_prod] No se pudieron cargar fotos de tickets:',err?.message||err); return serverImages;})
  ...
  function sortSummaryRowsFix15(rows){
    const mode=st().summaryTiendaSort||'tienda';
    const keyParts=r=>splitParts(rowKey(r)||r?.key||r?.label||'');
    return (Array.isArray(rows)?rows.slice():[]).sort((a,b)=>{
      const pa=keyParts(a), pb=keyParts(b);
      const a0=pa[0]||'', a1=pa.slice(1).join(' | ')||'';
      const b0=pb[0]||'', b1=pb.slice(1).join(' | ')||'';
      if(mode==='ticket') return a1.localeCompare(b1,'es',{numeric:true,sensitivity:'base'}) || a0.localeCompare(b0,'es',{numeric:true,sensitivity:'base'});
      return a0.localeCompare(b0,'es',{numeric:true,sensitivity:'base'}) || a1.localeCompare(b1,'es',{numeric:true,sensitivity:'base'});
    });
  }
  function detailedRowsFromState(){
    ...
    return sortSummaryRowsFix15([...filled.values(),...pending.values()]);
  }
  ...
  function mergeRowsWithFullState(rowsArg){
    ...
    return sortSummaryRowsFix15(out);
  }
  ...
  async function attachPhoto(label,ev){
    ...
    input.addEventListener('change',async()=>{
      ...
      try{
        ...
        await deleteServer(label).catch(err=>console.warn('[ControlEvent v28.5.2_prod] Limpieza previa TK:',err?.message||err));
  ...
  window.ControlEventV17CalculosFotos={install,redraw,attachPhoto,removePhoto,loadServerImages,imageFor,serverImages,version:'v28.5.2_prod_fix937_finalized_view_thumbs_stable'};
})();
