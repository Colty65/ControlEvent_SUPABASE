/* ControlEvent v3_0_exp - Cálculos por tienda/ticket: fotos con método tipo Documentos.
...
(function(){
  ...
  async function loadServerImages(force=false){
    ...
    fetchPromise=fetch('/api/ticket-images?eventId='+encodeURIComponent(ev)+'&_='+Date.now(),{cache:'no-store'})
      ...
      .catch(err=>{loadedEvent=ev;mergeKnownImages({});console.warn('[ControlEvent v3_0_exp] No se pudieron cargar fotos de tickets:',err?.message||err); return serverImages;})
  ...
  async function attachPhoto(label,ev){
    ...
    input.addEventListener('change',async()=>{
      ...
      try{
        ...
        await deleteServer(label).catch(err=>console.warn('[ControlEvent v3_0_exp] Limpieza previa TK:',err?.message||err));
  ...
  window.ControlEventV17CalculosFotos={install,redraw,attachPhoto,removePhoto,loadServerImages,imageFor,serverImages,version:'v3_0_exp_fix937_finalized_view_thumbs_stable'};
})();
