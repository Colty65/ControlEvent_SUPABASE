/* ControlEvent v30.8 - Mapa de recursos
...
(function(){
  ...
  function bindResponsableFilter(options){
    ...
    if(!filter || filter.__ceBoundV308) return;
    filter.__ceBoundV308 = true;
  ...
  function renderTicketLine(group){ return group.ticket || 'Pte. comprar / otros gastos'; }
  ...
  function renderMapaProductos(){
    ...
    normalizeMapaLabelsV308();
  }

  function normalizeMapaLabelsV308(){
  ...
  function forceShowMapa(options = {}){
    ...
    applyMapVisibility(); renderMapaProductos(); normalizeMapaLabelsV308(); closeMobileDrawer();
  ...
  function renderTabVisibilityV308(){ const result = oldRenderTabVisibility ? oldRenderTabVisibility.apply(this, arguments) : undefined; applyMapVisibility(); return result; }
  ...
  function renderV308(){ const result = oldRender ? oldRender.apply(this, arguments) : undefined; applyMapVisibility(); ensureMobileMenuAction(); if(currentTab() === TAB_NAME) forceShowMapa({reason:'render-v308', scroll:false}); return result; }
  ...
  function bindDirectMapaButton(){
    silenceMapTooltip();
    document.querySelectorAll('#tabMapaBtn,.mobile-menu-action[data-target="tabMapaBtn"]').forEach(el => {
      if(el.__ceMapaV308Bound) return; el.__ceMapaV308Bound = true;
  ...
  try{ renderTabVisibility = renderTabVisibilityV308; }catch(_){ }
  try{ window.renderTabVisibility = renderTabVisibilityV308; }catch(_){ }
  try{ render = renderV308; }catch(_){ }
  try{ window.render = renderV308; }catch(_){ }
  ...
  window.ControlEventMapaProductos = {version: VERSION, mode: 'mapa-recursos-v308', render: renderMapaProductos, build: buildMapaProductos, show: forceShowMapa, sync: () => { applyMapVisibility(); ensureMobileMenuAction(); if(currentTab() === TAB_NAME) renderMapaProductos(); }};
})();
