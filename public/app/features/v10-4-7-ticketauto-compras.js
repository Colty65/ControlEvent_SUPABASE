/* ControlEvent v10.4.7_prod - Entrada asistida de COMPRAS mediante foto de ticket e IA.
...
(function(){
  ...
  function enrichRowDefaults(row){
    ...
    if(p){
      // v10.4.7: no sustituye el literal leído por la IA. Mantiene el texto del ticket
  ...
  function aiPendingScore(pname, a){
    ...
    // Si solo coincide un genérico de sabor/formato (LIMON, LATA, BOTELLA...), no es candidato.
    if(!commonDistinct.length) return 0;
    // TE al limón <-> FUZE Tea limón: TE/TEA puede ser distintivo si coincide precio o hay LIMON común.
  ...
  console.info('[CE v10.4.7 Alta IA] producto similar, candidatos Pte.Compra, segmento/destino editables y actualización PRODUCTOS instalada. Prueba: window.__ceOpenTicketAutoV104()');
})();
