/* ControlEvent v45.7 - cierre de permisos por rol y refresco limpio de ventana activa.
   Objetivos:
...
(function(){
  ...
  function call(name, args){ const fn = getGlobalFn(name); if(typeof fn !== 'function') return undefined; try{ return fn.apply(window, args || []); }catch(error){ console.warn('[v45.7] Error en '+name, error); return undefined; } }
  ...
  function clearRoleResiduesForWritable(){
    if(isRO()) return;
    // v45.7: al pasar de RO a GD/RW, limpiar restos de display:none!important y clases de ocultación del rol anterior.
  ...
  async function refreshActive(reason){
    ...
    try{
      ...
      setTimeout(() => {
        ...
        try{ window.ControlEventViewRefreshStabilizer?.hydrate?.(tab, 'v45.7-soft-refresh'); }catch(_){ }
      }, 180);
      softNotice('Datos refrescados.');
      return true;
    }catch(error){
      console.warn('[v45.7] refresco limpio', error);
