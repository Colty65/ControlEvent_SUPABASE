/* ControlEvent v4_1_exp - Documentos del evento (fase 1: menú, gestión y foto DOCXX).
...
(function(){
  ...
  function findDoc(docId){
    ...
    if(recovered){
      ...
      persistDocumentsForEvent(found.eventId).catch(error => console.warn('[ControlEvent v4_1_exp] No se pudo materializar DOC recuperado:', error?.message || error));
      return found;
    }
    return null;
  }
  ...
  function saveNow(options = {}){
    ...
    if(!options.skipDocumentPersist){
      persistDocumentsForEvent().catch(error => console.warn('[ControlEvent v4_1_exp] No se pudieron persistir metadatos DOC:', error?.message || error));
    }
  }
  ...
  async function addDocument(){
    ...
    try{
      ...
      console.warn('[ControlEvent v4_1_exp] No se pudo subir DOC ahora, queda en estado local/protegido:', error?.message || error);
  ...
  async function replaceImage(docId, file){
    ...
    try{
      ...
      console.warn('[ControlEvent v4_1_exp] No se pudo subir DOC ahora, queda local:', error?.message || error);
      status('No se pudo subir al servidor ahora. Queda guardado localmente y se intentará sincronizar al guardar.', 'warn');
    }
    const s = ensureStateShape();
  ...
  async function removeImage(docId){
    ...
    await deleteDocumentImage(doc.eventId, code).catch(error => {
      console.warn('[ControlEvent v4_1_exp] No se pudo eliminar imagen en servidor:', error?.message || error);
      throw error;
    });
    const s = ensureStateShape();
    const key = docKey(doc.eventId, code);
  ...
  async function deleteDoc(docId){
    ...
    if(imageFor(doc)){
      await deleteDocumentImage(doc.eventId, code).catch(error => {
        ...
        console.warn('[ControlEvent v4_1_exp] No se pudo eliminar imagen en servidor; se elimina localmente:', error?.message || error);
      });
    }
    const s = ensureStateShape();
    const key = docKey(doc.eventId, code);
