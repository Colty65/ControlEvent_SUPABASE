/* ControlEvent v10.0_prod - Documentos del evento (fase 1: menú, gestión y foto DOCXX).
...
(function(){
  ...
  async function addDocument(){
    ...
    try{
      ...
      console.warn('[ControlEvent v10.0_prod] No se pudo subir DOC ahora, queda en estado local/protegido:', error?.message || error);
  ...
  async function replaceImage(docId, file){
    ...
    try{
      ...
      console.warn('[ControlEvent v10.0_prod] No se pudo subir DOC ahora, queda local:', error?.message || error);
      status('No se pudo subir al servidor ahora. Queda guardado localmente y se intentará sincronizar al guardar.', 'warn');
    }
    const s = ensureStateShape();
  ...
  async function removeImage(docId){
    ...
    await deleteDocumentImage(doc.eventId, code).catch(error => {
      console.warn('[ControlEvent v10.0_prod] No se pudo eliminar imagen en servidor:', error?.message || error);
      throw error;
    });
    const s = ensureStateShape();
