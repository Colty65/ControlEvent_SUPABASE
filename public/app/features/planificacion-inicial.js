...
(function(){
  ...
  function planBuyAfterDonation(productName, totalNeed, donatedUnits){
    ...
    if(!need) return 0;
    // En packs de 24 se redondea la necesidad total y DESPUÉS se resta lo donado.
    // Ej.: hacen falta 72 latas y hay 18 donadas => A comprar 54, no 72.
    if(isPackRoundedProduct(productName)) return Math.max(0, Math.round((roundPurchaseUnits(productName, need) - donated) * 100) / 100);
    return roundPurchaseUnits(productName, Math.max(0, need - donated));
  }
