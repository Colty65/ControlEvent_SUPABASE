/* ControlEvent v28.1_prod - Planificación inicial con Zuzu.
...
(function(){
  ...
  function mergeMasterRowsForPlanning(data){
    ...
    }catch(error){ console.warn('[ControlEvent v28.1_prod] No se pudo fusionar maestros para planificación:', error); }
  }
  async function ensureMasterRowsForMandatoryIncomes(){
    ...
    try{
      ...
      console.warn('[ControlEvent v28.1_prod] No se pudieron refrescar PERSONAS/TIENDAS para ingresos obligatorios:', error?.message || error);
    }
  }
  ...
  function clearPendingDiagnosticReview(indices, reason){
    ...
    (Array.isArray(indices) ? indices : []).forEach(idx => {
      ...
      if(!row) return;
      if(String(row.__ceDiagnosticStatus || '').toUpperCase() === 'REVISAR' || row.__ceDiagnosticResolved !== true){
        ...
        row.__cePlanReviewClearedFix45 = true;
  ...
  function collectPlanDiagnosticReviewIdsByIndex(idx){
    const n = Number(idx);
    if(!Number.isFinite(n) || n < 0) return [];
    const row = lastProposal[n];
    const ids = [n];
    const pid = String(row?.productId || '').trim();
    const pname = normalizeText(row?.productName || row?.producto || '');
    lastProposal.forEach((r, i) => {
      if(!r || i === n) return;
      if(pid && String(r.productId || '').trim() === pid) ids.push(i);
      else if(pname && normalizeText(r.productName || r.producto || '') === pname) ids.push(i);
    });
    return [...new Set(ids)];
  }
  function collectPlanDiagnosticReviewIdsByKey(key){
    const group = productGroupFromKey(key);
    if(!group) return [];
    return [...new Set([...(group.allIndices || []), ...(group.donationIndices || []), ...(group.purchaseIndices || [])])];
  }
  function markPlanDiagnosticReviewedByIndex(idx, reason){
    return clearPendingDiagnosticReview(collectPlanDiagnosticReviewIdsByIndex(idx), reason || 'usuario-dar-por-revisado');
  }
  function markPlanDiagnosticReviewedByKey(key, reason){
    return clearPendingDiagnosticReview(collectPlanDiagnosticReviewIdsByKey(key), reason || 'usuario-dar-por-revisado');
  }
  function planCssEsc(v){
    const s = String(v || '');
    try{ return (window.CSS && CSS.escape) ? CSS.escape(s) : s.replace(/[^a-zA-Z0-9_-]/g, '\\$&'); }catch(_){ return s.replace(/[^a-zA-Z0-9_-]/g, '\\$&'); }
  }
  function paintPlanReviewedInDom(btn, ids, key){
    const y = window.scrollY, x = window.scrollX;
    const scrollers = Array.from(document.querySelectorAll('.table-scroll,.plan-proposal-list,.ce-plan-scroll')).map(el => [el, el.scrollTop, el.scrollLeft]);
    const nodes = new Set();
    if(btn){
      const a = btn.closest('.plan-resource-edit-row,.plan-product-card,.plan-resource-subrow');
      if(a) nodes.add(a);
    }
    if(key){ document.querySelectorAll(`.plan-resource-edit-row[data-plan-resource-key="${planCssEsc(key)}"]`).forEach(n => nodes.add(n)); }
    (ids || []).forEach(id => {
      document.querySelectorAll(`[data-plan-index="${planCssEsc(id)}"],[data-plan-donation-index="${planCssEsc(id)}"],[data-plan-proposal-index="${planCssEsc(id)}"]`).forEach(n => {
        nodes.add(n);
        const parent = n.closest('.plan-resource-edit-row,.plan-product-card');
        if(parent) nodes.add(parent);
      });
    });
    nodes.forEach(node => {
      node.classList.remove('plan-review-pending','plan-needs-review');
      node.classList.add('plan-reviewed-ok');
      node.querySelectorAll('.plan-needs-review,.plan-review-pending').forEach(el => { el.classList.remove('plan-needs-review','plan-review-pending'); el.classList.add('plan-reviewed-ok'); });
      node.querySelectorAll('.plan-resource-donation-badge-review').forEach(el => {
        el.classList.remove('plan-resource-donation-badge-review');
        el.classList.add('plan-reviewed-badge');
        el.querySelectorAll('small').forEach(s => s.remove());
      });
      node.querySelectorAll('.plan-review-summary').forEach(el => {
        el.classList.add('plan-review-summary-reviewed');
        el.innerHTML = '<span><strong>✓ Revisado:</strong> decisión tomada. Esta ficha ya no queda pendiente.</span><button type="button" class="plan-review-clear-btn reviewed" disabled>Revisado</button>';
      });
      node.querySelectorAll('.plan-review-inline').forEach(el => {
        el.classList.add('plan-review-inline-reviewed');
        el.innerHTML = '<span>✓ Revisado manualmente</span>';
      });
    });
    if(btn){ btn.classList.add('reviewed'); btn.textContent='Revisado'; btn.disabled=true; }
    const remaining = document.querySelectorAll('.plan-resource-edit-row.plan-review-pending,.plan-product-card.plan-needs-review').length;
    document.querySelectorAll('.plan-review-global-note').forEach(el => {
      if(!remaining) el.remove();
      else el.innerHTML = `<strong>⚠ Productos a revisar:</strong> ${remaining} ficha(s) siguen marcadas desde el diagnóstico naranja.`;
    });
    requestAnimationFrame(() => {
      try{ window.scrollTo(x, y); }catch(_){}
      scrollers.forEach(([el, top, left]) => { try{ el.scrollTop = top; el.scrollLeft = left; }catch(_){} });
    });
  }
  function bindPlanDiagnosticReviewButtons(root){
    const scope = root || document;
    if(!window.__cePlanReviewDelegatedFix45){
      window.__cePlanReviewDelegatedFix45 = true;
      document.addEventListener('click', event => {
        ...
        let ids = [];
        if(idx !== null && idx !== '') ids = ids.concat(collectPlanDiagnosticReviewIdsByIndex(idx));
        if(key) ids = ids.concat(collectPlanDiagnosticReviewIdsByKey(key));
        ids = [...new Set(ids)];
        const changed = clearPendingDiagnosticReview(ids, 'usuario-dar-por-revisado');
        if(changed) paintPlanReviewedInDom(btn, ids, key);
      }, true);
    }
  ...
  function renderPlanResourceEditor(proposals){
    ...
    const tr = rows.map(r => {
      ...
      const reviewSummary = needsReview ? `<div class="plan-review-summary" title="${esc((r.reviewLines || []).map(x => x.diagnosticReason || '').filter(Boolean).join(' · ') || 'Caso pendiente de revisar')}"><span><strong>⚠ Atención:</strong> ${esc(r.reviewCount)} caso(s) marcado(s) como <b>REVISAR</b> en el diagnóstico. Esta ficha queda resaltada hasta que tomes una decisión.</span><button type="button" class="plan-review-clear-btn" data-plan-review-clear-key="${esc(r.key)}">Dar por revisado</button></div>` : '';
  ...
  function ceFix40PromptBlocksUltra(info){
    const lines = String(info || '').replace(/\r/g,'').split(/\n/);
    const out = [];
    let active = null;
    const seen = new Set();
    function header(line){
      const h = String(line || '').trim();
      ...
      if(m){
        const kind = normalizeText(m[1] || '');
        const ticketDonacion = /TIENDA/.test(kind) ? 'DONADO TIENDA' : (/OTRO/.test(kind) ? 'DONADO OTROS' : 'DONADO SOCIO');
        const rest = String(m[2] || '').trim();
        const parts = rest.split('/').map(x => x.trim()).filter(Boolean);
        const donorLabel = (parts[0] || (ticketDonacion === 'DONADO TIENDA' ? 'Tienda donante' : ticketDonacion === 'DONADO OTROS' ? 'Donante externo' : 'Donante indicado')).replace(/responsable\s*[:=]?.*$/i,'').trim();
        const rp = parts.find(x => /responsable/i.test(x));
        const responsableLabel = rp ? rp.replace(/responsable\s*[:=]?/i,'').trim() : (document.getElementById('planResponsable')?.selectedOptions?.[0]?.textContent || donorLabel);
        return {ticketDonacion, donorLabel, responsableLabel};
      }
      if(/^\s*(?:[-*•]\s*)?Producto\s+en\s+la\s+pe[nñ]a\b/i.test(h)) return {ticketDonacion:'DONADO SOCIO', donorLabel:'Peña El Arrastre', responsableLabel:document.getElementById('planResponsable')?.selectedOptions?.[0]?.textContent || 'Colty'};
      return null;
    }
    function isStop(line){ return /^\s*(?:Pistas?\s+de\s+compra|Reglas?\s+finales|Criterios?\s+de\s+c[aá]lculo|Datos\s+para\s+el\s+c[aá]lculo|Descripci[oó]n\s+conceptual|Resumen\s+de\s+men[uú]|Personas\s+y\s+consumo|Datos\s+generales|Objetivo\s+del\s+evento)\b/i.test(line || ''); }
    function isProduct(raw){
    ...
    lines.forEach((raw, idx) => {
      ...
      if(!line) return;
      const h = header(line);
      if(h){ active = h; return; }
      if(active && isStop(line)){ active = null; return; }
      if(!active || !isProduct(raw)) return;
      let productText = String(raw).replace(/^\s*[•\-*]\s*/,'');
      productText = productText.includes(':') ? productText.slice(0, productText.lastIndexOf(':')) : productText;
      productText = cleanPromptProductNameHf25(productText);
      const donorRef = ceHf27FindDonorRef(active.ticketDonacion, active.donorLabel || 'Donante indicado');
      ...
      const k = [active.ticketDonacion, donorRef || active.donorLabel, productText, units(raw)].join('|');
      if(seen.has(k)) return;
      seen.add(k);
      out.push({
        line:idx+1,
        raw:line,
        section:'FIX40 parser directo',
        productText,
        unidades:units(raw),
        ticketDonacion:active.ticketDonacion,
  ...
  function ceHf27PromptBlocks(){
    ...
    if(!out.length){
      const fb = ceFix40PromptBlocksUltra(info);
  ...
  function ceHf27RenderDiagnosticsOnly(){
    ...
    try{
      ...
      console.error('[ControlEvent v28.1_prod] Diagnóstico HF28 falló', error);
      box.classList.remove('hidden');
  ...
  function renderProposalRow(p, index){
    ...
    const needsReview = hasPendingDiagnosticReviewRow(p);
    const reviewNote = needsReview ? `<div class="plan-review-inline" title="${esc(p.reason || 'Caso pendiente de revisar')}"><span>⚠ REVISAR · viene del diagnóstico naranja. Revísalo en la propuesta detallada.</span></div>` : '';
    return `
  ...
  function bindAdvancedProposalControls(root){
    ...
    bindPlanDiagnosticReviewButtons(scope);
  ...
  function bindProposalControls(){
    ...
    bindPlanDiagnosticReviewButtons(box);
  ...
  async function applyReplicaToRealEvent(){
    ...
    try{
      ...
      console.error('[ControlEvent v28.1_prod] Error generando evento real desde planificación:', error);
  ...
  function cePlanFix29FilterFixedMenuRows(rows){
    // FIX45_REVISION_SALDO_CAPS: no-op. Se conserva el nombre solo por compatibilidad defensiva.
    return Array.isArray(rows) ? rows : [];
  }


  function cePlanFormatGeminiCost(value){
    const n = Number(value || 0);
    return (Number.isFinite(n) ? n : 0).toLocaleString('es-ES',{minimumFractionDigits:5,maximumFractionDigits:6});
  }
  function renderPlanGeminiCost(debug){
    const c = debug?.costEstimate;
  ...
  function renderPlanDebugTrace(debug){
    ...
    const traceShort = {
      ...
      costEstimate: debug.costEstimate,
  ...
  async function generateProposal(){
    ...
    try{
      ...
      const geminiCostInfo = renderPlanGeminiCost(data.debugPlanificacion);
      const tracePanel = renderPlanDebugTrace(data.debugPlanificacion);
      const topInfo = menuSummary + note + geminiCostInfo + tracePanel;
      ...
      console.warn('[ControlEvent v28.1_prod] Propuesta Zuzu no disponible; se intenta réplica local.', error);
      await ensureMasterRowsForMandatoryIncomes();
  ...
  function showPlanificacion(){
    ...
    try{ initForm(); }catch(error){ console.warn('[ControlEvent v28.1_prod] No se pudo inicializar el formulario de planificación.', error); }
    unlockPlanControls();
  ...
  function ensureHf27DiagnosticButton(){
    ...
    btn.onclick = function(event){
      ...
      try{ ceHf27RenderDiagnosticsOnly(); }
        ...
        console.error('[ControlEvent v28.1_prod] Error abriendo diagnóstico HF27/HF28', error);
        const box = document.getElementById('planificacionResultado');
        if(box){
          box.classList.remove('hidden');
    ...
    if(!document.__ceHf28DiagnosticGlobalClick){
      ...
      document.addEventListener('click', event => {
        ...
        try{ ceHf27RenderDiagnosticsOnly(); }
          ...
          console.error('[ControlEvent v28.1_prod] Error diagnóstico HF28', error);
          const box = document.getElementById('planificacionResultado');
          if(box){
            box.classList.remove('hidden');
            box.innerHTML = '<div class="planificacion-note compact-note warning"><strong>No se pudo abrir el diagnóstico:</strong> '+esc(error?.message || error)+'</div>';
