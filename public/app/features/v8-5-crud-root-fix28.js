/* ControlEvent v4_1_exp FIX30 - CRUD raíz fila-a-fila con baja por firma
...
(function(){
  ...
  function renderIngresosFast(){
    localStore();
    let painted=false;
    try{ if(typeof renderIngresosSummary==='function'){ renderIngresosSummary(); painted=true; } }catch(e){ log('renderIngresosSummary falló', e); }
    try{ if(typeof renderColabs==='function'){ renderColabs(); painted=true; } }catch(e){ log('renderColabs falló', e); }
    if(!painted) renderNow();
  }
  function markCollabRecent(id){
    try{
      const safe=String(id||'').replace(/"/g,'\"');
      document.querySelectorAll('[data-id="'+safe+'"],button[data-action="save-collab"][data-id="'+safe+'"]').forEach(el=>{
        const card=el.closest('.ce-v5011-pending-row,.rowline,.itemcard,.card,tr,.red-row')||el;
        card.classList.add('ce-fix15-recent-collab');
        setTimeout(()=>{ try{ card.classList.remove('ce-fix15-recent-collab'); }catch(_){} }, 4500);
      });
    }catch(_){ }
  }
  ...
  function historicalPersonSnapshot(personaId){
    const person=arr('personas').find(row=>String(row?.id||row?.ID||'')===String(personaId||''))||{};
    return {
      personaNombreSnapshot:String(person.nombre||person.Nombre||personaId||'').trim(),
      personaRangoSnapshot:String(person.rango||person.Rango||'SOCIO').trim().toUpperCase()||'SOCIO'
    };
  }
  function rowEventoFromForm(id){ const old=eventById(id)||{}; return {...old,id:String(id),titulo:val('edit-evento-titulo',id,old.titulo||'').trim(),nombreHablado:val('edit-evento-nombrehablado',id,old.nombreHablado||old.nombre_hablado||'').trim(),precio:money(val('edit-evento-precio',id,old.precio||0)),fechaIni:val('edit-evento-fechaini',id,old.fechaIni||'').trim(),fechaFin:val('edit-evento-fechafin',id,old.fechaFin||'').trim(),descripcion:val('edit-evento-descripcion',id,old.descripcion||'').trim(),situacion:val('edit-evento-situacion',id,old.situacion||'En curso')}; }
  function rowCollabFromForm(id,scope){
    ...
    if(String(row.personaId)!==String(old.personaId)||!old.personaNombreSnapshot||!old.personaRangoSnapshot) Object.assign(row,historicalPersonSnapshot(row.personaId));
  ...
  function splitAliases(v){return String(v||'').split(/[;,\n]+/).map(x=>x.trim()).filter(Boolean).filter((x,i,a)=>a.findIndex(y=>y.toLocaleLowerCase('es')===x.toLocaleLowerCase('es'))===i);}
  function rowPersonaFromForm(id){ const old=arr('personas').find(x=>String(x.id)===String(id))||{}; return {...old,id:String(id),nombre:val('edit-persona-nombre',id,old.nombre||'').trim(),nombreAmigo:val('edit-persona-nombreamigo',id,old.nombreAmigo||'').trim(),aliases:splitAliases(val('edit-persona-aliases',id,(old.aliases||[]).join(', '))),rango:val('edit-persona-rango',id,old.rango||'SOCIO')}; }
  ...
  async function addEvento(){ if(!requireGD()) return; const titulo=elVal('newEventoTitulo').trim(); if(!titulo) return; const row={id:uid(),titulo,nombreHablado:elVal('newEventoNombreHablado').trim(),precio:money(elVal('newEventoPrecio','0')),fechaIni:elVal('newEventoFechaIni').trim(),fechaFin:elVal('newEventoFechaFin').trim(),situacion:elVal('newEventoSituacion','En curso'),descripcion:elVal('newEventoDescripcion').trim()}; await upsert('eventos',row); replaceLocal('eventos',row); getState().selectedEventId=row.id; clear(['newEventoTitulo','newEventoNombreHablado','newEventoFechaIni','newEventoFechaFin','newEventoDescripcion']); setVal('newEventoPrecio','0.00'); await refreshFromDb('eventos'); }
  async function addPersona(){ if(!requireWrite())return; const nombre=elVal('newPersonaNombre').trim(); if(!nombre)return; const row={id:uid(),nombre,nombreAmigo:elVal('newPersonaNombreAmigo').trim(),aliases:splitAliases(elVal('newPersonaAliases')),rango:elVal('newPersonaRango','SOCIO')}; await upsert('personas',row); replaceLocal('personas',row); setVal('newPersonaNombre',''); setVal('newPersonaNombreAmigo',''); setVal('newPersonaAliases',''); await refreshFromDb('personas'); }
