/* ControlEvent v10_5_prod - guarda de recuperacion de INGRESOS sin escrituras destructivas. */
(function(){
  'use strict';
  ...
  function call(name, ...args){
    ...
    try{ return fn(...args); }catch(error){ console.warn('[ControlEvent v10_5_prod]', name, error); return undefined; }
  }
  ...
  function mergeFromServer(server, reason){
    ...
    if(changed){
      console.warn('[ControlEvent v10_5_prod] Recuperados datos desde /api/state:', reason);
    }
    return changed;
  }
  async function recover(reason){
    ...
    try{
      ...
      console.warn('[ControlEvent v10_5_prod] No se pudo recuperar /api/state.', error);
      return false;
    }finally{
      recoverBusy = false;
    }
    ...
    const wrapped = function(input, init){
      ...
      if(method === 'PUT' && /\/api\/state(?:$|\?)/.test(url) && init && typeof init.body === 'string'){
        try{
          ...
          if(blocked){
            console.warn('[ControlEvent v10_5_prod] Bloqueado guardado de colecciones vacias en /api/state.');
  ...
  function patchCollabsForEvent(){
    ...
    const wrapped = function(){
      let rows = [];
      try{ rows = old.apply(this, arguments) || []; }catch(error){ console.warn('[ControlEvent v10_5_prod] collabsForEvent original fallo.', error); }
