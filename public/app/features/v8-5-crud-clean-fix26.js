/* ControlEvent v27_prod_1.0 FIX26 - CRUD limpio fila-a-fila
...
(function(){
  'use strict';
  const TAG='__ceV85CrudCleanFix26';
  if(window[TAG]) return; window[TAG]=true;

  const WRITE_SCOPE='row-crud-v8-5-fix26';
  ...
  function log(){ try{ console.warn('[FIX26 CRUD LIMPIO]', ...arguments); }catch(_){} }
  ...
  async function handleMaintenance(action,id){
    ...
    if(action==='delete-compra'||action==='delete-donacion'){ await del('compras',id); removeLocal('compras',id); await refreshFromDb(); return; }
    ...
    if(action==='delete-collab'){ await del('colaboradores',id); removeLocal('colaboradores',id); await refreshFromDb(); return; }
  ...
  document.addEventListener('click', function(e){
    ...
    (async()=>{
      ...
      }catch(err){ console.error('[FIX26 CRUD LIMPIO]',err); alert(err.message||String(err)); }
    })();
    return false;
  }, true);

  function localOnlySave(){ localStore(); return Promise.resolve({ok:true, localOnly:true, fix:'FIX26'}); }
    ...
    const wrapped=function(input, init){
      ...
      if(method==='PUT' && /\/api\/state(?:$|\?)/i.test(url)){
        ...
        if(!restore){
          ...
          return Promise.resolve(new Response(JSON.stringify({ok:false,blocked:true,error:'FIX26: PUT /api/state bloqueado; use CRUD fila-a-fila'}),{status:409,headers:{'Content-Type':'application/json'}}));
        }
      }
  ...
  window.ControlEventCrudCleanFix26={active:true, version:'v8.5_prod_fix26', scope:WRITE_SCOPE};
  log('Activo: CRUD limpio fila-a-fila. Sin guardado global.');
})();
