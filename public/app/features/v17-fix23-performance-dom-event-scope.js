/* ControlEvent v17_prod FIX23 - rendimiento real:
...
(function(){
  ...
  function compactInfo(r, kind){
    ...
    const don = txt(r.donorRef || r.donante || '');
    const amount = kind === 'donacion' ? r.valor : r.importe;
    return {producto, tienda, tk, responsable, don, amount};
  }
    ...
    return `<div class="itemcard ce-fix23-compact-row ${pending?'red-row':''}" id="compraRow_${esc(r.id)}" data-ce-fix23-kind="${kind}" data-id="${esc(r.id)}">
      <div class="ce-fix23-compact-main">
        <div class="ce-fix23-compact-text">
          ...
          <div class="ce-fix23-meta">${esc(i.tienda)} · ${esc(i.tk)} · ${esc(i.responsable)}${i.don ? ' · '+esc(i.don) : ''}</div>
        </div>
  ...
  window.ControlEventFix23Performance = {version:'v17_prod_fix23_performance_dom_event_scope', run, renderCompras:()=>renderCompraKind('compra'), renderDonaciones:()=>renderCompraKind('donacion')};
})();
