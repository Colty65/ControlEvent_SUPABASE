/* ControlEvent v26_prod OPT2D - Gráficas sin avisos negros y con repintado estabilizado.
...
(function(){
  ...
  function commitStableHtml(wrap, html, delay){
    ...
    pendingTimer = setTimeout(() => {
      ...
      try{
        ...
        cacheCurrentChart();
      }catch(err){ console.warn('[v26_prod_opt_2c] commit gráficas', err); }
    }, Number(delay || 180));
  }
  ...
  function runGraphRender(base, ctx, options){
    ...
    renderTimer = setTimeout(() => {
      ...
      try{
        ...
        base.call(ctx || window.ControlEventV434 || window, Object.assign({}, options || {}, {force:true, reason:'v26_prod_opt_2c'}));
      ...
      }catch(err){ console.warn('[v26_prod_opt_2c] render gráficas', err); }
      finally{
  ...
  async function hydrateReceipts(force){
    ...
    try{ await race(window.ControlEventV469?.hydrateEventReceipts?.(force !== false), 2100); }catch(err){ console.warn('[v26_prod_opt_2c] hidratación justificantes', err); }
