/* ControlEvent v6.1_prod - parche quirurgico desde v5.1.0: version, cierre foto, GRAFICAS e iPhone sin bucles periodicos.
...
(function(){
  ...
  function injectStyle(){
    if(document.getElementById('ceV61FinalStyle')) return;
    ...
    style.textContent = `
      ...
      #ceV502ReceiptModal [data-close],#ceV468ReceiptModal [data-close],#ceV465ReceiptModal [data-close],.ce-v468-modal [data-close],.ce-v465-modal [data-close]{background:#fff!important;color:#000!important;border:1px solid #111827!important;z-index:10000090!important;pointer-events:auto!important;touch-action:manipulation!important;position:fixed!important;right:calc(14px + env(safe-area-inset-right))!important;bottom:calc(18px + env(safe-area-inset-bottom))!important;top:auto!important;left:auto!important;}
  ...
  // v6.1: protección final sin bucles periódicos: impedir que un cierre de foto pulse Salir y fijar versión.
  function installV61CloseAndLogoutGuard(){
    if(window.__ceV61CloseAndLogoutGuard) return;
    window.__ceV61CloseAndLogoutGuard = true;
    document.addEventListener('click', function(ev){
      const close = ev.target?.closest?.('.ce-v310-photo-close,.ce-v401-pc-modal-close,.ce-v40-modal-close,[data-close]');
      if(close){ try{ window.__ceBlockLogoutUntil = Date.now() + 850; }catch(_){} }
    }, true);
    document.addEventListener('click', function(ev){
      const target = ev.target?.closest?.('#btnLogout,#logoutBtn,#btnSalir,button[onclick*="logout"],button[onclick*="salir"],button[onclick*="Salir"]');
      if(target && Date.now() < (window.__ceBlockLogoutUntil || 0)){
        try{ ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation(); }catch(_){}
        return false;
      }
    }, true);
  }
  installV61CloseAndLogoutGuard();

  window.ControlEventV61ProdFinalFix = {version:VERSION, versionFile:VERSION_FILE, install, normalizeDownloadName, patchExcelJS};
})();
