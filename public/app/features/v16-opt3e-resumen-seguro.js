/* ControlEvent v16_prod OPT3E - Resumen seguro, sin tocar entrada/login ni carga general.
   Rebase sobre OPT2J validada. No parchea login, fetch, render global ni /api/state.
   Solo mejora el bloque #summaryTiendaTicket cuando Resumen ya existe y está visible. */
(function(){
  'use strict';
  if(window.__ceV16Opt3EResumenSeguro) return;
  window.__ceV16Opt3EResumenSeguro = true;

  const VERSION = 'v16_opt_3e';
  ...
  const metrics = window.ControlEventOpt3E = {
    version: VERSION,
  ...
  function schedule(reason, delay){
    ...
    clearTimeout(timer);
    timer = setTimeout(() => { try{ renderNow(false); }catch(err){ console.warn('[v16_opt_3e]', reason, err); } }, delay == null ? 80 : delay);
  }
  ...
  function ensureObserver(){
    ...
    observer = new MutationObserver(() => {
      ...
      const ok = !!root.querySelector('.ce-opt3e-row[data-ce-tip-v21]');
      if(!ok){ metrics.observerFixes++; schedule('mutation', 90); }
    });
  ...
  function install(){
    ...
    ensureObserver();
    if(isResumenVisible()) schedule('install', 60);
  }
  ...
  document.addEventListener('click', ev => {
    ...
    const rowEl = ev.target?.closest?.('#summaryTiendaTicket .ce-opt3e-row');
    if(rowEl && !ev.target.closest('button,input,select,a,img')){
      const row = rowEl.__ceOpt3eRow;
  ...
  window.ControlEventOpt3E = Object.assign(metrics, {install, renderNow, rowsForSummary});
  injectStyle();
  ['controlevent:app-ready','controlevent:runtime-ready','controlevent:data-loaded','controlevent:event-ready','controlevent:opt1-event-stable','controlevent:event-changed'].forEach(e => window.addEventListener(e, () => setTimeout(install, 120), true));
  document.addEventListener('click', () => setTimeout(install, 140), true);
