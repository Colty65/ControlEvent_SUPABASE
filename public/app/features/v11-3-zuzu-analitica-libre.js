/* ControlEvent v4_1_exp - Zuzu / Analítica libre de explotación del evento.
...
(function(){
  ...
  function ensureZuzuUserPreface(value){
    ...
    // v4_1_exp: la respuesta empieza directamente. Eliminamos también el saludo
  ...
  function responsePdfTitle(data, prompt){
    ...
    return 'ControlEvent_v4_1_exp-responde_Zuzu_a_'+subject+'-'+stamp+'.pdf';
  }
  ...
  function modalHtml(){
    return '<div class="ce-ai-overlay" id="ceGeminiLibreOverlay" role="dialog" aria-modal="true">'+
      '<div class="ce-ai-modal">'+
        '<div class="ce-ai-head"><h2>✨ Soy Zuzu, pregúntame lo que quieras...</h2><span class="ce-ai-version-badge">v4_1_exp</span><div id="ceAiEventTitle">'+eventTitleHtml()+'</div><div class="spacer"></div><button type="button" class="ce-ai-close" id="ceAiClose">Cerrar</button></div>'+
  ...
  function conversationTrailBodyHtml(){
    ...
    return hist.map(function(turn,idx){
      ...
      return '<div class="ce-ai-conversation-rail-turn"><div class="ce-ai-conversation-rail-turnno">Turno '+zuzuTurnNumber(turn,base+idx+1)+'</div><div class="ce-ai-conversation-rail-line"><b>P:</b> '+esc(q||'—')+'</div><div class="ce-ai-conversation-rail-line"><b>R:</b> '+esc(a||'—')+'</div></div>';
    }).join('');
  }
  function updateConversationTrail(){
    ...
    if(count) count.textContent=String(zuzuConversationTurnCount());
  ...
  function conversationResumeHtml(){
    ...
    return '<div class="ce-ai-card ce-ai-conversation-card ce-ai-conversation-screen-only"><h3>💬 Conversación abierta · '+zuzuConversationTurnCount()+' turnos</h3><div class="ce-ai-answer">El rastro resumido del hilo está visible a la derecha del prompt. Puedes continuar preguntando o pulsar 🧹 para empezar de cero.</div></div>';
  }
  ...
  function restoreConversationScreen(){
    ...
    syncZuzuConversationFromServer(ZUZU_LOCAL_HISTORY_LIMIT).then(function(){updateConversationMode();updateConversationTrail();var rr=$('ceAiResult');if(rr&&rr.getAttribute('data-ce-resume-only')==='1')rr.innerHTML=conversationResumeHtml();}).catch(function(){});
  }
  ...
  async function printSelectedZuzuTurns(selectedIndexes,includeTrace){
    ...
    var title=selected.length===1?responsePdfTitle({__prompt:selected[0].turn.user},selected[0].turn.user):('ControlEvent_v4_1_exp-conversacion_Zuzu-'+dateStamp(now)+'.pdf');
  ...
  async function openZuzuPdfPicker(){
    ...
    var rows=hist.map(function(turn,idx){
      ...
      return '<label class="ce-ai-pdf-turn"><input type="checkbox" class="ce-ai-pdf-turn-check" value="'+idx+'" checked><span><div class="ce-ai-pdf-turn-title">Turno '+zuzuTurnNumber(turn,idx+1)+'</div><div class="ce-ai-pdf-turn-line"><b>P:</b> '+esc(q||'—')+'</div><div class="ce-ai-pdf-turn-line"><b>R:</b> '+esc(a||'—')+'</div><div class="ce-ai-pdf-turn-badges">'+pdfPickerTurnBadges(turn)+'</div></span></label>';
    }).join('');
  ...
  function clearZuzu(ev){
    ...
    var p=$('ceAiPrompt'); if(p){ p.value=''; p.textContent=''; savePromptDraftValue(''); autoSizePrompt(p); }
  ...
  function zuzuPromptDraftKey(){return zuzuStorageKey('prompt_draft_p16');}
  function zuzuPromptLastKey(){return zuzuStorageKey('prompt_last_p16');}
  function savePromptDraftValue(value){try{var v=String(value==null?'':value);if(v)sessionStorage.setItem(zuzuPromptDraftKey(),v);else sessionStorage.removeItem(zuzuPromptDraftKey());}catch(_){}}
  function saveLastPromptValue(value){try{var v=String(value==null?'':value);if(v)sessionStorage.setItem(zuzuPromptLastKey(),v);}catch(_){}}
  function loadPromptDraft(){try{return String(sessionStorage.getItem(zuzuPromptDraftKey())||'');}catch(_){return '';}}
  function loadLastPrompt(){try{return String(sessionStorage.getItem(zuzuPromptLastKey())||'');}catch(_){return '';}}
  function loadPromptBackup(){return loadPromptDraft()||loadLastPrompt();}
  function autoSizePrompt(p){if(!p)return;try{p.style.height='auto';p.style.height=Math.min(220,Math.max(82,p.scrollHeight||96))+'px';}catch(_){}}
  function restorePromptBackup(){var p=$('ceAiPrompt'),v=loadLastPrompt()||loadPromptDraft();if(!p||!v){setStatus('No hay un prompt guardado para recuperar.','');return;}p.value=v;savePromptDraftValue(v);p.dispatchEvent(new Event('input',{bubbles:true}));try{p.focus();p.setSelectionRange(p.value.length,p.value.length);}catch(_){}autoSizePrompt(p);setStatus('Prompt recuperado. No se perderá hasta que lo borres tú.','ok');}
  function bindPromptDraftPersistence(){var p=$('ceAiPrompt');if(!p||p.dataset.ceP16Draft==='1')return;p.dataset.ceP16Draft='1';var saved=loadPromptBackup();if(saved&&!p.value)p.value=saved;autoSizePrompt(p);p.addEventListener('input',function(ev){if(ev&&ev.isTrusted===false){autoSizePrompt(p);return;}savePromptDraftValue(p.value);autoSizePrompt(p);},{passive:true});p.addEventListener('paste',function(ev){setTimeout(function(){if(!ev||ev.isTrusted!==false)savePromptDraftValue(p.value);autoSizePrompt(p);},0);},{passive:true});}
  function installPromptEventShield(){
    var p=$('ceAiPrompt');if(!p||p.dataset.cePromptShieldP16==='1')return;p.dataset.cePromptShieldP16='1';
    // P1.6: el escudo vive EN el textarea. No bloquea beforeinput/input en window capture,
    // porque eso impedía al protector de borrador enterarse de que el usuario estaba escribiendo.
    var block=function(ev){if(ev.type==='keydown'&&ev.key==='Escape'){closeModal();try{ev.preventDefault();ev.stopPropagation();}catch(_){}return false;}try{ev.stopPropagation();}catch(_){}return undefined;};
    ['keydown','keyup','keypress','beforeinput','input','paste','compositionstart','compositionupdate','compositionend'].forEach(function(type){p.addEventListener(type,block,{capture:true,passive:false});});
  }
  ...
  function renderZuzuVNextMode(){ var b=$('ceAiVNextMode'); if(!b)return; var on=isZuzuVNextMode(); b.classList.toggle('is-active',on); b.setAttribute('aria-pressed',on?'true':'false'); b.textContent=on?'🧪 VNext ACTIVO':'🧪 VNext'; var strip=$('ceAiConversationMode'); if(strip&&on){strip.className='ce-ai-mode-strip is-conversation';strip.innerHTML='<span class="ce-ai-mode-pill">🧪 VNext P1.24</span><span class="ce-ai-mode-help">Dialogue State Authority + instrumentación de latencia real Zuzu/simulador, presentación materializada visible para ITV y búsqueda histórica paralelizada. NHC.</span>';}}
  ...
  function openModal(){
    ...
    installPromptEventShield();bindPromptDraftPersistence();
    ...
    $('ceAiClear').onclick=function(ev){ clearZuzu(ev);savePromptDraftValue(''); };
    var restoreBtn=$('ceAiRestorePrompt');if(restoreBtn)restoreBtn.onclick=function(ev){if(ev){ev.preventDefault();ev.stopPropagation();}restorePromptBackup();};
  ...
  var ZUZU_WAIT_STATUS='Zuzu está trabajando…';
  function startZuzuThinking(){
    clearZuzuThinkingTimer();setZuzuButtonHeartbeat(true);window.__ceZuzuThinkingState={active:true};setStatus(ZUZU_WAIT_STATUS,'');
  }
  ...
  function zuzuConversationTurnCount(){
    var all=loadZuzuConversation(); if(!all.length)return 0;
    var maxSeq=0; for(var i=0;i<all.length;i++){var n=Number(all[i]&&((all[i].turnSeq!=null?all[i].turnSeq:all[i].seq)));if(Number.isFinite(n)&&n>maxSeq)maxSeq=n;}
    return Math.max(all.length,Math.trunc(maxSeq||0));
  }
  async function syncZuzuConversationFromServer(limit){
    var cid=loadZuzuServerConversationId(); if(!cid)return loadZuzuConversation();
    try{
  ...
  function loadZuzuServerConversationId(){ try{return String(sessionStorage.getItem(zuzuServerConversationIdKey())||'').trim();}catch(_){return '';} }
  function saveZuzuServerConversationId(value){ try{var v=String(value||'').trim();if(v)sessionStorage.setItem(zuzuServerConversationIdKey(),v);else sessionStorage.removeItem(zuzuServerConversationIdKey());localStorage.removeItem(zuzuServerConversationIdKey());}catch(_){ } }
  async function restoreServerConversationIfNeeded(){
    // RAW3: si el índice local está vacío, eso significa conversación NUEVA.
    // No se resucita silenciosamente un ledger anterior desde un id persistido.
  ...
  async function runAi(){
    ...
    // P1.6: snapshot doble antes de tocar voz/red/estado. Un prompt largo nunca depende del textarea vivo.
    savePromptDraftValue(prompt);saveLastPromptValue(prompt);window.__ceZuzuLastPromptSnapshot=prompt;
    ...
    // v4_1_exp: la conversación nativa de Gemini es el hilo principal. El evento de pantalla
    ...
    try{
      ...
      data.spokenAnswer=String(data.spokenAnswer||(data.meta&&data.meta.spokenAnswer)||data.answer||''); // RAW4: segunda redacción Gemini, específica para voz.
      ...
      // RAW4: incluso en conversación oral se mantiene la presentación escrita completa; la voz usa spokenAnswer aparte.
    ...
    saveZuzuServerConversationId('');
      ...
      try{
        document.dispatchEvent(new CustomEvent('ce:zuzu-response-rendered',{detail:{turnId:turnId,voiceConversation:voiceConversation,answer:String(data.answer||''),spokenAnswer:String(data.spokenAnswer||data.answer||''),title:String(data.title||'')}}));
  ...
  function traceHtml(data){
    // v4_1_exp: la traza está SIEMPRE disponible en pantalla, plegada por defecto.
  ...
  function detailedLineChartHtml(ch, labels, values){
    ...
    // v4_1_exp: globos completos y legibles. Se reparte la serie solo cuando hace falta,
  ...
  function tableHtml(tb){
    ...
    function colClass(name){
      ...
      if(/importe|precio|valor|coste|saldo|total/.test(n))return 'ce-col-money';
      if(/unidades|cantidad|registros|personas|asistentes|numero|nº/.test(n))return 'ce-col-number';
      if(/segmento|destino|tipo|ticket|tkxx|estado|fecha/.test(n))return 'ce-col-short';
      return '';
    }
