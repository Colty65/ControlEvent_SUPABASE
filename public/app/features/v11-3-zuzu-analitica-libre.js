...
(function(){
        ...
        '<div class="ce-ai-prompt">'+
          '<div class="ce-ai-prompt-grid">'+
            ...
            '<aside class="ce-ai-conversation-rail" aria-label="Rastro de la conversación">'+
              '<div class="ce-ai-conversation-rail-head"><span>💬 Rastro de conversación</span><span class="ce-ai-rail-typing" id="ceAiRailTyping" aria-label="Indicador animado de Zuzu"><i></i><i></i><i></i><i></i><i></i></span><span class="ce-ai-conversation-rail-count" id="ceAiConversationRailCount">0</span></div>'+
  ...
  function setZuzuRailTyping(on){ var el=$('ceAiRailTyping'); if(el)el.classList.toggle('is-on',!!on); }
  ...
  function traceHtml(data){
    ...
    if(usage){
      ...
      usageBlock='<div class="ce-ai-trace-item"><div class="ce-ai-trace-status '+(calls?'OK':'INFO')+'">'+(calls?'OK':'INFO')+'</div><div><strong>Consumo Gemini</strong></div><div class="ce-ai-trace-detail">'+
        ...
        (calls?'':' Ruta determinista de ControlEvent; sin consumo Gemini en este turno.')+
      '</div></div>';
    }
    ...
    if(shadowPending){
      shadowBlock='<div class="ce-ai-trace-item"><div class="ce-ai-trace-status INFO">DIAG</div><div><strong>Router Gemini · diagnóstico en sombra</strong></div><div class="ce-ai-trace-detail">Clasificación pendiente. Es una auditoría de enrutamiento; la ruta que realmente manda aparece como ROUTER CE ACTIVO cuando ControlEvent resuelve el turno directamente.</div></div>';
    }else if(shadow){
      if(shadow.ok&&shadow.decision){
        ...
        shadowBlock='<div class="ce-ai-trace-item"><div class="ce-ai-trace-status OK">DIAG</div><div><strong>Router Gemini · diagnóstico '+esc(d.mode||'')+'</strong></div><div class="ce-ai-trace-detail">TUBERÍA='+esc(d.route||'UNKNOWN')+esc(subject)+esc(event)+' · operación='+esc(d.operation||'OTHER')+' · confianza='+esc((conf*100).toFixed(0))+'%. '+esc(d.reason||'')+' <strong>Solo auditoría: no manda en la respuesta.</strong></div>'+guard+'</div>';
      }else{
        shadowBlock='<div class="ce-ai-trace-item"><div class="ce-ai-trace-status WARN">DIAG</div><div><strong>Router Gemini · diagnóstico no disponible</strong></div><div class="ce-ai-trace-detail">'+esc(shadow.error||'No se pudo clasificar este turno.')+' La ruta activa de ControlEvent no se ha visto afectada.</div></div>';
      }
    }
