/* ControlEvent v28.5_prod - guardado inmediato, buscadores en compras/donaciones y nuevas gráficas. */
(function(){
  'use strict';
  ...
  function injectStyle(){
    ...
    style.textContent = `
      ...
      .ce-v413-pie-svg{width:100%;max-width:190px;aspect-ratio:1;margin:0 auto 8px;display:block;touch-action:manipulation;}
      .ce-v413-pie-slice{cursor:pointer;stroke:#fff;stroke-width:.8px;transition:filter .12s ease;touch-action:manipulation;}
      ...
      .ce-v413-legend-row{display:flex;gap:6px;align-items:flex-start;cursor:pointer;border-radius:8px;padding:6px 7px;min-height:34px;touch-action:manipulation;}
  ...
  function pieDisplayAngles(nonzero, denom){
    const raw = nonzero.map(it => Math.abs(Number(it.value||0)) / denom * 360);
    if(raw.length <= 1) return raw;
    const minAngle = raw.length <= 6 ? 20 : Math.max(10, 120 / raw.length);
    const fixed = raw.map(a => a > 0 && a < minAngle ? minAngle : a);
    const totalFixed = fixed.reduce((a,b)=>a+b,0);
    if(totalFixed <= 360.01) return fixed;
    const smallExtra = fixed.reduce((a,b,i)=>a + Math.max(0, b - raw[i]), 0);
    const bigTotal = raw.reduce((a,b)=>a + (b >= minAngle ? b : 0), 0);
    return raw.map((a,i) => {
      if(a > 0 && a < minAngle) return fixed[i];
      if(bigTotal <= 0) return 360 / raw.length;
      return Math.max(minAngle, a - smallExtra * (a / bigTotal));
    });
  }
  function pieCard(title, total, items){
    ...
    const displayAngles = pieDisplayAngles(nonzero, denom);
    const slices = nonzero.length ? nonzero.map((it, idx) => {
      const start = angle;
      const inc = displayAngles[idx] || (Math.abs(Number(it.value||0)) / denom) * 360;
      angle += inc;
