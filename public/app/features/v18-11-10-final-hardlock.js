/* ControlEvent v18.11.10_prod - hardlock final de versión, trazabilidad y herramientas laterales. */
(function(){
  'use strict';
  ...
  function installStyle(){
    ...
    st.textContent = [
      '#ceVersionProof{position:fixed;top:8px;right:158px;z-index:100020;background:#0f172a;color:#fff;border:1px solid #38bdf8;border-radius:999px;padding:4px 10px;font-size:11px;font-weight:950;box-shadow:0 6px 18px rgba(15,23,42,.22);pointer-events:none}',
  ...
  function setGlobals(){
    safe(function(){
      ...
      window.ControlEventVersion={label:VERSION_LABEL,version:VERSION_TEXT,versionFile:VERSION_FILE,build:BUILD_ID,zip:ZIP_NAME,source:'v18-11-10-final-hardlock.js'};
  ...
  function apply(){ installStyle(); setGlobals(); if(document.body){ installVersionProof(); moveTools(); } cleanOldVisibleVersions(); patchZuzuModal(); }
  window.ControlEventVersionCheck=function(){
    var front={label:VERSION_LABEL,version:VERSION_TEXT,versionFile:VERSION_FILE,build:BUILD_ID,zip:ZIP_NAME,source:'v18-11-10-final-hardlock.js'};
