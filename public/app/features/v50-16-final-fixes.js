...
(function(){
  ...
  function callRenderPieces(tab){
    ...
    try{ window.ControlEventModules?.activate?.(active, {reason:'v50.18-refresh-active'}); }catch(_){ }
    ...
    if(active === 'graficas'){
      try{ window.renderGraficas?.({force:true, reason:'v50.18-refresh-active'}); }catch(_){ try{ window.renderGraficas?.(); }catch(__){ } }
    }
    if(active === 'ingresos'){
  ...
  async function refreshInPlace(ev){
    ...
    }catch(error){ console.warn('[v50.18] No se pudo refrescar estado en sitio', error); }
    clearAwaitingVisualState();
  ...
  function patchV5015ForceLogic(){
    // v50.18 dejaba una marca de "forzar selector" que podia seguir activa tras escoger evento.
