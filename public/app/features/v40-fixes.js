/* ControlEvent v41.2 - Ajustes finales
...
(function(){
  ...
  async function exportBackupV40(){
    ...
    if(!scope) return;
    // v41.2: se vuelve al backup generado por servidor. Evita el RangeError
  ...
  function exportInfoEventoLegacy(){
    ...
    try{
      if(window.ControlEventExcel?.run) return window.ControlEventExcel.run('exportExcel', {source:'v41.2-legacy-infoevento'});
    }catch(err){ return Promise.reject(err); }
  ...
  function installExcelGuards(){
    ...
    const backup = function(){ return exportBackupV40().catch(err => { console.error('[v41.2] BACKUP', err); alert(`No se pudo descargar la descarga de datos.\n\n${err?.name || 'Error'}: ${err?.message || err}`); }); };
