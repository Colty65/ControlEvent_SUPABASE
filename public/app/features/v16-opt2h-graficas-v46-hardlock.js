/* ControlEvent v28.1_prod OPT2J - bloqueo duro de gráficas antiguas + anti segundo repintado.
...
(function(){
  'use strict';
  const VERSION = 'v28.1_prod_opt_2j';
  const now = () => Date.now();
  ...
  function runFinal(reason){
    ...
    metrics.finalRuns++;
    try{ fn.call(window.ControlEventV462 || window.ControlEventV461 || window.ControlEventV460 || window, {force:true, reason:'v28.1_prod_opt_2h_' + (reason || 'final')}); }
    catch(err){ console.warn('[v28.1_prod_opt_2h] render V46', err); }
    const html = rawGet(w) || '';
  ...
  function patchWrap(){
    ...
    }catch(err){ console.warn('[v28.1_prod_opt_2h] patch wrap', err); }
  }
