/* ControlEvent v2.0_exp OPT2J - bloqueo duro de gráficas antiguas + anti segundo repintado.
...
(function(){
  ...
  function runFinal(reason){
    ...
    metrics.finalRuns++;
    try{ fn.call(window.ControlEventV462 || window.ControlEventV461 || window.ControlEventV460 || window, {force:true, reason:'v2.0_exp_opt_2h_' + (reason || 'final')}); }
    catch(err){ console.warn('[v2.0_exp_opt_2h] render V46', err); }
    const html = rawGet(w) || '';
  ...
  function patchWrap(){
    ...
    }catch(err){ console.warn('[v2.0_exp_opt_2h] patch wrap', err); }
  }
