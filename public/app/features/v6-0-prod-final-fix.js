/* ControlEvent v6.0_prod - cierre final de versión, Excel y visores sin bucles periódicos.
...
(function(){
  ...
  const VERSION_FILE = 'ControlEvent_v6_0_prod';
  const INSTALLED = '__ceV60ProdFinalFix';
  ...
  function patchAnchorDownloads(){
    ...
    if(!proto || !nativeClick || proto.__ceV60DownloadPatched) return;
    proto.__ceV60DownloadPatched = true;
  ...
  function scrubWorkbook(wb){
    if(!wb || wb.__ceV60Scrubbed) return wb;
    wb.__ceV60Scrubbed = true;
  ...
  function patchWorkbookInstance(wb){
    if(!wb || wb.__ceV60WritePatched || !wb.xlsx || typeof wb.xlsx.writeBuffer !== 'function') return wb;
    wb.__ceV60WritePatched = true;
  ...
  function patchExcelJS(){
    ...
    if(!X || !X.Workbook || X.__ceV60WorkbookPatched) return false;
    ...
    X.__ceV60WorkbookPatched = true;
    return true;
  }
  function wrapEnsureExcelJS(){
    const fn = window.ensureExcelJS;
    if(typeof fn !== 'function' || fn.__ceV60EnsureWrapped) return;
    const wrapped = async function(){
      const res = await fn.apply(this, arguments);
      patchExcelJS();
      return res;
    };
    wrapped.__ceV60EnsureWrapped = true;
  ...
  function injectStyle(){
    if(document.getElementById('ceV60FinalStyle')) return;
    const style = document.createElement('style');
    style.id = 'ceV60FinalStyle';
    style.textContent = `
      ...
      #ceV401PcPhotoModal .ce-v401-pc-modal-close,#ceV40TicketPhotoModal .ce-v40-modal-close,#ceV310PhotoViewer .ce-v310-photo-close,#ceV509ReceiptModal [data-close],#ceV504ReceiptModal [data-close],#ceV502ReceiptModal [data-close],.ce-v505-photo-close,.ce-v506-photo-close{position:fixed!important;right:calc(env(safe-area-inset-right,0px) + 18px)!important;bottom:calc(env(safe-area-inset-bottom,0px) + 18px)!important;top:auto!important;left:auto!important;z-index:10000110!important;background:#fff!important;color:#000!important;border:1px solid #111827!important;border-radius:12px!important;min-height:42px!important;min-width:92px!important;padding:8px 14px!important;box-shadow:0 4px 16px rgba(15,23,42,.24)!important;pointer-events:auto!important;touch-action:manipulation!important;}
      #ceV401PcPhotoModal .ce-v401-pc-modal-box,#ceV40TicketPhotoModal .ce-v40-modal-box,#ceV310PhotoViewer .ce-v310-photo-box,.ce-v505-photo-card,.ce-v506-photo-card{padding-bottom:74px!important;}
    `;
  ...
  window.ControlEventV60ProdFinalFix = {version:VERSION, versionFile:VERSION_FILE, install, normalizeDownloadName, patchExcelJS};
})();
