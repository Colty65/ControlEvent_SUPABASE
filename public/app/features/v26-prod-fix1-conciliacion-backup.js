...
(function(){
  ...
  function coreState(wb){
    ...
    const personas=personaRows.map(r=>({id:norm(pick(r,'PERSONA_ID')),nombre:norm(pick(r,'PERSONA_NOMBRE')),nombreAmigo:norm(pick(r,'PERSONA_NOMBRE_AMIGO')),rango:norm(pick(r,'PERSONA_RANGO'))||'SOCIO'})).filter(r=>r.id);
  ...
  function extendedTables(wb){
    return {
      ...
      personAliases:sheetRows(wb,'PERSONAS_ALIAS').map(r=>({persona_id:norm(pick(r,'PERSONA_ID')),alias:norm(pick(r,'ALIAS')),prioridad:number(pick(r,'PRIORIDAD'))||50,es_preferido:bool(pick(r,'ES_PREFERIDO')),activo:norm(pick(r,'ACTIVO'))===''?true:bool(pick(r,'ACTIVO'))})).filter(r=>r.persona_id&&r.alias),
      ...
      bankMovementSettlements:sheetRows(wb,'BANCO_CIERRE_MVTO').map(r=>({movement_id:norm(pick(r,'MOVEMENT_ID')),accepted_difference:number(pick(r,'ACCEPTED_DIFFERENCE')),note:norm(pick(r,'NOTE'))||null,accepted_by:norm(pick(r,'ACCEPTED_BY'))||null,accepted_at:norm(pick(r,'ACCEPTED_AT'))||undefined,updated_at:norm(pick(r,'UPDATED_AT'))||undefined})).filter(r=>r.movement_id),
