...
(function(){
  ...
  function installRenderOptimizer(){
    // Guardias conservadores: no tocan INFOEVENTO/BACKUP ni el motor de datos; solo evitan pintar listas ocultas.
    guardRenderFunction('renderIngresosSummary', 'tabIngresos');
    guardRenderFunction('renderColabs', 'tabIngresos');
    guardRenderFunction('renderCompras', 'tabCompras');
    guardRenderFunction('renderDonaciones', 'tabDonaciones');
    // Resumen/Graficas se mantienen sin guardia por seguridad de exportaciones; se limpian si no están visibles.
    const oldRender = window.render;
    if(typeof oldRender === 'function' && !oldRender.__cePerf442PruneWrapped){
      const wrapped = function(){
        const result = oldRender.apply(this, arguments);
        setTimeout(() => pruneInactiveDynamicDom('after-render'), 0);
        return result;
      };
      wrapped.__cePerf442PruneWrapped = true;
      wrapped.__cePerf442Original = oldRender;
      window.render = wrapped;
    }
    renderOptimizer.installed = true;
    setTimeout(() => pruneInactiveDynamicDom('install'), 120);
  }
...
Optimización v44.3: limpiezas ${window.__ceV443Stats?.prunes||0}, nodos ${window.__ceV443Stats?.clearedNodes||0}\nActualizado: ${last.updatedAt}\nArranque: ${last.bootMs} ms</pre>
  ...
  function ensureUi(){
    injectCss();
    const oldBtn = byId('cePerf442Button') || byId('cePerf441Button');
    if(oldBtn) oldBtn.remove();
    const oldPanel = byId('cePerf442Panel') || byId('cePerf441Panel');
  ...
  setInterval(() => { installWrappers(); installRenderOptimizer(); pruneInactiveDynamicDom('interval'); if(state.opened) updatePanel(); }, 3000);
})();
