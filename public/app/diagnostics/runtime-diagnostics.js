...
function collectWarnings(report){
  ...
  if(report.version.domTitle && !report.version.domTitle.includes('v30.3')) warnings.push('El título del documento no parece estar en v30.3.');
  if(report.version.bodyDataset && !report.version.bodyDataset.includes('v30.3')) warnings.push('body.dataset.ceVersion no coincide con v30.3.');
...
export function assertHealthy(){
  ...
  if(!report.ok){
    console.warn('[ControlEventDiagnostics/v30.3] Avisos detectados', report.warnings, report);
  }
  return report;
}
...
export function print(){
  ...
  if(report.warnings.length) console.warn('[ControlEventDiagnostics/v30.3]', report.warnings);
  return report;
}
