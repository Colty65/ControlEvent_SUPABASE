...
const DIAGNOSTICS_VERSION = 'v27.0';
...
function collectWarnings(report){
  ...
  if(report.version.domTitle && !report.version.domTitle.includes('v27.0')) warnings.push('El título del documento no parece estar en v27.0.');
  if(report.version.bodyDataset && !report.version.bodyDataset.includes('v27.0')) warnings.push('body.dataset.ceVersion no coincide con v27.0.');
...
export function assertHealthy(){
  ...
  if(!report.ok){
    console.warn('[ControlEventDiagnostics/v27.0] Avisos detectados', report.warnings, report);
  }
  return report;
}
...
export function print(){
  ...
  if(report.warnings.length) console.warn('[ControlEventDiagnostics/v27.0]', report.warnings);
  return report;
}
