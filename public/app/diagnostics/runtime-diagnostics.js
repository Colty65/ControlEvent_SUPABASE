...
const DIAGNOSTICS_VERSION = 'v30.5';
...
function collectWarnings(report){
  ...
  if(report.version.domTitle && !report.version.domTitle.includes('v30.5')) warnings.push('El título del documento no parece estar en v30.5.');
  if(report.version.bodyDataset && !report.version.bodyDataset.includes('v30.5')) warnings.push('body.dataset.ceVersion no coincide con v30.5.');
...
export function assertHealthy(){
  ...
  if(!report.ok){
    console.warn('[ControlEventDiagnostics/v30.5] Avisos detectados', report.warnings, report);
  }
  return report;
}
...
export function print(){
  ...
  if(report.warnings.length) console.warn('[ControlEventDiagnostics/v30.5]', report.warnings);
  return report;
}
