...
const DIAGNOSTICS_VERSION = 'v28.1.1';
...
function collectWarnings(report){
  ...
  if(report.version.domTitle && !report.version.domTitle.includes('v28.1.1')) warnings.push('El título del documento no parece estar en v28.0.');
  if(report.version.bodyDataset && !report.version.bodyDataset.includes('v28.1.1')) warnings.push('body.dataset.ceVersion no coincide con v28.0.');
...
export function assertHealthy(){
  ...
  if(!report.ok){
    console.warn('[ControlEventDiagnostics/v28.1.1] Avisos detectados', report.warnings, report);
  }
  return report;
}
...
export function print(){
  ...
  if(report.warnings.length) console.warn('[ControlEventDiagnostics/v28.1.1]', report.warnings);
  return report;
}
