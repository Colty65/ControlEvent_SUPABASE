...
router.put('/crud/productos/:id/precio', asyncHandler(async (req, res) => {
  requireRowWrite(req);
  res.json(await updateProductPrice(req.params.id, cleanBody(req.body || {})));
}));
...
router.post('/crud/eventos/:id/correccion-compras', asyncHandler(async (req, res) => {
  requireRowWrite(req);
  res.json(await openComprasCorrection(req.params.id, req.body?.minutes, req.body?.reason));
}));

router.delete('/crud/eventos/:id/correccion-compras', asyncHandler(async (req, res) => {
  requireRowWrite(req);
  res.json(await closeEventCorrection(req.params.id));
}));
...
router.delete('/crud/:collection/:id', asyncHandler(async (req, res) => {
  requireRowWrite(req);
  res.json(await deleteRecord(collection(req), req.params.id, cleanBody(req.body || {})));
}));
