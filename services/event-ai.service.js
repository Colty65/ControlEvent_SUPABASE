/* ControlEvent v26_prod_1.0 - Zuzu / Analítica libre sobre datos del evento.
...
function directModuleResultIfApplicable(prompt, context) {
  ...
  const filename = fileSafe(`${moduleName}_${eventos || 'ControlEvent'}_v26_prod_1.0.csv`);
...
function directAggregateResultIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [
      { filename: fileSafe(`${moduleName}_${eventos || 'ControlEvent'}_agrupado_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(groupedCsvColumns, groupedCsvRows) },
      { filename: fileSafe(`${moduleName}_${eventos || 'ControlEvent'}_detalle_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(detailColumns, rows) }
    ],
...
function directSegmentDestinationSituationPieIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [
      { filename: fileSafe(`Compras_segmento_destino_situacion_${events.join('_') || 'evento'}_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, ordered.map(g => ({ Segmento:g.Segmento, Destino:g.Destino, 'Pte. Compra (€)':round(g.totals['Pte. Compra'],2), 'TKxx (€)':round(g.totals.TKxx,2), 'GASTOS CORRIENTES (€)':round(g.totals['GASTOS CORRIENTES'],2), 'Total parcial (€)':round(g.total,2), '% del total general':totalGeneral ? `${round((g.total*100)/totalGeneral,2)} %` : '0 %', Líneas:g.lines }))) },
      { filename: fileSafe(`Compras_segmento_destino_situacion_detalle_${events.join('_') || 'evento'}_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(detailColumns, detail) }
    ],
    provider: 'control-event-local-segmento-destino-situacion',
    model: 'calculo-local-oficial'
  };
...
function directComparativeAllDataResultIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [{ filename: fileSafe(`Comparativa_eventos_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(columns, rows) }],
...
function directPersonIdentityIfApplicable(prompt, context) {
  ...
  const files = [];
  if (ficha.length) files.push({ filename: fileSafe(`PERSONA_${displayName}_ficha_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(colsFicha, ficha) });
  if (actividad.length) files.push({ filename: fileSafe(`PERSONA_${displayName}_actividad_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(colsAct, actividad) });
...
function directPersonRoleReportIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [{ filename: fileSafe(`Informe_participacion_${needle}_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(colsDetail, detail) }],
...
function directCashEvolutionIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [{ filename: fileSafe(`Evolucion_saldo_caja_${anio || 'ControlEvent'}_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(cols, rows) }],
...
function directPersonsCatalogIfApplicable(prompt, context) {
  ...
  return {
    ...
    files:[{filename:fileSafe(`${rangos.length ? 'PERSONAS_'+rangos.join('_') : 'PERSONAS_catalogo'}_v26_prod_1.0.csv`),mime:'text/csv;charset=utf-8',content:csvFromRows(cols,rows)}],
...
function directDonorDonationProductsIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: detail.length ? [{ filename: fileSafe(`Donaciones_${titleNames}_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(detailColumns, detail) }] : [],
...
function directChronologicalEventNarrativeIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [
      { filename: fileSafe('Informe_cronologico_eventos_v26_prod_1.0.csv'), mime: 'text/csv;charset=utf-8', content: csvFromRows(columns, summaryRows) },
      { filename: fileSafe('Informe_cronologico_eventos_detalle_modulos_v26_prod_1.0.csv'), mime: 'text/csv;charset=utf-8', content: csvFromRows(moduleColumns, moduleRows) }
    ],
...
function directEventReportIfApplicable(prompt, context) {
  ...
  if (policy.onePage) {
    ...
    return {
      ...
      files:[{filename:fileSafe('Informe_ejecutivo_v26_prod_1.0.csv'),mime:'text/csv;charset=utf-8',content:csvFromRows(['Área','Semáforo','Lectura'],executiveRows.map(r=>({Área:r[0],Semáforo:r[1],Lectura:r[2]})))}],
  ...
  return {
    ...
    files:[{filename:fileSafe('Informe_eventos_v26_prod_1.0.csv'),mime:'text/csv;charset=utf-8',content:csvFromRows(exportColumns,rows)}],
    provider:'control-event-local-informe-eventos',model:'calculo-local-oficial'
  };
}
...
function directOperationalRankingResultIfApplicable(prompt, context) {
  ...
  return {
    ...
    charts,
    tables,
    files: [{ filename: fileSafe('Rankings_operativos_v26_prod_1.0.csv'), mime:'text/csv;charset=utf-8', content: csvFromRows(['Seccion','Nombre','Registros','Importe'], tables.flatMap(t => t.columns.includes('Importe (€)') ? t.rows.map(r => ({ Seccion:t.title, Nombre:r[0], Registros:r[1], Importe:r[2] })) : [])) }],
...
function directProductConsumptionResultIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [
      { filename: fileSafe(`Productos_consumidos_coste${titleEvents}_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, byCost) },
      { filename: fileSafe(`Productos_consumidos_unidades${titleEvents}_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(columns, byUnits) },
      { filename: fileSafe(`Productos_consumidos_detalle${titleEvents}_v26_prod_1.0.csv`), mime:'text/csv;charset=utf-8', content: csvFromRows(detailColumns, rowsSrc.map(r=>({ 'Evento':r.evento, 'Origen':r.origen, 'Producto':r.producto, 'Unidades':round(r.unidades,3), 'Precio':round(r.precio,4), 'Importe/Valor':round(r.importe,2), 'Ticket/Tipo':r.detalle, 'Tienda/Donante':r.tercero, 'Responsable':r.responsable }))) }
    ],
...
function directDeterministicResultIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: rows.length ? [{ filename: fileSafe(`${first}_${eventos || 'ControlEvent'}_diagnostico_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(columns, rows) }] : [],
...
function directGraphResultIfApplicable(prompt, context) {
  ...
  return {
    ...
    files: [{ filename: fileSafe(`${moduleName}_${eventos || 'ControlEvent'}_grafica_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(columns, rows) }],
...
function logGeminiUsage(stage, model, payload) {
  try {
    ...
    console.log(`[ControlEvent v26_prod_1.0 Zuzu] ${stage} · ${model} · prompt=${u.promptTokenCount ?? ''} candidates=${u.candidatesTokenCount ?? ''} total=${u.totalTokenCount ?? ''} billableOut=${c.billableOutputTokens ?? c.outputTokens} · coste≈$${Number(c.costUsd||0).toFixed(6)}/€${Number(c.costEurApprox||0).toFixed(6)}`);
  } catch (_) {}
}
    ...
    try {
      ...
      try { parsed = JSON.parse(stripJsonText(outText)); }
        ...
        return {
          ...
          charts: [],
          tables: [],
          files: [{ filename: fileSafe('Zuzu_respuesta_zuzu_no_estructurada_v26_prod_1.0.txt'), mime: 'text/plain;charset=utf-8', content: outText.slice(0, 250000) }],
...
function directSqlSelectResultIfApplicable(prompt, context) {
  ...
  executed.forEach(item => {
    ...
    if (columns.length) {
      ...
      files.push({ filename: fileSafe(`ZUZU_SELECT_${item.indice}_v26_prod_1.0.csv`), mime: 'text/csv;charset=utf-8', content: csvFromRows(columns, arr(item.rows).map(r => Object.fromEntries(columns.map(c => [c, r?.[c]])))) });
    } else {
  ...
  return {
    ...
    warnings: arr(context?.advertencias).concat('Versión experimental v26_prod_1.0: SELECTs ejecutados literalmente mediante RPC ce_zuzu_select. Si la RPC no está instalada, no habrá resultados SQL reales.'),
    charts: [],
    tables,
    files,
...
function semanticPresentation(results=[],userPrompt=''){
  ...
  for(const r of arr(results).filter(x=>x.ok&&x.showTable&&arr(x.rows).length).slice(0,10)){
    ...
    if(files.length<4)files.push({filename:fileSafe(`${r.id}_${r.title}_v26_prod_1.0.csv`),mime:'text/csv;charset=utf-8',content:csvFromRows(columns,displayRows.map(row=>Object.fromEntries(columns.map(c=>[c,row?.[c]]))))});
  }
  return {tables,files};
}
...
function v26PlannerPrompt(userPrompt,state,selectedEventId,conversationHistory=[]){
  ...
  return `Eres el INTÉRPRETE de Zuzu en ControlEvent v26_prod_1.0.
...
function v26ResolvePersonFamily(state,value){
  ...
  const queryKey=canonicalNameKey(value),primaryKey=canonicalNameKey(primary.nombre),queryIsPair=isCanonicalPairName(value);const rows=semanticCatalogRows(state,'person');const family=new Map([[primary.id,{id:primary.id,nombre:primary.nombre,kind:'primary'}]]);
  if(!queryIsPair){
    for(const row of rows){const name=trim(row.nombre),key=canonicalNameKey(name);if(row.id===primary.id)continue;if(key===queryKey||key===primaryKey){family.set(row.id,{...row,kind:'alias'});continue;}if(isCanonicalPairName(name)){const parts=v26PairParts(name).map(canonicalNameKey);if(parts.includes(queryKey)||(primaryKey&&!isCanonicalPairName(primary.nombre)&&parts.includes(primaryKey)))family.set(row.id,{...row,kind:'composite'});}}
  }
  const members=[...family.values()];return{ok:true,id:primary.id,nombre:primary.nombre,score:primary.score,type:'person',ids:members.map(x=>x.id),names:members.map(x=>x.nombre),members,expanded:members.length>1};
}
...
async function v26ToolEventDossier(tool,state,selectedEventId){
  ...
  return{id:tool.id,name:tool.name,ok:true,title:`Dossier · ${rr.nombre}`,facts,tables:[v26Table('kpis',`Indicadores de ${rr.nombre}`,rows,{Indicador:v26TextSchema('Nombre del indicador'),Valor:v26TextSchema('Valor; la unidad depende del indicador y está definida en facts')})]};
}
...
async function v26ToolEventPeople(tool,state,selectedEventId){
  ...
  for(const c of arr(state?.colaboradores).filter(x=>trim(x?.eventId||x?.event_id)===rr.id)){
    const inc=h.income(c); rows.push({Persona:inc.identity.name,Rango:inc.identity.range,Número:inc.numero,'Forma de pago':trim(c?.situacion||c?.formaPago||c?.ingreso||'Pendiente'),'Importe obligatorio':inc.mandatory,'Importe voluntario':inc.voluntary,'Importe total':inc.total});
  }
  const att=arr(buildCanonicalAttendance(state,[rr.id])?.porEvento)[0]||{};
  return{id:tool.id,name:tool.name,ok:true,title:`Personas · ${rr.nombre}`,facts:{event:rr.nombre,records:rows.length,income_people_units:round(rows.reduce((a,r)=>a+num(r.Número),0),3),attendees_canonical:round(att?.totalAsistentesPersonas,3),members_attending:round(att?.sociosAsistentesPersonas,3),nonmembers_attending:round(att?.noSociosAsistentesPersonas,3),income_total:v26Money(rows.reduce((a,r)=>a+num(r['Importe total']),0)),attendance_criterion:trim(att?.criterio)},tables:[v26Table('people',`Participación e ingresos · ${rr.nombre}`,rows,{Persona:v26TextSchema('Persona o pareja registrada'),Rango:v26TextSchema('Rango histórico'),Número:v26CountSchema('personas','Número asociado al registro de ingreso; no equivale por sí solo a asistencia canónica'),'Forma de pago':v26TextSchema('Forma de pago / situación'),'Importe obligatorio':v26MoneySchema('Ingreso obligatorio'),'Importe voluntario':v26MoneySchema('Ingreso voluntario o ajuste'),'Importe total':v26MoneySchema('Obligatorio + voluntario')})]};
}
...
async function v26ToolPeopleActivity(tool,state){
  const people=byId(state?.personas),eventSets=new Map(),incomeRecords=new Map(),purchaseLines=new Map(),purchaseValue=new Map(),donationLines=new Map(),donationValue=new Map(),hitosMap=new Map(),lgMap=new Map();
  ...
  for(const c of arr(state?.compras)){const tt=ticketText(c),amount=valueOfLine(c);if(isDonationTicket(tt)){const pid=trim(text(c?.donorRef||c?.donor_ref).replace(/^P:/,''));if(pid){bump(donationLines,pid);bump(donationValue,pid,amount);}}else if(!isPendingTicket(tt)){const pid=trim(c?.responsableId||c?.responsable_id);if(pid){bump(purchaseLines,pid);bump(purchaseValue,pid,amount);}}}
  ...
  const ids=new Set([...eventSets.keys(),...purchaseLines.keys(),...donationLines.keys(),...hitosMap.keys(),...lgMap.keys()]);const rows=[];
  for(const id of ids){const name=trim(people.get(id)?.nombre)||id;if(!name||/^z_dev/i.test(name)||/^grupo/i.test(name)||/^peña/i.test(name)||/^personas?$/i.test(name))continue;rows.push({Entidad:name,'Eventos con participación':eventSets.get(id)?.size||0,'Registros de ingreso':num(incomeRecords.get(id)),'Registros compra responsabilidad':num(purchaseLines.get(id)),'Importe compra gestionado':v26Money(purchaseValue.get(id)),'Donaciones como donante':num(donationLines.get(id)),'Valor donado':v26Money(donationValue.get(id)),Hitos:num(hitosMap.get(id)),'Tareas LG':num(lgMap.get(id))});}
  ...
  const schema={Entidad:v26TextSchema('Persona o pareja registrada'),'Eventos con participación':v26CountSchema('eventos','Eventos con participación'),'Registros de ingreso':v26CountSchema('registros','Registros de ingreso'),'Registros compra responsabilidad':v26CountSchema('registros','Registros de compra bajo responsabilidad'),'Importe compra gestionado':v26MoneySchema('Importe de compras bajo responsabilidad'),'Donaciones como donante':v26CountSchema('registros','Registros de donación como donante'),'Valor donado':v26MoneySchema('Valor de producto donado'),Hitos:v26CountSchema('hitos','Hitos como responsable'),'Tareas LG':v26CountSchema('tareas','LG como responsable')};
  return{id:tool.id,name:tool.name,ok:true,title:'Implicación de personas y parejas',facts:{entities:rows.length,criterion:'ControlEvent no usa un índice opaco: entrega dimensiones separadas de participación, responsabilidad de compras, donaciones y gestión para que Gemini argumente la conclusión.'},tables:[v26Table('people_activity','Implicación por dimensiones',rows,schema)]};
}
...
function v26FinalPrompt(userPrompt,plan,results,repair='',conversationHistory=[],conversationContext={}){
  return `Eres Zuzu, ANALISTA de ControlEvent v26_prod_1.0. ControlEvent ya ha ejecutado las herramientas y te entrega HECHOS CANÓNICOS. Tu valor añadido es interpretarlos bien, detectar lo relevante y responder como un analista humano. NO inventes datos y NO recalcules totales: usa exactamente los hechos numéricos ya calculados por CE.
...
function v26BuildPresentation(final,results,userPrompt){
  const byId=new Map(arr(results).map(r=>[trim(r.id),r])); const tables=[],charts=[]; const seenT=new Set();
  for(const ref of arr(final?.showTables).slice(0,6)){
  ...
  const wantsCharts=semanticPromptExplicitlyRequestsCharts(userPrompt) || arr(final?.chartSpecs).length>0;
  if(wantsCharts){
    for(const spec of arr(final?.chartSpecs).slice(0,5)){
      const r=byId.get(trim(spec?.tool_id)),t=arr(r?.tables).find(x=>trim(x?.key)===trim(spec?.table_key));if(!t)continue;const lf=trim(spec?.label_field),vf=trim(spec?.value_field),labelMeta=v26TableFieldMeta(t,lf),valueMeta=v26TableFieldMeta(t,vf);if(labelMeta&&v26IsChartNumericMeta(labelMeta))continue;
...
async function runZuzuV26Tools({userPrompt,state,selectedEventId,flowTrace=[],conversationHistory=[],conversationContext={},conversationResolution={}}){
  ...
  for(const t of presentation.tables.slice(0,4)){
    ...
    files.push({filename:fileSafe(`${t.title}_v26_prod_1.0.csv`),mime:'text/csv;charset=utf-8',content:csvFromRows(t.columns,objs)});
  }
  ...
  return{
    ...
    meta:{
      generatedAt:new Date().toISOString(),version:'v26_prod_1.0',
...
// ============================================================================
// v26_prod_1.0 · ZUZU GEMINI INTERACTIONS
...
function v261AgentTools(){
  ...
  return [
    {type:'function',name:'event_dossier',description:'Obtiene hechos canónicos de un evento: fechas, estado, ingresos, compras realizadas y pendientes, producto donado, saldo, valoración, asistencia y conteos de gestión. Úsala para comprender cómo fue un evento.',parameters:{type:'object',properties:{event:eventArg,scope:scopeArg,detail:detailArg},required:['scope']}},
    ...
    {type:'function',name:'event_people',description:'Obtiene participantes/colaboradores e ingresos de un evento con criterio de asistencia canónica.',parameters:{type:'object',properties:{event:eventArg,scope:scopeArg,detail:detailArg},required:['scope']}},
    ...
    {type:'function',name:'event_bank',description:'Obtiene el cuadre/conciliación bancaria de un evento: periodo, resumen, movimientos y vínculos con tickets/ingresos.',parameters:{type:'object',properties:{event:eventArg,scope:scopeArg,detail:detailArg},required:['scope']}},
    ...
    {type:'function',name:'person_dossier',description:'Obtiene un dossier completo de una persona o pareja: apariciones individuales/compuestas, participación, ingresos, compras bajo responsabilidad, donaciones, Hitos y tareas LG. Resuelve automáticamente nombres dentro de parejas como «Colty y Esther».',parameters:{type:'object',properties:{person:{type:'string',description:'Nombre o alias de la persona/pareja.'},detail:detailArg},required:['person']}},
    {type:'function',name:'participation_events',description:'Lista de forma ligera los eventos en los que aparece una persona, incluyendo apariciones dentro de parejas.',parameters:{type:'object',properties:{person:{type:'string'},detail:detailArg},required:['person']}},
    {type:'function',name:'people_activity',description:'Devuelve dimensiones comparables de implicación de personas/parejas en toda la Peña: participación, compras gestionadas, donaciones, Hitos y tareas LG. No impone un ranking; úsala para que tú razones el criterio.',parameters:{type:'object',properties:{detail:detailArg}}},
...
function v261FinalSchema(){
  return {type:'object',properties:{
    ...
    warnings:{type:'array',items:{type:'string'}},
    show_tables:{type:'array',items:{type:'object',properties:{tool_id:{type:'string'},table_key:{type:'string'}},required:['tool_id','table_key']}},
    charts:{type:'array',items:{type:'object',properties:{title:{type:'string'},type:{type:'string',enum:['bar','horizontalBar','pie','donut','line','stackedBar']},tool_id:{type:'string'},table_key:{type:'string'},label_field:{type:'string'},value_field:{type:'string'},series_fields:{type:'array',items:{type:'string'}},unit:{type:'string'}},required:['title','type','tool_id','table_key','label_field']}}
  },required:['title','answer','warnings','show_tables','charts']};
}

function v261SystemInstruction(state,selectedEventId,{usuarioLogado,user,authUser,ce_acceso}={}){
...
function v261CompactToolResult(result,detail='standard'){
  ...
  return{id:trim(result?.id),name:trim(result?.name),ok:result?.ok!==false,title:trim(result?.title),facts:result?.facts||{},tables};
}
...
async function v261ExecuteAgentTool(call,state,selectedEventId,flowTrace=[]){
  ...
  if(['event_dossier','event_breakdowns','event_people','event_documentation','event_management','event_bank','event_weather','store_purchases'].includes(tool.name) && !tool.scope){
...
function v261ObjectiveIssues(final,results){
  const issues=[];const answer=trim(final?.answer);
  const unsupported=v26UnsupportedMoney(answer,results);if(unsupported.length)issues.push(`Importes monetarios no respaldados por hechos canónicos: ${unsupported.map(v26FormatEuro).join(', ')}.`);
...
async function runZuzuV261InteractionsAgent({userPrompt,state,selectedEventId,flowTrace=[],previousInteractionId='',conversationHistory=[],usuarioLogado,user,authUser,ce_acceso}){
  const model=v261InteractionModel(),tools=v261AgentTools(),systemInstruction=v261SystemInstruction(state,selectedEventId,{usuarioLogado,user,authUser,ce_acceso});
  let currentId=trim(previousInteractionId),payload;
  const cache=new Map(),allResults=[];
  const initialInput=userPrompt;
  try{payload=await v261CallInteraction({input:initialInput,previousInteractionId:currentId,model,systemInstruction,tools,flowTrace,stage:'V26.1 · Gemini conversación'});}
  catch(error){
    if(currentId&&v261PreviousIdFailure(error)){
      zuzuTracePush(flowTrace,'V26.1 · Recuperación de conversación','WARN','El previous_interaction_id ya no es válido. Gemini reconstruirá el hilo con los últimos turnos guardados por el navegador.');
      currentId='';payload=await v261CallInteraction({input:v261EmergencyConversationInput(userPrompt,conversationHistory),model,systemInstruction,tools,flowTrace,stage:'V26.1 · Gemini conversación recuperada'});
    }else throw error;
  }
  ...
  for(let cycle=1;cycle<=6;cycle++){
    ...
    zuzuTracePush(flowTrace,`V26.1 · Herramientas · ronda ${cycle}`,'OK',`Gemini solicitó ${calls.length} herramienta(s): ${calls.map(c=>trim(c.name)).join(', ')}.`);
    const functionResults=await Promise.all(calls.map(async call=>{
      ...
      try{
        ...
        zuzuTracePush(flowTrace,`V26.1 · Tool ${trim(call.name)}`,'OK',`${trim(full?.title)||'Resultado'}; tablas=${arr(full?.tables).length}.`);
        return{type:'function_result',name:trim(call.name),call_id:trim(call.id),result:compact};
      }catch(error){
        const msg=cleanGeminiError(error);zuzuTracePush(flowTrace,`V26.1 · Tool ${trim(call.name)}`,'WARN',msg);
        return{type:'function_result',name:trim(call.name),call_id:trim(call.id),is_error:true,result:{ok:false,error:msg}};
      }
    }));
    payload=await v261CallInteraction({input:functionResults,previousInteractionId:currentId,model,systemInstruction,tools,flowTrace,stage:`V26.1 · Gemini tras herramientas ${cycle}`});
    currentId=trim(payload?.id)||currentId;
  }
  ...
  if(issues.length){
    zuzuTracePush(flowTrace,'V26.1 · Auditor factual','RETRY',issues.join(' '));
    const repairInput=`ControlEvent ha verificado tu respuesta y ha encontrado estos problemas OBJETIVOS:\n- ${issues.join('\n- ')}\n\nCorrige SOLO esos problemas conservando la intención, el razonamiento y el tono de tu respuesta. No inventes datos. Devuelve de nuevo el JSON estructurado final.`;
    payload=await v261CallInteraction({input:repairInput,previousInteractionId:currentId,model,systemInstruction,tools,flowTrace,stage:'V26.1 · Gemini corrección factual',toolChoice:'none'});
    currentId=trim(payload?.id)||currentId;final=v261ParseFinal(payload);
    const remaining=v261ObjectiveIssues(final,allResults);if(remaining.length){zuzuTracePush(flowTrace,'V26.1 · Auditor factual','WARN',`Gemini mantiene ${remaining.length} incidencia(s) objetiva(s); se informa sin sustituir su respuesta: ${remaining.join(' ')}`);final.warnings=arr(final.warnings).concat(remaining);}
    else zuzuTracePush(flowTrace,'V26.1 · Auditor factual','OK','Gemini corrigió los hechos señalados; ControlEvent no redactó una respuesta alternativa.');
  }else zuzuTracePush(flowTrace,'V26.1 · Auditor factual','OK','No se detectan importes no respaldados ni una fórmula de valoración incorrecta.');
  final={...final,answer:v26PolishNarrative(v26FormatNarrativeMoney(final.answer,allResults))};
  const presentation=v26BuildPresentation(final,allResults,userPrompt),files=[];
  for(const t of presentation.tables.slice(0,6)){
    const objs=t.rows.map(r=>Object.fromEntries(t.columns.map((c,i)=>[c,r[i]])));
    files.push({filename:fileSafe(`${t.title}_v26_prod_1.0.csv`),mime:'text/csv;charset=utf-8',content:csvFromRows(t.columns,objs)});
  }
  const answer=trim(final.answer);
  zuzuTracePush(flowTrace,'V26.1 · Presentación','OK',`Gemini dirigió la jugada. Herramientas ejecutadas=${allResults.length}; tablas mostradas=${presentation.tables.length}; gráficas=${presentation.charts.length}.`);
  return{ok:true,rejected:false,title:final.title||'Respuesta de Zuzu',answer,warnings:arr(final.warnings),charts:presentation.charts,tables:presentation.tables,files,provider:'gemini-interactions-v26-prod-1.0',model,interactionId:currentId,meta:{generatedAt:new Date().toISOString(),version:'v26_prod_1.0',architecture:'Usuario -> Gemini Interactions (estado nativo) -> Gemini elige herramientas -> ControlEvent ejecuta hechos canónicos -> Gemini razona/redacta -> ControlEvent verifica hechos/presenta',interactionId:currentId,tools:[...new Set(allResults.map(r=>trim(r?.name)).filter(Boolean))],geminiUsageEstimate:summarizeGeminiUsageFromTrace(flowTrace),debugTrace:arr(flowTrace).slice(0,120)},debugTrace:arr(flowTrace).slice(0,120),showDebugTrace:true};
}
...
function v26LocalCapabilitiesHelp(userPrompt,{usuarioLogado,user,authUser,ce_acceso}={}){
  ...
  return{ok:true,rejected:false,title:'Qué puedes preguntarle a Zuzu',answer,warnings:[],charts:[],tables:[],files:[],provider:'control-event-local-capabilities',model:'',meta:{generatedAt:new Date().toISOString(),version:'v26_prod_1.0',architecture:'Ayuda local de capacidades; no requiere llamada a Gemini',geminiUsageEstimate:{calls:0,promptTokens:0,outputTokens:0,hiddenOutputTokens:0,totalTokens:0,costUsdApprox:0,costEurApprox:0},debugTrace:[{step:'V26 · Capacidades de Zuzu',status:'OK',detail:'Respuesta local basada en las capacidades reales de ControlEvent; sin consumo Gemini.'}]},debugTrace:[{step:'V26 · Capacidades de Zuzu',status:'OK',detail:'Respuesta local basada en las capacidades reales de ControlEvent; sin consumo Gemini.'}],showDebugTrace:true};
}

export async function analyzeEventPrompt({ prompt, selectedEventId, stateOverride, usuarioLogado, user, authUser, ce_acceso, previousInteractionId, conversationHistory, conversationContext } = {}) {
  const flowTrace = [];
  ...
  // v26_prod_1.0: las capacidades y cualquier pregunta conversacional las interpreta Gemini;
  ...
  zuzuTracePush(flowTrace,'V26.1 · Arquitectura','OK',`Gemini Interactions dirige conversación y herramientas. previous_interaction_id=${trim(previousInteractionId)?'presente':'nuevo hilo'}; evento de pantalla=${trim(selectedEventId)||'ninguno'} (solo contexto ambiental).`);
  return runZuzuV261InteractionsAgent({userPrompt,state,selectedEventId,flowTrace,previousInteractionId:trim(previousInteractionId),conversationHistory:safeHistory,usuarioLogado,user,authUser,ce_acceso});

  /* Código histórico conservado temporalmente para Planificación y trazabilidad de versiones previas.
     La ruta /event-ai/analyze de v26_prod_1.0 retorna antes de llegar aquí. */
...
// v26_prod_1.0 - Planificación inicial asistida por Zuzu.
...
export async function planificacionInicialZuzu({ mode, modelEventId, content, title, fechaIni, fechaFin, dias, personas, defaultResponsibleId, defaultStoreId, descripcion, info, applySaldoAjuste, applyProductCaps, usuarioLogado, user, authUser, ce_acceso } = {}) {
  ...
  return {
    ok: true,
    version: 'v26_prod_1.0_FIX47_CONSUMO_ABIERTO_VARIABLE',
    provider: aiProvider,
    model: aiModel,
    mode: m,
    modules,
