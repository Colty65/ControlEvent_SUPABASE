/* ControlEvent v28.2_prod - Alta asistida para lectura de tickets de compra.
...
async function callGeminiModel({ model, parts, apiKey, instrucciones = '', responsables = [], tiendas = [] }) {
  ...
  try { console.log(`[ControlEvent v28.2_prod Alta IA] OCR ticket · ${model} · prompt=${usage.promptTokens} outputFacturable=${usage.outputTokens} total=${usage.totalTokens} · coste≈$${Number(usage.costUsd||0).toFixed(6)}/€${Number(usage.costEurApprox||0).toFixed(6)}`); } catch (_) {}
...
async function callGemini({ dataUrl, instrucciones = '', responsables = [], tiendas = [] }) {
  ...
  for (const model of models) {
    try {
      ...
      try { console.warn(`[ControlEvent v28.2_prod Alta IA] Gemini REST falló con ${model}; se probará otro modelo si queda disponible.`, error?.message || error); } catch (_) {}
    }
  }
  if (lastError) {
...
export async function analyzeReceiptImage({ dataUrl, instrucciones, indicaciones, hint, responsables = [], tiendas = [] } = {}) {
  ...
  if (provider === 'openai') {
    try {
      ...
      if (geminiKey()) {
        try { console.warn('[ControlEvent v28.2_prod Alta IA] OpenAI falló; se reintenta con Gemini REST.', error?.message || error); } catch (_) {}
