/* ControlEvent v4_0_exp · ITV INTÉRPRETE GEMINI V2.1
...
/* V2.1 distingue transporte limpio de intención recuperable. Si Gemini añade basura después
...
CHAT request opcional = session_summary cuando se pide resumir la conversación ACTUAL.
...
- CHAT/session_summary se usa para resumir lo ocurrido en la sesión/conversación ACTUAL usando session_ledger.
...
const BASE_CASES=Object.freeze([
  ...
  c(2,'DATA','Sácame las compras realizadas de FUNCION 2026.',{}, {type:'DATA',request:'event_purchases',events:['FUNCION 2026']}),
  c(3,'DATA','¿Qué ingresos están pendientes en SySA 2026?',{}, {type:'DATA',request:'event_income_status',events:['SySA 2026']}),
  ...
  c(6,'MEMORY','¿Recuerdas algo de Pocholo?',{}, {type:'MEMORY',request:'search',people:['Pocholo']}),
  ...
  c(13,'CALCULATE','¿Cuál de esas filas tiene el Valor más alto y cuánto es?',{current_dataset:{dataset_id:'econ-sysa26',table_key:'economics_chart',title:'Economía · SySA 2026',columns:['Indicador','Valor'],row_count:2}}, {type:'CALCULATE',request:'MAX',field:'Valor'}),
  ...
  c(26,'CHAT','Resúmeme qué hemos hecho en esta conversación y qué queda abierto.',{session_ledger:[{kind:'memory',value:'Pocholo'},{kind:'event',value:'SySA 2026'},{kind:'dataset',value:'Economía · SySA 2026'},{kind:'person',value:'Colty'},{kind:'person',value:'Esther'}]}, {type:'CHAT',request:'session_summary'}),
...
function translateConcept(plan={},state={}){
  ...
  if(['CHAT','CLARIFY','UNSUPPORTED'].includes(type))return{ok:true,actions,issues,guard};
  if(type==='DATA'){
...
async function callGemini(caseDef={},externalSignal=null){
  const apiKey=geminiKey();if(!apiKey){const e=new Error('Falta GEMINI_API_KEY para ITV INTÉRPRETE GEMINI V2.1.');e.status=503;throw e;}
...
export async function runInterpreterCase({caseDef,signal=null}={}){
  ...
  try{
    ...
    return{ok:true,id:caseDef.id,baseId:caseDef.baseId||cdef.id,repeat:num(caseDef.repeat)||1,category:cdef.category,prompt:cdef.prompt,context:cdef.context,enriched:state,expected:cdef.expected,plan,raw:got.raw,conceptSignature:canonicalConceptSignature(plan),executionSignature:canonicalExecutionSignature(translation),translatedActions:translation.actions,executionGuard:translation.guard||null,status,metrics:{planParsed:parsed.parsed,transportClean:parsed.transportClean,transportRecovered:parsed.recovered,intentCorrect:intent.ok,translationCE:ce.ok,analysisPolicy:policy,guardedAmbiguity:Boolean(translation.guard?.blocked)},reasons:[...(parsed.parsed?[]:[parsed.error||'JSON inválido']),...intent.reasons,...ce.issues,...(translation.guard?.blocked?[`ejecución bloqueada: ${translation.guard.reason}`]:[]),...(policy?[]:[`analysis esperado ${Boolean(cdef.expected.analysis)}; recibido ${Boolean(plan.analysis)}`])],audits:ce.audits,durationMs:got.durationMs||Date.now()-started,usage:got.usage,model:got.model,executesCE:false};
  }catch(error){return{ok:false,id:caseDef.id,baseId:caseDef.baseId||cdef.id,repeat:num(caseDef.repeat)||1,category:cdef.category,prompt:cdef.prompt,context:cdef.context,enriched:state,expected:cdef.expected,plan:null,raw:'',conceptSignature:'',executionSignature:'',translatedActions:[],executionGuard:null,status:'KO',metrics:{planParsed:false,transportClean:false,transportRecovered:false,intentCorrect:false,translationCE:false,analysisPolicy:false},reasons:[error?.message||String(error)],audits:[],durationMs:Date.now()-started,usage:{promptTokens:0,outputTokens:0,totalTokens:0,costEur:0},model:interpreterModel(),executesCE:false,error:error?.message||String(error)};}
}
...
export function __interpreterLabForRegression(){return{BASE_CASES,ENTITY_FIXTURE,parseConceptPlan,enrichState,conceptualIntentMatch,translateConcept,translatorAudit,canonicalConceptSignature,canonicalExecutionSignature,ambiguityGuard,systemInstruction};}
