/* ControlEvent v3_0_exp - Motor seguro de contexto para Zuzu / Analítica libre.
...
export function buildEventAiContext(state, selectedEventId = '', userPrompt = '') {
  ...
  const context = {
    versionContexto: 'ControlEvent EventContext v3_0_exp - Zuzu contexto completo selectivo',
...
/* ControlEvent v3_0_exp - Zuzu: módulos seguros de extracción selectiva completa.
...
export function buildZuzuPlanningCatalog(state, selectedEventId = '', userPrompt = '') {
  ...
  return {
    version: 'ControlEvent Zuzu Planner v3_0_exp',
...
export function buildZuzuModuleContext(state, selectedEventId = '', userPrompt = '', plan = null) {
  ...
  const context = {
    versionContexto: 'ControlEvent Zuzu Modules v3_0_exp',
    generatedAt: new Date().toISOString(),
    seguridad: { modo: 'solo lectura', nota: 'EXPERIMENTAL v3_0_exp: Zuzu puede proponer SELECTS_PROPUESTOS. ControlEvent valida que sean SELECT de solo lectura y los ejecuta literalmente mediante RPC ce_zuzu_select; los módulos oficiales se conservan como respaldo/auditoría.' },
