/* ControlEvent v28.5.1_prod - Motor seguro de contexto para Zuzu / Analítica libre.
...
export function buildEventAiContext(state, selectedEventId = '', userPrompt = '') {
  ...
  const context = {
    versionContexto: 'ControlEvent EventContext v28.5.1_prod - Zuzu contexto completo selectivo',
...
/* ControlEvent v28.5.1_prod - Zuzu: módulos seguros de extracción selectiva completa.
...
function zuzuIncomeAmounts(row, ev, helpers) {
  ...
  const voluntaryKeys = ['importeVoluntario','voluntario','donation','importe','importeDonacion','aportacionVoluntaria'];
  const totalKeys = ['total','importeTotal','importe_total'];

  // Regla canónica idéntica al RESUMEN PRESUPUESTARIO:
  // 1) Si existe un importe voluntario explícito, se respeta literalmente, incluso si es negativo.
  //    Los negativos son ajustes válidos para reducir el obligatorio calculado (p. ej. pareja que paga una sola cuota).
  // 2) Solo si no existe importe voluntario explícito se recupera desde TOTAL - obligatorio.
  // 3) Nunca se fuerza el voluntario a cero: hacerlo inflaba los informes de Zuzu.
  let voluntario;
  if (rowHasNumber(row, voluntaryKeys)) {
    voluntario = firstNumber(row, voluntaryKeys, 0);
  } else if (rowHasNumber(row, totalKeys)) {
    const totalGuardado = firstNumber(row, totalKeys, 0);
    voluntario = isSocio ? round(totalGuardado - obligatorio, 2) : round(totalGuardado, 2);
  } else {
    voluntario = 0;
  }

  voluntario = round(voluntario, 2);
  return {
    rango,
    numero: round(numero, 3),
    obligatorio: round(obligatorio, 2),
    voluntario,
    total: round(obligatorio + voluntario, 2)
  };
}
...
function zuzuAsksMissingAttendees(prompt) {
  const p = norm(prompt);
...
export function buildZuzuPlanningCatalog(state, selectedEventId = '', userPrompt = '') {
  ...
  return {
    version: 'ControlEvent Zuzu Planner v28.5.1_prod',
    ...
    conteosSistema: {
      ...
      hitos: arr(state?.hitos).length,
      lg: arr(state?.lgs).length
    },
...
function zuzuDependencyLabel(ref, hitoById, lgById) {
  const tipo = trim(ref?.tipo || ref?.type || '').toUpperCase() === 'HITO' ? 'HITO' : 'LG';
  const id = trim(ref?.id || ref?.value);
  if (!id) return '';
  if (tipo === 'HITO') return `Hito completo: ${trim(hitoById.get(id)?.nombreHito || hitoById.get(id)?.nombre_hito || id)}`;
  return `LG: ${trim(lgById.get(id)?.descripcion || id)}`;
}
function zuzuModuleHitos(state, eventIds) {
  const evById = byId(state?.eventos);
  const lgs = arr(state?.lgs);
  return arr(state?.hitos)
    .filter(h => eventIds.includes(trim(h?.eventId || h?.event_id)))
    .map(h => {
      const eventId = trim(h?.eventId || h?.event_id);
      const hitoId = trim(h?.id);
      const children = lgs.filter(lg => trim(lg?.eventId || lg?.event_id) === eventId && trim(lg?.hitoId || lg?.hito_id) === hitoId);
      const cumplidas = children.filter(lg => lg?.cumplida === true).length;
      return {
        Evento: trim(evById.get(eventId)?.titulo) || 'Sin evento',
        Hito: trim(h?.nombreHito || h?.nombre_hito) || 'Sin nombre',
        'Descripción': trim(h?.descripcion),
        'Fecha mínima': trim(h?.fechaMinima || h?.fecha_minima),
        'Fecha máxima': trim(h?.fechaMaxima || h?.fecha_maxima),
        'Responsable general': trim(h?.responsableNombre || h?.responsable_nombre) || 'Sin responsable',
        'LG totales': children.length,
        'LG cumplidas': cumplidas,
        Estado: children.length > 0 && cumplidas === children.length ? 'Cumplido' : (cumplidas > 0 ? 'En progreso' : 'Pendiente')
      };
    });
}
function zuzuModuleLg(state, eventIds) {
  const evById = byId(state?.eventos);
  const hitoById = byId(state?.hitos);
  const lgById = byId(state?.lgs);
  return arr(state?.lgs)
    .filter(lg => eventIds.includes(trim(lg?.eventId || lg?.event_id)))
    .map(lg => {
      const eventId = trim(lg?.eventId || lg?.event_id);
      const hito = hitoById.get(trim(lg?.hitoId || lg?.hito_id));
      const previas = arr(lg?.dependenciasPrevias || lg?.dependencias_previas).map(ref => zuzuDependencyLabel(ref, hitoById, lgById)).filter(Boolean);
      const posteriores = arr(lg?.dependenciasPosteriores || lg?.dependencias_posteriores).map(ref => zuzuDependencyLabel(ref, hitoById, lgById)).filter(Boolean);
      return {
        Evento: trim(evById.get(eventId)?.titulo) || 'Sin evento',
        Hito: trim(hito?.nombreHito || hito?.nombre_hito) || 'Sin Hito',
        'Descripción LG': trim(lg?.descripcion) || 'Sin descripción',
        'Fecha mínima': trim(lg?.fechaMinima || lg?.fecha_minima),
        'Fecha máxima': trim(lg?.fechaMaxima || lg?.fecha_maxima),
        Notas: trim(lg?.notas),
        Responsable: trim(lg?.responsableNombre || lg?.responsable_nombre) || 'Sin responsable',
        Estado: lg?.cumplida === true ? 'Cumplida' : 'Pendiente',
        'Dependencias previas': previas.join(' | ') || 'Ninguna',
        'Dependencias posteriores': posteriores.join(' | ') || 'Ninguna'
      };
    });
}
...
function zuzuAttendanceExcludedName(name) {
  const n = norm(name);
  return !n || /^personas\b/.test(n) || /^grupo\b/.test(n) || /^pe[ñn]a\b/.test(n) || /^z\s*_?\s*de/.test(n) || /^z\s*dev/.test(n);
}
function zuzuAttendanceNameKey(name) {
  return norm(name).replace(/[^a-z0-9ñ]+/g, ' ').replace(/\s+/g, ' ').trim();
}
function zuzuAttendanceIsPair(name) { return /\s+y\s+/i.test(trim(name)); }
function zuzuAttendancePairParts(name) { return trim(name).split(/\s+y\s+/i).map(trim).filter(Boolean); }
function zuzuAttendanceRawNumber(row) {
...
function zuzuAttendanceRowSize(row, name) {
  const n = zuzuAttendanceRawNumber(row);
  if (n !== null && n > 0) return n;
  const parts = zuzuAttendancePairParts(name);
  return zuzuAttendanceIsPair(name) ? Math.max(2, parts.length || 2) : 1;
}
function zuzuCanonicalSocioCensusFromState(state) {
  const base = arr(state?.personas)
    .map(row => ({ row, name: trim(row?.nombre || row?.Nombre || row?.NOMBRE), rango: norm(row?.rango || row?.Rango || row?.RANGO), id: trim(row?.id || row?.ID) }))
    .filter(x => x.rango === 'socio' && !zuzuAttendanceExcludedName(x.name));
  const pairs = base.filter(x => zuzuAttendanceIsPair(x.name)).map(x => {
    const parts = zuzuAttendancePairParts(x.name);
    return { kind:'pair', name:x.name, key:zuzuAttendanceNameKey(x.name), parts, size:Math.max(2, parts.length || 2), id:x.id, row:x.row };
  });
  const out = [];
  const seen = new Set();
  ...
  for (const x of base) {
    if (zuzuAttendanceIsPair(x.name)) continue;
    const key = zuzuAttendanceNameKey(x.name);
    if (pairs.some(pair => pair.parts.some(part => zuzuAttendanceNameKey(part) === key))) continue;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push({ kind:'single', name:x.name, key, parts:[x.name], size:1, id:x.id, row:x.row });
  }
...
function zuzuCanonicalMetricsFromModules(modulos, asistenciaCanonica = null) {
  ...
  const porEvento = [...events].map(ev => {
    ...
    const comprasPendientes = com.filter(r => { const t=trim(r?.['Ticket u otros gastos']); return !t || /pte\.?\s*compra|pendiente/i.test(t); });
    const comprasRealizadas = com.filter(r => { const t=trim(r?.['Ticket u otros gastos']); return !!t && !/pte\.?\s*compra|pendiente/i.test(t); });
    const donPorTipo = {};
    ...
    return {
      ...
      'Socios censo personas': num(arr(asistenciaCanonica?.porEvento).find(a=>trim(a.Evento)===ev)?.sociosCensoPersonas),
      'Socios asistentes personas': num(arr(asistenciaCanonica?.porEvento).find(a=>trim(a.Evento)===ev)?.sociosAsistentesPersonas),
      'No socios asistentes personas': num(arr(asistenciaCanonica?.porEvento).find(a=>trim(a.Evento)===ev)?.noSociosAsistentesPersonas),
      'Asistentes total personas': num(arr(asistenciaCanonica?.porEvento).find(a=>trim(a.Evento)===ev)?.totalAsistentesPersonas),
      'Socios no asistentes personas': num(arr(asistenciaCanonica?.porEvento).find(a=>trim(a.Evento)===ev)?.sociosNoAsistentesPersonas),
      ...
      'Valoracion con donaciones': round(comprasTotal + comprasPte + donacionesTotal, 2)
    };
  });
  return {
    fuente: 'ControlEvent calculado sobre módulos extraídos con reglas de RESUMEN PRESUPUESTARIO',
    reglaIngresos: 'Ingresos total = suma de Importe obligatorio + Importe voluntario de INGRESOS. El importe voluntario puede ser positivo, cero o negativo y nunca debe recortarse a cero.',
    reglaCompras: 'Compras realizadas = suma de Importe de COMPRAS excluyendo Pte. Compra. Incluye TKxx y GASTOS CORRIENTES. Un ticket/otros gastos vacío es Pte.Compra.',
    ...
    reglaValoracion: 'Valoración del evento = gastos previstos (compras realizadas + Pte.Compra) + donaciones valoradas.',
    porEvento
  };
}
...
export function buildZuzuModuleContext(state, selectedEventId = '', userPrompt = '', plan = null) {
  ...
  let eventIds = [];
  if (strictPlan) {
    eventIds = arr(p.eventIds || p.event_ids).map(trim).filter(Boolean);
    if (!eventIds.length) eventIds = zuzuFindEventIds(events, '', userPrompt, p, { noSelectedFallback: true });
    // En alcance cerrado no se permite caer a todos los eventos ni al evento activo arrastrado.
  } else if (allRowsMode) {
    eventIds = zuzuFindEventIds(events, selectedId, userPrompt, p, { noSelectedFallback: true });
    if (!eventIds.length && (zuzuAllEventsRequested(userPrompt, p) || extractModules.includes('EVENTOS'))) eventIds = arr(events).map(e => trim(e?.id)).filter(Boolean);
    if (!eventIds.length && extractModules.some(m => eventScopedModules.includes(m))) eventIds = selectedId ? [selectedId] : arr(events).map(e => trim(e?.id)).filter(Boolean);
  } else {
    ...
    if (!eventIds.length && extractModules.includes('EVENTOS')) eventIds = arr(events).map(e => trim(e?.id)).filter(Boolean);
  }
  if (masterCatalogOnly) eventIds = [];
  if (!eventIds.length && extractModules.some(m => eventScopedModules.includes(m) || m === 'EVENTOS')) {
  ...
  if (fullEventDataRequested) {
    ...
    if (extractModules.includes('HITOS') && !arr(modulos.HITOS).length && rawExpected('HITOS') > 0) {
      modulos.HITOS = zuzuModuleHitos(safeState, eventIds);
      if (modulos.HITOS.length) recoveredOperationalModules.push('HITOS');
    }
    if (extractModules.includes('LG') && !arr(modulos.LG).length && rawExpected('LG') > 0) {
      modulos.LG = zuzuModuleLg(safeState, eventIds);
      if (modulos.LG.length) recoveredOperationalModules.push('LG');
    }
  }
  ...
  const context = {
    versionContexto: 'ControlEvent Zuzu Modules v28.5.1_prod',
    generatedAt: new Date().toISOString(),
    seguridad: { modo: 'solo lectura', nota: 'EXPERIMENTAL v28.5.1_prod: Zuzu puede proponer SELECTS_PROPUESTOS. ControlEvent valida que sean SELECT de solo lectura y los ejecuta literalmente mediante RPC ce_zuzu_select; los módulos oficiales se conservan como respaldo/auditoría.' },
    ...
    instruccionesFuncionalesZuzu: [
      ...
      { id: 'EXP-7-SELECTS-ZUZU', regla: 'En v28.5.1_prod experimental, si planZuzu.selectsPropuestos contiene SELECTs válidos, ControlEvent intenta ejecutarlos literalmente como SELECT de solo lectura mediante ce_zuzu_select. Si modulosExtraidos.SELECTS_SQL_ZUZU existe, úsalo como fuente principal de esos SELECTs.' },
    ...
    instrucciones: {
      ...
      ingresos: 'INGRESOS usa la salida probada: Evento; Nombre; Numero; Importe obligatorio; Importe voluntario; Ingreso; Just.ing. En socios, Importe obligatorio = Numero * Precio del evento. Importe voluntario puede ser negativo como ajuste y debe conservarse literalmente.',
