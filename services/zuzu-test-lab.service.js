...
async function buildGoldenDialogueCases(state={}){
  ...
  const turns=[
    ...
    {prompt:'Vale, cambia de asunto: háblame de Colty y Esther.',tool:'resolve_entity',requiresTool:true,changeFocus:true,entities:['Colty','Esther']},
    {prompt:'¿En qué eventos aparecen?',tool:'query_ce',operations:['person_events'],persons:['Colty','Esther']},
...
export async function previewZuzuLanguageBattery({level='BASIC',seed}={}){
  ...
  return{ok:true,replayContractVersion:4,source:'language',batteryCode:golden?'GOLDEN-P117-110':goldenDialogue?'GOLDEN-DIALOG-P21-14':dialogue?'DIALOGUE-P21-24':`LANG-${normalizedLevel}-${profile.count}`,languageProfile:{...profile,goldenFixed:golden||goldenDialogue},generatedAt:nowIso(),seed:normalizedSeed,dataCounts:batteryDataCounts(state),tests:{FAST:0,'AI-SMOKE':0,'FULL-CERT':cases.length},cases:{FAST:[],'AI-SMOKE':[],'FULL-CERT':cases.map(c=>publicBatteryCase(c,'FULL-CERT'))},estimated:{'FULL-CERT':{turns:cases.length,costEurRange:`hasta ~${(profile.count*(dialogue?0.05:0.025)).toFixed(2).replace('.',',')} € según modelo/tokens`,hardCapSuggested:round(profile.count*(dialogue?0.05:0.025),2)}},notes:[golden?'GOLDEN 110: prompts y escenarios congelados desde P1.17; los oráculos se recalculan con el estado actual de CE.':goldenDialogue?'GOLDEN DIÁLOGO 14: conversación fija P2.1; mismas frases y mismo orden para comparar versiones sin cambiar de carretera.':dialogue?'DIÁLOGO P2.1: solo el primer mensaje es semilla; los siguientes los genera un usuario sintético leyendo la respuesta real y los artefactos visibles de Zuzu.':`Batería de alcance ${profile.label}: ${profile.count} preguntas.`,`Expectativa de partida: ${profile.expectedBand}.`,profile.description,'NHC: estas frases viven exclusivamente en ITV; no añaden reglas lingüísticas al runtime de Zuzu.',dialogue?'Cada turno adaptativo usa dos papeles IA separados: Zuzu + simulador ITV. Sus llamadas y costes se informan por separado.':'FULL-CERT ejecuta expresamente el motor VNext y conserva el historial visible dentro de cada escenario.','P2.1 conserva la separación de coste/llamadas de Zuzu y simulador y añade verificación semántica de la GOLDEN.']};
}
...
function p2GoldenDialogueAssessment(caseDef={},result={}){
  ...
  const personCalls=calls.filter(x=>trim(x?.tool)==='query_ce'&&!trim(x?.error)&&trim(x?.effectiveOperation||x?.normalizedArgs?.operation)==='person_events').map(x=>trim(x?.normalizedArgs?.person||x?.rawArgs?.person)).filter(Boolean);
  for(const person of arr(d.expectedPersons).map(trim).filter(Boolean))if(!personCalls.some(x=>norm(x)===norm(person)))reasons.push(`faltó consultar los eventos de ${person}`);
