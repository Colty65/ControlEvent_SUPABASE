...
function p124PresentationEvidence(result={}){const m=result?.meta?.presentationEvidence;if(m&&typeof m==='object')return m;const tables=arr(result?.tables).filter(Boolean).slice(0,8).map(t=>{const rows=arr(t?.rows),columns=arr(t?.columns).length?arr(t.columns).map(trim).filter(Boolean):Object.keys(rows[0]||{}).map(trim).filter(Boolean);return{key:trim(t?.key),title:trim(t?.title),rowCount:rows.length,columns:columns.slice(0,20)};});return{tableCount:tables.length,tables,chartCount:arr(result?.charts).filter(Boolean).length,materialized:tables.length>0||arr(result?.charts).length>0};}
