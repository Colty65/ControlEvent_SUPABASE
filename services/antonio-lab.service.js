...
const BUILD='ZUZU-LAB-V3.16.1-LIVE-PROTOCOL-DUAL-LIVE';
const LAB_TTS_VOICE='Iapetus';
export function antonioLabConfig(){return {ok:true,build:BUILD,configured:Boolean(geminiKey()),provider:'Gemini STT + VNext/CE + Gemini Live 3.1 Native Audio + Live 2.5 fallback',sttModel:model(),ttsModel:'gemini-3.1-flash-live-preview',voiceId:LAB_TTS_VOICE,voiceProfile:'adulta, clara, natural, cercana y poco teatral',fallbackVoiceId:LAB_TTS_VOICE,recoveryTtsModel:'gemini-2.5-flash-native-audio-preview-12-2025',wakeMode:'Activar = escucha continua; Dormir = requiere llamada para volver',paidNewServices:0,ttsTransport:'Live API WebSocket directo con token efímero · fallback Live 2.5',notes:'La boca normal no usa generateContent TTS. Live 3.1 es principal y Live 2.5 es respaldo.'}}


const LIVE_PRIMARY_MODEL='gemini-3.1-flash-live-preview';
const LIVE_FALLBACK_MODEL='gemini-2.5-flash-native-audio-preview-12-2025';
const LIVE_MODELS=new Set([LIVE_PRIMARY_MODEL,LIVE_FALLBACK_MODEL]);
export async function createAntonioLiveToken(options={}){
  ...
  // Token efímero de un solo uso. Sin setup incrustado: la configuración efectiva la manda el WebSocket del cliente.
  ...
  return {ok:true,token:clean(payload.name,1400),model:selected,voice:LAB_TTS_VOICE,expiresAt:expireTime};
}
