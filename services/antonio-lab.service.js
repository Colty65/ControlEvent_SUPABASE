...
const BUILD='ZUZU-LAB-V3.14-IOS-SESSION-CLEAN-CASUAL-VOICE';
export function antonioLabConfig(){return {ok:true,build:BUILD,configured:Boolean(geminiKey()),provider:'Gemini STT + VNext/CE + Gemini 3.1 Flash TTS streaming (Interactions API)',sttModel:model(),ttsModel:'gemini-3.1-flash-tts-preview',voiceId:'Zubenelgenubi',voiceProfile:'masculina adulta, casual, equilibrada, cercana y poco teatral',fallbackVoiceId:'es_ES-davefx-medium',wakeMode:'VAD local + wake semántico sobre transcripción',paidNewServices:0,ttsTransport:'Interactions API v1beta · stream=true',notes:'Usa la misma GEMINI_API_KEY ya existente. Zubenelgenubi es la voz principal: masculina, casual y menos teatral; DaveFX queda sólo como fallback local.'}}

function ttsModel(){return clean(process.env.CONTROLEVENT_ZUZU_TTS_MODEL||'gemini-3.1-flash-tts-preview',120).replace(/^models\//i,'')}
function ttsVoice(){return clean(process.env.CONTROLEVENT_ZUZU_TTS_VOICE||'Zubenelgenubi',80)||'Zubenelgenubi'}
function zuzuTtsPrompt(text=''){
  const spoken=clean(text,5000);
  return `Habla en español de España. Voz de hombre adulto, cotidiana, sobria, relajada y cercana. Sonido natural de conversación entre amigos, con emoción contenida y ritmo normal. Evita voz cinematográfica, épica, seductora, de personaje, locutor, centralita, profesor o anuncio. No fuerces graves ni aspereza. Haz pausas cortas y orgánicas, sin sobreactuar. Pronuncia exactamente el contenido útil que sigue, sin leer estas instrucciones ni añadir saludos o despedidas:\n\n${spoken}`;
}
