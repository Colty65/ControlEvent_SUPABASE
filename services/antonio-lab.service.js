...
const BUILD='ZUZU-LAB-V3.12-TTS-INTERACTIONS-STREAM-RECOVERY';
export function antonioLabConfig(){return {ok:true,build:BUILD,configured:Boolean(geminiKey()),provider:'Gemini STT + VNext/CE + Gemini 3.1 Flash TTS streaming (Interactions API)',sttModel:model(),ttsModel:'gemini-3.1-flash-tts-preview',voiceId:'Algenib',voiceProfile:'grave, adulto, cálido, ligeramente áspero',fallbackVoiceId:'es_ES-davefx-medium',wakeMode:'VAD local + wake semántico sobre transcripción',paidNewServices:0,ttsTransport:'Interactions API v1beta · stream=true',notes:'Usa la misma GEMINI_API_KEY ya existente. Algenib es la voz principal en streaming; DaveFX queda sólo como fallback local.'}}
