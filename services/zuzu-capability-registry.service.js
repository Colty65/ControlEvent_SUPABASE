...
const P={
  ...
  visible_columns:{type:'array',items:{type:'string'}},hidden_columns:{type:'array',items:{type:'string'}},view_filters:{type:'array',description:'Filtros de la vista actual. Varios filtros positivos (eq/contains) sobre la MISMA columna son alternativas OR; filtros sobre columnas distintas se combinan AND.',items:{type:'object',properties:{field:{type:'string'},operator:{type:'string',enum:['eq','neq','contains','not_contains']},value:{type:'string'}},required:['field','operator','value']}},remove_view_filters:{type:'array',description:'Retira filtros concretos sin tocar los demás.',items:{type:'object',properties:{field:{type:'string'},operator:{type:'string',enum:['eq','neq','contains','not_contains']},value:{type:'string'}},required:['field']}},reset_filters:{type:'boolean',description:'true SOLO si el usuario pide explícitamente quitar/restaurar TODOS los filtros o recuperar todas las filas. Ordenar o recuperar columnas NO resetea filtros.'},view_sort:{type:'array',description:'Orden de la vista. Usa siempre view_sort; nunca inventes sort ni order_by para view_current.',items:{type:'object',properties:{field:{type:'string'},direction:{type:'string',enum:['asc','desc']}},required:['field']}},reset_table:{type:'boolean',description:'Restaura por completo la vista original solo si el usuario lo pide explícitamente.'},
  ...
  derive_operation:{type:'string',description:'Operación algebraica. Para preguntas como cuál es el mayor/más alto usa MAX; menor usa MIN; ranking/top usa RANK; suma usa SUM; promedio AVG; conteo COUNT.',enum:['SUM','COUNT','DISTINCT_COUNT','MAX','MIN','AVG','RANK','DIFFERENCE']},field:{type:'string',description:'Columna métrica sobre la que se calcula. En una tabla Indicador/Valor, field=Valor.'},derive_field:{type:'string',description:'Alias compatible de field; preferir field.'},label_field:{type:'string',description:'Columna que identifica la fila ganadora. En una tabla Indicador/Valor, label_field=Indicador.'},table_key:{type:'string'},top_n:{type:'integer'},
...
const OP_DESCRIPTIONS={
  ...
  derive:'Calcula SUM/COUNT/DISTINCT_COUNT/MAX/MIN/AVG/RANK/DIFFERENCE sobre un dataset factual anterior. Para «cuál tiene el valor más alto/mayor» usa derive_operation=MAX, field=columna numérica y label_field=columna descriptiva; NO sustituyas esa pregunta por view_current+sort. La procedencia vive en context/source_args.',
  ...
  view_current:'Transforma o vuelve a mostrar únicamente la vista del dataset actual. view_filters positivos de una misma columna son OR. Cambiar columnas u orden conserva los filtros existentes. reset_filters solo cuando el usuario pide explícitamente quitar todos los filtros. Para responder máximos/mínimos usa derive, no view_current.',
...
export function auditCapabilityCall(rawArgs={}){
  ...
  // P2.1 · tolerancias PURAMENTE ESTRUCTURALES: aliases de forma, nunca de intención.
  if(sanitized.sort!==undefined){if(sanitized.view_sort===undefined&&Array.isArray(sanitized.sort)){sanitized.view_sort=sanitized.sort;classification=mark(repairs,'sort → view_sort',classification,'COMPATIBLE');}else classification=mark(repairs,'sort redundante descartado; manda view_sort',classification,'COMPATIBLE');delete sanitized.sort;}
