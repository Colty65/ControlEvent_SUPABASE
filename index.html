/* ControlEvent v28.7.1 - Carga diferida por pantalla sin tocar operativa estable */
import { VERSION } from '../version.js';

const SCREEN_LAZY_VERSION = 'v28.7.1';
const state = {
...
async function activateScreen(name, options = {}){
  ...
  try{
    ...
    state.activations.push(item);
    console.warn('[ControlEventScreenLazy/v28.7.1] No se pudo activar pantalla', name, error);
    return item;
  }
}
...
function print(){
  const report = info();
  console.group('[ControlEventScreenLazy/v28.7.1] Carga diferida por pantalla');
