...
function ticketTokenForImageKey(value) {
  const match = text(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().match(/\bTK\s*0*([0-9]{1,4})\b/);
...
async function matchingTicketImageRows(eventId, key) {
  const ev = text(eventId).trim();
  const raw = text(key).trim();
  if (!ev || !raw) return [];
  const exact = ticketImageKeyForEvent(ev, raw);
  const rawNoEvent = raw.startsWith(`${ev}|`) ? raw.slice(ev.length + 1) : raw;
  const tk = ticketTokenForImageKey(rawNoEvent || raw);
  ...
  for (const row of data || []) {
    ...
    let match = imageKey === exact || imageKey === raw || label === rawNoEvent || label === raw || base === rawNoEvent || base === raw;
    if (!match && tk) {
...
export async function uploadTicketImage({ eventId, key, dataUrl, eventSnapshot }) {
  ...
  // v16 OPT3N: si se sube una foto de un TKxx, primero elimina todas las variantes antiguas
  // de ese mismo TK en el evento. Así no quedan miniaturas repetidas ni vuelve la foto anterior.
  let deletedBeforeUpload = 0;
  if (ticketTokenForImageKey(key)) {
    const oldRows = await matchingTicketImageRows(ev, key);
    if (oldRows.length) {
      const result = await deleteTicketImageRows(oldRows, '[v16 OPT3N ticket-image-replace]');
      deletedBeforeUpload = result.deletedRows || result.deleted || oldRows.length;
    }
  }
...
export async function deleteTicketImage({ eventId, key }) {
  ...
  const rows = await matchingTicketImageRows(ev, key);
  if (rows.length) {
    return deleteTicketImageRows(rows, '[v16 OPT3N ticket-image-delete]');
  }
  ...
  if (error) throw error;
  return { deletedRows: 0, storageDeleted: 0 };
}
...
export async function imagesForEvent(eventId) {
  ...
  for (const img of data || []) {
    ...
    const url = imageCacheBusted(img.public_url || img.pathname || img.storage_path, stamp);
    const pathname = imageCacheBusted(img.pathname || img.public_url || img.storage_path, stamp);
