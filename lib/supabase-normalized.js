...
async function rpcProductCatalogUpdate(rowId, patch = {}) {
  ...
  const { data, error } = await db().rpc('ce_update_producto_catalogo_libre', args);
  if (error) {
    const msg = text(error.message || error.details || error.hint || error.code).trim();
    if (/ce_update_producto_catalogo_libre|could not find|does not exist|pgrst202/i.test(msg)) return null;
    throw error;
  }
  return Array.isArray(data) ? data[0] : data;
}
...
async function productCatalogUpdateViaRpcOrThrow(rowId, patch = {}) {
  ...
  if (!data?.id) {
    const err = new Error('No está disponible la RPC ce_update_producto_catalogo_libre. Ejecuta ControlEvent_SQL_FIX_PRODUCTOS_CATALOGO_LIBRE.sql en Supabase SQL Editor y vuelve a probar.');
    err.status = 409;
    throw err;
  }
  return data;
...
export async function upsertCrudRecord(collection, payload) {
  ...
  if (key === 'productos') {
    ...
    if (existsError) throw existsError;
    if (existing?.id) {
      const sameName = text(existing.nombre).trim() === text(row.nombre).trim();
      // Mantenimientos PRODUCTOS: el precio/tienda/segmento/destino general del catálogo
      // NO son el precio histórico usado en eventos. Se actualizan por RPC para evitar
      // el trigger CE_BLOQUEO_FINALIZADO de la tabla ce_productos.
      if (sameName) {
