...
async function removeStoragePaths(paths, logPrefix = '[v8.4.1]') {
...
async function deleteTicketImageRows(rows, logPrefix = '[v8.4.1]') {
...
export async function deleteTicketImagesForEvent(eventId) {
  ...
  const result = await deleteTicketImageRows([...byKey.values()], '[v8.4.1]');
...
export async function cleanupOrphanTicketImages(liveEventIds = null) {
  ...
  const result = await deleteTicketImageRows(orphans, '[v8.4.1 orphan-cleanup]');
  console.warn(`[v8.4.1] Limpieza CE_TICKET_IMAGES: ${result.deletedRows} filas huérfanas eliminadas.`);
