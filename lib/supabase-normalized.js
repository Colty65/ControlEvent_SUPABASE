...
async function saveImagesFromState(state) {
  ...
  for (const [rawKey, value] of Object.entries(images)) {
    if (!value) continue;
    const key = canonicalImageKey(rawKey, value);
    // No persistir claves globales TKxx sin evento; son las que mezclaban fotos entre eventos.
    if (!key.includes('|') && hasTicketToken(key)) continue;
