...
export const META_KEYS = ['selectedEventId', 'comprasSort', 'summaryTiendaSort', 'eventCodeMap', 'entityCodeMaps', 'eventDocuments', 'eventDocumentMeta'];
export const TABLES = [
  'ce_users',
  'ce_eventos',
  'ce_personas',
...
function decodeBase64UrlText(value) {
  const raw = text(value).trim().replace(/\.[a-z0-9]+(?:\?.*)?$/i, '').split('.v')[0];
  if (!raw) return '';
...
function storagePathFor(key, ext) {
  ...
  return `${imageEventId(key).replace(/[^a-zA-Z0-9_-]/g, '_')}/${safeFilePart(key)}.v${token}.${ext || 'jpg'}`;
}
function cacheBustedUrl(value, storagePath) {
  const src = text(value).trim();
  const token = text(storagePath).trim().split('.v').pop()?.replace(/\.[a-z0-9]+$/i, '') || Date.now().toString(36);
  if (!src || /^data:/i.test(src)) return src;
  const sep = src.includes('?') ? '&' : '?';
  return `${src}${sep}ce_img=${encodeURIComponent(token)}`;
}
...
async function selectTicketImageRowsByKeys(keys) {
  const cleanKeys = [...new Set(arr(keys).map(k => text(k).trim()).filter(Boolean))];
  const rows = [];
  for (let i = 0; i < cleanKeys.length; i += 100) {
    const chunk = cleanKeys.slice(i, i + 100);
...
async function removeReplacedStoragePaths(previousRows, newRows, logPrefix = '[v17 fotos]') {
  const nextPaths = new Set(arr(newRows).map(row => text(row?.storage_path).trim()).filter(Boolean));
  const oldPaths = arr(previousRows)
    .map(row => text(row?.storage_path).trim())
    .filter(path => path && !nextPaths.has(path));
  return removeStoragePaths(oldPaths, logPrefix);
}
async function deleteReplacedAliasRows(previousRows, newRows, logPrefix = '[v17 aliases]') {
  const nextKeys = new Set(arr(newRows).map(row => text(row?.image_key).trim()).filter(Boolean));
  const aliases = arr(previousRows).filter(row => {
    const key = text(row?.image_key).trim();
    return key && !nextKeys.has(key);
  });
  if (!aliases.length) return { deletedRows: 0, storageDeleted: 0 };
  return deleteTicketImageRows(aliases, logPrefix);
}
function ticketImageDeleteCandidates(eventId, key) {
  const ev = text(eventId).trim();
  const raw = text(key).trim();
  const out = [];
  const add = value => {
  ...
  const withoutEvent = ev && raw.startsWith(`${ev}|`) ? raw.slice(ev.length + 1) : raw;
  const label = imageLabel(raw);
  [raw, withoutEvent, label].forEach(value => {
    add(value);
    if (ev) add(ticketImageKeyForEvent(ev, value));
  });
  const ingresoMatch = [raw, withoutEvent, label].map(value => text(value).match(/INGRESO[:|]([^|\s]+)/i)).find(Boolean);
  if (ingresoMatch) {
    const id = text(ingresoMatch[1]).trim();
    [`INGRESO:${id}`, `INGRESO|${id}`, ev ? `INGRESO:${ev}|${id}` : '', ev ? `${ev}|INGRESO:${id}` : '', ev ? `${ev}|INGRESO|${id}` : ''].forEach(add);
  }
  const doc = documentCodeFrom(raw) || documentCodeFrom(withoutEvent) || documentCodeFrom(label);
  if (doc) {
    add(doc);
    if (ev) add(`${ev}|${doc}`);
  }
  const tk = ticketTokenFrom(raw) || ticketTokenFrom(withoutEvent) || ticketTokenFrom(label);
  if (tk && ev) {
    add(`${ev}|${tk}`);
    const parts = withoutEvent.split('|').map(part => text(part).trim()).filter(Boolean);
    if (parts.length >= 2) {
      add(`${ev}|${parts[0]} | ${tk}`);
      add(`${ev}|${parts[0]}|${tk}`);
    }
...
function eventToDb(row) {
  return {
    id: id(row),
    titulo: text(row.titulo ?? row.nombre ?? row.name),
    nombre_hablado: nullableText(row.nombreHablado ?? row.nombre_hablado ?? row.tituloVoz ?? row.titulo_voz),
    precio: money(row.precio),
    fecha_ini: text(row.fechaIni ?? row.fecha_ini),
    fecha_fin: text(row.fechaFin ?? row.fecha_fin),
    situacion: text(row.situacion || row.estado || 'En curso'),
...
function eventFromDb(row) {
  return {
    id: row.id,
    titulo: row.titulo || '',
    nombreHablado: row.nombre_hablado || '',
...
function personToDb(row) {
  return {
    ...
    rango: text(row.rango || 'SOCIO').toUpperCase(),
    nombre_amigo: nullableText(row.nombreAmigo ?? row.nombre_amigo)
  };
}
function personFromDb(row) {
  return {
    id: row.id,
    nombre: row.nombre || '',
    rango: row.rango || 'SOCIO',
    nombreAmigo: row.nombre_amigo || '',
    createdAt: row.created_at || '',
    updatedAt: row.updated_at || ''
  };
}
function personAliasFromDb(row) {
  return {
    personaId: row.persona_id || '',
    alias: row.alias || '',
    prioridad: Number(row.prioridad ?? 50),
    preferido: row.es_preferido === true,
...
function aliasNorm(value) {
  ...
  if (!raw) return '';
  try { return raw.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9ñ]+/g, ' ').replace(/\s+/g, ' ').trim(); }
  catch (_) { return raw.toLowerCase().replace(/\s+/g, ' ').trim(); }
}
function aliasesFromInput(input = {}) {
  const hasAliases = Object.prototype.hasOwnProperty.call(input, 'aliases') || Object.prototype.hasOwnProperty.call(input, 'otrosAlias') || Object.prototype.hasOwnProperty.call(input, 'otros_alias');
  const hasFriend = Object.prototype.hasOwnProperty.call(input, 'nombreAmigo') || Object.prototype.hasOwnProperty.call(input, 'nombre_amigo');
  if (!hasAliases && !hasFriend) return null;
  const preferred = text(input.nombreAmigo ?? input.nombre_amigo).trim();
  const raw = input.aliases ?? input.otrosAlias ?? input.otros_alias ?? [];
  const source = Array.isArray(raw) ? raw : text(raw).split(/[;,\n]+/);
  const rows = [];
  const seen = new Set();
  function add(value, prioridad = 50, preferido = false) {
    const alias = text(typeof value === 'object' ? (value.alias ?? value.value) : value).trim();
    const key = aliasNorm(alias);
    if (!alias || !key || seen.has(key)) return;
    seen.add(key);
    const pr = Number(typeof value === 'object' ? value.prioridad : prioridad);
    rows.push({ alias, prioridad: Number.isFinite(pr) ? pr : prioridad, preferido: !!(typeof value === 'object' ? value.preferido : preferido) });
  }
  if (preferred) add(preferred, 10, true);
  source.forEach(value => add(value, 50, false));
  return { preferred, rows };
}
async function replacePersonAliasesCrud(personaId, input = {}) {
  const parsed = aliasesFromInput(input);
  if (!parsed) return null;
  const pid = text(personaId).trim();
  if (!pid) return [];
  let res = await db().from('ce_persona_aliases').delete().eq('persona_id', pid);
  if (res.error) {
    const err = new Error('No está disponible ce_persona_aliases. Ejecuta ControlEvent_SQL_BANK4_20_PERSONAS_SOCIALES.sql en Supabase SQL Editor.');
    err.status = 409;
    err.cause = res.error;
    throw err;
  }
  const dbRows = parsed.rows.map(row => ({ persona_id: pid, alias: row.alias, prioridad: row.prioridad, es_preferido: row.preferido, activo: true }));
  if (dbRows.length) {
    res = await db().from('ce_persona_aliases').upsert(dbRows, { onConflict: 'persona_id,alias' });
    if (res.error) throw res.error;
  }
  // El alias preferido vive también en ce_personas para consulta humana rápida.
  // Si un trigger histórico impide tocar esa columna, la tabla de aliases sigue siendo
  // la fuente operativa y el estado la proyecta como nombreAmigo.
  try {
    const mirror = await db().from('ce_personas').update({ nombre_amigo: parsed.preferred || null }).eq('id', pid);
    if (mirror.error) console.warn('[BANK4_20 nombre_amigo] No se pudo reflejar en ce_personas; ce_persona_aliases sigue autoritativa:', mirror.error.message || mirror.error);
  } catch (error) {
    console.warn('[BANK4_20 nombre_amigo] Reflejo no disponible:', error?.message || error);
  }
  return dbRows.map(personAliasFromDb);
}
function storeToDb(row) {
  return { id: id(row), nombre: text(row.nombre) };
}
function storeFromDb(row) {
  return { id: row.id, nombre: row.nombre || '', createdAt: row.created_at || '', updatedAt: row.updated_at || '' };
}
...
function productFromDb(row) {
  const out = {
    ...
    precio: Number(row.default_precio || 0),
    createdAt: row.created_at || '',
    updatedAt: row.updated_at || ''
  };
...
function eventPersonSnapshotFromDb(row) {
  return {
    eventId: row.event_id || '',
    personaId: row.persona_id || '',
    nombreSnapshot: row.nombre_snapshot || '',
    rangoSnapshot: String(row.rango_snapshot || 'SOCIO').toUpperCase(),
...
function purchaseToDb(row) {
  return {
    ...
    donacion_situacion: nullableText(row.donacionSituacion || row.donacion_situacion)
  };
}
function purchaseFromDb(row) {
  return {
    ...
    donacionSituacion: row.donacion_situacion || '',
    createdAt: row.created_at || '',
    updatedAt: row.updated_at || ''
  };
}
...
async function selectPaged(table, { order = 'id', columns = '*', apply = null } = {}) {
  // FIX44/FIX48: Supabase/PostgREST devuelve como máximo 1000 filas por defecto.
  // Cualquier lectura de tabla debe paginar, también cuando se filtra por event_id.
  const pageSize = 1000;
  let from = 0;
  const out = [];
...
async function selectAll(table, order = 'id', columns = '*') {
  return selectPaged(table, { order, columns });
}

async function selectByEventId(table, eventId, order = 'id', columns = '*') {
  const ev = text(eventId).trim();
  if (!ev) return [];
  return selectPaged(table, { order, columns, apply: query => query.eq('event_id', ev) });
}

async function selectTicketImagesForEvent(eventId) {
  const ev = text(eventId).trim();
  if (!ev) return [];
  const columns = 'image_key,event_id,label,public_url,pathname,storage_path,content_type,size_bytes';
  const direct = await selectPaged('ce_ticket_images', {
    order: 'image_key',
    columns,
    apply: query => query.eq('event_id', ev)
  });
  // Compatibilidad con filas antiguas que tuvieran event_id vacío pero image_key con prefijo de evento.
  const prefixed = await selectPaged('ce_ticket_images', {
    order: 'image_key',
    columns,
    apply: query => query.like('image_key', `${ev}|%`)
  });
  const byKey = new Map();
  for (const row of [...direct, ...prefixed]) {
    const key = text(row?.image_key).trim();
    if (key) byKey.set(key, row);
  }
  return [...byKey.values()].sort((a, b) => text(a.image_key).localeCompare(text(b.image_key)));
}

function scopedMetaValue(key, value, eventId) {
  const ev = text(eventId).trim();
  if (!ev) return value;
  if (key === 'selectedEventId') return ev;
  if (key === 'eventDocuments' && Array.isArray(value)) {
    return value.filter(doc => text(doc?.eventId).trim() === ev);
  }
  if (key === 'eventDocumentMeta' && value && typeof value === 'object' && !Array.isArray(value)) {
    return Object.fromEntries(Object.entries(value).filter(([k]) => text(k).trim().startsWith(`${ev}|`)));
  }
  return value;
}
...
async function uploadDataUrlImage(key, dataUrl) {
  ...
  const publicUrl = cacheBustedUrl(pub?.publicUrl || storagePath, storagePath);
  return {
    image_key: key,
...
async function saveImagesFromState(state) {
  ...
  for (const [rawKey, value] of Object.entries(images)) {
    ...
    if (isDataUrl(value)) {
      const uploaded = await uploadDataUrlImage(key, value);
      uploadedKeys.add(text(uploaded?.image_key).trim());
      rows.push(uploaded);
    } else {
      rows.push(refToImageRow(key, value));
    }
  }
  ...
  const freshRows = safeRows.filter(row => uploadedKeys.has(text(row?.image_key).trim()) && text(row?.storage_path).trim());
  const previousRows = freshRows.length ? await selectTicketImageRowsByKeys(freshRows.map(row => row.image_key)) : [];
  const { error } = await db().from('ce_ticket_images').upsert(safeRows, { onConflict: 'image_key' });
  if (error) throw error;
  if (previousRows.length) await removeReplacedStoragePaths(previousRows, freshRows, '[v17 save-images]');
  return safeRows.length;
}

async function stateFromDbParallel(options = {}) {
  const scopedEventId = text(typeof options === 'string' ? options : options?.eventId).trim();
  const bootOnly = !scopedEventId && options && typeof options === 'object' && options.boot === true;
  const state = {};

  // VNext P1.2: las lecturas independientes de Supabase se lanzan en paralelo. La versión
  // legacy conserva stateFromDb secuencial; esta ruta se activa solo con {parallel:true}.
  const basePromise = Promise.all(COLLECTIONS.map(async collection => {
    const [table, , , fromDb] = tableMap[collection];
    const isEventHeavy = collection === 'colaboradores' || collection === 'compras';
    const rows = bootOnly && isEventHeavy
      ? []
      : (scopedEventId && isEventHeavy ? await selectByEventId(table, scopedEventId) : await selectAll(table));
    return [collection, fromRows(rows, fromDb)];
  }));
  const aliasPromise = selectAll('ce_persona_aliases', 'persona_id', 'persona_id,alias,prioridad,es_preferido,activo,created_at,updated_at')
    .then(rows => ({ ok:true, rows }))
    .catch(error => ({ ok:false, error, rows:[] }));
  const snapshotPromise = (bootOnly
      ? Promise.resolve([])
      : (scopedEventId ? selectByEventId('ce_event_person_snapshots', scopedEventId, 'persona_id') : selectAll('ce_event_person_snapshots', 'event_id')))
    .then(rows => ({ ok:true, rows }))
    .catch(error => ({ ok:false, error, rows:[] }));
  const metaPromise = selectAll('ce_meta', 'key');
  const imagesPromise = (bootOnly
      ? Promise.resolve([])
      : (scopedEventId
        ? selectTicketImagesForEvent(scopedEventId)
        : selectAll('ce_ticket_images','image_key','image_key,event_id,label,public_url,pathname,storage_path,content_type,size_bytes')));

  const [baseEntries, aliasLoad, snapshotLoad, meta, images] = await Promise.all([
    basePromise, aliasPromise, snapshotPromise, metaPromise, imagesPromise
  ]);
  for (const [collection, rows] of baseEntries) state[collection] = rows;

  if (!aliasLoad.ok) {
    console.warn('[BANK4_20] ce_persona_aliases no disponible:', aliasLoad.error?.message || aliasLoad.error);
    state.personAliases = [];
    state.__personAliasesMissing = true;
  } else state.personAliases = fromRows(aliasLoad.rows, personAliasFromDb);

  const aliasesByPerson = new Map();
  for (const alias of state.personAliases) {
    const pid = text(alias?.personaId).trim();
    if (!pid || alias?.activo === false) continue;
    if (!aliasesByPerson.has(pid)) aliasesByPerson.set(pid, []);
    aliasesByPerson.get(pid).push(alias);
  }
  state.personas = arr(state.personas).map(person => {
    const rows = (aliasesByPerson.get(text(person?.id).trim()) || []).slice().sort((a,b) => Number(a.prioridad||50) - Number(b.prioridad||50));
    const preferred = rows.find(row => row.preferido)?.alias || person?.nombreAmigo || '';
    return { ...person, nombreAmigo: preferred, aliases: rows.map(row => row.alias).filter(alias => aliasNorm(alias) !== aliasNorm(preferred)) };
  });

  if (!snapshotLoad.ok) {
    console.warn('[FIX9.3] ce_event_person_snapshots no disponible:', snapshotLoad.error?.message || snapshotLoad.error);
    state.eventPersonSnapshots = [];
    state.__eventPersonSnapshotsMissing = true;
  } else state.eventPersonSnapshots = fromRows(snapshotLoad.rows, eventPersonSnapshotFromDb);

  const snapshotIndex = new Map((state.eventPersonSnapshots || []).map(row => [`${row.eventId}|${row.personaId}`, row]));
  const personIndex = new Map((state.personas || []).map(row => [String(row.id || ''), row]));
  const eventIndex = new Map((state.eventos || []).map(row => [String(row.id || ''), row]));
  state.colaboradores = (state.colaboradores || []).map(row => {
    const snap = snapshotIndex.get(`${row.eventId}|${row.personaId}`);
    if (!snap) return row;
    const current = personIndex.get(String(row.personaId || '')) || {};
    const event = eventIndex.get(String(row.eventId || '')) || {};
    const rango = String(snap.rangoSnapshot || 'SOCIO').toUpperCase();
    const base = rango === 'SOCIO' ? Number(row.numero || 0) * Number(event.precio || 0) : 0;
    const donation = Number(row.importe || 0);
    return { ...row, personaNombreSnapshot:snap.nombreSnapshot, personaRangoSnapshot:rango, personaNombre:snap.nombreSnapshot, personaRango:rango, rango, persona:{...current,id:row.personaId,nombre:snap.nombreSnapshot,rango,__historicalSnapshot:true}, base, donation, total:base+donation, __ceHistoricalPerson:true };
  });

  for (const row of meta) {
    if (bootOnly && /document|ticket|image|foto/i.test(text(row.key))) { state[row.key] = Array.isArray(row.value) ? [] : null; continue; }
    state[row.key] = scopedMetaValue(row.key, row.value, scopedEventId);
  }
  if (scopedEventId) state.selectedEventId = scopedEventId;
  else if (bootOnly) state.selectedEventId = '';
  ...
  for (const img of images || []) {
    ...
    state.ticketImageRefs[img.image_key] = { key:img.image_key, url:img.public_url||value, pathname:img.pathname||value, contentType:img.content_type||'', size:img.size_bytes||0 };
  }
  if (scopedEventId) { state.__eventScoped = true; state.__eventScopedId = scopedEventId; }
  if (bootOnly) { state.__bootScoped = true; state.__eventScoped = false; state.__eventScopedId = ''; }
  state.__parallelRead = true;
  return state;
}

export async function stateFromDb(options = {}) {
  if (options && typeof options === 'object' && options.parallel === true) return stateFromDbParallel({ ...options, parallel:false });
  const scopedEventId = text(typeof options === 'string' ? options : options?.eventId).trim();
  const bootOnly = !scopedEventId && options && typeof options === 'object' && options.boot === true;
  const state = {};
  ...
  for (const collection of COLLECTIONS) {
    ...
    const rows = bootOnly && isEventHeavy
      ? []
      : (scopedEventId && isEventHeavy ? await selectByEventId(table, scopedEventId) : await selectAll(table));
    state[collection] = fromRows(rows, fromDb);
  }

  // BANK4_20: identidad social persistente. Se carga siempre porque es catálogo pequeño
  // y sirve tanto a texto como a voz antes del fuzzy general de PERSONAS.
  try {
    const aliasRows = await selectAll('ce_persona_aliases', 'persona_id', 'persona_id,alias,prioridad,es_preferido,activo,created_at,updated_at');
    state.personAliases = fromRows(aliasRows, personAliasFromDb);
  } catch (error) {
    console.warn('[BANK4_20] ce_persona_aliases no disponible:', error?.message || error);
    state.personAliases = [];
    state.__personAliasesMissing = true;
  }
  const aliasesByPerson = new Map();
  ...
  // FIX9.3: fotografía histórica de PERSONAS por EVENTO. No es una colección CRUD
  // del navegador; la mantienen triggers SQL y se consulta para cálculos históricos.
  try {
    const snapshotRows = bootOnly
      ? []
      : (scopedEventId
          ? await selectByEventId('ce_event_person_snapshots', scopedEventId, 'persona_id')
          : await selectAll('ce_event_person_snapshots', 'event_id'));
    state.eventPersonSnapshots = fromRows(snapshotRows, eventPersonSnapshotFromDb);
  } catch (error) {
    // Compatibilidad temporal durante el despliegue: la app sigue cargando, pero la
    // función histórica no queda activa hasta ejecutar el SQL de FIX9.3.
    console.warn('[FIX9.3] ce_event_person_snapshots no disponible:', error?.message || error);
    state.eventPersonSnapshots = [];
    state.__eventPersonSnapshotsMissing = true;
  }
  ...
  state.colaboradores = (state.colaboradores || []).map(row => {
    ...
    return {
      ...row,
      personaNombreSnapshot: snap.nombreSnapshot,
      personaRangoSnapshot: rango,
      personaNombre: snap.nombreSnapshot,
      personaRango: rango,
      rango,
      persona: {...current, id: row.personaId, nombre: snap.nombreSnapshot, rango, __historicalSnapshot: true},
      base,
      donation,
      total: base + donation,
  ...
  const images = bootOnly
    ? []
    : (scopedEventId
      ? await selectTicketImagesForEvent(scopedEventId)
      : await selectAll(
          'ce_ticket_images',
          'image_key',
          'image_key,event_id,label,public_url,pathname,storage_path,content_type,size_bytes'
        ));
  state.ticketImages = {};
  state.ticketImageRefs = {};
  for (const img of images || []) {
  ...
  if (scopedEventId) {
    state.__eventScoped = true;
    state.__eventScopedId = scopedEventId;
  }
  if (bootOnly) {
    state.__bootScoped = true;
    state.__eventScoped = false;
    state.__eventScopedId = '';
  }
  return state;
}

export async function stateFromDbForEvent(eventId) {
  return stateFromDb({ eventId });
}
...
async function isProductCatalogUpdateAllowed(row) {
  const idv = text(row?.id).trim();
  if (!idv) return false;
  const { data, error } = await db().from('ce_productos').select('id,nombre,segmento,destino').eq('id', idv).maybeSingle();
  if (error) throw error;
  if (!data?.id) return false;
  // Catálogo PRODUCTOS: el precio general/default_tienda_id NO forma parte del dato histórico
  // de compras/donaciones ya guardadas en eventos. Aunque el producto haya aparecido en un
  // evento Finalizado, debe poder corregirse/subirse su precio de catálogo para eventos futuros.
  // Mantenemos protegido el cambio de identidad maestra desde mantenimientos normales.
  return text(data.nombre).trim() === text(row.nombre).trim()
    && text(data.segmento).trim() === text(row.segmento).trim()
    && text(data.destino).trim() === text(row.destino).trim();
}

function normalizeEventDateForCompare(value) {
  const raw = text(value).trim();
  if (!raw) return '';
...
function sameEventDbField(a, b, numeric = false, date = false) {
  if (numeric) return Math.round(Number(a || 0) * 100) === Math.round(Number(b || 0) * 100);
  if (date) return normalizeEventDateForCompare(a) === normalizeEventDateForCompare(b);
  return text(a).replace(/\s+/g, ' ').trim() === text(b).replace(/\s+/g, ' ').trim();
}
function eventUpdateOnlyChangesSituation(existing, incoming) {
  return sameEventDbField(existing?.titulo, incoming?.titulo)
    ...
    && sameEventDbField(existing?.fecha_ini, incoming?.fecha_ini, false, true)
    && sameEventDbField(existing?.fecha_fin, incoming?.fecha_fin, false, true)
...
async function assertCrudAllowed(collection, rowOrId, action, table = '', pk = 'id') {
  ...
  if ((collection === 'personas' || collection === 'tiendas') && row && !isDelete) {
    // Tablas maestras generales: editar nombre/rango de PERSONAS o nombre de TIENDAS
    // no debe depender del evento seleccionado ni de que existan eventos históricos Finalizados.
    // La protección se mantiene en las BAJAS, porque borrar sí afectaría a históricos.
    return;
  }

  if (collection === 'productos' && row && !isDelete && await isProductCatalogUpdateAllowed(row)) {
    return;
  }
...
async function selectTicketImageRowsForEventTicket(eventId, key) {
  const ev = text(eventId).trim();
  const raw = text(key).trim();
  const candidates = ticketImageDeleteCandidates(ev, raw);
  const byKey = new Map();
  const collect = async (query) => {
    const { data, error } = await query;
    if (error) throw error;
    for (const row of data || []) {
      const imageKey = text(row?.image_key).trim();
      if (imageKey) byKey.set(imageKey, row);
    }
  };
  for (let i = 0; i < candidates.length; i += 100) {
    await collect(db().from('ce_ticket_images').select('image_key,event_id,label,storage_path').in('image_key', candidates.slice(i, i + 100)));
  }
  const labels = [...new Set(candidates.flatMap(candidate => [candidate, imageLabel(candidate), stripKnownEventPrefix(candidate, ev)]).map(label => text(label).trim()).filter(Boolean))];
  for (let i = 0; ev && i < labels.length; i += 100) {
    await collect(db().from('ce_ticket_images').select('image_key,event_id,label,storage_path').eq('event_id', ev).in('label', labels.slice(i, i + 100)));
  }
  const tk = ticketTokenFrom(raw) || ticketTokenFrom(imageLabel(raw));
  if (ev && tk) {
    // FIX final v17: para Cálculos por tienda/ticket el TKxx es la unidad contable de foto.
    // Se eliminan también claves/labels antiguas del mismo evento que contengan el TKxx,
    // aunque no coincidan exactamente con el texto actual de la fila.
    await collect(db().from('ce_ticket_images').select('image_key,event_id,label,storage_path').eq('event_id', ev).ilike('image_key', `%${tk}%`));
    await collect(db().from('ce_ticket_images').select('image_key,event_id,label,storage_path').eq('event_id', ev).ilike('label', `%${tk}%`));
  }
  return [...byKey.values()];
}
...
export async function uploadTicketImage({ eventId, key, dataUrl, eventSnapshot }) {
  ...
  const imageKey = ticketImageKeyForEvent(ev, key);
  const previousRows = await selectTicketImageRowsForEventTicket(ev, imageKey);
  const row = await uploadDataUrlImage(imageKey, dataUrl);
  ...
  if (error) throw error;
  await deleteReplacedAliasRows(previousRows, [row], '[v17 upload-aliases]');
  await removeReplacedStoragePaths(previousRows.filter(prev => text(prev?.image_key).trim() === row.image_key), [row], '[v17 upload-image]');
  return { key: row.image_key, url: row.public_url, pathname: row.pathname };
}

export async function deleteTicketImage({ eventId, key }) {
  const ev = text(eventId).trim();
  await assertEventNotFinalized(ev, 'eliminar foto');
  const rows = await selectTicketImageRowsForEventTicket(ev, key);
  if (!rows.length) return { deletedRows: 0, storageDeleted: 0 };
  return deleteTicketImageRows(rows, '[v17 delete-image-tk]');
}
...
async function removeEventDocumentsFromMeta(eventId) {
  ...
  const metaRows = [{ key: 'eventDocuments', value: next }];
  try {
    const { data: metaData, error: metaError } = await db().from('ce_meta').select('value').eq('key', 'eventDocumentMeta').maybeSingle();
    if (!metaError && metaData?.value && typeof metaData.value === 'object' && !Array.isArray(metaData.value)) {
      const nextMeta = Object.fromEntries(Object.entries(metaData.value).filter(([k]) => !text(k).trim().startsWith(`${ev}|`)));
      metaRows.push({ key: 'eventDocumentMeta', value: nextMeta });
    }
  } catch (_) {}
  const { error: upsertError } = await db().from('ce_meta').upsert(metaRows, { onConflict: 'key' });
  if (upsertError) throw upsertError;
  return value.length - next.length;
}


function sanitizeDocumentMetaKey(eventId, value) {
  const ev = text(eventId).trim();
  const raw = text(value).trim();
  const code = documentCodeFrom(raw);
  return ev && code ? `${ev}|${code}` : '';
}
function sanitizeEventDocumentForMeta(eventId, doc, index = 0) {
  const ev = text(eventId).trim();
  const code = documentCodeFrom(doc?.codigo || doc?.imageKey || doc?.id) || `DOC${String(index + 1).padStart(2, '0')}`;
  const idv = `${ev}|${code}`;
  return {
    id: idv,
    eventId: ev,
    codigo: code,
    imageKey: code,
    imageUrl: imageSourceForCanonical(doc?.imageUrl || doc?.image_url || doc?.url || ''),
    fecha: text(doc?.fecha || doc?.date || '').trim(),
    descripcion: text(doc?.descripcion || doc?.description || '').trim(),
    createdAt: text(doc?.createdAt || doc?.created_at || '').trim() || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    recovered: false
  };
}

export async function saveEventDocumentsForEvent(eventId, docs = [], meta = {}) {
  const ev = text(eventId).trim();
  if (!ev) {
    const err = new Error('Falta eventId para guardar documentos.');
    err.status = 400;
    throw err;
  }
  await assertEventNotFinalized(ev, 'modificar documentos');
  const safeDocs = arr(docs)
    .map((doc, index) => sanitizeEventDocumentForMeta(ev, doc, index))
    .filter(doc => doc.eventId && doc.codigo);

  const { data: docRow, error: docError } = await db().from('ce_meta').select('value').eq('key', 'eventDocuments').maybeSingle();
  if (docError) throw docError;
  const currentDocs = Array.isArray(docRow?.value) ? docRow.value : [];
  const nextDocs = currentDocs.filter(doc => text(doc?.eventId || doc?.event_id).trim() !== ev).concat(safeDocs);

  const { data: metaRow, error: metaError } = await db().from('ce_meta').select('value').eq('key', 'eventDocumentMeta').maybeSingle();
  if (metaError) throw metaError;
  const currentMeta = metaRow?.value && typeof metaRow.value === 'object' && !Array.isArray(metaRow.value) ? metaRow.value : {};
  const nextMeta = Object.fromEntries(Object.entries(currentMeta).filter(([k]) => !text(k).trim().startsWith(`${ev}|`)));

  if (meta && typeof meta === 'object' && !Array.isArray(meta)) {
    Object.entries(meta).forEach(([key, value]) => {
      const metaKey = sanitizeDocumentMetaKey(ev, key);
      if (!metaKey) return;
      const v = value && typeof value === 'object' ? value : {};
      nextMeta[metaKey] = {
        fecha: text(v.fecha || '').trim(),
        descripcion: text(v.descripcion || v.description || '').trim()
      };
    });
  }
  safeDocs.forEach(doc => {
    const key = `${ev}|${doc.codigo}`;
    nextMeta[key] = {
      fecha: text(nextMeta[key]?.fecha || doc.fecha || '').trim(),
      descripcion: text(nextMeta[key]?.descripcion || doc.descripcion || '').trim()
    };
  });

  const { error: upsertError } = await db().from('ce_meta').upsert([
    { key: 'eventDocuments', value: nextDocs },
    { key: 'eventDocumentMeta', value: nextMeta }
  ], { onConflict: 'key' });
  if (upsertError) throw upsertError;
  return { ok: true, eventId: ev, documents: safeDocs.length };
}
...
async function rpcDonacionSituacion(rowId, situacion) {
  const value = text(situacion).trim();
  const idv = text(rowId).trim();
  if (!['Supuesta','Comprometida','Entregada'].includes(value)) {
    const err = new Error(`Situación de donación no válida: ${value || 'vacía'}`);
    err.status = 400;
    throw err;
  }
  if (!idv) {
    const err = new Error('Falta id de donación.');
    err.status = 400;
    throw err;
  }

  // BANK4.3: la situación ya no depende de una RPC SQL adicional. El propio backend,
  // autenticado con service_role, actualiza exclusivamente la nueva columna. Antes se
  // comprueba que la fila sea realmente una donación y que el evento siga En curso.
  // La protección SQL de eventos Finalizados continúa activa como segunda barrera.
  const { data: existing, error: readError } = await db()
    .from('ce_compras')
    .select('id,event_id,ticket_donacion,donacion_situacion')
    .eq('id', idv)
    .maybeSingle();
  if (readError) throw readError;
  if (!existing?.id) {
    const err = new Error(`No existe la donación ${idv}.`);
    err.status = 404;
    throw err;
  }
  if (!isDonationTicket(existing.ticket_donacion)) {
    const err = new Error(`El registro ${idv} no es una donación de producto.`);
    err.status = 409;
    throw err;
  }
  await assertEventNotFinalized(existing.event_id, 'cambiar la situación de una donación');

  const { data, error } = await db()
    .from('ce_compras')
    .update({ donacion_situacion: value })
    .eq('id', idv)
    .select('*')
    .maybeSingle();
  if (error) throw error;
  if (!data?.id) {
    const err = new Error(`No se pudo guardar la situación de la donación ${idv}.`);
    err.status = 409;
    throw err;
  }
  return data;
...
async function rpcOpenComprasCorrection(rowId, minutes = 30, reason = 'Corrección manual de COMPRAS') {
  const { data, error } = await db().rpc('ce_crud_open_compras_correction', {
    p_event_id: text(rowId).trim(),
    p_minutes: Number.isFinite(Number(minutes)) ? Math.max(1, Math.round(Number(minutes))) : 30,
    p_reason: text(reason || 'Corrección manual de COMPRAS')
  });
  if (error) throw error;
  return Array.isArray(data) ? data[0] : data;
}

async function rpcCloseEventCorrection(rowId) {
  const { data, error } = await db().rpc('ce_crud_close_event_correction', {
    p_event_id: text(rowId).trim()
  });
  if (error) throw error;
  return Array.isArray(data) ? data[0] : data;
...
async function rpcCatalogLibre(functionName, args, missingPattern) {
  const { data, error } = await db().rpc(functionName, args);
  if (error) {
    const msg = text(error.message || error.details || error.hint || error.code).trim();
    if (missingPattern.test(msg)) return null;
    throw error;
  }
  return Array.isArray(data) ? data[0] : data;
}

async function rpcPersonCatalogUpdate(rowId, row = {}) {
  return rpcCatalogLibre('ce_update_persona_catalogo_libre', {
    p_id: text(rowId).trim(),
    p_nombre: text(row.nombre).trim(),
    p_rango: text(row.rango || 'SOCIO').toUpperCase()
  }, /ce_update_persona_catalogo_libre|could not find|does not exist|pgrst202/i);
}

async function rpcStoreCatalogUpdate(rowId, row = {}) {
  return rpcCatalogLibre('ce_update_tienda_catalogo_libre', {
    p_id: text(rowId).trim(),
    p_nombre: text(row.nombre).trim()
  }, /ce_update_tienda_catalogo_libre|could not find|does not exist|pgrst202/i);
}

async function rpcProductCatalogUpdateLegacy(rowId, patch = {}) {
  const legacyPatch = { ...patch };
  delete legacyPatch.nombre;
  if (!Object.keys(legacyPatch).length) return null;
  const args = {
    p_id: text(rowId).trim(),
    p_set_precio: Object.prototype.hasOwnProperty.call(legacyPatch, 'default_precio'),
    p_default_precio: Object.prototype.hasOwnProperty.call(legacyPatch, 'default_precio') ? money(legacyPatch.default_precio) : null,
    p_set_tienda: Object.prototype.hasOwnProperty.call(legacyPatch, 'default_tienda_id'),
    p_default_tienda_id: Object.prototype.hasOwnProperty.call(legacyPatch, 'default_tienda_id') ? nullableText(legacyPatch.default_tienda_id) : null,
    p_set_segmento: Object.prototype.hasOwnProperty.call(legacyPatch, 'segmento'),
    p_segmento: Object.prototype.hasOwnProperty.call(legacyPatch, 'segmento') ? text(legacyPatch.segmento).trim() : null,
    p_set_destino: Object.prototype.hasOwnProperty.call(legacyPatch, 'destino'),
    p_destino: Object.prototype.hasOwnProperty.call(legacyPatch, 'destino') ? text(legacyPatch.destino).trim() : null
  };
  return rpcCatalogLibre('ce_update_producto_catalogo_libre', args, /ce_update_producto_catalogo_libre|could not find|does not exist|pgrst202/i);
}

async function rpcProductCatalogUpdate(rowId, patch = {}) {
  const args = {
    p_id: text(rowId).trim(),
    p_set_nombre: Object.prototype.hasOwnProperty.call(patch, 'nombre'),
    p_nombre: Object.prototype.hasOwnProperty.call(patch, 'nombre') ? text(patch.nombre).trim() : null,
    p_set_precio: Object.prototype.hasOwnProperty.call(patch, 'default_precio'),
    p_default_precio: Object.prototype.hasOwnProperty.call(patch, 'default_precio') ? money(patch.default_precio) : null,
    p_set_tienda: Object.prototype.hasOwnProperty.call(patch, 'default_tienda_id'),
    p_default_tienda_id: Object.prototype.hasOwnProperty.call(patch, 'default_tienda_id') ? nullableText(patch.default_tienda_id) : null,
    p_set_segmento: Object.prototype.hasOwnProperty.call(patch, 'segmento'),
    p_segmento: Object.prototype.hasOwnProperty.call(patch, 'segmento') ? text(patch.segmento).trim() : null,
    p_set_destino: Object.prototype.hasOwnProperty.call(patch, 'destino'),
    p_destino: Object.prototype.hasOwnProperty.call(patch, 'destino') ? text(patch.destino).trim() : null
  };
  const v2 = await rpcCatalogLibre('ce_update_producto_catalogo_libre_v2', args, /ce_update_producto_catalogo_libre_v2|could not find|does not exist|pgrst202/i);
  if (v2?.id) return v2;
  // Compatibilidad: si solo se refresca precio/tienda/segmento/destino y la BBDD aun tiene
  // la RPC anterior instalada, la usamos. Para cambiar nombre maestro hace falta la nueva v2.
  if (!Object.prototype.hasOwnProperty.call(patch, 'nombre')) return rpcProductCatalogUpdateLegacy(rowId, patch);
  return null;
}

function productCatalogPatchFromDbRow(row = {}) {
  const patch = {};
  if (Object.prototype.hasOwnProperty.call(row, 'nombre')) patch.nombre = text(row.nombre).trim();
  if (Object.prototype.hasOwnProperty.call(row, 'default_precio')) patch.default_precio = money(row.default_precio);
  if (Object.prototype.hasOwnProperty.call(row, 'default_tienda_id')) patch.default_tienda_id = nullableText(row.default_tienda_id);
  if (Object.prototype.hasOwnProperty.call(row, 'segmento')) patch.segmento = text(row.segmento).trim();
  if (Object.prototype.hasOwnProperty.call(row, 'destino')) patch.destino = text(row.destino).trim();
  return patch;
}
async function personCatalogUpdateViaRpcOrThrow(rowId, row = {}) {
  const data = await rpcPersonCatalogUpdate(rowId, row);
  if (!data?.id) {
    const err = new Error('No están disponibles las RPC de catálogo libre. Ejecuta ControlEvent_SQL_FIX_MAESTROS_CATALOGO_LIBRE.sql en Supabase SQL Editor y vuelve a probar.');
    err.status = 409;
    throw err;
  }
  return data;
}
async function storeCatalogUpdateViaRpcOrThrow(rowId, row = {}) {
  const data = await rpcStoreCatalogUpdate(rowId, row);
  if (!data?.id) {
    const err = new Error('No están disponibles las RPC de catálogo libre. Ejecuta ControlEvent_SQL_FIX_MAESTROS_CATALOGO_LIBRE.sql en Supabase SQL Editor y vuelve a probar.');
    err.status = 409;
    throw err;
...
async function productCatalogUpdateViaRpcOrThrow(rowId, patch = {}) {
  const data = await rpcProductCatalogUpdate(rowId, patch);
  if (!data?.id) {
    const err = new Error('No está disponible la RPC ce_update_producto_catalogo_libre_v2. Ejecuta ControlEvent_SQL_FIX_MAESTROS_CATALOGO_LIBRE.sql en Supabase SQL Editor y vuelve a probar.');
    err.status = 409;
    throw err;
  }
  return data;
...
export async function openComprasCorrectionCrud(recordId, minutes = 30, reason = 'Corrección manual de COMPRAS') {
  const rowId = text(recordId).trim();
  if (!rowId) {
    const err = new Error('Falta id de evento para abrir corrección de COMPRAS');
    err.status = 400;
    throw err;
  }
  const data = await rpcOpenComprasCorrection(rowId, minutes, reason);
  return { ok: true, collection: 'eventos', action: 'open-correction-compras', id: rowId, correction: data };
}

export async function closeEventCorrectionCrud(recordId) {
  const rowId = text(recordId).trim();
  if (!rowId) {
    const err = new Error('Falta id de evento para cerrar corrección');
    err.status = 400;
    throw err;
  }
  const data = await rpcCloseEventCorrection(rowId);
  return { ok: true, collection: 'eventos', action: 'close-correction', id: rowId, correction: data };
}
...
export async function updateProductPriceCrud(productId, payload = {}) {
  const rowId = text(productId || payload?.id).trim();
  if (!rowId) {
    const err = new Error('Falta id de producto para actualizar precio.');
    err.status = 400;
    throw err;
  }
  const price = money(payload.defaultPrecio ?? payload.precio);
  if (!(price > 0)) {
    const err = new Error('Falta precio válido para actualizar PRODUCTOS.');
    err.status = 400;
    throw err;
  }
  const patch = { default_precio: price };
  if (payload.defaultTiendaId !== undefined || payload.tiendaId !== undefined) {
    patch.default_tienda_id = nullableText(payload.defaultTiendaId ?? payload.tiendaId);
  }
  // v10.3: Ticket IA puede corregir SEGMENTO/DESTINO de un producto existente
  // al detectar un error. Se permite junto al refresco de precio porque son atributos
  // maestros del producto, no datos contables de eventos finalizados.
  if (payload.segmento !== undefined) patch.segmento = text(payload.segmento).trim();
  if (payload.destino !== undefined) patch.destino = text(payload.destino).trim();
  let data = await productCatalogUpdateViaRpcOrThrow(rowId, patch);
  if (!data?.id) {
    const err = new Error(`No existe producto para actualizar precio: ${rowId}`);
    err.status = 404;
    throw err;
  }
  return { ok: true, collection: 'productos', action: 'price-refresh-v10.0', id: rowId, item: productFromDb(data) };
}
...
export async function upsertCrudRecord(collection, payload) {
  ...
  if (input.__priceRefreshOnly === true || text(input.__priceRefreshOnly).trim().toLowerCase() === 'true') {
    row.__priceRefreshOnly = true;
  }
  ...
  if (key === 'productos' && (input.__priceRefreshOnly === true || text(input.__priceRefreshOnly).trim().toLowerCase() === 'true')) {
    // v9.4: refresco seguro de último precio conocido desde Tickets IA.
    // Solo toca default_precio y default_tienda_id; no cambia nombre, segmento ni destino,
    // y por eso no debe bloquearse aunque el producto aparezca en eventos finalizados.
    const patch = {};
    if (input.defaultPrecio !== undefined || input.precio !== undefined) patch.default_precio = money(input.defaultPrecio ?? input.precio);
    if (input.defaultTiendaId !== undefined || input.tiendaId !== undefined) patch.default_tienda_id = nullableText(input.defaultTiendaId ?? input.tiendaId);
    if (input.segmento !== undefined) patch.segmento = text(input.segmento).trim();
    if (input.destino !== undefined) patch.destino = text(input.destino).trim();
    if (!Object.keys(patch).length) {
      const { data, error } = await db().from('ce_productos').select('*').eq('id', rowId).maybeSingle();
      if (error) throw error;
      return { ok: true, collection: key, action: 'noop-price-refresh', id: rowId, item: data ? fromDb(data) : input };
    }
    let data = await productCatalogUpdateViaRpcOrThrow(rowId, patch);
    if (!data?.id) {
      const err = new Error(`No existe producto para actualizar precio: ${rowId}`);
    ...
    return { ok: true, collection: key, action: 'price-refresh', id: rowId, item: fromDb(data) };
  }

  if (key === 'eventos') {
    const existing = await eventRowByIdForGuard(rowId);
    if (existing && isFinalizedDbRow(existing)) {
      const nextSituation = text(row?.situacion || existing.situacion || 'En curso').trim() || 'En curso';
      const situationChanged = text(existing.situacion).trim() !== nextSituation;
      // Un formulario puede enviar título/fechas/descripción con formatos distintos aunque el
      // usuario solo cambie el desplegable. Para reabrir un evento Finalizado usamos siempre
      // la RPC específica, que modifica exclusivamente SITUACIÓN y conserva intacto el resto.
      if (situationChanged) {
        const saved = await rpcEventSituation(rowId, nextSituation);
        return { ok: true, collection: key, action: 'update-situacion-rpc', id: rowId, item: saved ? fromDb(saved) : input };
      }
    }
  }
  ...
  if (key === 'compras') {
    ...
    const { data: existing, error: existsError } = await db().from('ce_compras').select('id,ticket_donacion,donacion_situacion').eq('id', rowId).maybeSingle();
    if (existsError) throw existsError;
    let saved = existing?.id ? await rpcCompraUpdate(row) : await rpcCompraInsert(row);
    // v4_0_exp BANK4.2: la situación de DONACIONES forma parte del registro funcional.
    // Las RPC históricas de COMPRAS no reciben todavía ese argumento; se conserva el valor
    // previo si el formulario antiguo no lo envía y se persiste por la RPC específica.
    if (isDonationTicket(text(row.ticket_donacion).trim())) {
      const desiredSituation = text(row.donacion_situacion || existing?.donacion_situacion || 'Comprometida').trim() || 'Comprometida';
      saved = await rpcDonacionSituacion(rowId, desiredSituation);
    }
    return { ok: true, collection: key, action: existing?.id ? 'update' : 'insert', id: rowId, item: saved ? fromDb(saved) : input };
  }

  if (key === 'personas') {
    const { data: existing, error: existsError } = await db().from('ce_personas').select('*').eq('id', rowId).maybeSingle();
    if (existsError) throw existsError;
    if (existing?.id) {
      // Mantenimientos PERSONAS: nombre/rango son catálogo maestro. Se actualizan por RPC
      // para no chocar con CE_BLOQUEO_FINALIZADO cuando la persona aparece en eventos históricos.
      const saved = await personCatalogUpdateViaRpcOrThrow(rowId, row);
      const aliasRows = await replacePersonAliasesCrud(rowId, input);
      const item = fromDb(saved);
      if (aliasRows) {
        const preferred = aliasRows.find(a => a.preferido)?.alias || text(input.nombreAmigo ?? input.nombre_amigo).trim();
        item.nombreAmigo = preferred;
        item.aliases = aliasRows.filter(a => !a.preferido).map(a => a.alias);
      }
      return { ok: true, collection: key, action: 'catalog-update-rpc-social', id: rowId, item };
    }
  }

  if (key === 'tiendas') {
    const { data: existing, error: existsError } = await db().from('ce_tiendas').select('*').eq('id', rowId).maybeSingle();
    if (existsError) throw existsError;
    if (existing?.id) {
      // Mantenimientos TIENDAS: el nombre de la tienda es catálogo maestro. Se actualiza por RPC
      // para no chocar con CE_BLOQUEO_FINALIZADO cuando la tienda aparece en eventos históricos.
      const saved = await storeCatalogUpdateViaRpcOrThrow(rowId, row);
      return { ok: true, collection: key, action: 'catalog-update-rpc', id: rowId, item: fromDb(saved) };
    }
  }

  if (key === 'productos') {
    const { data: existing, error: existsError } = await db().from('ce_productos').select('*').eq('id', rowId).maybeSingle();
    if (existsError) throw existsError;
    if (existing?.id) {
      // Mantenimientos PRODUCTOS: nombre/segmento/destino/precio/tienda son catálogo maestro.
      // No cambian las líneas históricas de COMPRAS/DONACIONES ya creadas; se guardan por RPC
      // para evitar CE_BLOQUEO_FINALIZADO al estar usado en eventos finalizados.
      const patch = productCatalogPatchFromDbRow(row);
      const saved = await productCatalogUpdateViaRpcOrThrow(rowId, patch);
      return { ok: true, collection: key, action: 'catalog-update-rpc', id: rowId, item: fromDb(saved) };
    }
  }
  ...
  const aliasRows = key === 'personas' ? await replacePersonAliasesCrud(rowId, input) : null;
  const { data, error } = await db().from(table).select('*').eq(pk, rowId).maybeSingle();
  if (error) throw error;
  const item = data ? fromDb(data) : input;
  if (key === 'personas' && aliasRows) {
    item.nombreAmigo = aliasRows.find(a => a.preferido)?.alias || text(input.nombreAmigo ?? input.nombre_amigo).trim();
    item.aliases = aliasRows.filter(a => !a.preferido).map(a => a.alias);
  }
  return { ok: true, collection: key, action: 'upsert', id: rowId, item };
}


export async function updateDonationSituationCrud(rowId, situacion) {
  const idv = text(rowId).trim();
  if (!idv) {
    const err = new Error('Falta id de donación.');
    err.status = 400;
  ...
  const saved = await rpcDonacionSituacion(idv, situacion);
  if (!saved?.id) {
    const err = new Error('No se pudo guardar la situación de la donación en ce_compras.');
    err.status = 409;
    throw err;
  }
  return { ok: true, collection: 'compras', action: 'donacion-situacion', id: idv, item: purchaseFromDb(saved) };
}
...
export async function deleteCrudRecord(collection, recordId, payload = {}) {
  ...
  if (key === 'personas') {
    try { const aliasDelete = await db().from('ce_persona_aliases').delete().eq('persona_id', rowId); if (aliasDelete.error) console.warn('[BANK4_20] No se pudieron borrar aliases de persona:', aliasDelete.error.message || aliasDelete.error); } catch (_) { }
