...
export const META_KEYS = ['selectedEventId', 'comprasSort', 'summaryTiendaSort', 'eventCodeMap', 'entityCodeMaps'];
export const TABLES = [
  'ce_users',
  'ce_eventos',
  'ce_personas',
...
export async function deleteTicketImagesForEvent(eventId) {
  ...
  if (!ev) return { deleted: 0, storageDeleted: 0 };
  ...
  await collect(db().from('ce_ticket_images').select('storage_path').eq('event_id', ev));
  await collect(db().from('ce_ticket_images').select('storage_path').like('image_key', `${ev}|%`));

  let storageDeleted = 0;
  const allPaths = Array.from(paths).filter(Boolean);
  for (let i = 0; i < allPaths.length; i += 100) {
    ...
    try {
      ...
      else console.warn('[v8.4] Storage remove parcial fallido:', error.message || error);
    } catch (err) {
      console.warn('[v8.4] Storage remove parcial fallido:', err?.message || err);
    }
  }

  const { error: err1 } = await db().from('ce_ticket_images').delete().eq('event_id', ev);
  if (err1) throw err1;
  const { error: err2 } = await db().from('ce_ticket_images').delete().like('image_key', `${ev}|%`);
  if (err2) throw err2;

  return { deleted: allPaths.length, storageDeleted };
}
