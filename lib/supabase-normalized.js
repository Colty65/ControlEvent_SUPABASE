...
function stripImageCacheParam(value) {
  const src = text(value).trim();
  if (!src) return '';
  try {
    const url = new URL(src);
    ['ce_img', 'ceimg', 'ce_t', '_ce'].forEach(param => url.searchParams.delete(param));
    return url.toString();
  } catch {
    return src.replace(/([?&])(ce_img|ceimg|ce_t|_ce)=[^&#]*/g, '').replace(/[?&]$/, '');
  }
}
function imageCacheToken(img) {
  return `${Date.now()}_${text(img?.size_bytes || img?.size || 0)}`;
}
function displayImageUrl(value, img = {}) {
  const src = stripImageCacheParam(value);
  if (!src || isDataUrl(src)) return src;
  if (!/^https?:\/\//i.test(src)) return src;
  const sep = src.includes('?') ? '&' : '?';
  return `${src}${sep}ce_img=${encodeURIComponent(imageCacheToken(img))}`;
}
...
function refToImageRow(key, value) {
  ...
  if (typeof value === 'object') {
    const src = stripImageCacheParam(value.public_url || value.publicUrl || value.url || value.pathname || value.path || value.storage_path || '');
...
export async function stateFromDb() {
  ...
  for (const img of images || []) {
    const rawValue = img.public_url || img.pathname || img.storage_path || '';
    const value = displayImageUrl(rawValue, img);
    state.ticketImages[img.image_key] = value;
    state.ticketImageRefs[img.image_key] = {
      key: img.image_key,
      url: value,
      pathname: displayImageUrl(img.pathname || rawValue, img),
...
export async function uploadTicketImage({ eventId, key, dataUrl }) {
  const row = await uploadDataUrlImage(`${eventId}|${text(key).includes('|') ? text(key).split('|').slice(1).join('|') : text(key)}`, dataUrl);
  const { error } = await db().from('ce_ticket_images').upsert(row, { onConflict: 'image_key' });
  if (error) throw error;
  return { key: row.image_key, url: displayImageUrl(row.public_url, row), pathname: displayImageUrl(row.pathname, row) };
}

export async function deleteTicketImage({ eventId, key }) {
  const imageKey = text(key).includes('|') ? text(key) : `${eventId}|${key}`;
...
export async function imagesForEvent(eventId) {
  const { data, error } = await db()
    .from('ce_ticket_images')
    .select('image_key,public_url,pathname,storage_path,content_type,size_bytes')
