...
async function matchingTicketImageRows(eventId, key, tkHint = '') {
  ...
  const { data, error } = await db()
    ...
    .select('image_key,event_id,label,storage_path')
    .eq('event_id', ev);
...
export async function uploadTicketImage({ eventId, key, dataUrl, eventSnapshot, tk, replace = true }) {
  ...
  // Supabase/CDN/navegador puede servir la foto antigua si se reutiliza exactamente
  // el mismo storage_path tras borrar y volver a subir. La image_key lógica sigue
  // siendo la misma; solo cambia el fichero físico.
  const row = await uploadDataUrlImage(ticketImageKeyForEvent(ev, key), dataUrl, { unique: true });
  ...
  return { key: row.image_key, url: imageCacheBusted(row.public_url, stamp), pathname: imageCacheBusted(row.pathname, stamp), storage_path: row.storage_path || '', replaced: deletedBeforeUpload, unique: true };
}
...
export async function imagesForEvent(eventId) {
  const { data, error } = await db()
    .from('ce_ticket_images')
    .select('image_key,event_id,label,public_url,pathname,storage_path,updated_at')
