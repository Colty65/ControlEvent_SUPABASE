...
async function selectTicketImageRowsForMutation(eventId, key) {
  const ev = text(eventId).trim();
  const raw = text(key).trim();
  const byKey = new Map();
  ...
  for (let i = 0; i < candidates.length; i += 100) {
    await collect(db().from('ce_ticket_images').select('image_key,storage_path').in('image_key', candidates.slice(i, i + 100)));
  }
  const labels = [...new Set(candidates.flatMap(candidate => [candidate, imageLabel(candidate)]).map(label => text(label).trim()).filter(Boolean))];
  for (let i = 0; ev && i < labels.length; i += 100) {
    await collect(db().from('ce_ticket_images').select('image_key,storage_path').eq('event_id', ev).in('label', labels.slice(i, i + 100)));
  }

  // v17 PROD: para Cálculos por tienda y ticket no basta la clave exacta.
  // Versiones anteriores guardaron alias distintos para el mismo TKxx (con/sin tienda,
  // con/sin espacios o solo eventId|TKxx). Si queda un alias vivo, el render antiguo
  // puede coger su URL y parece que se ha vuelto a adjuntar la foto anterior.
  const withoutEvent = ev && raw.startsWith(`${ev}|`) ? raw.slice(ev.length + 1) : raw;
  const tk = ticketTokenFrom(raw) || ticketTokenFrom(withoutEvent) || ticketTokenFrom(imageLabel(raw));
  if (ev && tk) {
    const like = `%${tk}%`;
    await collect(db().from('ce_ticket_images').select('image_key,storage_path').eq('event_id', ev).ilike('image_key', like));
    await collect(db().from('ce_ticket_images').select('image_key,storage_path').eq('event_id', ev).ilike('label', like));
  }
  return [...byKey.values()];
}
...
export async function uploadTicketImage({ eventId, key, dataUrl, eventSnapshot }) {
  ...
  const previousRows = await selectTicketImageRowsForMutation(ev, imageKey);
...
export async function deleteTicketImage({ eventId, key }) {
  ...
  const rows = await selectTicketImageRowsForMutation(ev, key);
  if (!rows.length) return { deletedRows: 0, storageDeleted: 0 };
  return deleteTicketImageRows(rows, '[v17 delete-image]');
}
