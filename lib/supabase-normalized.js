...
async function selectAll(table, order = 'id', columns = '*') {
  // FIX44: Supabase/PostgREST devuelve como máximo 1000 filas por defecto.
  // La app estaba cargando solo las primeras 1000 ce_compras, por eso algunas
  // compras/donaciones existían en BBDD pero no aparecían en pantalla.
  const pageSize = 1000;
  let from = 0;
  const out = [];
