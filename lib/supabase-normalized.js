...
export async function stateFromDb() {
  ...
  for (const img of images || []) {
    ...
    const value = cacheBustedImageUrl(rawValue);
    state.ticketImages[img.image_key] = value;
    state.ticketImageRefs[img.image_key] = {
      key: img.image_key,
      url: cacheBustedImageUrl(img.public_url || rawValue),
      pathname: cacheBustedImageUrl(img.pathname || rawValue),
...
export async function uploadTicketImage({ eventId, key, dataUrl }) {
  ...
  if (error) throw error;
  return { key: row.image_key, url: cacheBustedImageUrl(row.public_url), pathname: cacheBustedImageUrl(row.pathname) };
}
...
export async function imagesForEvent(eventId) {
  ...
  for (const img of data || []) {
    ...
    images[img.image_key] = {
      key: img.image_key,
      url: cacheBustedImageUrl(rawValue),
      pathname: cacheBustedImageUrl(img.pathname || rawValue)
    };
  }
  return images;
