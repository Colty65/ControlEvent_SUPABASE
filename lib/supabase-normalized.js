...
function refToImageRow(key, value) {
  ...
  if (typeof value === 'string') {
    return {
      ...
      event_id: imageEventId(key),
      label: imageLabel(key),
      public_url: /^https?:\/\//.test(value) ? value : null,
      pathname: value
    };
  }
  if (typeof value === 'object') {
    return {
      ...
      event_id: imageEventId(key),
      label: imageLabel(key),
      public_url: value.public_url || value.publicUrl || value.url || null,
      pathname: value.pathname || value.path || value.url || '',
...
export async function saveStateToDb(incoming) {
  ...
  for (const collection of COLLECTIONS) {
    ...
    const rows = arr(state[collection]).map((row, index) => toDb({ ...row, id: id(row, `${collection}-${index}`) }));
    if (
