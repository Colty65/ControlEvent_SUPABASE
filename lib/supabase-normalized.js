...
async function isProductPriceRefreshOnly(row) {
  const idv = text(row?.id).trim();
  if (!idv) return false;
  const flag = row?.__priceRefreshOnly === true || text(row?.__priceRefreshOnly).trim().toLowerCase() === 'true';
  if (!flag) return false;
  const { data, error } = await db().from('ce_productos').select('id,nombre,segmento,destino').eq('id', idv).maybeSingle();
  if (error) throw error;
  if (!data?.id) return false;
  // Solo se permite saltar el bloqueo de finalizados cuando NO cambian los atributos maestros.
  // El ticket automático únicamente refresca último precio conocido y tienda habitual.
...
async function assertCrudAllowed(collection, rowOrId, action, table = '', pk = 'id') {
  ...
  if (collection === 'productos' && row && !isDelete && await isProductPriceRefreshOnly(row)) {
    return;
  }
...
export async function updateProductPriceCrud(productId, payload = {}) {
  ...
  // v10.2: Ticket IA puede corregir SEGMENTO/DESTINO de un producto existente
