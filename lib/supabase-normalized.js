...
async function selectTicketImageRowsForMutation(eventId, key) {
  ...
  // v17_prod: Cálculos por tienda y ticket pudo dejar alias distintos del mismo TKxx
  // (solo TKxx, tienda|TKxx o tienda | TKxx). En sustitución/baja se limpian todos
  // los alias del TKxx dentro del evento para que no reaparezca una URL antigua.
  const withoutEvent = ev && raw.startsWith(`${ev}|`) ? raw.slice(ev.length + 1) : raw;
