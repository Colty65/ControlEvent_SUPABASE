...
function ticketTokenForImageKey(value) {
  const raw = text(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase();
  const match = raw.match(/(?:^|[^A-Z0-9])(?:TK|TICKET)\s*[-_]*\s*0*([0-9]{1,4})(?:[^A-Z0-9]|$)/);
  return match ? `TK${String(Number(match[1])).padStart(2, '0')}` : '';
}
function imageCacheBusted(value, stamp) {
  const src = text(value).trim();
  if (!src) return src;
  const token = stamp ? encodeURIComponent(text(stamp)) : Date.now();
  return `${src}${src.includes('?') ? '&' : '?'}ceImg=${token}`;
}
async function matchingTicketImageRows(eventId, key, tkHint = '') {
  const ev = text(eventId).trim();
  const raw = text(key).trim();
  const tk = ticketTokenForImageKey(tkHint) || ticketTokenForImageKey(raw);
  if (!ev || (!raw && !tk)) return [];
  const exact = raw ? ticketImageKeyForEvent(ev, raw) : '';
  const rawNoEvent = raw && raw.startsWith(`${ev}|`) ? raw.slice(ev.length + 1) : raw;
  const data = await selectTicketImagesForEvent(ev);
  const out = new Map();
  for (const row of data || []) {
    const imageKey = text(row?.image_key).trim();
    const label = text(row?.label).trim();
    const base = imageLabel(imageKey);
    let match = false;
    if (raw) {
      match = imageKey === exact || imageKey === raw || label === raw || label === rawNoEvent || base === raw || base === rawNoEvent;
    }
    if (!match && tk) {
      match = ticketTokenForImageKey(imageKey) === tk || ticketTokenForImageKey(label) === tk || ticketTokenForImageKey(base) === tk;
    }
    if (match && imageKey) out.set(imageKey, row);
  }
  return [...out.values()];
}
...
function storagePathFor(key, ext, options = {}) {
  const eventFolder = imageEventId(key).replace(/[^a-zA-Z0-9_-]/g, '_');
  const base = safeFilePart(key);
  const suffix = options && options.unique
    ? `__${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
    : '';
  return `${eventFolder}/${base}${suffix}.${ext || 'jpg'}`;
}
...
async function selectTicketImagesForEvent(eventId) {
  const ev = text(eventId).trim();
  if (!ev) return [];
  const columns = 'image_key,event_id,label,public_url,pathname,storage_path,content_type,size_bytes,updated_at';
...
async function uploadDataUrlImage(key, dataUrl, options = {}) {
  const client = db();
  const parsed = parseDataUrl(dataUrl);
  const storagePath = storagePathFor(key, parsed.ext, options);
  const { error: uploadError } = await client.storage.from(BUCKET).upload(storagePath, parsed.buffer, {
    contentType: parsed.contentType,
    upsert: !options?.unique,
    cacheControl: options?.unique ? '0' : '60'
  });
...
async function saveImagesFromState(state) {
  ...
  for (const [rawKey, value] of Object.entries(images)) {
    ...
    if (isDataUrl(value)) {
      // v16 OPT3S: los guardados generales de estado tambien pueden venir de rutas
      // antiguas del front. Si llegan con dataURL, se suben a un fichero fisico nuevo
      // para no volver a dejar la misma URL .../TKxx.jpg y evitar cache de foto vieja.
      const row = await uploadDataUrlImage(key, value, { unique: true });
      try{ Object.defineProperty(row, '__ceDataUrlRow', { value: true, enumerable: false }); }catch(_){ row.__ceDataUrlRow = true; }
      rows.push(row);
    } else {
  ...
  // v8.5 FIX16 + v16 OPT3S: varias referencias antiguas pueden canonizarse al mismo image_key.
  // Las URL/path que manda el cliente son cache local y no deben pisar una fila mas nueva
  // creada por la accion directa de foto. Solo las dataURL recien subidas pueden reemplazar;
  // las referencias URL no se guardan desde el estado local.
  ...
  const mergedRows = [...rowByImageKey.values()];
  if (!mergedRows.length) return 0;

  const stripInternal = row => { const out = { ...(row || {}) }; delete out.__ceDataUrlRow; return out; };
  const dataRows = mergedRows.filter(row => row.__ceDataUrlRow === true);
  const refRows = mergedRows.filter(row => row.__ceDataUrlRow !== true);
  let saved = 0;

  if (dataRows.length) {
    const { error } = await db().from('ce_ticket_images').upsert(dataRows.map(stripInternal), { onConflict: 'image_key' });
    if (error) throw error;
    saved += dataRows.length;
  }

  if (refRows.length) {
    // v16 OPT3S: las filas que vienen solo como URL/path desde el estado del navegador
    // son referencias de cache. No se reinsertan ni se usan para actualizar ce_ticket_images,
    // porque eran la causa de que una foto eliminada o sustituida reapareciera con la URL vieja.
    // La tabla se alimenta desde /api/ticket-images o desde dataURL real.
    saved += 0;
  }
...
export async function stateFromDb(options = {}) {
  ...
  for (const collection of COLLECTIONS) {
    ...
    const rows = scopedEventId && (collection === 'colaboradores' || collection === 'compras')
      ? await selectByEventId(table, scopedEventId)
      : await selectAll(table);
    state[collection] = fromRows(rows, fromDb);
  }

  const meta = await selectAll('ce_meta', 'key');
  for (const row of meta) state[row.key] = scopedMetaValue(row.key, row.value, scopedEventId);
  if (scopedEventId) state.selectedEventId = scopedEventId;
  attachStableCodesToState(state);

  const images = scopedEventId
...
export async function uploadTicketImage({ eventId, key, dataUrl, eventSnapshot, tk, replace = true }) {
  ...
  let deletedBeforeUpload = 0;
  if (replace !== false) {
    const oldRows = await matchingTicketImageRows(ev, key, tk);
    if (oldRows.length) {
      const result = await deleteTicketImageRows(oldRows, '[v16 OPT3O ticket-image-replace]');
      deletedBeforeUpload = result.deletedRows || oldRows.length;
    }
  }

  // Cada sustitución explícita de foto debe generar una URL física nueva.
  // Además, para tickets TKxx no se reutiliza tampoco la image_key física de la fila.
  // Así se evita que una ruta antigua tipo .../TK05.jpg pueda reaparecer por alias/caché heredada.
  const logicalKey = ticketImageKeyForEvent(ev, key);
  const logicalLabel = imageLabel(logicalKey);
  const isTicketPhoto = !!(ticketTokenForImageKey(tk) || ticketTokenForImageKey(key) || ticketTokenForImageKey(logicalKey));
  const uploadKey = isTicketPhoto
    ? `${logicalKey}|IMG_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`
    : logicalKey;
  const row = await uploadDataUrlImage(uploadKey, dataUrl, { unique: true });
  row.event_id = ev;
  row.label = logicalLabel;
  ...
  if (isTicketPhoto && !String(row.storage_path || '').includes('__')) {
    const err = new Error('La subida de foto no ha generado ruta física única. Operación cancelada para evitar reutilizar la imagen vieja.');
    err.status = 409;
    throw err;
  }
  ...
  return { key: row.image_key, image_key: row.image_key, event_id: row.event_id, label: row.label, url: imageCacheBusted(row.public_url, stamp), pathname: imageCacheBusted(row.pathname, stamp), storage_path: row.storage_path || '', replaced: deletedBeforeUpload, unique: true, logical_key: logicalKey };
}

export async function deleteTicketImage({ eventId, key, tk }) {
  const ev = text(eventId).trim();
  await assertEventNotFinalized(ev, 'eliminar foto');
  const rows = await matchingTicketImageRows(ev, key, tk);
  if (rows.length) {
    const result = await deleteTicketImageRows(rows, '[v16 OPT3O ticket-image-delete]');
    return { deletedRows: result.deletedRows || rows.length, storageDeleted: result.storageDeleted || 0 };
  }
  ...
  if (error) throw error;
  return { deletedRows: 0, storageDeleted: data?.storage_path ? 1 : 0 };
}
...
export async function upsertCrudRecord(collection, payload) {
  ...
  if (key === 'eventos') {
    ...
    if (existing && isFinalizedDbRow(existing)) {
      ...
      if (eventUpdateOnlyChangesSituation(existing, row)) {
...
export async function imagesForEvent(eventId) {
  const data = await selectTicketImagesForEvent(text(eventId));
  const images = {};
  for (const img of data || []) {
    const stamp = img.updated_at || Date.now();
    images[img.image_key] = {
      key: img.image_key,
      event_id: img.event_id || imageEventId(img.image_key),
      label: img.label || imageLabel(img.image_key),
      url: imageCacheBusted(img.public_url || img.pathname || img.storage_path, stamp),
      pathname: imageCacheBusted(img.pathname || img.public_url || img.storage_path, stamp),
