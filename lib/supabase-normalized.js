...
export async function uploadTicketImage({ eventId, key, dataUrl, eventSnapshot }) {
  ...
  const previousRows = await selectTicketImageRowsByKeys(ticketImageDeleteCandidates(ev, imageKey));
...
export async function deleteTicketImage({ eventId, key }) {
  ...
  const candidates = ticketImageDeleteCandidates(ev, key);
  if (!candidates.length) return { deletedRows: 0, storageDeleted: 0 };
  const byKey = new Map();
  ...
  return deleteTicketImageRows([...byKey.values()], '[v17 delete-image]');
}
