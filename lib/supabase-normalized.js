...
export async function saveStateToDb(incoming) {
  ...
  const liveEventIds = new Set(arr(state.eventos).map(e => text(e?.id).trim()).filter(Boolean));
  await cleanupOrphanTicketImages(liveEventIds);
}
...
export async function repairTicketImageEventIds() {
  const { data: events, error: eventsError } = await db().from('ce_eventos').select('id');
  if (eventsError) throw eventsError;
  const liveIds = new Set((events || []).map(row => text(row.id).trim()).filter(Boolean));

  const { data, error } = await db()
    .from('ce_ticket_images')
    .select('image_key,event_id,label,storage_path,public_url,pathname');
  if (error) throw error;

  const toDelete = [];
  const toUpdate = [];
  for (const row of data || []) {
    const current = text(row?.event_id).trim();
    const keyEvent = imageEventId(row?.image_key);
    const pathEvent = eventIdFromStoragePath(row?.storage_path || row?.pathname || '');
    const urlEvent = imageEventIdFromValue(row?.public_url || row?.pathname || '');
    const liveCandidate = [keyEvent, pathEvent, urlEvent]
      .map(v => text(v).trim())
      .find(v => v && v !== 'sin_evento' && liveIds.has(v));

    if (current && liveIds.has(current)) {
      // Si la clave contiene de forma inequívoca otro evento vivo, corregimos event_id
      // para que la agrupación por event_id y la carga por evento sean coherentes.
      if (liveCandidate && liveCandidate !== current) {
        toUpdate.push({ image_key: row.image_key, event_id: liveCandidate, label: imageLabel(row.image_key) || row.label || '' });
      }
      continue;
    }
  ...
  for (const row of toUpdate) {
    const { error: updateError } = await db()
      .from('ce_ticket_images')
      .update({ event_id: row.event_id, label: row.label })
      .eq('image_key', row.image_key);
    if (updateError) throw updateError;
    updatedRows += 1;
  }

  const deleted = await deleteTicketImageRows(toDelete, '[v8.5 ticket-image-repair]');
  const finalCleanup = await cleanupOrphanTicketImages(liveIds);
  return {
    ok: true,
    liveEvents: liveIds.size,
    scannedRows: (data || []).length,
    updatedRows,
    deletedRows: (deleted.deletedRows || 0) + (finalCleanup.deletedRows || 0),
    storageDeleted: (deleted.storageDeleted || 0) + (finalCleanup.storageDeleted || 0)
  };
}
