...
async function rpcDonacionSituacion(rowId, situacion) {
  ...
  const { data, error } = await db().rpc('ce_crud_donacion_situacion', {
...
export async function updateDonationSituationCrud(rowId, situacion) {
  ...
  if (!saved?.id) {
    const err = new Error('No está disponible la RPC ce_crud_donacion_situacion. Ejecuta sql/ce_donaciones_situacion_v4.sql en Supabase SQL Editor y vuelve a probar.');
    err.status = 409;
    throw err;
  }
