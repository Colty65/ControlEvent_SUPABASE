...
async function ensureExistingEventIdForImageUpload(eventId, eventSnapshot) {
  const ev = text(eventId).trim();
  if (!ev) return assertExistingEventId(ev, 'subir foto');
  const { data, error } = await db().from('ce_eventos').select('id').eq('id', ev).maybeSingle();
  if (error) throw error;
  if (data?.id) return ev;

  // v8.5 FIX17: para un evento recien creado, el cliente puede adjuntar solo la fila
  // del evento a la subida de foto. Se crea/asegura ese evento concreto, sin tocar
  // ce_colaboradores ni ce_compras ni ejecutar PUT /api/state completo.
  const snap = eventSnapshot && typeof eventSnapshot === 'object' ? eventSnapshot : null;
  if (snap && text(snap.id).trim() === ev && text(snap.titulo || snap.nombre).trim()) {
    const row = eventToDb({
      id: ev,
      titulo: snap.titulo || snap.nombre || '',
      precio: snap.precio ?? 0,
      fechaIni: snap.fechaIni || snap.fecha_ini || '',
      fechaFin: snap.fechaFin || snap.fecha_fin || '',
      situacion: snap.situacion || 'En curso',
      descripcion: snap.descripcion || ''
    });
    const { error: upsertError } = await db().from('ce_eventos').upsert([row], { onConflict: 'id' });
    if (upsertError) throw upsertError;
    return ev;
  }
  return assertExistingEventId(ev, 'subir foto');
}
...
export async function deleteCrudRecord(collection, recordId) {
...
export async function applyCrudDeltas(deltas = {}) {
  const result = { ok: true, upserts: 0, deletes: 0, details: [] };
  ...
  for (const [collection, ids] of Object.entries(deltas.deletes || {})) {
    for (const id of arr(ids)) {
      const info = await deleteCrudRecord(collection, id);
      result.deletes++;
      result.details.push(info);
    }
  }
