/* ControlEvent v31.3 - Mapa de recursos
...
(function(){
  ...
  function bindDonationSummaryJump(){
    ...
    if(btn && !btn.__ceDonationJumpBoundV313){
      btn.__ceDonationJumpBoundV313 = true;
  ...
  function bindDelegatedDonationJumpOnce(){
    if(document.__ceMapaDonationJumpDelegatedV313) return;
    document.__ceMapaDonationJumpDelegatedV313 = true;
  ...
  function bindDirectMapaButton(){
    ...
    silenceMapTooltip();
    document.querySelectorAll('#tabMapaBtn,.mobile-menu-action[data-target="tabMapaBtn"]').forEach(el => {
      if(el.__ceMapaV313Bound) return; el.__ceMapaV313Bound = true;
  ...
  window.ControlEventMapaProductos = {version: VERSION, mode: 'mapa-recursos-v313', render: renderMapaProductos, build: buildMapaProductos, show: forceShowMapa, goDonados: scrollToDonationHeader, goTop: scrollToMapaTop, sync: () => { applyMapVisibility(); ensureMobileMenuAction(); if(currentTab() === TAB_NAME) renderMapaProductos(); }};
})();
