# v17_prod_FIX38_PLANIFICACION_JSON_DIRECTO_Y_SALDO

Base: FIX37_PLANIFICACION_JSON_BARRIL_EXACTO.

Cambios aplicados solo en Planificación inicial / Encargo total a Zuzu:

1. Gemini recibe el prompt del usuario formateado como fuente principal, sin convertirlo en una estructura interna pesada.
2. Gemini recibe PRODUCTOS de catálogo relevantes con nombre, precio, segmento, destino y tienda.
...
5. Si Gemini devuelve donaciones directas, se respetan sus donantes/responsables y el parser local solo añade faltantes.
6. El encabezado genérico “Donaciones y existencias confirmadas” ya no se toma como donante “Existencias”.
7. Si el catálogo cambia formato/capacidad (por ejemplo barril 50l vs 30l), se conserva el producto escrito por Gemini/usuario como revisable, sin forzar sustitución.
